...
const char *
LibRaw::HassyRawFormat_idx2HR(unsigned idx) // HR means "human-readable"
{
...
void LibRaw::parseHassyModel() {
  ...
  static const char *Hasselblad_Ctrl[] = {
      // manually selectable options only
      "ELD",        "ELX",           "Winder CW",  "CW",      "Pinhole",
      "Flash Sync", "SWC",           "200 (Mod)",  "200",     "500 Mech.",
      "500",        "H Series",      "H-Series",   "H1",      "H2",
      "Black Box",  "LENSCONTROL S", "LENSCTRL S", "Generic",
  };
  ...
  if (!imHassy.format) {
    ...
    } else if ((imHassy.nIFD_CM[0] != -1) && (imHassy.nIFD_CM[1] == -1) &&
  ...
  if (imHassy.SensorUnitConnector[0]) {
    ...
    if (!strncmp(imHassy.SensorUnitConnector, "Hasselblad ", 11))
      memmove(imHassy.SensorUnitConnector, imHassy.SensorUnitConnector + 11,
              64 - 11);
  ...
  if (imHassy.format ==
      LIBRAW_HF_AdobeDNG) { // Adobe DNG, use LocalizedCameraModel
    ...
    if ((ps = strrchr(imgdata.color.LocalizedCameraModel, '-')))
      ...
      c = int(strlen(imgdata.color.LocalizedCameraModel));
    ...
    if (strcasestr(imgdata.color.UniqueCameraModel, "coated")) {
      strncpy(normalized_model, imgdata.color.UniqueCameraModel,
              sizeof(imgdata.color.UniqueCameraModel) - 1);
    ...
    if ((ps = strrchr(imgdata.color.UniqueCameraModel, '/'))) {
      ...
      c = int(strlen(imgdata.color.UniqueCameraModel));
    int cc = MIN(c, (int)sizeof(tmp_model) - 1);
  ...
  if (!imHassy.HostBody[0]) {
    ps = strchr(model, '-');
    if (ps) { // check if model contains both host body and sensor version,
              // resolution, MS info
  ...
  if (strstr(model, "503CWD")) {
    ...
    if (model[6] == 'I' && model[7] == 'I')
      ...
      strcpy(model, "CFV");
  } else if (strstr(model, "Hasselblad") && (model[10] != ' ')) {
    strcpy(model, "CFV");
    ilm.CameraMount = LIBRAW_MOUNT_DigitalBack;
  } else {
    FORC(int(sizeof Hasselblad_SensorEnclosures /
             sizeof *Hasselblad_SensorEnclosures)) {
...
#define cpynorm(str)                                                           \
  ...
  if ((imHassy.SensorCode == 4) && (imHassy.CoatingCode < 2)) {
    strcpy(imHassy.Sensor, "-16");
    cpynorm("16-Uncoated");

  } else if ((imHassy.SensorCode == 6) && (imHassy.CoatingCode < 2)) {
    strcpy(imHassy.Sensor, "-22");
    cpynorm("22-Uncoated");

  } else if ((imHassy.SensorCode == 8) && (imHassy.CoatingCode == 1)) {
    strcpy(imHassy.Sensor, "-31");
    cpynorm("31-Uncoated");

  } else if ((imHassy.SensorCode == 9) && (imHassy.CoatingCode < 2)) {
    strcpy(imHassy.Sensor, "-39");
    cpynorm("39-Uncoated");

  } else if ((imHassy.SensorCode == 9) && (imHassy.CoatingCode == 4)) {
    strcpy(imHassy.Sensor, "-39");
    strcpy(model, "H3DII");
    add_MP_toName = 1;
    cpynorm("39-Coated");

  } else if ((imHassy.SensorCode == 13) && (imHassy.CoatingCode == 4)) {
    strcpy(imHassy.Sensor, "-40");
    cpynorm("40-Coated");

  } else if ((imHassy.SensorCode == 13) && (imHassy.CoatingCode == 5)) {
    strcpy(imHassy.Sensor, "-40");
    cpynorm("40-Coated5");

  } else if ((imHassy.SensorCode == 11) && (imHassy.CoatingCode == 4)) {
  ...
  } else if ((imHassy.SensorCode == 11) && (imHassy.CoatingCode == 5)) {
    strcpy(imHassy.Sensor, "-50");
    cpynorm("50-Coated5");

  } else if ((imHassy.SensorCode == 15) && (imHassy.CoatingCode == 5)) {
  ...
  } else if ((imHassy.SensorCode == 12) && (imHassy.CoatingCode == 4)) {
    strcpy(imHassy.Sensor, "-60");
    cpynorm("60-Coated");

  } else if ((imHassy.SensorCode == 17) && (imHassy.CoatingCode == 5)) {
    ...
    if (!imHassy.SensorCode)
      imHassy.SensorCode = 8;
             ...
             ((raw_width == 7212) &&
              (raw_height ==
               5412)) || // CF-39, CFV-39, possibly v.II; Phocus crop
             // uncropped, when the exact size is unknown, should be:
             // - greater or equal to the smallest Phocus crop for the current
             // size
  ...
  } else if (((raw_width == 8282) &&
              (raw_height == 6240)) || // (CFV-50, H3DII-50, H5D-50)
    ...
    if (!strncmp(model, "H5D", 3)) {
      ...
      if (!strncmp(model, "H3D", 3)) {
        ...
        add_MP_toName = 0;
             ...
             ((raw_width == 8384) &&
              (raw_height == 6304)) || // (X1D-50c, "X1D II 50C", "CFV II 50C")
    ...
    if ((raw_width == 8384) ||
      ...
      if (strstr(imHassy.CaptureSequenceInitiator, " II ")) {
        ...
        if (!strncasecmp(imHassy.CaptureSequenceInitiator, "X1D II 50C", 10)) {
          strcpy(model, "X1D II 50C");
        } else if (!strncasecmp(imHassy.CaptureSequenceInitiator, "CFV II 50C",
                                10)) {
          strcpy(model, "CFV II 50C");
        }
      } else {
  ...
  } else if (((raw_width == 10320) &&
              (raw_height == 7752)) || // Phocus crop, A5D-80
  ...
  if ((raw_width == 4090) || ((raw_width == 4096) && (raw_height == 4096)) ||
  ...
  if (dng_version && (imHassy.SensorCode == 13) && (imHassy.CoatingCode == 4)) {
  ...
  if ((c == LIBRAW_HF_3FR) || (c == LIBRAW_HF_FFF))
  ...
  if (((imHassy.CaptureSequenceInitiator[0] == 'H') &&
    ...
    if (imgdata.lens.Lens[0] == 'H') {
      ...
      } else if (!imgdata.lens.Lens[0] && (aperture > 1.0f) &&
                 (focal_len > 10.0f)) {
