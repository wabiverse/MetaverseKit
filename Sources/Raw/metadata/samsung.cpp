...
void LibRaw::parseSamsungMakernotes(int /*base*/, unsigned tag, unsigned type,
  ...
  if (tag == 0x0002) {
    ...
    if (imSamsung.DeviceType == 0x2000) {
      ...
      ilm.CameraFormat = LIBRAW_FORMAT_APSC;
                   ...
                   model, "NX mini",
                   7)) { // device type 0x1000: 'NX mini', EX2F, EX1, WB2000
  ...
  } else if ((tag == 0xa011) && ((len == 1) || (len == 2)) &&
    ...
    if (icWBC[LIBRAW_WBI_Auto][0] < (icWBC[LIBRAW_WBI_Auto][1] >> 1)) {
      icWBC[LIBRAW_WBI_Auto][1] = icWBC[LIBRAW_WBI_Auto][1] >> 4;
      icWBC[LIBRAW_WBI_Auto][3] = icWBC[LIBRAW_WBI_Auto][3] >> 4;
    }
  } else if (tag == 0xa023) {
    ushort ki[4] = {8, 9, 10, 0};
    FORC4 icWBC[LIBRAW_WBI_Ill_A][RGGB_2_RGBG(c)] =
        get4() - imSamsung.key[ki[c]];
    if (icWBC[LIBRAW_WBI_Ill_A][0] < (icWBC[LIBRAW_WBI_Ill_A][1] >> 1)) {
      icWBC[LIBRAW_WBI_Ill_A][1] = icWBC[LIBRAW_WBI_Ill_A][1] >> 4;
      icWBC[LIBRAW_WBI_Ill_A][3] = icWBC[LIBRAW_WBI_Ill_A][3] >> 4;
    }
  } else if (tag == 0xa024) {
    FORC4 icWBC[LIBRAW_WBI_D65][RGGB_2_RGBG(c)] = get4() - imSamsung.key[c + 1];
    if (icWBC[LIBRAW_WBI_D65][0] < (icWBC[LIBRAW_WBI_D65][1] >> 1)) {
      icWBC[LIBRAW_WBI_D65][1] = icWBC[LIBRAW_WBI_D65][1] >> 4;
      icWBC[LIBRAW_WBI_D65][3] = icWBC[LIBRAW_WBI_D65][3] >> 4;
    }
  } else if (tag == 0xa025) {
