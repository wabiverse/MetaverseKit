...
void LibRaw::PentaxLensInfo(unsigned long long id, unsigned len) // tag 0x0207
{
  ...
  if ((id < PentaxID_K100D) ||
      (((id == PentaxID_K100D) || (id == PentaxID_K110D) ||
...
void LibRaw::parsePentaxMakernotes(int /*base*/, unsigned tag, unsigned type,
  ...
  if (tag == 0x0005) {
  ...
  } else if (tag == 0x0008) { /* 4 is raw, 7 is raw w/ pixel shift, 8 is raw w/
                                 dynamic pixel shift */
    ...
    if (imCommon.afcount < LIBRAW_AFDATA_MAXCOUNT) {
      ...
      if ((len < 25) && (len >= 11)) {
        imPentax.AFPointMode =
            (imCommon.afdata[imCommon.afcount].AFInfoData[3] >> 4) & 0x0f;
        imPentax.FocusMode[1] =
            imCommon.afdata[imCommon.afcount].AFInfoData[3] & 0x0f;
        imPentax.AFPointSelected[1] =
            sget2(imCommon.afdata[imCommon.afcount].AFInfoData + 4);
        // Pentax K-m has multiexposure set to 8 when no multi-exposure is in
        // effect
        imPentax.MultiExposure =
            imCommon.afdata[imCommon.afcount].AFInfoData[10] & 0x0f;
      }
      imCommon.afcount++;
    }
  } else if (tag == 0x0207) {
    ...
    if ((unique_id != PentaxID_K_1) && (unique_id != PentaxID_K_3) &&
        (unique_id != PentaxID_K_3_II) && (unique_id != PentaxID_K_1_Mark_II)) {
