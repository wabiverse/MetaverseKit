  ...
  while (nrecs--) {
    ...
    if (type == 0x3004) {
      ...
      if (!strncasecmp(imCommon.firmware, "Firmware Version", 16))
        memmove(imCommon.firmware, imCommon.firmware + 16,
                strlen(imCommon.firmware) - 15);
    ...
    } else if (type == 0x102c) // CanonColorInfo2 / Appendix A: Pro90IS, G1, G2,
                               // S30, S40
    {
      int CanonColorInfo2_type = get2(); // G1 1028, G2 272, Pro90 IS 769, S30
                                         // 274, S40 273, EOS D30 276
    ...
    } else if (type == 0x0032) // CanonColorInfo1
    {
      if (len == 768) { // EOS D30
        ...
        for (unsigned linenum = 0; linenum < Canon_D30_linenums_2_StdWBi.size();
             linenum++) {
          if (Canon_D30_linenums_2_StdWBi[linenum] != LIBRAW_WBI_Unknown) {
            ...
            //         if (Canon_wbi2std[imCanon.wbi] ==
            //         *(Canon_D30_linenums_2_StdWBi + linenum)) {
            //           FORC4 cam_mul[c] = icWBC[*(Canon_D30_linenums_2_StdWBi
            //           + linenum)][c]; Got_AsShotWB = 1;
            //           }
          }
        }
        ...
        if (!wbi)
          cam_mul[0] = -1; // use my auto white balance
                  ...
                  0.001f) || // Pro1, G3, G5, G6, S45, S50, S60, S70
                 Appendix_A) // G2, S30, S40
      {
        ...
        } else if ((CanonColorInfo1_key == key[0]) &&
                   (len == 3072)) { // S60, S70, G6
          linenums_2_StdWBi = Canon_KeyIs0x0410_Len3072_linenums_2_StdWBi;
          WB_table_offset = 16;

        } else if (!CanonColorInfo1_key &&
                   (len == 2048)) { // G2, S30, S40; S45, S50, G3, G5
          key[0] = key[1] = 0;
        ...
        if ((Canon_wbi2std[wbi] == LIBRAW_WBI_Auto) ||
            (Canon_wbi2std[wbi] == LIBRAW_WBI_Unknown) || Got_AsShotWB)
          UseWBfromTable_as_AsShot = 0;
        ...
        if (UseWBfromTable_as_AsShot) {
          ...
          if (Canon_wbi2std[wbi] == LIBRAW_WBI_Custom)
            ...
            temp_wbi = wbi;
          for (AsShotWB_linenum = 0;
               AsShotWB_linenum < linenums_2_StdWBi.size();
               AsShotWB_linenum++) {
            if (Canon_wbi2std[temp_wbi] ==
                linenums_2_StdWBi[AsShotWB_linenum]) {
              break;
            }
          }
        }
        ...
        for (unsigned linenum = 0; linenum < linenums_2_StdWBi.size();
             linenum++) {
          if (linenums_2_StdWBi[linenum] != LIBRAW_WBI_Unknown) {
            FORC4 icWBC[linenums_2_StdWBi[linenum]][GRBG_2_RGBG(c)] =
                get2() ^ key[c & 1];
