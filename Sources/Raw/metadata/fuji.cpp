...
int LibRaw::guess_RAFDataGeneration(
    uchar *RAFData_start) // returns offset to first valid width/height pair
{
  ...
  /* RAFDataGeneration codes, values are 4 bytes, little endian
     ...
     RAFData gen. 0: no RAFData
       ...
       E550, E900, (F500 / F505?) F550, F600 / F605, F700, F770 / F775, F800,
     F810, F900 HS10 HS11, HS20 / HS22, HS30 / HS33 / HS35, HS50 S1, SL1000,
     S100, S200 / S205, S20Pro, S2Pro, S3Pro, S5Pro S5000, S5100 / S5500, S5200
     / S5600, S6000 / S6500, S7000, S9000 / S9500, S9100 / S9600

     RAFData gen. 1, offset to WH pair (offsetWH_inRAFData) = 0:
     - number in bytes 0..1 is less than 10000
     - contains WH pair, recommended image size WH pair, 16 bytes unknown, 2*13
     values (for crops, scales?) X100, X-Pro1, X-S1, X10, XF1
     ...
     - bytes 0..1 contain a number greater than 10000; bytes 2..3 contain zero;
       ...
       - contains WH pair, recommended image size WH pair, 16 bytes unknown,
     2*13 values X-E1
     ...
     - contains a table of 3+2*13 values; first 3 values look like WHW
     - H in WHW group has a different meaning if the shot is taken in crop 2
     mode GFX 100, GFX 100S, GFX 50R, GFX 50S, GFX 50S II X-E2S, X-E3, X-H1,
     X-S10 X-T2, X-T3, X-T4, X-T20, X-T30 X-Pro2, X-Pro3 X100F, X100V
...
#define get_average_WB(wb_index)                                               \
  ...
  if (strstr(model, "S2Pro") || strstr(model, "S20Pro") ||
      strstr(model, "F700") || strstr(model, "S5000") ||
      strstr(model, "S7000")) {
    use_WBcorr_coeffs = 1;
  ...
  // At least 0x36 bytes because of memcpy(imFuji.RAFVersion, PrivateMknBuf +
  // 0x32, 4);
  if ((PrivateMknLength >= 0x36) && (PrivateMknLength < 10240000) &&
      (PrivateMknBuf =
           (uchar *)malloc(PrivateMknLength + 1024))) // 1024b for safety
  {
    ...
    while (isdigit(imFuji.SerialSignature[c]) && (c > 0))
      c--;
    ilm.CamID = unique_id =
        (unsigned long long)atoi(imFuji.SerialSignature + c + 1);
    ...
    while (PrivateEntries--) {
      ...
      if (PrivateTagID == 0x2000) {
        ...
        for (int wb_ind = LIBRAW_WBI_Custom1;
             wb_ind < LIBRAW_WBI_Custom1 + nWBs; wb_ind++) {
        ...
        order = 0x4949;
        if (PrivateTagBytes != 4096) // not one of Fuji X-A3, X-A5, X-A7, X-A10,
                                     // X-A20, X-T100, X-T200, XF10
        {
          int is34 = 0;
          CHECKSPACE(8);
          ...
          for (posWB = 0; posWB < (int)PrivateTagBytes - 16; posWB++) {
            ...
            if ((!memcmp(PrivateMknBuf + posWB, "TSNERDTS", 8) &&
              ...
              icWBC[LIBRAW_WBI_Auto][0] = sget2(PrivateMknBuf + posWB + 2);
              icWBC[LIBRAW_WBI_Auto][2] = sget2(PrivateMknBuf + posWB + 4);
              break;
            }
          }
          ...
          if ((imFuji.RAFDataVersion == 0x0260) || // X-Pro3, GFX 100S
              ...
              !strcmp(model, "X-Pro3") || !strcmp(model, "GFX 100S") ||
              !strcmp(model, "GFX100S") || !strcmp(model, "GFX 50S II") ||
              !strcmp(model, "GFX50S II") || !strcmp(model, "X100V") ||
              !strcmp(model, "X-T4") || !strcmp(model, "X-E4") ||
              !strcmp(model, "X-T30 II") || !strcmp(model, "X-S10"))
            is34 = 1;
          ...
          } else if (imFuji.RAFDataVersion == 0x0150) // XQ1, XQ2
          {
            wb_section_offset = 0x1414;
                     ...
                     imFuji.RAFDataVersion == 0x0251 ||
                     imFuji.RAFDataVersion == 0x0351 ||
                     imFuji.RAFDataVersion == 0x0451 ||
                     imFuji.RAFDataVersion == 0x0551) {
          ...
          } else if (imFuji.RAFDataVersion ==
                     0x025f) // X-T30, GFX 50R, GFX 100 (? RAFDataVersion
                             // 0x045f)
          {
          ...
          /* try for unknown RAF Data versions */
            ...
            if (isWB(PrivateMknBuf + posPrivateMknBuf + 0x141c))
              wb_section_offset = 0x141c;
          } else if (!strcmp(model, "GFX 50S") || !strcmp(model, "GFX 50R")) {
            ...
            if (isWB(PrivateMknBuf + posPrivateMknBuf + 0x21c8))
              wb_section_offset = 0x21c8;
          } else if ((!strcmp(model, "X-E4")) || (!strcmp(model, "X-T30 II"))) {
          ...
          if (wb_section_offset &&
            ...
            for (int wb_ind = 0; wb_ind < (int)Fuji_wb_list1.size();
                 posWB += 6, wb_ind++) {
              CHECKSPACE_ABS2(posWB, 6);
            ...
            for (int posEndCCTsection = posWB; posEndCCTsection < (posWB + 30);
              ...
              if (sget2(PrivateMknBuf + posEndCCTsection) != Gval) {
                if (is34)
                  wb_section_offset =
                      posEndCCTsection -
                      34 * 3 * 2; // 34 records, 3 2-byte values in a record
                else
                  wb_section_offset =
                      posEndCCTsection -
                      31 * 3 * 2; // 31 records, 3 2-byte values in a record
                found = 1;
                break;
              }
            }
            ...
            if (found) {
              for (int iCCT = 0; iCCT < 31; iCCT++) {
                ...
                icWBCCTC[iCCT][2] = icWBCCTC[iCCT][4] =
        ...
        } else // process 4K raf data
        {
          ...
          int nWB, tWB, pWB;
          int iCCT = 0;
          imFuji.RAFDataGeneration =
              4096; // X-A3, X-A5, X-A7, X-A10, X-A20, X-T100, X-T200, XF10
...
void LibRaw::parse_fuji_thumbnail(int offset) {
  ...
  if (get2() == 0xFFD8) {
    while (1) {
      ...
      int len = get2();
      if (len > xmpsz + 2) {
        if ((fread(buf, 1, xmpsz, ifp) == xmpsz) &&
            !memcmp(buf, xmpmarker, xmpsz)) // got it
        {
          xmplen = len - xmpsz - 2;
...
#define get_average_WB(wb_index)                                               \
  ...
  while (entries--) {
    ...
    } else if (tag == 0x2ff0) // WB_GRGBLevels
    {
      get_average_WB(LIBRAW_WBI_AsShot);
      FORC4 cam_mul[c] = icWBC[LIBRAW_WBI_AsShot][c];
    } else if ((tag == 0x4000) && ((len == 8) || (len == 16))) {
      ...
      if (imFuji.BlackLevel[0] == 4)
        ...
        FORC4 cblack[c] =
            (imFuji.BlackLevel[c + 1] + imFuji.BlackLevel[c + 5]) / 2;
    ...
    } else if (tag == 0x2f00) // WB_GRGBLevels
    {
      int nWBs = get4();
      nWBs = MIN(nWBs, 6);
      for (int wb_ind = LIBRAW_WBI_Custom1; wb_ind < LIBRAW_WBI_Custom1 + nWBs;
           wb_ind++) {
    ...
    else if (tag == 0xc000) // RAFData
    {
      ...
      if (len > 20000) {
        ...
        for (int i = 0; i < (int)((sizeof imFuji.RAFData_ImageSizeTable) /
                                  (sizeof imFuji.RAFData_ImageSizeTable[0]));
             i++) {
          imFuji.RAFData_ImageSizeTable[i] = get4();
        }

        //         if ((width > raw_width)
        //             || (raw_inset_present && (width <
        //             imgdata.sizes.raw_inset_crops[0].cwidth))
        //         )
        //           width = raw_width;
        //         if ((height > raw_height)
        //             || (raw_inset_present && (height <
