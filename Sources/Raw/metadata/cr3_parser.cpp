...
int LibRaw::selectCRXFrame(short trackNum, unsigned frameIndex) {
  ...
  crx_data_header_t *hdr =
      &libraw_internal_data.unpacker_data.crx_header[trackNum];
  ...
  for (int i = 0; i < hdr->chunk_count; i++) {
    ...
    while ((stsc_index < hdr->stsc_count) &&
           (i + 1 == hdr->stsc_data[stsc_index + 1].first))
      stsc_index++;
    ...
    for (int j = 0; j < hdr->stsc_data[stsc_index].count; j++) {
      ...
      sample_size = hdr->sample_size > 0 ? hdr->sample_size
                                         : hdr->sample_sizes[current_sample];
...
void LibRaw::selectCRXTrack() {
  ...
  // If the file has multiple media tracks and multiple frames it is currently
  // unsupported.
  ...
  int tracki = -1;
  for (int i = 0, trackcnt = 0; i <= maxTrack && i < LIBRAW_CRXTRACKS_MAXCOUNT;
       i++) {
    if (bitcounts[i] == maxbitcount) {
  ...
  if (tracki >= 0 &&
      tracki < LIBRAW_CRXTRACKS_MAXCOUNT /* && frame_select > 0 */) {
...
#define track libraw_internal_data.unpacker_data.crx_header[trackNum]
  ...
  while (relpos_inDir + 6 < track.MediaSize) {
    if (track.MediaOffset + relpos_inDir >
        ifp->size() - 6) // need at least 6 bytes
    {
      err = -11;
      goto ctmd_fin;
    }
    ...
    if ((tItem == 7) || (tItem == 8) || (tItem == 9)) {
      ...
      while (relpos_inBox + 8 < relpos_inDir + szItem) {
        if (track.MediaOffset + relpos_inBox >
            ifp->size() - 8) // need at least 8 bytes
        {
          err = -11;
          goto ctmd_fin;
        }
        ...
        if ((Tag == 0x927c) && ((tItem == 7) || (tItem == 8))) {
          fseek(ifp, track.MediaOffset + relpos_inBox + 8L, SEEK_SET);
          short q_order = order;
          order = get2();
          if (bad_hdr()) {
            err = -13;
          ...
          parse_makernote(track.MediaOffset + relpos_inBox + 8, 0);
...
int LibRaw::parseCR3(INT64 oAtomList, INT64 szAtomList, short &nesting,
  ...
  */
  ...
  const unsigned char UIID_CanonPreview[17] =
      "\xea\xf4\x2b\x5e\x1c\x98\x4b\x88\xb9\xfb\xb7\xdc\x40\x6e\x4d\x16";
  const unsigned char UUID_XMP[17] =
  ...
  while ((oAtom + 8LL) <= (oAtomList + szAtomList)) {
    ...
    if (!strcmp(AtomNameStack, "uuid")) // Top level uuid
    {
      ...
      if (!memcmp(UIID, UUID_XMP, 16) && szAtom > 24LL && szAtom < 1024000LL) {
        ...
        xmpdata[szAtom - 24] = 0;
      } else if (!memcmp(UIID, UIID_CanonPreview, 16) && szAtom > 48LL &&
                 szAtom < 100LL * 1024000LL) {
        ...
        if (!memcmp(xdata + 12, "PRVW", 4)) {
          ...
          if (imgdata.thumbs_list.thumbcount < LIBRAW_THUMBNAIL_MAXCOUNT) {
            ...
            if (do_add) {
              ...
              imgdata.thumbs_list.thumblist[idx].tmisc =
                  (3 << 5) | 8; // 3 samples/8 bps
              imgdata.thumbs_list.thumblist[idx].twidth =
                  (xdata[22] << 8) + xdata[23];
              imgdata.thumbs_list.thumblist[idx].theight =
                  (xdata[24] << 8) + xdata[25];
              imgdata.thumbs_list.thumbcount++;
            }
          }
    ...
    if (!strcmp(AtomNameStack, "moovuuid")) {
      ...
      if (!libraw_internal_data.unpacker_data.cr3_ifd0_length)
        libraw_internal_data.unpacker_data.cr3_ifd0_length =
            unsigned(szAtomContent);
      ...
      if (!libraw_internal_data.unpacker_data.cr3_exif_length)
        libraw_internal_data.unpacker_data.cr3_exif_length =
            unsigned(szAtomContent);
...
#define current_track libraw_internal_data.unpacker_data.crx_header[nTrack]
      ...
      if (szAtomContent >= 12) {
        ...
        current_track.stsc_data = (crx_sample_to_chunk_t *)malloc(
            entries * sizeof(crx_sample_to_chunk_t));
      ...
      if (szAtomContent >= 12) {
        ...
        if (sample_size) {
          ...
          current_track.sample_sizes =
              (int32_t *)malloc(entries * sizeof(int32_t));
      ...
      if (szAtomContent >= 16) {
        ...
        current_track.chunk_offsets =
            (INT64 *)malloc(entries * sizeof(int64_t));
