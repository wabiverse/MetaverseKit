...
void LibRaw::parse_exif_interop(int base) {
  ...
  while (entries--) {
    ...
    if (callbacks.exif_cb) {
      callbacks.exif_cb(callbacks.exifparser_data, tag | 0x40000, type, len,
                        order, ifp, base);
            ...
            if (pos) {
              for (l = 0; l < 3; l++) // skip last row
              {
                ...
                if (num > 0.01)
                 ...
                 (!strncmp(model, "DSC-V3", 6) ||
                  !strncmp(model, "DSC-F828", 8))) {
        parseSonySRF(len);
        break;
...
void LibRaw::parse_gps_libraw(int base) {
  ...
  while (entries--) {
    ...
    if (callbacks.exif_cb) {
      callbacks.exif_cb(callbacks.exifparser_data, tag | 0x50000, type, len,
                        order, ifp, base);
      fseek(ifp, savepos, SEEK_SET);
    }
