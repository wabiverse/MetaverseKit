...
int LibRaw::parse_tiff_ifd(int base) {
  ...
  while (entries--) {
    ...
    if (!is_pana_raw) { /* processing of EXIF tags that collide w/ PanasonicRaw
                           tags */
      switch (tag) {
      case 0x0001:
        if (len == 4)
      ...
      case 0xf007: // 61447
        ...
        if (!tiff_ifd[ifd].bps && tiff_ifd[ifd].offset > 0) {
          ...
          if (ljpeg_start(&jh, 1)) {
            if (!dng_version && !strcasecmp(make, "SONY") &&
                tiff_ifd[ifd].phint == 32803 &&
                tiff_ifd[ifd].comp == 7) // Sony/lossless compressed IFD
            {
              ...
              if ((tiff_ifd[ifd].t_width > 4 * tiff_ifd[ifd].t_height) &
                  ~jh.clrs) {
      ...
      case 0x002d: /*  45, RawFormat */
                   /* pana_encoding: tag 0x002d (45dec)
                      ...
                      mode            7 -            RW2 DC-S1R (probably            DC-S1
                      too) in            "pixel shift on" mode
                   */
        pana_encoding = get2();
        break;
      ...
      case 0x002e: /*  46, JpgFromRaw */
        if ((type != LIBRAW_EXIFTAG_TYPE_UNDEFINED) || (fgetc(ifp) != 0xff) ||
            (fgetc(ifp) != 0xd8))
          break;
    ...
    case 0xf003: /* 61443, Fuji RAF 0xf003 */
      if (!tiff_ifd[ifd].samples ||
          tag != 0x0102) // ??? already set by tag 0x115
        ...
        if (fwb[3] < 0x100) {
          ...
          if ((fwb[3] == 17) && // Tungsten WB
            ...
            if ((imFuji.RAFDataVersion == 0x0260) || // X-Pro3, GFX 100S
                ...
                !strcmp(model, "X-T4") || !strcmp(model, "X-H2S") ||
                !strcmp(model, "X-E4") || !strcmp(model, "X-T30 II") ||
                !strcmp(model, "X-S10"))
              // is34 cameras have 34 CCT values instead of 31, manual still
              // claims 2500 to 10000 K aligned 3000 K to Incandescent, as it is
              // usual w/ other Fujifilm cameras
              is34 = 1;

            for (int fi = 0;
                 fi < int(libraw_internal_data.unpacker_data.lenRAFData - 3);
                 fi++) // looking for Tungsten WB
            {
                  ...
                  (fwb[2] == rafdata[fi + 2])) // found Tungsten WB
              {
                if (rafdata[fi - 15] !=
                  ...
                  continue;
                for (int wb_ind = 0, ofst = fi - 15;
                     wb_ind < (int)Fuji_wb_list1.size(); wb_ind++, ofst += 3) {
                ...
                for (fj = fi; fj < (fi + 15);
                     fj += 3) // looking for the end of the WB table
                {
    ...
    case 0xc7b5: /* 51125 DefaultUserCrop */
      if (len == 4) {
        ...
        if (cnt == 4 // valid values
            && tiff_ifd[ifd].dng_levels.user_crop[0] <
                   tiff_ifd[ifd].dng_levels.user_crop[2] // top < bottom
            && tiff_ifd[ifd].dng_levels.user_crop[1] <
                   tiff_ifd[ifd].dng_levels.user_crop[3] // left < right
        )
    ...
    case 0xc634: /* 50740 : DNG Adobe, DNG Pentax, Sony SR2, DNG Private */
    {
      ...
      if (!strcmp(mbuf, "Adobe")) {
        ...
        while (curr_pos + 8 - start_pos <= len) {
          ...
          if (!strncmp(mbuf, "Pano",
                       4)) { // PanasonicRaw, yes, they use "Pano" as signature
          ...
          if (!strncmp(mbuf, "MakN", 4)) {
            ...
            if (!strncmp(mbuf, "SR2 ", 4)) {
              ...
              if (SR2SubIFDLength && (SR2SubIFDLength < 10240000) &&
                ...
                parseSonySR2((uchar *)buf_SR2, SR2SubIFDOffset, SR2SubIFDLength,
                             AdobeDNG);
...
static LibRaw_internal_thumbnail_formats
tiff2thumbformat(int _comp, int _phint, int _bps, const char *_make);
...
void LibRaw::apply_tiff() {
  ...
  if (dng_version) {
    ...
    if (ifdc) {
      if (ifdc > 1 && (imgdata.rawparams.options &
                       LIBRAW_RAWOPTIONS_DNG_PREFER_LARGEST_IMAGE)) {
        ...
        for (int q = 0; q < ifdc && q < LIBRAW_IFD_MAXCOUNT * 2; q++) {
          ...
          if (tiff_ifd[ifdidx].phint == 4)
            arr[q].databits /=
                4; // Force lower bit count for Transp. mask images
        }
    ...
    for (i = 0; i < (int)tiff_nifds; i++) {
      ...
      if (tiff_ifd[i].phint == 2 && tiff_ifd[i].extrasamples > 0 &&
          tiff_ifd[i].samples > 3)
    ...
    case 1:
      if (dng_version && tiff_sampleformat == 3 &&
          (tiff_bps > 8 && (tiff_bps % 8 == 0) &&
           (tiff_bps <= 32))) // only 16,24, and 32 are allowed
      {
  ...
  if (!dng_version) {
    ...
    if (is_raw && raw >= 0 && tiff_ifd[raw].phint == 2 &&
        tiff_ifd[raw].extrasamples > 0 && tiff_ifd[raw].samples > 3)
  ...
  if (imgdata.rawparams.options &
      LIBRAW_RAWOPTIONS_CHECK_THUMBNAILS_ALL_VENDORS)
    fsizecheck = ifp->size();
  else if ((imgdata.rawparams.options &
  ...
  for (i = 0; i < (int)tiff_nifds; i++)
    if (i != raw &&
      ...
      if (fsizecheck > 0LL) {
        bool ok = true;
        if (tiff_ifd[i].strip_byte_counts_count &&
            tiff_ifd[i].strip_offsets_count)
          for (int s = 0; s < MIN(tiff_ifd[i].strip_byte_counts_count,
                                  tiff_ifd[i].strip_offsets_count);
               s++) {
            if (tiff_ifd[i].strip_offsets[s] +
                    tiff_ifd[i].strip_byte_counts[s] >
                fsizecheck) {
              ok = false;
              break;
            }
      ...
      if ((INT64(tiff_ifd[i].t_width) * INT64(tiff_ifd[i].t_height) /
           INT64(SQR(tiff_ifd[i].bps) + 1)) >
          (INT64(thumb_width) * INT64(thumb_height) /
           INT64(SQR(thumb_misc) + 1))) {
      ...
      if (imgdata.thumbs_list.thumbcount < LIBRAW_THUMBNAIL_MAXCOUNT &&
        ...
        for (int idx = 0; idx < imgdata.thumbs_list.thumbcount; idx++)
          if (imgdata.thumbs_list.thumblist[idx].toffset ==
              tiff_ifd[i].offset) {
            already = true;
            break;
          }
        if (!already) {
          ...
          imgdata.thumbs_list.thumblist[idx].tformat = tiff2thumbformat(
              tiff_ifd[i].comp, tiff_ifd[i].phint, tiff_ifd[i].bps, make);
          ...
          imgdata.thumbs_list.thumblist[idx].tmisc =
              tiff_ifd[i].bps | (tiff_ifd[i].samples << 5);
  ...
  if (thm >= 0) {
    ...
    thumb_format = tiff2thumbformat(tiff_ifd[thm].comp, tiff_ifd[thm].phint,
                                    tiff_ifd[thm].bps, make);
  }
}
...
tiff2thumbformat(int _comp, int _phint, int _bps, const char *_make) {
  ...
  case 65000:
    return _phint == 6 ? LIBRAW_INTERNAL_THUMBNAIL_KODAK_YCBCR
                       : LIBRAW_INTERNAL_THUMBNAIL_KODAK_RGB;
  }
