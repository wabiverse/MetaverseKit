...
void LibRaw::parse_phase_one(int base) {
  ...
  while (entries--) {
    ...
    data = get4();
    save = ftell(ifp);
    bool do_seek = (tag < 0x0108 ||
                    tag > 0x0110); // to make it single rule, not copy-paste
    if (do_seek)
    ...
    case 0x0226:
      for (i = 0; i < 9; i++)
        imgdata.color.P1_color[1].romm_cam[i] =
            (float)getreal(LIBRAW_EXIFTAG_TYPE_FLOAT);
      break;
    case 0x0301:
      model[63] = 0;
  ...
  if ((ilm.MaxAp4CurFocal > 0.7f) && (ilm.MinAp4CurFocal > 0.7f)) {
