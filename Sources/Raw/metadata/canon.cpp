...
void LibRaw::setCanonBodyFeatures(unsigned long long id) {
  ...
  ilm.CamID = id;
  if ((id == CanonID_EOS_1D) || (id == CanonID_EOS_1D_Mark_II) ||
      (id == CanonID_EOS_1D_Mark_II_N) || (id == CanonID_EOS_1D_Mark_III) ||
      (id == CanonID_EOS_1D_Mark_IV)) {
    ilm.CameraFormat = LIBRAW_FORMAT_APSH;
    ilm.CameraMount = LIBRAW_MOUNT_Canon_EF;
  } else if ((id == CanonID_EOS_1Ds) || (id == CanonID_EOS_1Ds_Mark_II) ||
             (id == CanonID_EOS_1Ds_Mark_III) || (id == CanonID_EOS_1D_X) ||
             (id == CanonID_EOS_1D_X_Mark_II) ||
             (id == CanonID_EOS_1D_X_Mark_III) || (id == CanonID_EOS_1D_C) ||
             (id == CanonID_EOS_5D) || (id == CanonID_EOS_5D_Mark_II) ||
             (id == CanonID_EOS_5D_Mark_III) ||
             (id == CanonID_EOS_5D_Mark_IV) || (id == CanonID_EOS_5DS) ||
             (id == CanonID_EOS_5DS_R) || (id == CanonID_EOS_6D) ||
             (id == CanonID_EOS_6D_Mark_II)) {
    ilm.CameraFormat = LIBRAW_FORMAT_FF;
    ilm.CameraMount = LIBRAW_MOUNT_Canon_EF;
  } else if ((id == CanonID_EOS_M) || (id == CanonID_EOS_M2) ||
             (id == CanonID_EOS_M3) || (id == CanonID_EOS_M5) ||
             (id == CanonID_EOS_M10) || (id == CanonID_EOS_M50) ||
             (id == CanonID_EOS_M50_Mark_II) || (id == CanonID_EOS_M6) ||
             (id == CanonID_EOS_M6_Mark_II) || (id == CanonID_EOS_M100)) {
    ilm.CameraFormat = LIBRAW_FORMAT_APSC;
    ilm.CameraMount = LIBRAW_MOUNT_Canon_EF_M;
  } else if ((id == CanonID_EOS_R) || (id == CanonID_EOS_RP) ||
             (id == CanonID_EOS_R3) || (id == CanonID_EOS_R5) ||
             (id == CanonID_EOS_R6)) {
    ilm.CameraFormat = LIBRAW_FORMAT_FF;
  ...
  else if ((id == CanonID_EOS_D30) || (id == CanonID_EOS_D60) ||
           (id > 0x80000000ULL)) {
    ilm.CameraFormat = LIBRAW_FORMAT_APSC;
    ilm.CameraMount = LIBRAW_MOUNT_Canon_EF;
  }
...
int CanonCameraInfo_checkFirmwareRecordLocation(uchar *offset) {
  ...
  if (isdigit(*offset) && isdigit(*(offset + 2)) && isdigit(*(offset + 4)) &&
      (*(offset + 1) == '.') && (*(offset + 3) == '.') &&
...
void LibRaw::processCanonCameraInfo(unsigned long long id, uchar *CameraInfo,
  ...
  if (tagtypeIs(LIBRAW_EXIFTAG_TYPE_LONG)) {
    if ((maxlen == 94) || (maxlen == 138) || (maxlen == 148) ||
      ...
      imCommon.CameraTemperature =
          float(sget4(CameraInfo + ((maxlen - 3) << 2)));
    else if (maxlen == 72)
      imCommon.CameraTemperature =
          float(sget4(CameraInfo + ((maxlen - 1) << 2)));
    else if ((maxlen == 85) || (maxlen == 93))
      imCommon.CameraTemperature =
          float(sget4(CameraInfo + ((maxlen - 2) << 2)));
    else if ((maxlen == 96) || (maxlen == 104))
      imCommon.CameraTemperature =
          float(sget4(CameraInfo + ((maxlen - 4) << 2)));
  }
  ...
  } else if (tagtypeIs(LIBRAW_EXIFTAG_TYPE_UNDEFINED) && (mgck == 0xaaaa) &&
             (dng_writer == nonDNG)) { // CameraOrientation
    int c, i;
...
void LibRaw::Canon_WBCTpresets(short WBCTversion) {
  ...
  if (WBCTversion == 0) { // tint, as shot R, as shot B, CСT
    ...
    if ((unique_id == CanonID_EOS_M3) || (unique_id == CanonID_EOS_M10) ||
...
void LibRaw::parseCanonMakernotes(unsigned tag, unsigned /*type*/, unsigned len,
                                  unsigned dng_writer) {
...
#define CR3_ColorData(offset)                                                  \
  ...
  if (tag == 0x0001) {
    ...
    if (dng_writer == nonDNG) {
      ...
      if (imCanon.ISOgain[1] != 0x7fff) {
        imCommon.real_ISO = floorf(
            ...
            libraw_powf64l(
                2.f,
                float(imCanon.ISOgain[0] + imCanon.ISOgain[1]) / 32.f - 5.f));
  ...
  } else if ((tag == 0x0012) || (tag == 0x0026) || (tag == 0x003c)) {
    if (!imCommon.afcount) {
    ...
    for (unsigned linenum = 0; linenum < Canon_G9_linenums_2_StdWBi.size();
         linenum++) {
      if (Canon_G9_linenums_2_StdWBi[linenum] != LIBRAW_WBI_Unknown) {
        FORC4 icWBC[Canon_G9_linenums_2_StdWBi[linenum]][GRBG_2_RGBG(c)] =
            get4();
        if (Canon_wbi2std[imCanon.wbi] == Canon_G9_linenums_2_StdWBi[linenum]) {
          FORC4 cam_mul[c] =
              float(icWBC[Canon_G9_linenums_2_StdWBi[linenum]][c]);
          Got_AsShotWB = 1;
        }
      }
  ...
  } else if (tag == 0x00e0) // SensorInfo
  {
    ...
    case 796:
      imCanon.ColorDataVer =
          3; // -1D Mark II N, 5D, 30D, 400D; ColorDataSubVer: 1
    ...
    case 1346: // 1100D, 60D; ColorDataSubVer: 9
      ...
      if ((imCanon.ColorDataSubVer == 4) || (imCanon.ColorDataSubVer == 5)) {
    ...
    case 5120: // G10, G11, G12, G15, G16
      ...
      if (imCanon.ColorDataSubVer == 0xfffc) // ColorDataSubVer: 65532 (-4)
                                             // G7 X Mark II, G9 X Mark II, G1 X
                                             // Mark III M5, M100, M6
      {
      ...
      } else if (imCanon.ColorDataSubVer ==
                 0xfffd) // ColorDataSubVer: 65533 (-3)
                         // M10, M3
                         // G1 X, G1 X Mark II
                         // G3 X, G5 X, G7 X, G9 X
                         // G10, G11, G12, G15, G16
    ...
    case 1816: // RP, 250D, SX70 HS; ColorDataSubVer: 18
               // M6 Mark II, M200, 90D, G5 X Mark II, G7 X Mark III, 850D;
               // ColorDataSubVer: 19
