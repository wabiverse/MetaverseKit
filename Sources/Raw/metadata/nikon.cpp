  ...
  if ((len < 20) || (len == 58) || (len == 108)) {
    ...
    case 108: // "Z 9"
      ...
      if (i == 17) {
        ...
        if ((ilm.LensID == 11) || (ilm.LensID == 12) || (ilm.LensID == 26))
...
void LibRaw::parseNikonMakernote(int base, int uptag,
                                 unsigned /*dng_writer */) {
  ...
  while (entries--) {
    ...
    if (tag == 0x0002) {
      ...
      if (tagtypeIs(LIBRAW_EXIFTOOLTAGTYPE_binary)) {
        if (len == 2560) { // E5400, E8400, E8700, E8800
          ...
          icWBC[LIBRAW_WBI_Auto][1] = icWBC[LIBRAW_WBI_Auto][3] = icWBC
              [LIBRAW_WBI_Daylight][1] = icWBC[LIBRAW_WBI_Daylight][3] = icWBC
                  [LIBRAW_WBI_Tungsten][1] = icWBC[LIBRAW_WBI_Tungsten][3] =
      ...
      imCommon.real_ISO = int(
          100.0 * libraw_powf64l(2.0, double((uchar)fgetc(ifp)) / 12.0 - 5.0));
      if (!iso_speed || (iso_speed == 65535)) {
        iso_speed = imCommon.real_ISO;
      }
    } else if (tag == 0x0022) {
      imNikon.Active_D_Lighting = get2();
    } else if (tag == 0x003b) { // WB for multi-exposure (ME); all 1s for
                                // regular exposures
      imNikon.ME_WB[0] = getreal(type);
      imNikon.ME_WB[2] = getreal(type);
      imNikon.ME_WB[1] = getreal(type);
      imNikon.ME_WB[3] = getreal(type);
    } else if (tag == 0x003d) { // not corrected for file bitcount, to be
                                // patched in open_datastream
      ...
      black += i;
               ...
               0x0045) { /* upper left pixel (x,y), size (width,height) */
      ...
      */
      ...
      if ((imNikon.ColorBalanceVersion >= 400) &&
          (imNikon.ColorBalanceVersion <=
           405)) { // 1 J1, 1 V1, 1 J2, 1 V2, 1 J3, 1 S1, 1 AW1, 1 S2, 1 J4, 1
                   // V3, 1 J5
        ilm.CameraFormat = LIBRAW_FORMAT_1INCH;
        ilm.CameraMount = LIBRAW_MOUNT_Nikon_CX;
      } else if ((imNikon.ColorBalanceVersion >= 500) &&
                 (imNikon.ColorBalanceVersion <=
                  502)) { // P7700, P7800, P330, P340
    ...
    } else if (tag == 0x00a7) // shutter count
    {
      ...
      if (ShotInfo_len && (imNikon.ShotInfoVersion >= 208)) {
        unsigned RotationOffset = 0, OrientationOffset = 0;
        ...
        */
        ...
        case 231: // ShotInfoD4S, Rotation, Roll/Pitch/Yaw
          ...
          if (RotationOffset < ShotInfo_len) {
            imNikon.MakernotesFlip =
                (*(ShotInfo_buf + RotationOffset) >> 4) & 0x03;
          }
          break;
        ...
        if (OrientationOffset && ((OrientationOffset + 12) < ShotInfo_len) &&
            OrientationOffset < 0xffff) {
          if (imNikon.ShotInfoVersion == 231) // ShotInfoD4S
            imNikon.RollAngle =
                AngleConversion_a(morder, ShotInfo_buf + OrientationOffset);
          else
            imNikon.RollAngle =
                AngleConversion(morder, ShotInfo_buf + OrientationOffset);
          imNikon.PitchAngle =
              AngleConversion(morder, ShotInfo_buf + OrientationOffset + 4);
          imNikon.YawAngle =
              AngleConversion(morder, ShotInfo_buf + OrientationOffset + 8);
        }
        if ((RotationOffset) && (imNikon.MakernotesFlip < 4) &&
            (imNikon.MakernotesFlip >= 0))
