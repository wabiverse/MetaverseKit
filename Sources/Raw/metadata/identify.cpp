...
int LibRaw::setMakeFromIndex(unsigned makei) {
  if (makei <= LIBRAW_CAMERAMAKER_Unknown ||
      makei >= LIBRAW_CAMERAMAKER_TheLastOne)
    return 0;
...
void LibRaw::fixupArri() {
  ...
  for (int i = 0; i < int(sizeof(alist) / sizeof(alist[0])); i++)
    if (!strncasecmp(model, alist[i].a_model, strlen(alist[i].a_model)) &&
        software &&
        !strncasecmp(software, alist[i].a_software,
                     strlen(alist[i].a_software)) &&
...
void LibRaw::identify() {
  ...
  if (fsize64 > LIBRAW_MAX_NONDNG_RAW_FILE_SIZE &&
      fsize64 > LIBRAW_MAX_DNG_RAW_FILE_SIZE)
  ...
  } else if (!memcmp(head + 25, "ARECOYK", 7)) // 'KYOCERA' right-to-left
  {
    ...
    model[0x20] = 0;
    c = 11;
    while (imFuji.SerialSignature[c] > 0 &&
           isdigit(imFuji.SerialSignature[c]) && (c > 0))
      c--;
    if (c < 11)
    ...
    if ((unsigned char)software[0] == 0xff)
      software[0] = 0;
    software[63] = 0;
    data_offset = 4096;
  ...
  if (make[0] == 0) {
    ...
    if (!(strncmp(model, "ov", 2) && strncmp(model, "RP_", 3) &&
      ...
      } else if (!strncasecmp(model, "RP_OV", 5) ||
                 !strncasecmp(model, "ov5647", 6)) {
        const long offsets[] = {
    ...
    } else if (!(strncmp(model, "ov5647", 6) &&
  ...
  for (i = 0; i < int(sizeof CorpTable / sizeof *CorpTable); i++) {
    if (strcasestr(make, CorpTable[i].CorpName)) { /* Simplify company names */
      maker_index = CorpTable[i].CorpId;
      break;
    }
  }

  if (makeIs(LIBRAW_CAMERAMAKER_HMD_Global) &&
      !strncasecmp(model, "Nokia", 5)) {
    maker_index = LIBRAW_CAMERAMAKER_Nokia;
  } else if (makeIs(LIBRAW_CAMERAMAKER_JK_Imaging) &&
             !strncasecmp(model, "Kodak", 5)) {
    maker_index = LIBRAW_CAMERAMAKER_Kodak;
  } else if (makeIs(LIBRAW_CAMERAMAKER_Ricoh) &&
             !strncasecmp(model, "PENTAX", 6)) {
  ...
  if (dng_version) {
    ...
    case 1:
      // Uncompressed float: decoder set in apply_tiff for valid files; not set
      // for non-valid with sampleformat==3
    ...
    case 7:
      ...
      break;
    case 8:
      if (tiff_sampleformat == 3 && tiff_bps > 8 && (tiff_bps % 8 == 0) &&
          tiff_bps <= 32)
        load_raw = &LibRaw::deflate_dng_load_raw;
      else if ((tiff_sampleformat == 0 || tiff_sampleformat == 1) &&
               tiff_bps >= 8 && tiff_bps <= 16)
        load_raw = &LibRaw::deflate_dng_load_raw;
      break;
#ifdef USE_GPRSDK
    case 9:
    ...
    if (makeIs(LIBRAW_CAMERAMAKER_Leica)) {
      ...
      if (!strncasecmp(model, "Q2 MONO", 7))
        height -= 18;
             ...
             (OlyID == OlyID_STYLUS_1) && // don't use normalized_model below,
                                          // it is 'Stylus 1'
             (strchr(model + 6, 's') || strchr(model + 6, 'S'))) {
      width -= 16;
    }
    goto dng_skip;
  }
  ...
  if (makeIs(LIBRAW_CAMERAMAKER_Canon) && !fsize && tiff_bps != 15) {
    ...
    if ((unique_id | 0x20000ULL) ==
        0x2720000ULL) // "PowerShot G11", "PowerShot S90": 0x2700000, 0x2720000
                      // possibly "PowerShot SX120 IS" (if not chdk hack?):
                      // 0x2710000
    {
      left_margin = 8;
      top_margin = 16;
    ...
    if (!fromtable &&
        imCanon.AverageBlackLevel) // not known, but metadata known
    {
      ...
      if ((imCanon.SensorWidth == raw_width) &&
        ...
        left_margin =
            (imCanon.DefaultCropAbsolute.l + 1) & 0xfffe; // round to 2
  ...
  if (!(imgdata.rawparams.options &
        LIBRAW_RAWOPTIONS_ZEROFILTERS_FOR_MONOCHROMETIFFS) &&
...
dng_skip:
  ...
  if (raw_width < 22 || raw_width > 64000 || raw_height < 22 ||
      pixel_aspect < 0.1 || pixel_aspect > 10. || raw_height > 64000)
    is_raw = 0;
...
#define COPYARR(to, from) memmove(&to, &from, sizeof(from))
    ...
    if (iifd < (int)tiff_nifds && iifd >= 0) {
      ...
      if (!(imgdata.rawparams.options &
            LIBRAW_RAWOPTIONS_DONT_CHECK_DNG_ILLUMINANT)) {
      ...
      if (!noFujiDNGCrop) // Promote DNG Crops to raw_inset_crops
      {
        ...
        if (sidx >= 0 && sidx == sidx2 &&
          ...
          if ((lm + ww < int(raw_width) + int(left_margin)) &&
              (tm + hh <
               int(raw_height) + int(top_margin))) // Crop data is correct
          {
            ...
            if (sidx3 >= 0 && sidx3 == sidx) // No need to check values range,
                                             // it is checked at parse
            {
              int dt = int(imgdata.sizes.raw_inset_crops[0].cheight *
                           tiff_ifd[sidx].dng_levels.user_crop[0]);
              int dl = int(imgdata.sizes.raw_inset_crops[0].cwidth *
                           tiff_ifd[sidx].dng_levels.user_crop[1]);
              int db = int(imgdata.sizes.raw_inset_crops[0].cheight *
                           tiff_ifd[sidx].dng_levels.user_crop[2]);
              int dr = int(imgdata.sizes.raw_inset_crops[0].cwidth *
                           tiff_ifd[sidx].dng_levels.user_crop[3]);

              int dh = db - dt;
              int dw = dr - dl;

              if (dh > 0 && dw > 0 &&
                  dh < imgdata.sizes.raw_inset_crops[0]
                           .cheight // No need to repeat crop for 0,0,1,1
                  && dw < imgdata.sizes.raw_inset_crops[0].cwidth) {
                imgdata.sizes.raw_inset_crops[1].cleft =
                    imgdata.sizes.raw_inset_crops[0].cleft + dl;
                imgdata.sizes.raw_inset_crops[1].cwidth = dw;
                imgdata.sizes.raw_inset_crops[1].ctop =
                    imgdata.sizes.raw_inset_crops[0].ctop + dt;
      ...
      else if (tiff_ifd[iifd].sample_format <= 2 && tiff_ifd[iifd].bps > 0 &&
               tiff_ifd[iifd].bps < 32)
        FORC4
      imgdata.color.dng_levels.dng_whitelevel[c] =
          (1 << tiff_ifd[iifd].bps) - 1;
    ...
    if (load_raw == &LibRaw::lossy_dng_load_raw) {
      maximum = 0xffff;
      FORC4 imgdata.color.linear_max[c] =
          imgdata.color.dng_levels.dng_whitelevel[c] = 0xffff;
    } else {
      maximum = imgdata.color.dng_levels.dng_whitelevel[0];
    }
    black = imgdata.color.dng_levels.dng_black;
    if (tiff_samples == 2 && !imgdata.color.dng_levels.dng_cblack[2] &&
      ...
      imgdata.color.dng_levels.dng_cblack[0] =
          imgdata.color.dng_levels.dng_cblack[1] = 0;
      imgdata.color.dng_levels.dng_cblack[4] =
          imgdata.color.dng_levels.dng_cblack[5] = 0;
      imgdata.color.dng_levels.dng_fcblack[0] =
          imgdata.color.dng_levels.dng_fcblack[1] = 0.0f;
      imgdata.color.dng_levels.dng_fcblack[4] =
          imgdata.color.dng_levels.dng_fcblack[5] = 0.0f;
    } else if (tiff_samples == 2 &&
               imgdata.color.dng_levels.dng_cblack[4] *
                       imgdata.color.dng_levels.dng_cblack[5] * tiff_samples ==
                   imgdata.color.dng_levels
                       .dng_cblack[LIBRAW_CBLACK_SIZE - 1]) {
      unsigned ff = filters;
      if (filters > 999 && colors == 3)
        filters |= ((filters >> 2 & 0x22222222) | (filters << 2 & 0x88888888)) &
                   filters << 1;
      ...
      int i = 6 + shot_select;
      for (unsigned row = 0; row < imgdata.color.dng_levels.dng_cblack[4];
           row++)
        for (unsigned col = 0; col < imgdata.color.dng_levels.dng_cblack[5];
             col++) {
      ...
      imgdata.color.dng_levels.dng_cblack[4] =
          imgdata.color.dng_levels.dng_cblack[5] = 0;
      filters = ff;
    } else if (tiff_samples > 2 && tiff_samples <= 4 &&
               imgdata.color.dng_levels.dng_cblack[4] *
                       imgdata.color.dng_levels.dng_cblack[5] * tiff_samples ==
                   imgdata.color.dng_levels
                       .dng_cblack[LIBRAW_CBLACK_SIZE - 1]) {
...
void LibRaw::identify_finetune_pentax() {
    ...
    for (int i = 0; i < (int)tiff_nifds; i++)
      if (tiff_ifd[i].offset == data_offset) {
        if (tiff_ifd[i].phint == 34892)
          return; // Linear DNG made from Pentax source
        break;
      }
  }

  if (makeIs(LIBRAW_CAMERAMAKER_Pentax) || makeIs(LIBRAW_CAMERAMAKER_Samsung)) {
    if (height == 2624 && width == 3936) // Pentax K10D, Samsung GX10;
    {
      height = 2616;
      width = 3896;
    }
    if (height == 3136 && width == 4864) // Pentax K20D, Samsung GX20;
    {
      height = 3124;
      width = 4688;
      filters = 0x16161616;
  ...
  if (makeIs(LIBRAW_CAMERAMAKER_Pentax)) {
    if ((width == 4352) &&
        ((unique_id == PentaxID_K_r) || (unique_id == PentaxID_K_x))) {
      width = 4309;
      filters = 0x16161616;
    }
    if ((width >= 4960) &&
        ((unique_id == PentaxID_K_5) || (unique_id == PentaxID_K_5_II) ||
    ...
    if ((width == 6304) &&
        (unique_id == PentaxID_K_3_III)) // From DNG ActiveArea
    {
      left_margin = 26;
      width = 6224;
      top_margin = 34;
  ...
  } else if (makeIs(LIBRAW_CAMERAMAKER_Ricoh) && (height == 3014) &&
             (width == 4096)) // Ricoh GX200
    width = 4014;
}
  ...
  if (fsize == 4771840) { // hack Nikon 3mpix: E880, E885, E990, E995;
    ...
    if (!timestamp && !nikon_e2100())
      strcpy(model, "E2500");
                                 ...
                                 // E3700; Pentax "Optio 33WR"; Olympus C-740UZ
    if (!timestamp)
      nikon_3700();
...
void LibRaw::identify_finetune_dcr(char head[64], int fsize, int flen) {
  ...
  else if (makeIs(LIBRAW_CAMERAMAKER_Sony) && (raw_width > 3888) && !black &&
           !cblack[0]) {
  ...
  if (is_foveon) {
    ...
    if ((unique_id == PentaxID_K_1) || (unique_id == PentaxID_K_1_Mark_II)) {
      top_margin = 18;
      height = raw_height - top_margin;
      if (raw_width == 7392) {
        left_margin = 6;
      ...
      load_raw = &LibRaw::unpacked_load_raw;
      /* data_error = -1; */ /* No way to know why data_error was raised in
                                dcraw.c, looks not needed esp. for
                                unpacked_load_raw */
    ...
    if (tiff_bps == 15) { // Canon sRAW
      ...
      if (width == 7200 && height == 3888) { // Canon EOS 5DS (R);
    ...
    if (!strcmp(model, "D1")) {
      ...
      width -= 44;
    } else if (!strcmp(model, "D3200") || !strcmp(model, "D600") ||
               !strcmp(model, "D610") ||
    ...
    if (OlyID == OlyID_C_740UZ) { // (fsize == 4775936)
      ...
      if (load_raw == &LibRaw::unpacked_load_raw) {
        load_flags = 4;
        if (imOly.ValidBits == 10)
          load_flags += 2;
      }
      tiff_bps = imOly.ValidBits;

      if ((OlyID == OlyID_E_300) || (OlyID == OlyID_E_500)) {
        width -= 20;
        if (load_raw == &LibRaw::unpacked_load_raw) {
          maximum = 0xfc3;
          memset(cblack, 0, sizeof cblack);
      ...
      } else if ((OlyID == OlyID_TG_5) || (OlyID == OlyID_TG_6)) {
        width -= 26;
      }
    }
    ...
    if (!FujiCropMode && imFuji.RAFDataGeneration &&
    ...
    else if (FujiCropMode ==
             4) // electronic shutter, high speed mode (1.25x crop)
    {
      height = raw_height;
    }
    ...
    if (imFuji.RAFDataGeneration && (imFuji.RAFDataGeneration != 4096)) {
      ...
      case 6160: // uncompressed (unpacked) X100F, X-T2, X-T20, X-Pro2, X-H1,
                 // X-E3
        left_margin = 0;
        break;
    ...
    } else if (!strcmp(model, "NX3000")) // Samsung NX3000; raw_width: 5600
    {
      ...
      colors = 3;
               ...
               3714) // Samsung NX2000, NX300M, NX300, NX30,
                     // "NX U" (aka:
    ...
    } else if (!strcmp(normalized_model, "EX1")) // Samsung EX1; raw_width: 3688
    {
      ...
      if ((width -= 6) > 3682) {
        ...
        top_margin = 8;
                       ...
                       "WB2000")) // Samsung WB2000; raw_width: 3728
    {
      order = 0x4949;
      height -= 3;
      top_margin = 2;
  ...
  else if (makeIs(LIBRAW_CAMERAMAKER_ST_Micro) &&
           !strcmp(model, "STV680 VGA")) {
    black = 16;
    ...
    if ((imHassy.SensorCode == 4) &&
        !strncmp(model, "V96C", 4)) { // Hasselblad V96C
      strcpy(model, "V96C");
      strcpy(normalized_model, model);
      height -= (top_margin = 6);
      ...
      filters = 0x61616161;
               ...
               imHassy.uncropped) { // various Hasselblad '-39'
      height = 5444;
      width = 7248;
      top_margin = 4;
      left_margin = 7;
      filters = 0x61616161;
               ...
               imHassy.uncropped) { // Hasselblad H4D-40, H5D-40
      height -= 84;
      width -= 82;
      top_margin = 4;
      left_margin = 41;
      filters = 0x61616161;
               ...
               imHassy.uncropped) { // Hasselblad H5D-50
      height -= 84;
      width -= 82;
      top_margin = 4;
      left_margin = 41;
      ...
      tiff_bps = 16;
               ...
               imHassy.uncropped) { // Hasselblad H4D-60
      if (black > 500) {            // (imHassy.format == LIBRAW_HF_FFF)
        ...
        maximum = 0x8101;
               ...
               imHassy.uncropped) { // Hasselblad H6D-100c, A6D-100c
      left_margin = 64;
      width = 11608;
      top_margin = 108;
      height = raw_height - top_margin;
  ...
  if (load_raw == &LibRaw::sinar_4shot_load_raw) {
    ...
    if (!strcmp(model, "DC-LX100M2") && raw_height == 3568 &&
        raw_width == 4816 && filters == 3)
      filters = 4;
  ...
  } else if (makeIs(LIBRAW_CAMERAMAKER_Contax) && !strcmp(model, "N Digital")) {
    height = 2047;
    width = 3072;
    filters = 0x61616161;
    data_offset = 0x1a00;
    ...
    if (!strcmp(model, "DSC-F828")) { // Sony DSC-F828
    ...
    } else if (raw_width == 4288) { // Sony ILCE-7S, ILCE-7SM2, ILCE-7SM3,
                                    // DSLR-A700, DSLR-A500;
      width -= 32;
      ...
      // ILCE-6500, ILCE-6400;
      width -= 24;
      if (strstr(normalized_model, "RX1") || strstr(normalized_model, "A99"))
        width -= 6;
      ...
      /* FIXME: need APS-C samples, both losslesscompressed and uncompressed or
       * lossy */
    }
