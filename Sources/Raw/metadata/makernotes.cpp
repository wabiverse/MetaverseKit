...
void LibRaw::parseSigmaMakernote(int base, int /*uptag*/,
                                 unsigned /*dng_writer*/) {
  unsigned wb_table1[] = {LIBRAW_WBI_Auto,     LIBRAW_WBI_Daylight,
                          LIBRAW_WBI_Shade,    LIBRAW_WBI_Cloudy,
                          LIBRAW_WBI_Tungsten, LIBRAW_WBI_Fluorescent,
                          LIBRAW_WBI_Flash,    LIBRAW_WBI_Custom,
                          LIBRAW_WBI_Custom1,  LIBRAW_WBI_Custom2};
    ...
    if (tag == 0x0027) {
      ...
      if ((len >= tblsz) && (len % 3 == 0) && len / 3 <= tblsz) {
        for (i = 0; i < (len / 3); i++) {
          ...
          icWBC[wb_table1[i]][1] = icWBC[wb_table1[i]][3] =
              (int)(getreal(type) * 10000.0);
...
void LibRaw::parse_makernote_0xc634(int base, int uptag, unsigned dng_writer) {
  ...
  if (!strcmp(buf, "OLYMPUS") || !strcmp(buf, "PENTAX ") ||
    ...
    order = 0x4949;
    fseek(ifp, 2, SEEK_CUR);
  } else if (!strcmp(buf, "OLYMP") || !strcmp(buf, "Ricoh")) {
    fseek(ifp, -2, SEEK_CUR);
  ...
  if (!strncasecmp(make, "SONY", 4) || !strncasecmp(make, "Konica", 6) ||
  ...
  while (entries--) {
    ...
    if (!strncmp(make, "Canon", 5)) {
      ...
      } else if ((tag == 0x002a) && tagtypeIs(LIBRAW_EXIFTAG_TYPE_SRATIONAL) &&
                 (len == 12) && imHassy.SensorUnitConnector[0]) {
        FORC4 for (int i = 0; i < 3; i++) imHassy.mnColorMatrix[c][i] =
            getreal(type);

      } else if ((tag == 0x0031) && imHassy.SensorUnitConnector[0]) {
      ...
      if (!OlympusDNG_SubDirOffsetValid &&
          ((len > 4) ||
           ...
           ((tagtypeIs(LIBRAW_EXIFTAG_TYPE_LONG) ||
             tagtypeIs(LIBRAW_EXIFTAG_TYPE_SLONG)) &&
            (len > 1)) ||
...
void LibRaw::parse_makernote(int base, int uptag) {
  ...
  */
  ...
  nf:
    order = 0x4949;
    fseek(ifp, 2, SEEK_CUR);
  } else if (!strcmp(buf, "OLYMP") || !strncmp(buf, "LEICA", 5) ||
             !strcmp(buf, "Ricoh")) {
    fseek(ifp, -2, SEEK_CUR);
  ...
  if (!is_Sony &&
      (!strncasecmp(make, "SONY", 4) || !strncasecmp(make, "Konica", 6) ||
       !strncasecmp(make, "Minolta", 7) ||
       (!strncasecmp(make, "Hasselblad", 10) &&
        (!strncasecmp(model, "Stellar", 7) || !strncasecmp(model, "Lunar", 5) ||
         !strncasecmp(model, "Lusso", 5) || !strncasecmp(model, "HV", 2))))) {
  ...
  while (entries--) {
    ...
    if (imKodak.MakerNoteKodak8a) {
      if ((tag == 0xff00) && tagtypeIs(LIBRAW_EXIFTAG_TYPE_LONG) &&
          (len == 1)) {
        INT64 _pos1 = get4();
      ...
      if (!strncmp(model, "GR", 2) || !strncmp(model, "GXR", 3)) {
      ...
      if ((tag == 0xb028) && (len == 1) &&
          tagtypeIs(LIBRAW_EXIFTAG_TYPE_LONG)) // DSLR-A100
      {
        if ((c = get4())) {
    ...
    if (!strncasecmp(make, "Hasselblad", 10) && !is_Sony) {
      if (tag == 0x0011)
        ...
        imHassy.CoatingCode = getint(type);
               ...
               (len == 12)) {
        FORC4 for (int ii = 0; ii < 3; ii++) imHassy.mnColorMatrix[c][ii] =
            getreal(type);
    ...
    if ((tag == 0x0015) && tagtypeIs(LIBRAW_EXIFTAG_TYPE_ASCII) &&
        is_raw) { // Hasselblad
      stmread(imHassy.SensorUnitConnector, len, ifp);
    }
    ...
    if ((tag == 0x0088) && // Minolta, possibly Olympus too
        tagtypeIs(LIBRAW_EXIFTAG_TYPE_LONG) && (thumb_offset = get4()))
      thumb_offset += base;
    ...
    if (tagtypeIs(LIBRAW_EXIFTAG_TYPE_UNDEFINED) && // Nikon
        ((tag == 0x008c) || (tag == 0x0096))) {
      meta_offset = ftell(ifp);
    }

    if ((tag == 0x00a1) && tagtypeIs(LIBRAW_EXIFTAG_TYPE_UNDEFINED) &&
