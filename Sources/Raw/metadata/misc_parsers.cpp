...
void LibRaw::parse_rollei() {
  ...
  do {
    ...
    if (!strcmp(line, "CUTRECT")) {
      sscanf(val, "%hu %hu %hu %hu", &imgdata.sizes.raw_inset_crops[0].cleft,
  ...
  if (!fseek(ifp, 0xB0 - int(sizeof(brcm_tag)), SEEK_CUR) &&
    ...
    if (header.format == BRCM_FORMAT_BAYER) {
      ...
      case BRCM_BAYER_RAW10:
        ...
        raw_stride =
            (((((header.h_width + header.padding_right) * 5) + 3) >> 2) + 31) &
            (~31);
        width = header.h_width;
        raw_height = height = header.h_height;
        is_raw = 1;
      ...
      case BRCM_BAYER_RAW12:
        ...
        raw_stride =
            (((((header.h_width + header.padding_right) * 3) + 1) >> 1) + 31) &
            (~31);
        width = header.h_width;
        raw_height = height = header.h_height;
        is_raw = 1;
      ...
      case BRCM_BAYER_RAW14:
        ...
        raw_stride =
            (((((header.h_width + header.padding_right) * 7) + 3) >> 2) + 31) &
            (~31);
        width = header.h_width;
        raw_height = height = header.h_height;
        is_raw = 1;
      ...
      case BRCM_BAYER_RAW16:
        ...
        // 1 pixel per 2 bytes
        raw_stride =
            (((header.h_width + header.padding_right) << 1) + 31) & (~31);
        width = header.h_width;
        raw_height = height = header.h_height;
        is_raw = 1;
        order = 0x4d4d;
