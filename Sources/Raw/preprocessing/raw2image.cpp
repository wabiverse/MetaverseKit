...
int LibRaw::raw2image(void) {
  ...
  try {
    ...
    // Copy area size
    int copyheight =
        MAX(0, MIN(int(S.height), int(S.raw_height) - int(S.top_margin)));
    int copywidth =
        MAX(0, MIN(int(S.width), int(S.raw_width) - int(S.left_margin)));
...
#pragma omp parallel for schedule(dynamic) default(none) firstprivate(cblack)  \
    shared(dmaxp)
#endif
  for (int row = 0; row < int(S.raw_height) - int(S.top_margin) * 2; row++) {
    int col;
    unsigned short ldmax = 0;
    for (col = 0; col < IO.fuji_width << int(
                            !libraw_internal_data.unpacker_data.fuji_layout) &&
                  col + int(S.left_margin) < int(S.raw_width);
         col++) {
      unsigned r, c;
...
#if defined(LIBRAW_USE_OPENMP)
#pragma omp parallel for schedule(dynamic) default(none) shared(dmaxp)         \
    firstprivate(cblack, maxHeight)
#endif
