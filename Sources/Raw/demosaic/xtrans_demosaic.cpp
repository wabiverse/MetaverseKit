...
#pragma omp parallel for schedule(dynamic) default(none)                       \
    firstprivate(buffers, allhex, passes, sgrow, sgcol, ndir) shared(dir)
#endif
    ...
    for (int left = 3; left < width - 19; left += LIBRAW_AHD_TILE - 16) {
      ...
      for (int pass = 0; pass < passes; pass++) {
        ...
        for (int row = (top - sgrow + 4) / 3 * 3 + sgrow; row < mrow - 2;
             row += 3)
          for (int col = (left - sgcol + 4) / 3 * 3 + sgcol; col < mcol - 2;
               col += 3) {
            ...
            for (int i = 1, d = 0; d < 6;
                 d++, i ^= LIBRAW_AHD_TILE ^ 1, h ^= 2) {
              for (c = 0; c < 2; c++, h ^= 2) {
                int g = 2 * rix[0][1] - rix[i << c][1] - rix[-i << c][1];
                color[h][d] = g + rix[i << c][h] + rix[-i << c][h];
                if (d > 1)
                  diff[d] +=
                      SQR((float)rix[i << c][1] - (float)rix[-i << c][1] -
                          (float)rix[i << c][h] + (float)rix[-i << c][h]) +
                      SQR((float)g);
              }
              if (d > 1 && (d & 1))
                if (diff[d - 1] < diff[d])
        ...
        for (int row = top + 3; row < mrow - 3; row++)
          for (int col = left + 3; col < mcol - 3; col++) {
            ...
            for (int d = 0; d < 4;
                 d++, rix += LIBRAW_AHD_TILE * LIBRAW_AHD_TILE) {
              int i = d > 1 || ((d ^ c) & 1) ||
                              ((ABS(rix[0][1] - rix[c][1]) +
                                ABS(rix[0][1] - rix[-c][1])) <
            ...
            for (int col = left + 2; col < mcol - 2; col++)
              if ((col - sgcol) % 3) {
                ...
                for (int d = 0; d < 8;
                  ...
                  if (hex[d] + hex[d + 1]) {
                    int g =
                        3 * rix[0][1] - 2 * rix[hex[d]][1] - rix[hex[d + 1]][1];
                    for (c = 0; c < 4; c += 2)
                      rix[0][c] = CLIP(
