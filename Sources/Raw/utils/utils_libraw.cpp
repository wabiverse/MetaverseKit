...
int LibRaw::is_jpeg_thumb() {
  return libraw_internal_data.unpacker_data.thumb_format ==
...
int LibRaw::find_ifd_by_offset(int o) {
  for (unsigned i = 0; i < libraw_internal_data.identify_data.tiff_nifds &&
                       i < LIBRAW_IFD_MAXCOUNT;
       i++)
...
short LibRaw::tiff_sget(unsigned save, uchar *buf, unsigned buf_len,
                        INT64 *tag_offset, unsigned *tag_id, unsigned *tag_type,
                        INT64 *tag_dataoffset, unsigned *tag_datalen,
                        int *tag_dataunitlen) {
  uchar *pos = buf + *tag_offset;
  if ((((*tag_offset) + 12) > buf_len) ||
  ...
  *tag_datalen = sget4(pos);
  pos += 4;
  *tag_dataunitlen =
      tagtype_dataunit_bytes[(*tag_type <= LIBRAW_EXIFTAG_TYPE_IFD8) ? *tag_type
                                                                     : 0];
  if ((*tag_datalen * (*tag_dataunitlen)) > 4) {
    *tag_dataoffset = sget4(pos) - save;
    if ((*tag_dataoffset + *tag_datalen) >
        buf_len) { // abnormal, tag data buffer overrun
      return -2;
    }
  } else
...

{
  ...
  if (adjindex >= 0) {
    ...
    RS.width = S.width =
        MIN(rICC[adjindex].cwidth, int(S.raw_width) - int(S.left_margin));
    RS.height = S.height =
        MIN(rICC[adjindex].cheight, int(S.raw_height) - int(S.top_margin));
  }
  return adjindex + 1;
}
