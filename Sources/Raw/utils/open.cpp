...
int LibRaw::open_file(const wchar_t *, INT64) { return LIBRAW_NOT_IMPLEMENTED; }
#else
...
#if defined(WIN32) || defined(_WIN32)
#ifndef LIBRAW_WIN32_UNICODEPATHS
int LibRaw::open_file(const wchar_t *) { return LIBRAW_NOT_IMPLEMENTED; }
#else
...
int LibRaw::open_datastream(LibRaw_abstract_datastream *stream) {
  ...
  try {
    ...
    // promote the old single thumbnail to the thumbs_list if not present
    // already
    if (imgdata.thumbs_list.thumbcount < LIBRAW_THUMBNAIL_MAXCOUNT) {
      bool already = false;
      if (imgdata.thumbnail.tlength ||
          libraw_internal_data.internal_data.toffset)
        for (int i = 0; i < imgdata.thumbs_list.thumbcount; i++)
          if (imgdata.thumbs_list.thumblist[i].toffset ==
                  libraw_internal_data.internal_data.toffset &&
              imgdata.thumbs_list.thumblist[i].tlength ==
      ...
      if (!already) {
        ...
        imgdata.thumbs_list.thumblist[idx].toffset =
            libraw_internal_data.internal_data.toffset;
        imgdata.thumbs_list.thumblist[idx].tlength = imgdata.thumbnail.tlength;
        imgdata.thumbs_list.thumblist[idx].tflip = 0xffff;
        imgdata.thumbs_list.thumblist[idx].tformat =
            libraw_internal_data.unpacker_data.thumb_format;
        imgdata.thumbs_list.thumblist[idx].tmisc =
            libraw_internal_data.unpacker_data.thumb_misc;
        // promote if set
    ...
    imgdata.lens.Lens[sizeof(imgdata.lens.Lens) - 1] =
        0; // make sure lens is 0-terminated
...
#define isRIC imgdata.sizes.raw_inset_crops[0]
    ...
    if (!imgdata.idata.dng_version && makeIs(LIBRAW_CAMERAMAKER_Canon) &&
        ...
        && isRIC.cwidth < (imgdata.sizes.raw_width * 4 /
                           5)) // less than 80% of sensor width
    {
      isRIC.cleft = isRIC.ctop = 0xffff;
      isRIC.cwidth = isRIC.cheight = 0;
    }
    ...
    if (!imgdata.idata.dng_version &&
        (makeIs(LIBRAW_CAMERAMAKER_Sony) &&
         !strcmp(imgdata.idata.normalized_model, "DSC-F828")) &&
    ...
    if (load_raw == &LibRaw::lossless_jpeg_load_raw && MN.canon.RecordMode &&
        makeIs(LIBRAW_CAMERAMAKER_Kodak) &&
        /* Not normalized models here, it is intentional */
        (!strncasecmp(imgdata.idata.model, "EOS D2000",
                      9) || // if we want something different for B&W cameras,
         !strncasecmp(imgdata.idata.model, "EOS D6000",
                      9))) // it's better to compare with CamIDs
    {
    ...
    if (load_raw == &LibRaw::panasonic_load_raw) {
      ...
      if (libraw_internal_data.unpacker_data.pana_encoding == 6) {
        ...
        if (!ds)
          ds = libraw_internal_data.internal_data.input->size() -
...
#undef NIKON_14BIT_SIZE
    ...
    if (makeIs(LIBRAW_CAMERAMAKER_Sony) && imgdata.color.maximum > 0 &&
    ...
    if (makeIs(LIBRAW_CAMERAMAKER_Canon)) {
      if (MN.canon.DefaultCropAbsolute.l !=
          -1) // tag 0x00e0 SensorInfo was parsed
      {
        if (imgdata.sizes.raw_aspect !=
            LIBRAW_IMAGE_ASPECT_UNKNOWN) { // tag 0x009a AspectInfo was parsed
          isRIC.cleft += MN.canon.DefaultCropAbsolute.l;
          isRIC.ctop += MN.canon.DefaultCropAbsolute.t;
        } else {
          isRIC.cleft = MN.canon.DefaultCropAbsolute.l;
          isRIC.ctop = MN.canon.DefaultCropAbsolute.t;
          isRIC.cwidth = MN.canon.DefaultCropAbsolute.r -
                         MN.canon.DefaultCropAbsolute.l + 1;
          isRIC.cheight = MN.canon.DefaultCropAbsolute.b -
...
#undef isRIC
      if (imgdata.color.raw_bps < 14 && !imgdata.idata.dng_version &&
          load_raw != &LibRaw::canon_sraw_load_raw) {
        int xmax = (1 << imgdata.color.raw_bps) - 1;
        if (MN.canon.SpecularWhiteLevel >
            xmax) // Adjust 14-bit metadata to real bps
        {
    ...
    if (makeIs(LIBRAW_CAMERAMAKER_Canon) &&
      ...
      if ((ratio < 0.57 || ratio > 0.75) && MN.canon.SensorHeight > 1 &&
        ...
        if ((imgdata.sizes.raw_width * (imgdata.sizes.raw_height * 7ul)) / 4 ==
          ...
          parse_fuji_compressed_header();
                 ...
                 libraw_internal_data.internal_data.input->size() <
                     (libraw_internal_data.unpacker_data.data_size +
                      libraw_internal_data.unpacker_data.data_offset)) {
        parse_fuji_compressed_header(); // try to use compressed header: X-H2S
                                        // may record wrong data size
      }
    }
    // set raw_inset_crops[1] via raw_aspect
    if (imgdata.sizes.raw_aspect >=
            LIBRAW_IMAGE_ASPECT_MINIMAL_REAL_ASPECT_VALUE &&
        imgdata.sizes.raw_aspect <=
            LIBRAW_IMAGE_ASPECT_MAXIMAL_REAL_ASPECT_VALUE
        /* crops[0] is valid*/
        ...
        imgdata.sizes.raw_inset_crops[0].cwidth > 0 &&
        imgdata.sizes.raw_inset_crops[0].cheight > 0
        /* crops[1] is not set*/
        && (imgdata.sizes.raw_inset_crops[1].cleft == 0xffff) &&
        (imgdata.sizes.raw_inset_crops[1].ctop == 0xffff)) {
      float c0_ratio = float(imgdata.sizes.raw_inset_crops[0].cwidth) /
                       float(imgdata.sizes.raw_inset_crops[0].cheight);
      float c1_ratio = float(imgdata.sizes.raw_aspect) / 1000.f;
      if (c0_ratio / c1_ratio < 0.98 ||
          c0_ratio / c1_ratio > 1.02) // set crops[1]
      {
        if (c1_ratio >
            c0_ratio) // requested image is wider, cut from top/bottom
        {
          int newheight =
              int(imgdata.sizes.raw_inset_crops[0].cwidth / c1_ratio);
          int dtop = (imgdata.sizes.raw_inset_crops[0].cheight - newheight) / 2;
          imgdata.sizes.raw_inset_crops[1].ctop =
              imgdata.sizes.raw_inset_crops[0].ctop + dtop;
          ...
          int newwidth =
              int(imgdata.sizes.raw_inset_crops[0].cheight * c1_ratio);
          int dleft = (imgdata.sizes.raw_inset_crops[0].cwidth - newwidth) / 2;
          imgdata.sizes.raw_inset_crops[1].cleft =
              imgdata.sizes.raw_inset_crops[0].cleft + dleft;
          imgdata.sizes.raw_inset_crops[1].cwidth = newwidth;
          imgdata.sizes.raw_inset_crops[1].ctop =
              imgdata.sizes.raw_inset_crops[0].ctop;
          imgdata.sizes.raw_inset_crops[1].cheight =
              imgdata.sizes.raw_inset_crops[0].cheight;
        }
      }
    }
    ...
    if (adjust_margins) // adjust crop_inset margins
      for (int i = 0; i < 2; i++) {
        if (imgdata.sizes.raw_inset_crops[i].cleft &&
            imgdata.sizes.raw_inset_crops[i].cleft < 0xffff &&
            imgdata.sizes.raw_inset_crops[i].cwidth &&
            imgdata.sizes.raw_inset_crops[i].cwidth < 0xffff &&
            (imgdata.sizes.raw_inset_crops[i].cleft % adjust_margins) &&
            (imgdata.sizes.raw_inset_crops[i].cwidth > adjust_margins)) {
          int newleft =
              ((imgdata.sizes.raw_inset_crops[i].cleft / adjust_margins) + 1) *
              adjust_margins;
        ...
        if (imgdata.sizes.raw_inset_crops[i].ctop &&
            imgdata.sizes.raw_inset_crops[i].ctop < 0xffff &&
            imgdata.sizes.raw_inset_crops[i].cheight &&
            imgdata.sizes.raw_inset_crops[i].cheight < 0xffff &&
            (imgdata.sizes.raw_inset_crops[i].ctop % adjust_margins) &&
            (imgdata.sizes.raw_inset_crops[i].cheight > adjust_margins)) {
          int newtop =
              ((imgdata.sizes.raw_inset_crops[i].ctop / adjust_margins) + 1) *
              adjust_margins;
    ...
    if (imgdata.rawparams.use_dngsdk &&
        !(imgdata.rawparams.options &
          (LIBRAW_RAWOPTIONS_DNG_STAGE2 | LIBRAW_RAWOPTIONS_DNG_STAGE3 |
           LIBRAW_RAWOPTIONS_DNG_DISABLEWBADJUST)))
#endif
    {
      // Fix DNG white balance if needed: observed only for Kalpanika X3F tools
      // produced DNGs
    ...
    if (imgdata.idata.dng_version && makeIs(LIBRAW_CAMERAMAKER_Panasonic) &&
    ...
    if (makeIs(LIBRAW_CAMERAMAKER_Sony) && imgdata.idata.dng_version) {
      if (S.raw_width == 3984)
        S.width = 3925;
    ...
    if (makeIs(LIBRAW_CAMERAMAKER_Sony) && !imgdata.idata.dng_version) {
      ...
      if (((!strncasecmp(imgdata.idata.model, "ILCE-7RM", 8) ||
           ...
           (S.raw_width == 5216 ||
            S.raw_width == 6304)) // A7RM2/M3/A99M2 in APS mode; A7RM4 in APS-C
          ||
...
#endif
            (mnCamID == SonyID_SLT_A99)) // 2 reasons: some cameras are SLT-A99,
                                         // no 'V'; some are Hasselblad HV
           && S.raw_width > 3750 &&
    ...
    // FIXME: it is possible that DNG contains 4x main frames + some previews;
    // in this case imgdata.idata.raw_count will be greater than 4
    ...
    if (load_raw == &LibRaw::packed_load_raw &&
        makeIs(LIBRAW_CAMERAMAKER_Sony)) // 12 bit sony, but metadata may be for
                                         // 14-bit range
    {
      if (C.maximum > 4095)
        C.maximum = 4095;
    ...
    else if (makeIs(LIBRAW_CAMERAMAKER_Canon) && MN.canon.multishot[0] >= 8 &&
             MN.canon.multishot[1] > 0)
      imgdata.color.as_shot_wb_applied =
          LIBRAW_ASWB_APPLIED | LIBRAW_ASWB_CANON;
    else if (makeIs(LIBRAW_CAMERAMAKER_Nikon) && MN.nikon.ExposureMode == 1)
      imgdata.color.as_shot_wb_applied =
    ...
    if (makeIs(LIBRAW_CAMERAMAKER_Nikon) && (!C.linear_max[0]) &&
        (C.maximum > 1024) && (load_raw != &LibRaw::nikon_load_sraw)) {
    ...
    if (
      ...
      for (int cnt = LIBRAW_WBI_Unknown; cnt <= LIBRAW_WBI_StudioTungsten;
           cnt++) {
      ...
      for (int cnt = 0; cnt < 4; cnt++)
        imgdata.color.pre_mul[cnt] = C.WB_Coeffs[LIBRAW_WBI_Daylight][cnt];
    }

    // Adjust BL for Panasonic
    if (load_raw == &LibRaw::panasonic_load_raw &&
        makeIs(LIBRAW_CAMERAMAKER_Panasonic) && ID.pana_black[0] &&
        ID.pana_black[1] && ID.pana_black[2]) {
