...
void LibRaw::kodak_thumb_loader() {
  ...
  if (INT64(T.theight) * INT64(T.twidth) >
      1024ULL * 1024ULL * LIBRAW_MAX_THUMBNAIL_MB)
    throw LIBRAW_EXCEPTION_IO_CORRUPT;
...
#define Tformat libraw_internal_data.unpacker_data.thumb_format
  ...
  float out[3],
      out_cam[3][4] = {{2.81761312f, -1.98369181f, 0.166078627f, 0},
  ...
  if (imgdata.rawparams.options &
      LIBRAW_RAWOPTIONS_NO_ROTATE_FOR_KODAK_THUMBNAILS)
...
int LibRaw::thumbOK(INT64 maxsz) {
  ...
  if (Tformat == LIBRAW_INTERNAL_THUMBNAIL_JPEG)
    ...
    tsize =
        tcol * T.twidth * T.theight *
        ((imgdata.rawparams.options & LIBRAW_RAWOPTIONS_USE_PPM16_THUMBS) ? 2
                                                                          : 1);
#ifdef USE_X3FTOOLS
