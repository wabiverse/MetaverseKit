...
int LibRaw::unpack_thumb_ex(int idx) {
  if (idx < 0 || idx >= imgdata.thumbs_list.thumbcount ||
      idx >= LIBRAW_THUMBNAIL_MAXCOUNT)
...
#define THUMB_SIZE_CHECKT(A)                                                   \
  do {                                                                         \
    if (INT64(A) > 1024LL * 1024LL * LIBRAW_MAX_THUMBNAIL_MB)                  \
      return LIBRAW_UNSUPPORTED_THUMBNAIL;                                     \
    if (INT64(A) > 0 && INT64(A) < 64LL)                                       \
...
#define THUMB_SIZE_CHECKWH(W, H)                                               \
  do {                                                                         \
    if (INT64(W) * INT64(H) > 1024ULL * 1024ULL * LIBRAW_MAX_THUMBNAIL_MB)     \
      return LIBRAW_UNSUPPORTED_THUMBNAIL;                                     \
    if (INT64(W) * INT64(H) < 64ULL)                                           \
      return LIBRAW_NO_THUMBNAIL;                                              \
  } while (0)
    ...
    if (!ID.toffset &&
...
#ifdef USE_6BY9RPI
        && !(imgdata.thumbnail.tlength > 0 &&
             libraw_internal_data.unpacker_data.load_flags & 0x4000 &&
      ...
      if (Tformat == LIBRAW_INTERNAL_THUMBNAIL_JPEG) {
        ...
        if (load_raw == &LibRaw::crxLoadRaw && T.tlength > 0xE0) {
          // Check if it is canon H.265 preview:  CISZ at bytes 4-6, CISZ prefix
          // is 000n
          if (tthumb[0] == 0 && tthumb[1] == 0 && tthumb[2] == 0 &&
              !memcmp(tthumb + 4, "CISZ", 4)) {
        ...
        if (T.tlength &&
            (int)T.tlength <
                t_length) // try to find tiff ifd with needed offset
        {
          int pifd =
              find_ifd_by_offset(libraw_internal_data.internal_data.toffset);
        ...
        if (t_bytesps > 2)
          throw LIBRAW_EXCEPTION_IO_CORRUPT; // 16-bit thumb, but parsed for
                                             // more bits
        int o_bps =
            (imgdata.rawparams.options & LIBRAW_RAWOPTIONS_USE_PPM16_THUMBS)
                ? 2
                : 1;
