...
struct tile_stripe_data_t {
  ...
  tile_stripe_data_t()
      : tiled(false), striped(false), tileCnt(0), tileWidth(0), tileHeight(0),
        tilesH(0), tilesV(0), maxBytesInTile(0) {}
  void init(tiff_ifd_t *ifd, const libraw_image_sizes_t &,
            const unpacker_data_t &, short _order,
...
void tile_stripe_data_t::init(tiff_ifd_t *ifd,
                              const libraw_image_sizes_t &sizes,
                              const unpacker_data_t &unpacker_data,
                              short _order,
                              LibRaw_abstract_datastream *stream) {
  tiled = (unpacker_data.tile_width <= sizes.raw_width) &&
          (unpacker_data.tile_length <= sizes.raw_height);
  striped =
      (ifd->rows_per_strip > 0 && ifd->rows_per_strip < sizes.raw_height) &&
      ifd->strip_byte_counts_count > 0;

  tileWidth = tiled ? unpacker_data.tile_width : sizes.raw_width;
  tileHeight = tiled ? unpacker_data.tile_length
                     : (striped ? ifd->rows_per_strip : sizes.raw_height);
  tilesH = tiled ? (sizes.raw_width + tileWidth - 1) / tileWidth : 1;
  tilesV = tiled ? (sizes.raw_height + tileHeight - 1) / tileHeight
                 : (striped ? ((sizes.raw_height + ifd->rows_per_strip - 1) /
                               ifd->rows_per_strip)
                            : 1);
...
void LibRaw::deflate_dng_load_raw() {
  ...
  if (ifd->sample_format == 3)
    float_raw_image = (float *)calloc(tiles.tileCnt * tiles.tileWidth *
  ...
  for (size_t y = 0, t = 0; y < imgdata.sizes.raw_height;
    ...
    for (size_t x = 0; x < imgdata.sizes.raw_width; x += tiles.tileWidth, ++t) {
      libraw_internal_data.internal_data.input->seek(tiles.tOffsets[t],
                                                     SEEK_SET);
      libraw_internal_data.internal_data.input->read(cBuffer.data(), 1,
                                                     tiles.tBytes[t]);
      unsigned long dstLen = tileBytes;
      int err = uncompress(uBuffer.data() + tileRowBytes, &dstLen,
                           cBuffer.data(), (unsigned long)tiles.tBytes[t]);
      if (err != Z_OK) {
        throw LIBRAW_EXCEPTION_DECODE_RAW;
        return;
      } else {
        ...
        {
          unsigned char *dst =
              uBuffer.data() + row * tiles.tileWidth * bytesps * ifd->samples;
          unsigned char *src = dst + tileRowBytes;
          DecodeFPDelta(src, dst, tiles.tileWidth / xFactor,
                        ifd->samples * xFactor, bytesps);
          float lmax =
              expandFloats(dst, tiles.tileWidth * ifd->samples, bytesps);
          max = MAX(max, lmax);
...
void LibRaw::convertFloatToInt(float dmin /* =4096.f */,
  ...
  if (tmax < dmin || tmax > dmax) {
    ...
    for (int i = 0; i < int(sizeof(imgdata.color.cblack) /
                            sizeof(imgdata.color.cblack[0]));
         i++)
      if (i != 4 && i != 5)
...
#endif
    void
    swap32(uchar *data, int len) {
  unsigned *d = (unsigned *)data;
  for (int i = 0; i < len / 4; i++) {
    unsigned x = d[i];
    d[i] = (x << 24) + ((x << 8) & 0x00FF0000) + ((x >> 8) & 0x0000FF00) +
           (x >> 24);
  }
}
...
void LibRaw::uncompressed_fp_dng_load_raw() {
  ...
  INT64 allocsz = INT64(tiles.tileCnt) * INT64(tiles.tileWidth) *
  ...
  bool difford = (libraw_internal_data.unpacker_data.order == 0x4949) ==
                 (ntohs(0x1234) == 0x1234);
  float max = 0.f;

  std::vector<uchar> rowbuf(
      tiles.tileWidth * sizeof(float) *
      ifd->samples); // line buffer for last tile in tile row

  for (size_t y = 0, t = 0; y < imgdata.sizes.raw_height;
       y += tiles.tileHeight) {
    for (unsigned x = 0;
         x < imgdata.sizes.raw_width && t < (unsigned)tiles.tileCnt;
         x += tiles.tileWidth, ++t) {
      libraw_internal_data.internal_data.input->seek(tiles.tOffsets[t],
                                                     SEEK_SET);
      size_t rowsInTile = y + tiles.tileHeight > imgdata.sizes.raw_height
      ...
      {
        unsigned char *dst =
            fullrowbytes > inrowbytes ? rowbuf.data()
        ...
        else if (bytesps == 3 && (libraw_internal_data.unpacker_data.order ==
                                  0x4949)) // II-16bit
        ...
        if (fullrowbytes >
            inrowbytes) // last tile in row: copy buffer to destination
          memmove(&float_raw_image[((y + row) * imgdata.sizes.raw_width + x) *
                                   ifd->samples],
                  dst, outrowbytes);
        max = MAX(max, lmax);
      }
