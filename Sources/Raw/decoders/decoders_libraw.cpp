 ...
 *
 * PhaseOne IIQ-Sv2 decoder is inspired by code provided by Daniel Vogelbacher
<daniel@chaospixel.com>
 *
...
static inline void unpack7bytesto4x16(unsigned char *src,
                                      unsigned short *dest) {
...
#define wb(i) ((unsigned short)buffer[lastoffset + 15 - i])
  pixelbuffer[0] =
      (wb(0) << 4) | (wb(1) >> 4); // 12 bit: 8/0 + 4 upper bits of /1
  pixelbuffer[1] =
      (((wb(1) & 0xf) << 8) | (wb(2))) & 0xfff; // 12 bit: 4l/1 + 8/2

  pixelbuffer[2] = (wb(3) >> 6) & 0x3; // 2; 2u/3, 6 low bits remains in wb(3)
  pixelbuffer[3] = ((wb(3) & 0x3f) << 2) |
                   (wb(4) >> 6); // 8; 6l/3 + 2u/4; 6 low bits remains in wb(4)
  pixelbuffer[4] = ((wb(4) & 0x3f) << 2) |
                   (wb(5) >> 6); // 8: 6l/4 + 2u/5; 6 low bits remains in wb(5)
  pixelbuffer[5] = ((wb(5) & 0x3f) << 2) |
                   (wb(6) >> 6); // 8: 6l/5 + 2u/6, 6 low bits remains in wb(6)

  pixelbuffer[6] = (wb(6) >> 4) & 0x3; // 2, 4 low bits remains in wb(6)
  pixelbuffer[7] =
      ((wb(6) & 0xf) << 4) |
      (wb(7) >> 4); // 8: 4 low bits from wb(6), 4 upper bits from wb(7)
  pixelbuffer[8] =
      ((wb(7) & 0xf) << 4) |
      (wb(8) >> 4); // 8: 4 low bits from wb7, 4 upper bits from wb8
  pixelbuffer[9] =
      ((wb(8) & 0xf) << 4) |
      (wb(9) >> 4); // 8: 4 low bits from wb8, 4 upper bits from wb9

  pixelbuffer[10] =
      (wb(9) >> 2) & 0x3; // 2: bits 2-3 from wb9, two low bits remain in wb9
  pixelbuffer[11] = ((wb(9) & 0x3) << 6) |
                    (wb(10) >> 2); // 8: 2 bits from wb9, 6 bits from wb10
  pixelbuffer[12] = ((wb(10) & 0x3) << 6) |
                    (wb(11) >> 2); // 8: 2 bits from wb10, 6 bits from wb11
  pixelbuffer[13] = ((wb(11) & 0x3) << 6) |
                    (wb(12) >> 2); // 8: 2 bits from wb11, 6 bits from wb12
...
void decode_S_type(int32_t out_width, uint32_t *img_input,
                   ushort *outbuf /*, int bit_depth*/) {
#if 0
  ...
  const uint8_t bit_indicator[8 * 4] = {
      9, 8, 0, 7, 6, 6, 5, 5, 1, 1, 1, 1, 4, 4, 4, 4,
      3, 3, 3, 3, 3, 3, 3, 3, 2, 2, 2, 2, 2, 2, 2, 2,
  };

  const uint8_t skip_bits[8 * 4] = {5, 5, 5, 5, 4, 4, 4, 4, 3, 3, 3,
  ...
  if (out_width - 7 > 0) {
    ...
    for (int blk_id = 0; blk_id < block_count; ++blk_id) {
      ...
      uint32_t bp_shift[2] = {bit_check[0] - extra_bits[bidx],
                              bit_check[1] - extra_bits[bidx]};

      int pix_sub[2] = {0xFFFF >> (pix_sub_init - bit_check[0]),
      ...
      for (int i = 0; i < 8; i++) // MAIN LOOP for pixel decoding
      {
        ...
        if (bit_check[i & 1] == 9)
          ...
          value = prev_pix_value[i & 1] +
                  ((uint32_t)stream.get(take_bits) << bp_shift[i & 1]) -
                  pix_sub[i & 1];
...
void LibRaw::phase_one_load_raw_s() {
  if (!libraw_internal_data.unpacker_data.strip_offset ||
  ...
  libraw_internal_data.internal_data.input->seek(
      libraw_internal_data.unpacker_data.strip_offset, SEEK_SET);
  for (unsigned row = 0; row < imgdata.sizes.raw_height; row++) {
    stripes[row].row = row;
    stripes[row].offset =
        INT64(get4()) + libraw_internal_data.unpacker_data.data_offset;
  }
  stripes[imgdata.sizes.raw_height].row = imgdata.sizes.raw_height;
  stripes[imgdata.sizes.raw_height].offset =
      libraw_internal_data.unpacker_data.data_offset +
      INT64(libraw_internal_data.unpacker_data.data_size);
  std::sort(stripes.begin(), stripes.end());
  INT64 maxsz = imgdata.sizes.raw_width * 3 +
                2; // theor max: 17 bytes per 8 pix + row header
  ...
  for (unsigned row = 0; row < imgdata.sizes.raw_height; row++) {
    if (stripes[row].row >= imgdata.sizes.raw_height)
      continue;
    ushort *datap =
        imgdata.rawdata.raw_image + stripes[row].row * imgdata.sizes.raw_width;
    libraw_internal_data.internal_data.input->seek(stripes[row].offset,
                                                   SEEK_SET);
    INT64 readsz = stripes[row + 1].offset - stripes[row].offset;
    if (readsz > maxsz)
      throw LIBRAW_EXCEPTION_IO_CORRUPT;

    if (libraw_internal_data.internal_data.input->read(datavec.data(), 1,
                                                       readsz) != readsz)
      derror(); // TODO: check read state

    decode_S_type(imgdata.sizes.raw_width, (uint32_t *)datavec.data(),
                  datap /*, 14 */);
  }
}
