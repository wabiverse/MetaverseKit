...
void init_main_grads(const fuji_compressed_params *params,
...
void LibRaw::init_fuji_block(fuji_compressed_block *info,
...
void LibRaw::copy_line_to_xtrans(fuji_compressed_block *info, int cur_line,
...
void LibRaw::copy_line_to_bayer(fuji_compressed_block *info, int cur_line,
                                int cur_block, int cur_block_width) {
  ushort *lineBufB[3];
  ushort *lineBufG[6];
  ushort *lineBufR[3];
...
static inline void fuji_read_code(fuji_compressed_block *info, int *data,
                                  int bits_to_read) {
...
static inline int fuji_decode_sample_even(fuji_compressed_block *info,
                                          const fuji_compressed_params *params,
                                          ushort *line_buf, int pos,
                                          fuji_grads *grad_params) {
  int interp_val = 0;
  // ushort decBits;
  int errcnt = 0;
...
static inline int fuji_decode_sample_odd(fuji_compressed_block *info,
                                         const fuji_compressed_params *params,
                                         ushort *line_buf, int pos,
                                         fuji_grads *grad_params) {
  int interp_val = 0;
...
void LibRaw::xtrans_decode_block(fuji_compressed_block *info,
                                 const fuji_compressed_params *params,
                                 int /*cur_line*/) {
...
void LibRaw::fuji_bayer_decode_block(fuji_compressed_block *info,
                                     const fuji_compressed_params *params,
                                     int /*cur_line*/) {
  int r_even_pos = 0, r_odd_pos = 1;
  int g_even_pos = 0, g_odd_pos = 1;
...
void LibRaw::fuji_decode_strip(fuji_compressed_params *params, int cur_block,
                               INT64 raw_offset, unsigned dsize,
                               uchar *q_bases) {
  ...
  if (!libraw_internal_data.unpacker_data.fuji_lossless) {
    int buf_size = sizeof(fuji_compressed_params) +
                   (2 << libraw_internal_data.unpacker_data.fuji_bits);
...
void LibRaw::fuji_compressed_load_raw() {
  ...
  if (libraw_internal_data.internal_data.input->read(
          block_sizes, 1, sizesToRead) != sizesToRead) {
    free(block_sizes);
  ...
  if (!libraw_internal_data.unpacker_data.fuji_lossless) {
    ...
    libraw_internal_data.internal_data.input->seek(
        raw_offset + libraw_internal_data.unpacker_data.data_offset, SEEK_SET);
  ...
  fuji_decode_loop(&common_info,
                   libraw_internal_data.unpacker_data.fuji_total_blocks,
                   raw_block_offsets, block_sizes, q_bases);
...
void LibRaw::fuji_decode_loop(fuji_compressed_params *common_info, int count,
                              INT64 *raw_block_offsets, unsigned *block_sizes,
                              uchar *q_bases) {
  int cur_block;
  const int lineStep =
      (libraw_internal_data.unpacker_data.fuji_total_lines + 0xF) & ~0xF;
...
void LibRaw::parse_fuji_compressed_header() {
  unsigned signature, lossless, h_raw_type, h_raw_bits, h_raw_height,
  ...
  libraw_internal_data.internal_data.input->seek(
      libraw_internal_data.unpacker_data.data_offset, SEEK_SET);
  if (libraw_internal_data.internal_data.input->read(
          header, 1, sizeof(header)) != sizeof(header))
    return;
  ...
  // general validation
  if (signature != 0x4953 || lossless > 1 || h_raw_height > 0x4002 ||
      h_raw_height < 6 || h_raw_height % 6 || h_block_size < 1 ||
      h_raw_width > 0x4200 || h_raw_width < 0x300 || h_raw_width % 24 ||
      h_raw_rounded_width > 0x4200 || h_raw_rounded_width < h_block_size ||
      ...
      h_total_lines > 0xAAB || h_total_lines == 0 ||
