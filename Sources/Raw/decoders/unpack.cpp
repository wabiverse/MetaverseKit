  ...
  try {
    ...
    if (imgdata.idata.dng_version && dnghost &&
        libraw_internal_data.unpacker_data.tiff_samples !=
            2 // Fuji SuperCCD; it is better to detect is more rigid way
...
#endif
#ifdef USE_RAWSPEED3
    if (!raw_was_read() && (!IO.fuji_width) // Do not use for fuji rotated
        &&
        ((imgdata.idata.raw_count == 1)
         // Canon dual pixel, 1st frame
         || (makeIs(LIBRAW_CAMERAMAKER_Canon) && imgdata.idata.raw_count == 2 &&
      ...
      if (_rawspeed3_handle &&
          ID.input->size() > 0) // large bound is checked at identify
      {
        ...
        try {
          ...
          int status = rawspeed3_decodefile(_rawspeed3_handle, &rs3ret,
                                            _rawspeed_buffer, ID.input->size(),
#ifdef USE_RAWSPEED_BITS
                                            !(imgdata.rawparams.use_rawspeed &
                                              LIBRAW_RAWSPEEDV3_FAILONUNKNOWN)
#else
                                            false
#endif
          );
          ...
          if (status == rawspeed3_ok
#ifdef USE_RAWSPEED_BITS
              || (status == rawspeed3_ok_warnings &&
                  (imgdata.rawparams.use_rawspeed &
                   LIBRAW_RAWSPEEDV3_IGNOREERRORS))
#endif
          ) {

            if ((S.raw_width != rs3ret.width) ||
    ...
    if (!raw_was_read()) {
      ...
      if (imgdata.idata.dng_version &&
          (libraw_internal_data.unpacker_data.tiff_samples == 2 ||
           imgdata.idata.raw_count > 1))
        rawspeed_enabled = 0;
      ...
      if (makeIs(LIBRAW_CAMERAMAKER_Canon) &&
          (libraw_internal_data.identify_data.unique_id ==
           CanonID_EOS_6D_Mark_II))
        rawspeed_enabled = 0;
      ...
      if (load_raw == &LibRaw::nikon_load_raw &&
          ...
          (!strncasecmp(imgdata.idata.model, "Z", 1) ||
           !strncasecmp(imgdata.idata.model, "D780", 4)))
        rawspeed_enabled = 0;
    ...
    if (!raw_was_read()) // RawSpeed failed or not run
    {
      ...
      if (load_raw == &LibRaw::unpacked_load_raw &&
          (!strcasecmp(imgdata.idata.make, "Nikon") ||
           !strcasecmp(imgdata.idata.make, "Hasselblad")))
        C.maximum = 65535;
      (this->*load_raw)();
      if (zero_rawimage)
        imgdata.rawdata.raw_image = 0;
