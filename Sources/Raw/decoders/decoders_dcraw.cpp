    ...
    for (ecol = slice = 0; slice <= cr2_slice[0]; slice++) {
      ...
      for (row = 0; row < height; row += (jh.clrs >> 1) - 1) {
        ...
        for (col = scol; col < ecol; col += 2, jcol += jh.clrs) {
          ...
          } else if (imgdata.rawparams.specials &
    ...
    if (!(imgdata.rawparams.specials & LIBRAW_RAWSPECIAL_SRAW_NO_RGB))
      for (; rp < ip[0]; rp += 4) {
        ...
        if ((unique_id == CanonID_EOS_5D_Mark_II) ||
            (unique_id == CanonID_EOS_7D) || (unique_id == CanonID_EOS_50D) ||
...
void LibRaw::nikon_read_curve() {
  ...
  if ((csize = get2()) > 1)
    step = max / (csize - 1);
  if (ver0 == 0x44 && (ver1 == 0x20 || (ver1 == 0x40 && step > 3)) &&
      step > 0) {
    if (ver1 == 0x40) {
      step /= 4;
      max /= 4;
...
void LibRaw::nokia_load_raw() {
  ...
  maximum = 0x3ff;
#ifdef USE_6BY9RPI
  if (!strcmp(make, "OmniVision") || !strcmp(make, "Sony") ||
      !strcmp(make, "RaspberryPi"))
    return;
#else
...
void LibRaw::quicktake_100_load_raw() {
  ...
  for (row = 2; row < height + 2; row++) {
    ...
    for (col = 2 + (row & 1); col < width + 2; col += 2) {
      val = ((pixel[(row - 1) * 644 + col - 1] +
              2 * pixel[(row - 1) * 644 + col + 1] +
              pixel[row * 644 + col - 2]) >>
             2) +
            gstep[getbits(4)];
      ...
      if (row == 2)
        pixel[(row - 1) * 644 + col + 1] = pixel[(row - 1) * 644 + col + 3] =
            val;
    }
    ...
    for (row = 2 + rb; row < height + 2; row += 2) {
      ...
      for (col = 3 - (row & 1); col < width + 2; col += 2) {
        if (row < 4 || col < 4)
          ...
          val = ABS(pixel[(row - 2) * 644 + col] - pixel[row * 644 + col - 2]) +
                ABS(pixel[(row - 2) * 644 + col] -
                    pixel[(row - 2) * 644 + col - 2]) +
                ABS(pixel[row * 644 + col - 2] -
                    pixel[(row - 2) * 644 + col - 2]);
          sharp = val < 4    ? 0
                  : val < 8  ? 1
                  : val < 16 ? 2
        ...
        val =
            ((pixel[(row - 2) * 644 + col] + pixel[row * 644 + col - 2]) >> 1) +
  ...
  for (row = 2; row < height + 2; row++) {
    checkCancel();
    for (col = 3 - (row & 1); col < width + 2; col += 2) {
      val = ((pixel[row * 644 + col - 1] + (pixel[row * 644 + col] << 2) +
              pixel[row * 644 + col + 1]) >>
             1) -
            0x100;
  ...
  try {
    for (row = 0; row < height; row++) {
      ...
      for (dp = data, col = 0; col < raw_width - 30; dp += 16) {
        ...
        if (!(imgdata.rawparams.specials &
              LIBRAW_RAWSPECIAL_SONYARW2_ALLFLAGS) // no flag set
            || (imgdata.rawparams.specials &
