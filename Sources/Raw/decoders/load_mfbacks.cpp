...
inline uint32_t min32(uint32_t x, uint32_t y) { return x < y ? x : y; }

inline uint32_t max32(uint32_t x, uint32_t y) { return x > y ? x : y; }
...
void LibRaw::phase_one_fix_pixel_grad(unsigned row, unsigned col) {
  static const int8_t grad_sets[7][12][2] = {{{-4, -2},
                                              {4, 2},
                                              {-3, -1},
                                              {1, 1},
                                              {-1, -1},
  ...
  for (int i = 0; i < 7; ++i) {
    ...
    for (int j = 0; j < 12; j += 2)
      grad[i] += abs32(
          p1raw(row + grad_sets[i][j][0], col + grad_sets[i][j][1]) -
...
int LibRaw::phase_one_correct() {
  ...
  try {
    while (entries--) {
      ...
      apply: /* apply to whole image */
      ...
      } else if (tag == 0x041f && !qlin_applied) { /* Quadrant linearization */
        ...
        qlin_applied = 1;
      } else if (tag == 0x041e && !qmult_applied) { /* Quadrant multipliers -
                                                       output calibraion */
        ...
        for (row = 0; row < raw_height; row++) {
          ...
          for (col = 0; col < raw_width; col++) {
            i = qmult[row >= (unsigned)ph1.split_row]
                     [col >= (unsigned)ph1.split_col] *
                RAW(row, col);
            RAW(row, col) = LIM(i, 0, 65535);
          }
        }
        qmult_applied = 1;
      } else if (tag == 0x0431 && !qmult_applied) { /* Quadrant combined - four
                                                       tile gain calibration */
    ...
    if (!badCols.empty()) {
      ...
      for (i = 0; i < (int)badCols.size(); ++i) {
        bool nextIsolated =
            i == ((int)(badCols.size() - 1)) || badCols[i + 1] > badCols[i] + 4;
        for (row = 0; row < raw_height; ++row)
...
void LibRaw::hasselblad_load_raw() {
  struct jhead jh;
  int shot, row, col, *back[5] = {0, 0, 0, 0, 0}, len[2], diff[12], pred, sh, f,
                      c;
  unsigned s;
