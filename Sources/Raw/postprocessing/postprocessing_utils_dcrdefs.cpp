...
void LibRaw::convert_to_rgb() {
  ...
  if (!raw_color) {
    ...
    int i, j, k;
    prof_desc_len =
        snprintf(NULL, 0, "%s gamma %g toe slope %g", name[output_color - 1],
                 floorf(1000.f / gamm[0] + .5f) / 1000.f,
                 floorf(gamm[1] * 1000.0f + .5f) / 1000.f) +
        1;
    prof_desc.resize(prof_desc_len);
    sprintf(prof_desc.data(), "%s gamma %g toe slope %g",
            name[output_color - 1], floorf(1000.f / gamm[0] + .5f) / 1000.f,
            floorf(gamm[1] * 1000.0f + .5f) / 1000.f);
...
void LibRaw::scale_colors() {
  ...
  if (use_auto_wb ||
      (use_camera_wb &&
       (cam_mul[0] < -0.5 // LibRaw 0.19 and older: fallback to auto only if
                          // cam_mul[0] is set to -1
        || (cam_mul[0] <= 0.00001f // New default: fallback to auto if no
                                   // cam_mul parsed from metadata
