...
#ifdef USE_DNGSDK
static dng_ifd *search_single_ifd(const std::vector<dng_ifd *> &v,
                                  uint64 offset, int &idx, dng_stream &stream) {
  idx = -1;
...
static dng_ifd *search_for_ifd(const dng_info &info, uint64 offset, ushort w,
                               ushort h, int &ifdIndex, dng_stream &stream) {
  dng_ifd *ret = 0;
  ret = search_single_ifd(info.fIFD, offset, ifdIndex, stream);
  int dummy;
  if (!ret)
    ret = search_single_ifd(info.fChainedIFD, offset, dummy, stream);
  if (!ret) {
...
int LibRaw::valid_for_dngsdk() {
  ...
  if (libraw_internal_data.unpacker_data.tiff_compress == 34892 &&
    ...
    int ifdindex = -1;
    dng_ifd *rawIFD = search_for_ifd(
        info, libraw_internal_data.unpacker_data.data_offset,
        imgdata.sizes.raw_width, imgdata.sizes.raw_height, ifdindex, stream);
...
#ifdef USE_GPRSDK
  if (load_raw == &LibRaw::vc5_dng_load_raw_placeholder) // regardless of flags
                                                         // or use_dngsdk value!
    return 1;
#endif
  ...
  if (!imgdata.idata.filters &&
      (imgdata.rawparams.use_dngsdk & LIBRAW_DNG_LINEAR))
    return 1;
...
int LibRaw::try_dngsdk() {
  ...
  try {
    ...
    //(new dng_simple_image(rawIFD->Bounds(), rawIFD->fSamplesPerPixel,
    //rawIFD->PixelType(), host->Allocator()));
    ...
    if (((libraw_internal_data.unpacker_data.tiff_compress == 34892 &&
         ...
         ((tiff_ifd[ifdindex].dng_levels.parsedfields &
           (LIBRAW_DNGFM_OPCODE2 | LIBRAW_DNGFM_OPCODE3)) &&
          (imgdata.rawparams.options &
           (LIBRAW_RAWOPTIONS_DNG_STAGE2_IFPRESENT |
            LIBRAW_RAWOPTIONS_DNG_STAGE3_IFPRESENT)))) &&
        ifdindex >= 0) {
    ...
    if (ptype == ttShort && !stage23used && !is_curve_linear()) {
      ...
      // Alloc
      if ((imgdata.rawparams.options & LIBRAW_RAWOPTIONS_DNGSDK_ZEROCOPY) &&
          !stage23used) {
        zerocopy = true;
      } else {
        imgdata.rawdata.raw_alloc = malloc(pixels * TagTypeSize(ptype));
  ...
  return (dngnegative || imgdata.rawdata.raw_alloc) ? LIBRAW_SUCCESS
