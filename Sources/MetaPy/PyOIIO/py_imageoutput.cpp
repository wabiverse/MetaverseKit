...
bool ImageOutput_write_scanline(ImageOutput &self, int y, int z,
                                py::buffer &buffer) {
...
bool ImageOutput_write_scanlines(ImageOutput &self, int ybegin, int yend, int z,
                                 py::buffer &buffer) {
  const ImageSpec &spec(self.spec());
  if (spec.tile_width != 0) {
  ...
  oiio_bufinfo buf(buffer.request(), spec.nchannels, spec.width, yend - ybegin,
                   1, 2);
  if (!buf.data || buf.error.size()) {
    self.errorfmt("Pixel data array error: {}",
                  buf.error.size() ? buf.error.c_str() : "unspecified");
...
bool ImageOutput_write_tile(ImageOutput &self, int x, int y, int z,
                            py::buffer &buffer) {
  const ImageSpec &spec(self.spec());
  if (spec.tile_width == 0) {
...
bool ImageOutput_write_tiles(ImageOutput &self, int xbegin, int xend,
  ...
  py::gil_scoped_release gil;
  return self.write_tiles(xbegin, xend, ybegin, yend, zbegin, zend, buf.format,
                          buf.data, buf.xstride, buf.ystride, buf.zstride);
}

bool ImageOutput_write_image(ImageOutput &self, py::buffer &buffer) {
  const ImageSpec &spec(self.spec());
...
bool ImageOutput_write_deep_scanlines(ImageOutput &self, int ybegin, int yend,
                                      int z, const DeepData &deepdata) {
  py::gil_scoped_release gil;
  return self.write_deep_scanlines(ybegin, yend, z, deepdata);
}

bool ImageOutput_write_deep_tiles(ImageOutput &self, int xbegin, int xend,
                                  int ybegin, int yend, int zbegin, int zend,
...
bool ImageOutput_write_deep_image(ImageOutput &self, const DeepData &deepdata) {
  py::gil_scoped_release gil;
