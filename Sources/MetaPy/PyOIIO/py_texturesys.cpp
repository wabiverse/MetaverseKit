...
public:
  ...
  py::tuple get_missingcolor() const {
    return C_to_tuple(m_missingcolor_data.data(), m_missingcolor_data.size());
  }
  ...
  py::class_<TextureSystemWrap>(m, "TextureSystem")
      ...
      .def(
          ...
          [](const TextureSystemWrap &ts, const std::string &filename,
             TextureOptWrap &options, float s, float t, float dsdx, float dtdx,
            ...
            float *result = OIIO_ALLOCA(float, nchannels);
            {
              ...
              ts.m_texsys->texture(ustring(filename), options, s, t, dsdx, dtdx,
                                   dsdy, dtdy, nchannels, result);
            }
            return C_to_tuple(result, nchannels);
          },
          "filename"_a, "options"_a, "s"_a, "t"_a, "dsdx"_a, "dtdx"_a, "dsdy"_a,
          "dtdy"_a, "nchannels"_a)
          ...
          [](const TextureSystemWrap &ts, const std::string &filename,
            ...
            float *result = OIIO_ALLOCA(float, nchannels);
            {
              ...
              ts.m_texsys->texture3d(
                  ustring(filename), options, Imath::V3f(P[0], P[1], P[2]),
                  Imath::V3f(dPdx[0], dPdx[1], dPdx[2]),
                  Imath::V3f(dPdy[0], dPdy[1], dPdy[2]),
                  Imath::V3f(dPdz[0], dPdz[1], dPdz[2]), nchannels, result);
            }
            return C_to_tuple(result, nchannels);
          },
      ...
      .def(
          ...
          [](const TextureSystemWrap &ts, const std::string &filename,
            ...
            {
              py::gil_scoped_release gil;
              ts.m_texsys->environment(
                  ustring(filename), options, Imath::V3f(R[0], R[1], R[2]),
                  Imath::V3f(dRdx[0], dRdx[1], dRdx[2]),
                  Imath::V3f(dRdy[0], dRdy[1], dRdy[2]), nchannels, result);
            }
            return C_to_tuple(result, nchannels);
          },
      ...
      .def(
          ...
          [](TextureSystemWrap &ts, const std::string &filename) {
            ...
            ts.m_texsys->inventory_udim(ustring(filename), filenames, nutiles,
                                        nvtiles);
