...
static ImageBuf ImageBuf_from_buffer(const py::buffer &buffer) {
  ImageBuf ib;
  ...
  if (info.ndim == 3) {
    ...
    ib.errorfmt("ImageBuf-from-numpy-array must have 2, 3, or 4 dimensions");
  }
  return ib;
}

py::tuple ImageBuf_getpixel(const ImageBuf &buf, int x, int y, int z = 0,
...
py::tuple ImageBuf_interppixel(const ImageBuf &buf, float x, float y,
                               const std::string &wrapname = "black") {
  ImageBuf::WrapMode wrap = ImageBuf::WrapMode_from_string(wrapname);
  int nchans = buf.nchannels();
  float *pixel = OIIO_ALLOCA(float, nchans);
...
py::tuple ImageBuf_interppixel_NDC(const ImageBuf &buf, float x, float y,
                                   const std::string &wrapname = "black") {
  ImageBuf::WrapMode wrap = ImageBuf::WrapMode_from_string(wrapname);
  int nchans = buf.nchannels();
  float *pixel = OIIO_ALLOCA(float, nchans);
...
py::tuple ImageBuf_interppixel_bicubic(const ImageBuf &buf, float x, float y,
                                       const std::string &wrapname = "black") {
  ImageBuf::WrapMode wrap = ImageBuf::WrapMode_from_string(wrapname);
  int nchans = buf.nchannels();
  float *pixel = OIIO_ALLOCA(float, nchans);
...
void ImageBuf_setpixel(ImageBuf &buf, int x, int y, int z, py::object p) {
...
void ImageBuf_setpixel2(ImageBuf &buf, int x, int y, py::object p) {
  ImageBuf_setpixel(buf, x, y, 0, p);
}

void ImageBuf_setpixel1(ImageBuf &buf, int i, py::object p) {
  std::vector<float> pixel;
  py_to_stdvector(pixel, p);
  if (pixel.size())
...
py::object ImageBuf_get_pixels(const ImageBuf &buf, TypeDesc format,
                               ROI roi = ROI::All()) {
...
void ImageBuf_set_deep_value(ImageBuf &buf, int x, int y, int z, int c, int s,
                             float value) {
  buf.set_deep_value(x, y, z, c, s, value);
}

void ImageBuf_set_deep_value_uint(ImageBuf &buf, int x, int y, int z, int c,
                                  int s, uint32_t value) {
  buf.set_deep_value(x, y, z, c, s, value);
}

bool ImageBuf_set_pixels_buffer(ImageBuf &self, ROI roi, py::buffer &buffer) {
  if (!roi.defined())
    roi = self.roi();
  ...
  oiio_bufinfo buf(buffer.request(), roi.nchannels(), roi.width(), roi.height(),
  ...
  if (!buf.data || buf.size != size) {
    self.errorfmt("ImageBuf.set_pixels: array size ({}) did not match ROI size "
                  "w={} h={} d={} ch={} (total {})",
...
void ImageBuf_set_write_format(ImageBuf &self,
...
void declare_imagebuf(py::module &m) {
  ...
  py::class_<ImageBuf>(m, "ImageBuf")
      ...
      .def(
          "init_spec",
          [](ImageBuf &self, std::string filename, int subimage, int miplevel) {
            py::gil_scoped_release gil;
      ...
      .def(
          "read",
          [](ImageBuf &self, int subimage, int miplevel, int chbegin, int chend,
      ...
      .def("spec", &ImageBuf::spec, py::return_value_policy::reference_internal)
      ...
      .def_property_readonly(
          "name", [](const ImageBuf &self) { return PY_STR(self.name()); })
      .def_property_readonly(
          "file_format_name",
          [](const ImageBuf &self) { return PY_STR(self.file_format_name()); })
      ...
      .def("interppixel_bicubic", &ImageBuf_interppixel_bicubic, "x"_a, "y"_a,
           "wrap"_a = "black")
      .def("interppixel_bicubic_NDC", &ImageBuf_interppixel_bicubic_NDC, "x"_a,
           "y"_a, "wrap"_a = "black")
      ...
      .def("deep_insert_samples", &ImageBuf::deep_insert_samples, "x"_a, "y"_a,
