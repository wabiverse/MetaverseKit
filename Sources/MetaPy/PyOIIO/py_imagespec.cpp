...
static py::tuple ImageSpec_get_channelformats(const ImageSpec &spec,
...
static void ImageSpec_set_channelformats(ImageSpec &spec,
                                         const py::object &py_channelformats) {
  spec.channelformats.clear();
  py_to_stdvector(spec.channelformats, py_channelformats);
}

static py::tuple ImageSpec_get_channelnames(const ImageSpec &spec) {
  return C_to_tuple(cspan<std::string>(spec.channelnames));
}

static void ImageSpec_set_channelnames(ImageSpec &spec,
                                       const py::object &py_channelnames) {
  spec.channelnames.clear();
  py_to_stdvector(spec.channelnames, py_channelnames);
}

static py::object ImageSpec_getattribute_typed(const ImageSpec &spec,
  ...
  py::class_<ImageSpec>(m, "ImageSpec")
      ...
      .def(
          ...
          [](ImageSpec &spec, const std::string &name, TypeDesc type,
             const py::object &obj) { attribute_typed(spec, name, type, obj); })
      // .def("attribute", [](ImageSpec &spec, const std::string &name, TypeDesc
      // type, const py::list &obj) {
      ...
      .def(
          ...
          [](const ImageSpec &spec, const std::string &name,
             const std::string &def) {
            return py::bytes(std::string(spec.get_string_attribute(name, def)));
          },
          "name"_a, "defaultval"_a = "")
