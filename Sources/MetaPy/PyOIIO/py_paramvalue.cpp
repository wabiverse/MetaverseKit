...
static ParamValue ParamValue_from_pyobject(string_view name, TypeDesc type,
                                           int nvalues,
                                           ParamValue::Interp interp,
                                           const py::object &obj) {
  size_t expected_size = size_t(type.numelements() * type.aggregate * nvalues);
  ParamValue pv;
...
// Based on attribute_typed in PyOIIO.h, but with nvalues.
template <typename T, typename POBJ>
bool attribute_typed(T &myobj, string_view name, TypeDesc type, int nvalues,
                     const POBJ &dataobj) {
  if (type.basetype == TypeDesc::INT) {
    std::vector<int> vals;
...
void declare_paramvalue(py::module &m) {
  ...
  py::class_<ParamValue>(m, "ParamValue")
      .def_property_readonly(
          "name",
          [](const ParamValue &self) { return PY_STR(self.name().string()); })
#if OIIO_VERSION_LESS(3, 0, 0)
      .def_property_readonly(
          "type",
          [](const ParamValue &self) { return PY_STR(self.type().c_str()); })
  ...
  // by changing it until 3.0. It should really look like this:
      ...
      .def(py::init([](const std::string &name, TypeDesc type, int nvalues,
             ...
             return ParamValue_from_pyobject(name, type, nvalues, interp, obj);
           }),
  ...
  py::class_<ParamValueList>(m, "ParamValueList")
      ...
      .def(
          ...
          [](ParamValueList &self, const std::string &name, TypeDesc type,
             const py::object &obj) { attribute_typed(self, name, type, obj); })
      .def("attribute", [](ParamValueList &self, const std::string &name,
                           TypeDesc type, int nvalues, const py::object &obj) {
