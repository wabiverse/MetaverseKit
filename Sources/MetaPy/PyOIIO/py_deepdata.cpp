...
void DeepData_init(DeepData &dd, int npix, int nchan,
                   py::object py_channeltypes, py::object py_channelnames) {
...
void DeepData_init_spec(DeepData &dd, const ImageSpec &spec) {
  py::gil_scoped_release gil;
  dd.init(spec);
}

void DeepData_set_nsamples(DeepData &dd, int pixel, int nsamples) {
  dd.set_samples(pixel, uint32_t(nsamples));
}

void DeepData_set_deep_value_float(DeepData &dd, int pixel, int channel,
                                   int sample, float value) {
  dd.set_deep_value(pixel, channel, sample, value);
}

void DeepData_set_deep_value_uint(DeepData &dd, int pixel, int channel,
...
void declare_deepdata(py::module &m) {
  ...
  py::class_<DeepData>(m, "DeepData")
      ...
      .def_property_readonly(
          "Zback_channel", [](const DeepData &d) { return d.Zback_channel(); })
      ...
      .def(
          "capacity",
          [](const DeepData &dd, int pixel) { return (int)dd.capacity(pixel); },
          "pixel"_a)
      ...
      .def("erase_samples", &DeepData::erase_samples, "pixel"_a, "samplepos"_a,
           "nsamples"_a = 1)
      .def("channelname",
           [](const DeepData &dd, int c) { return PY_STR(dd.channelname(c)); })
