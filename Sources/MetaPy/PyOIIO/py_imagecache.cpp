...
public:
  ...
  py::object get_pixels(const std::string &filename, int subimage, int miplevel,
                        int xbegin, int xend, int ybegin, int yend, int zbegin,
                        int zend, TypeDesc datatype);
};
...
py::object ImageCacheWrap::get_pixels(const std::string &filename_,
                                      ...
                                      int zbegin, int zend, TypeDesc datatype) {
  ...
  bool ok;
  {
    ...
    ok = m_cache->get_pixels(filename, subimage, miplevel, xbegin, xend, ybegin,
                             yend, zbegin, zend, datatype, data.get());
  }
  if (ok)
...
void declare_imagecache(py::module &m) {
  ...
  py::class_<ImageCacheWrap>(m, "ImageCache")
      ...
      .def(
          "getattribute",
          [](const ImageCacheWrap &ic, const std::string &name, TypeDesc type) {
            if (type == TypeUnknown)
      ...
      .def(
          ...
          "filename"_a, "subimage"_a = 0, "miplevel"_a = 0, "native"_a = false)
      ...
      .def_property_readonly(
          "has_error",
          [](ImageCacheWrap &self) { return self.m_cache->has_error(); })
      .def(
          "geterror",
