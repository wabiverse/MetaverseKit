...
oiio_bufinfo::oiio_bufinfo(const py::buffer_info &pybuf, int nchans, int width,
  ...
  if (pixeldims == 3) {
    ...
    } else if (pybuf.ndim == 1 && pybuf.shape[0] == int64_t(width) *
                                                        int64_t(height) *
                                                        int64_t(nchans)) {
...
py::object make_pyobject(const void *data, TypeDesc type, int nvalues,
...
static py::object oiio_getattribute_typed(const std::string &name,
...
OIIO_DECLARE_PYMODULE(PYMODULE_NAME) {
  ...
  m.def("attribute",
        [](const std::string &name, float val) { OIIO::attribute(name, val); });
  m.def("attribute",
