  ...
  namespace OPENVDB_VERSION_NAME
  {
    namespace tools
    {
      ...
      namespace point_partitioner_internal
      {
        ...
        struct ComputePointOrderOp
        {
          ComputePointOrderOp(PointIndexType *pointOrder,
              ...
              : mPointOrder(pointOrder), mBucketCounters(bucketCounters), mBucketOffsets(bucketOffsets)
          {
          }
        ...
        struct CreateOrderedPointIndexArrayOp
        {
          CreateOrderedPointIndexArrayOp(PointIndexType *orderedIndexArray,
              ...
              : mOrderedIndexArray(orderedIndexArray), mPointOrder(pointOrder), mIndices(indices)
          {
          }
        ...
        struct VoxelOrderOp
        {
          ...
          VoxelOrderOp(IndexArray &indices, const IndexArray &pages, const VoxelOffsetArray &offsets)
              : mIndices(indices.get()), mPages(pages.get()), mVoxelOffsets(offsets.get())
          {
          }
        ...
        struct MergeBinsOp
        {
          ...
          MergeBinsOp(IndexPairListMapPtr *bins,
                      ...
                      Coord *coords,
                      size_t numSegments)
              : mBins(bins), mIndexSegments(indexSegments), mOffsetSegments(offsetSegments), mCoords(coords), mNumSegments(numSegments)
          {
          }
        ...
        private:
          struct CopyData
          {
            CopyData(IndexPairListPtr **indexLists,
                ...
                : mIndexLists(indexLists), mArrayOffsets(arrayOffsets), mIndices(indices), mOffsets(offsets)
            {
            }
        ...
        struct BinPointIndicesOp
        {
          ...
          BinPointIndicesOp(IndexPairListMapPtr *data,
              ...
              : mData(data), mPoints(&points), mVoxelOffsets(voxelOffsets), mXForm(m), mBinLog2Dim(binLog2Dim), mBucketLog2Dim(bucketLog2Dim), mNumSegments(numSegments), mCellCenteredTransform(cellCenteredTransform)
          {
          }
          ...
          void operator()(const tbb::blocked_range<size_t> &range) const
          {
            ...
            for (size_t n = range.begin(), N = range.end(); n != N; ++n)
            {
              ...
              if (!dataPtr)
                dataPtr.reset(new IndexPairListMap());
              ...
              for (size_t i = start; i != end; ++i)
              {
                ...
                {
                  ijk = cellCentered ? mXForm.worldToIndexCellCentered(pos) : mXForm.worldToIndexNodeCentered(pos);
        ...
        struct OrderSegmentsOp
        {
          ...
          OrderSegmentsOp(SegmentPtr *indexSegments, SegmentPtr *offsetSegments,
              ...
              : mIndexSegments(indexSegments), mOffsetSegments(offsetSegments), mPageOffsetArrays(pageOffsetArrays), mPageIndexArrays(pageIndexArrays), mBinVolume(binVolume)
          {
          }
            ...
            bool cellCenteredTransform = true)
        {
          ...
          if (points.size() > (numThreads * 2))
            numTasks = numThreads * 2;
          else if (points.size() > numThreads)
            numTasks = numThreads;
            ...
            bool cellCenteredTransform)
        {
          ...
          if (recordVoxelOffsets)
            voxelOffsets.reset(new VoxelOffsetType[points.size()]);
          else
            voxelOffsets.reset();
          ...
          tbb::parallel_for(segmentRange, OrderSegmentsOp<PointIndexType>(indexSegments.get(), offsetSegments.get(),
      ...
      inline PointPartitioner<PointIndexType, BucketLog2Dim>::PointPartitioner()
          : mPointIndices(nullptr), mVoxelOffsets(nullptr), mPageOffsets(nullptr), mPageCoordinates(nullptr), mPageCount(0), mUsingCellCenteredTransform(true)
      {
      }
