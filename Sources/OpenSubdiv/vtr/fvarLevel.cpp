...
//  level.  It is typically constructed and initialized similarly to levels --
//  the base level in a Factory and subsequent levels by refinement.
//
...
FVarLevel::FVarLevel(Level const &level)
    : _level(level), _isLinear(false), _hasLinearBoundaries(false),
      _hasDependentSharpness(false), _valueCount(0) {}
...
//
//  Constructing the mapping between vertices and their face-varying values
//  involves:
//
//      - iteration through all vertices to mark edge discontinuities and
//      classify
//      - allocation of vectors mapping vertices to their multiple (sibling)
//      values
//      - iteration through all vertices and their distinct values to tag
//      topologically
//
//  Once values have been identified for each vertex and tagged, refinement
//  propagates the tags to child values using more simplified logic (child
//  values inherit the topology of their parent) and no further analysis is
//  required.
//
void FVarLevel::completeTopologyFromFaceValues(int regularBoundaryValence) {

  //
  //  Assign some members and local variables based on the interpolation options
  //  (the members support queries that are expected later):
  //
  //  Given the growing number of options and behaviors to support, this is
  //  likely going to get another pass.  It may be worth identifying the
  //  behavior for each "feature", i.e. determine smooth or sharp for corners,
  //  creases and darts, but the fact that the rule for one value may be
  //  dependent on that of another complicates this.
  //
  using Sdc::Options;

  Options::VtxBoundaryInterpolation geomOptions =
      _options.GetVtxBoundaryInterpolation();
  Options::FVarLinearInterpolation fvarOptions =
      _options.GetFVarLinearInterpolation();
  ...
  _hasDependentSharpness =
      (fvarOptions == Options::FVAR_LINEAR_CORNERS_PLUS1) ||
      (fvarOptions == Options::FVAR_LINEAR_CORNERS_PLUS2);

  bool geomCornersAreSmooth =
      (geomOptions != Options::VTX_BOUNDARY_EDGE_AND_CORNER);
  ...
  bool sharpenBothIfOneCorner =
      (fvarOptions == Options::FVAR_LINEAR_CORNERS_PLUS2);

  bool sharpenDarts = sharpenBothIfOneCorner || _hasLinearBoundaries;

  //
  //  It's awkward and potentially inefficient to try and accomplish everything
  //  in one pass over the vertices...
  //
  //  Make a first pass through the vertices to identify discts edges and to
  //  determine the number of values-per-vertex for subsequent allocation.  The
  //  presence of a discts edge warrants marking vertices at BOTH ends as having
  //  mismatched topology wrt the vertices (part of why full topological
  //  analysis is deferred).
  //
  //  So this first pass will allocate/initialize the overall structure of the
  //  topology. Given N vertices and M (as yet unknown) sibling values, the
  //  first pass achieves the following:
  //
  //      - assigns a local vector indicating which of the N vertices "match"
  //          - requires a single value but must also have no discts incident
  //          edges
  //      - determines the number of values associated with each of the N
  //      vertices
  ...
  //
  //  The second pass initializes remaining members based on the total number of
  //  siblings M after allocating appropriate vectors dependent on M.
  //
  ...
  for (int vIndex = 0; vIndex < _level.getNumVertices(); ++vIndex) {
    ...
    for (int i = 0; i < vFaces.size(); ++i) {
      vValues[i] = _faceVertValues[_level.getOffsetOfFaceVertices(vFaces[i]) +
                                   vInFace[i]];
    }

    //
    //  Inspect the incident edges of the vertex and tag those whose FVar values
    //  are discts between the two (or more) faces sharing that edge.  When
    //  manifold, we know an edge is discts when two successive fvar-values
    //  differ -- so we will make use of the local buffer of values.
    //  Unfortunately we can't infer anything about the edges for a non-manifold
    //  vertex, so that case will be more complex.
    //
    ...
    if (vIsManifold) {
      //
      //  We want to use face indices here as we are accessing the fvar-values
      //  per face.  The indexing range here maps to the interior edges for
      //  boundary and interior verts:
      //
    ...
    //
    //  While we've tagged the vertex as having mismatched FVar topology in the
    //  presence of any discts edges, we also need to account for different
    //  treatment of vertices along geometric boundaries if the FVar
    //  interpolation rules affect them.  So inspect all boundary vertices that
    //  have not already been tagged.
    //
    ...
    //
    //  Inspect the set of fvar-values around the vertex to identify the number
    //  of unique values.  While doing so, associate a "sibling index" (over the
    //  range of unique values) with each value around the vertex (this latter
    //  need makes it harder to make simple use of std::sort() and uniq() on the
    //  set of values)
    //
    ...
    //
    //  Some non-manifold cases can have multiple fvar-values but without any
    //  discts edges that would previously have identified mismatch (e.g. two
    //  faces meeting at a common vertex), so deal with that case now that we've
    //  counted values:
    //
  ...
  //
  //  Now that we know the total number of additional sibling values (M values
  //  in addition to the N vertex values) allocate space to accommodate all N +
  //  M vertex values.
  //
  //  Then make the second pass through the vertices to identify the values
  //  associated with each and to inspect and tag local face-varying topology
  ...
  for (int vIndex = 0; vIndex < _level.getNumVertices(); ++vIndex) {
    ...
    //
    //  First step is to assign the values associated with the faces by
    //  retrieving them from the faces.  If the face-varying topology around
    //  this vertex matches the vertex topology, there is little more to do as
    //  other members were bulk-initialized to match, so we can continue
    //  immediately:
    //
    IndexArray vValues = getVertexValues(vIndex);

    if (vFaces.size() > 0) {
      vValues[0] = _faceVertValues[_level.getOffsetOfFaceVertices(vFaces[0]) +
                                   vInFace[0]];
    } else {
      vValues[0] = 0;
    }
    ...
    if (vValues.size() > 1) {
      ...
      for (int i = 1, nextSibling = 1; i < vFaces.size(); ++i) {
        if (vFaceSiblings[i] == nextSibling) {
          vValues[nextSibling++] =
              _faceVertValues[_level.getOffsetOfFaceVertices(vFaces[i]) +
                              vInFace[i]];
        }
      }
    }
    ...
    //  revisited soon.  This intentionally avoids the overhead of identifying
    //  the local topology of the values along its boundaries -- necessary for
    //  smooth boundary values but not for sharp as far as refining and limiting
    //  the values is concerned.  But ultimately we need more information than
    //  just the sharp tag when it comes to identifying and gathering FVar
    //  patches.
    //
    ...
    //  Until then...
    //
    //  If all values for this vertex are to be designated as sharp, the value
    //  tags have already been initialized for this by default, so we can
    //  continue.  On further inspection there may be other cases where all are
    //  determined to be sharp, but use what information we can now to avoid
    //  that inspection:
    //
    //  Regarding sharpness of the vertex itself, its vertex tags reflect the
    //  inf- or semi-sharp nature of the vertex and edges around it, so be
    //  careful not to assume too much from say, the presence of an incident
    //  inf-sharp edge. We can make clear decisions based on the sharpness of
    //  the vertex itself.
    //
    ...
    bool allCornersAreSharp =
        _hasLinearBoundaries || vTag._infSharp || vTag._nonManifold ||
    ...
    //
    //  Spans are identified as sharp or smooth based on their own local
    //  topology, but the sharpness of one span may be dependent on the
    //  sharpness of another if certain linear-interpolation options were
    //  specified.  Mark both as infinitely sharp where possible (rather than
    //  semi-sharp) to avoid re-assessing this dependency as sharpness is
    //  reduced during refinement.
    //
    ...
    if (!allCornersAreSharp) {
      if (_hasDependentSharpness && (vValues.size() == 2)) {
        ...
        allCornersAreSharp = vValueSpans[0]._infSharpEdgeCount ||
                             vValueSpans[1]._infSharpEdgeCount ||
                             vValueSpans[0]._disctsEdgeCount ||
                             vValueSpans[1]._disctsEdgeCount;

        //  Detect a sharp corner, making both sharp:
        if (sharpenBothIfOneCorner) {
          allCornersAreSharp |=
              (vValueSpans[0]._size == 1) || (vValueSpans[1]._size == 1);
        }

        //  If only one semi-sharp, need to mark the other as dependent on it:
        hasDependentValuesToSharpen =
            (vValueSpans[0]._semiSharpEdgeCount > 0) !=
            (vValueSpans[1]._semiSharpEdgeCount > 0);
      }
    }

    //
    //  Inspect each vertex value to determine if it is a smooth boundary
    //  (crease) and tag it accordingly.  If not semi-sharp, be sure to consider
    //  those values sharpened by the topology of other values.
    //
    for (int i = 0; i < vValues.size(); ++i) {
      ...
      valueTag._infIrregular =
          vSpan._infSharpEdgeCount
              ? ((vSpan._size - vSpan._infSharpEdgeCount) > 1)
              : (isInfSharp ? (vSpan._size > 1) : valueTag._xordinary);

      if (!isInfSharp) {
        //
        //  Remember that a semi-sharp value (or one dependent on one) needs to
        //  be treated as a corner (at least three sharp edges or one sharp
        //  vertex) until the sharpness has decayed, so don't tag them as
        //  creases here. But do initialize and maintain the ends of the crease
        //  until needed.
        //
        ...
        if (hasCreaseEnds()) {
          ...
          if ((i == 0) && (vSpan._start != 0)) {
            valueCrease._endFace =
                (LocalIndex)(vSpan._start + vSpan._size - 1 - vFaces.size());
          } else {
...
//
//  Values tagged as creases have their two "end values" identified relative to
//  the incident faces of the vertex for compact storage and quick retrieval.
//  This method identifies the values for the two ends of such a crease value:
//
void FVarLevel::getVertexCreaseEndValues(Index vIndex, Sibling vSibling,
...
bool FVarLevel::validate() const {
  ...
  for (int fIndex = 0; fIndex < _level.getNumFaces(); ++fIndex) {
    ConstIndexArray fVerts = _level.getFaceVertices(fIndex);
    ConstIndexArray fValues = getFaceValues(fIndex);
    Sibling const *fSiblings =
        &fvSiblingVector[_level.getOffsetOfFaceVertices(fIndex)];
    ...
    for (int fvIndex = 0; fvIndex < fVerts.size(); ++fvIndex) {
      ...
      if (testValue != fvValue) {
        printf("Error:  unexpected value %d for sibling %d of face-vert %d.%d "
               "= %d (expecting %d)\n",
  ...
  for (int vIndex = 0; vIndex < _level.getNumVertices(); ++vIndex) {
    ...
    for (int j = 0; j < vFaces.size(); ++j) {
      ...
      if (vValue != fvValue) {
        printf("Error:  value mismatch between face-vert %d.%d and vert-face "
               "%d.%d (%d != %d)\n",
...
void FVarLevel::print() const {
  ...
  for (int i = 0; i < _level.getNumFaces(); ++i) {
    ...
    Sibling const *fSiblings =
        &fvSiblingVector[_level.getOffsetOfFaceVertices(i)];
  ...
  for (int i = 0; i < _level.getNumEdges(); ++i) {
    ...
    if (eTag._mismatch) {
      ...
      printf("    edge%4d:  verts = [%4d%4d], discts = [%d,%d]\n", i, eVerts[0],
             eVerts[1], eTag._disctsV0, eTag._disctsV1);
    }
  }
}
...
void FVarLevel::initializeFaceValuesFromFaceVertices() {
  ...
  std::memcpy(dstFaceValues, &srcFaceVerts[0],
              srcFaceVerts.size() * sizeof(Index));
}

void FVarLevel::initializeFaceValuesFromVertexFaceSiblings() {
  //
  //  Iterate through all face-values first and initialize them with the first
  //  value associated with each face-vertex.  Then make a second sparse pass
  //  through the vertex-faces to offset those with multiple values.  This turns
  //  out to be much more efficient than a single iteration through the
  //  vertex-faces since the first pass is much more memory coherent.
  //
...
void FVarLevel::buildFaceVertexSiblingsFromVertexFaceSiblings(
    std::vector<Sibling> &fvSiblings) const {

  fvSiblings.resize(_level.getNumFaceVerticesTotal());
  std::memset(&fvSiblings[0], 0,
              _level.getNumFaceVerticesTotal() * sizeof(Sibling));
  ...
  for (int vIndex = 0; vIndex < _level.getNumVertices(); ++vIndex) {
    ...
    if (getNumVertexValues(vIndex) > 1) {
      ...
      for (int j = 0; j < vFaces.size(); ++j) {
        if (vSiblings[j] > 0) {
          fvSiblings[_level.getOffsetOfFaceVertices(vFaces[j]) + vInFace[j]] =
              vSiblings[j];
        }
      }
    }
...
//    - given an edge, return values corresponding to its vertices within a
//    given face
//    - given a vertex, return values corresponding to verts at the ends of its
//    edges
//
void FVarLevel::getEdgeFaceValues(Index eIndex, int fIncToEdge,
  ...
  if ((getNumVertexValues(eVerts[0]) + getNumVertexValues(eVerts[1])) > 2) {
    ...
    valuesPerVert[1] =
        fValues[((eInFace + 1) < fValues.size()) ? (eInFace + 1) : 0];

    //  Given the way these two end-values are used (both weights the same) we
    //  really don't need to ensure the value pair matches the vertex pair...
  ...
  for (int i = 0; i < vEdges.size(); ++i) {
    ...
    if (getNumVertexValues(vOther) == 1) {
      ...
      if (vOther == fVerts[eInFace0]) {
        ...
        int valueInFace =
            (eInFace0 == (fValues.size() - 1)) ? 0 : (eInFace0 + 1);
...
//
//  The "size" (number of faces in which each value occurs), is most immediately
//  useful in determining whether a value is a corner or smooth boundary, while
//  other properties such as the first face and whether or not the span is
//  interrupted by discts, semi- sharp or infinite edges, are useful to fully
//  qualify smooth boundaries by the caller.
//
void FVarLevel::gatherValueSpans(Index vIndex, ValueSpan *vValueSpans) const {
  ...
  if (vIsNonManifold) {
    //  This needs more work as spans around a non-manifold vertex may
    //  themselves be manifold.  Just mark all spans with a discts edge for now
    //  to trigger them non-manifold
