...
FVarRefinement::FVarRefinement(Refinement const &refinement,
    ...
    : _refinement(refinement), _parentLevel(refinement.parent()),
      _parentFVar(parentFVarLevel), _childLevel(refinement.child()),
...
void FVarRefinement::applyRefinement() {
  ...
  //
  //  The refined face-values are technically redundant as they can be
  //  constructed from the face-vertex siblings -- do so here as a post-process
  //
...
//
//  Quickly estimate the memory required for face-varying vertex-values in the
//  child and allocate them.  For uniform refinement this estimate should
//  exactly match the desired result.  For sparse refinement the excess should
//  generally be low as the sparse boundary components generally occur where
//  face-varying data is continuous.
//
void FVarRefinement::estimateAndAllocateChildValues() {
  ...
  for (; cVert < cVertEnd; ++cVert) {
    ...
    maxVertexValueCount += _parentFVar.edgeTopologyMatches(pEdge)
                               ? 1
                               : _parentLevel.getEdgeFaces(pEdge).size();
  }
  ...
  //
  //  Now allocate/initialize for the maximum -- use resize() and trim the size
  //  later to avoid the constant growing with reserve() and incremental sizing.
  //  We know the estimate should be close and memory wasted should be small, so
  //  initialize all to zero as well to avoid writing in all but affected areas:
  //
...
inline int FVarRefinement::populateChildValuesForEdgeVertex(Index cVert,
                                                            Index pEdge) {
  ...
  //  have as many child values as parent faces incident the edge.  But even
  //  when refinement is sparse, if this edge-vertex is not complete, we will
  //  still be guaranteed that a child face exists for each parent face since
  //  one of the edge's end vertices must be complete and therefore include all
  //  child faces.
  //
  ...
  FVarLevel::SiblingArray cVertFaceSiblings =
      _childFVar.getVertexFaceSiblings(cVert);
  ...
  for (int i = 0; i < cVertFaceSiblings.size(); ++i) {
    ...
    if (pEdgeFaces.size() == 2) {
      ...
      //  Non-manifold case with > 2 parent faces -- match child faces to
      //  parent:
...
inline int FVarRefinement::populateChildValuesForVertexVertex(Index cVert,
                                                              Index pVert) {

  //
  //  We should not be getting incomplete vertex-vertices from feature-adaptive
  //  refinement (as neighboring vertices will be face-vertices or
  //  edge-vertices). This will get messy when we do (i.e. sparse refinement of
  //  Bilinear or more flexible and specific sparse refinement of Catmark) but
  ...
  if (cValueCount > 1) {
    ...
    FVarLevel::ConstSiblingArray pVertFaceSiblings =
        _parentFVar.getVertexFaceSiblings(pVert);
    FVarLevel::SiblingArray cVertFaceSiblings =
        _childFVar.getVertexFaceSiblings(cVert);
...
void FVarRefinement::populateChildValues() {
  ...
  //
  //  Be sure to match the same vertex ordering as Refinement, i.e.
  //  face-vertices first vs vertex-vertices first, etc.  A few optimizations
  //  within the use of face-varying data take advantage of this assumption, and
  //  it just makes sense to be consistent (e.g. if there is a 1-to-1
  //  correspondence between vertices and their FVar-values, their children will
  //  correspond).
  //
...
void FVarRefinement::populateChildValuesFromVertexVertices() {
  ...
  for (; cVert < cVertEnd; ++cVert) {
    ...
    _childFVar._vertSiblingOffsets[cVert] = _childFVar._valueCount;
    if (_parentFVar.valueTopologyMatches(
            _parentFVar.getVertexValueOffset(pVert))) {
      _childFVar._vertSiblingCounts[cVert] = 1;
      _childFVar._valueCount++;
    } else {
...
void FVarRefinement::propagateEdgeTags() {
  ...
  //              * what was this comment:  "discts based on parent face-edges
  //              at ends"
  //      Edge-edges:
  ...
  for (int eIndex = 0; eIndex < _refinement.getNumChildEdgesFromFaces();
       ++eIndex) {
    _childFVar._edgeTags[eIndex] = eTagMatch;
  }
  for (int eIndex = _refinement.getNumChildEdgesFromFaces();
       eIndex < _childLevel.getNumEdges(); ++eIndex) {
...
void FVarRefinement::propagateValueTags() {
  ...
  //
  //  Value tags correspond to vertex-values and originate from all three
  //  sources:
  //      Face-values:
  ...
  //
  //  Values from edge-vertices -- for edges that are split, tag as mismatched
  //  and tag as corner or crease depending on the presence of creases in the
  //  parent:
  //
  ...
  FVarLevel::ValueTag &valTagSplitEdge =
      _parentFVar.hasSmoothBoundaries() ? valTagCrease : valTagMismatch;
  ...
  //
  //  Values from vertex-vertices -- inherit tags from parent values when
  //  complete otherwise (not yet supported) need to identify the parent value
  //  for each child:
  //
  cVert = _refinement.getFirstChildVertexFromVertices();
  cVertEnd = cVert + _refinement.getNumChildVerticesFromVertices();
  ...
  for (; cVert < cVertEnd; ++cVert) {
    ...
    FVarLevel::ConstValueTagArray pValueTags =
        _parentFVar.getVertexValueTags(pVert);
    FVarLevel::ValueTagArray cValueTags = _childFVar.getVertexValueTags(cVert);
...
void FVarRefinement::propagateValueCreases() {
  ...
  //
  //  For each child vertex from an edge that has FVar values and is complete,
  //  initialize the crease-ends for those values tagged as smooth boundaries
  //
  //  Note that this does depend on the nature of the topological split, i.e.
  //  how many child faces are incident the new child vertex for each face that
  //  becomes a crease, so identify constants to be used in each iteration
  //  first:
  //
  ...
  for (; cVert < cVertEnd; ++cVert) {
    ...
    FVarLevel::CreaseEndPairArray cValueCreaseEnds =
        _childFVar.getVertexValueCreaseEnds(cVert);
  ...
  //
  //  For each child vertex from a vertex that has FVar values and is complete,
  //  initialize the crease-ends for those values tagged as smooth or semi-sharp
  //  (to become smooth eventually):
  //
  cVert = _refinement.getFirstChildVertexFromVertices();
  cVertEnd = cVert + _refinement.getNumChildVerticesFromVertices();
  for (; cVert < cVertEnd; ++cVert) {
    ...
    FVarLevel::ConstCreaseEndPairArray pCreaseEnds =
        _parentFVar.getVertexValueCreaseEnds(pVert);
    FVarLevel::CreaseEndPairArray cCreaseEnds =
        _childFVar.getVertexValueCreaseEnds(cVert);
...
void FVarRefinement::reclassifySemisharpValues() {
  ...
  //
  //  Reclassify the tags of semi-sharp vertex values to smooth creases
  //  according to changes in sharpness:
  //
  //  Vertex values introduced on edge-verts can never be semi-sharp as they
  //  will be introduced on discts edges, which are implicitly infinitely sharp,
  //  so we can skip them entirely.
  //
  //  So we just need to deal with those values descended from parent vertices
  //  that were semi-sharp.  The child values will have inherited the semi-sharp
  //  tag from their parent values -- we will be able to clear it in many simple
  //  cases but ultimately will need to inspect each value:
  //
  ...
  for (; cVert < cVertEnd; ++cVert) {
    ...
    //  If the parent vertex wasn't semi-sharp, the child vertex and values
    //  can't be:
    Index pVert = _refinement.getChildVertexParentIndex(cVert);
    ...
    //  If the child is no longer semi-sharp, we can just clear those values
    //  marked (i.e. make them creases, others may remain corners) and continue:
    //
    ...
    //  There are some semi-sharp edges left -- for those values tagged as
    //  semi-sharp, see if they are still semi-sharp and clear those that are
    //  not:
    //
    FVarLevel::CreaseEndPairArray const cValueCreaseEnds =
        _childFVar.getVertexValueCreaseEnds(cVert);

    //  Beware accessing the child's vert-edges -- full topology may not be
    //  enabled:
    ...
    if (_childLevel.getNumVertexEdgesTotal()) {
      ...
      ConstLocalIndexArray pVertInEdge =
          _parentLevel.getVertexEdgeLocalIndices(pVert);
      for (int i = 0; i < pVertEdges.size(); ++i) {
        cVertEdgeBuffer[i] =
            _refinement.getEdgeChildEdges(pVertEdges[i])[pVertInEdge[i]];
      }
    ...
    for (int j = 0; j < cValueTags.size(); ++j) {
      if (cValueTags[j]._semiSharp && !cValueTags[j]._depSharp) {
        ...
        if (vEndFace > vStartFace) {
          for (int k = vStartFace + 1; !isStillSemiSharp && (k <= vEndFace);
               ++k) {
            isStillSemiSharp = _childLevel.getEdgeTag(cVertEdges[k])._semiSharp;
          }
        } else if (vStartFace > vEndFace) {
          for (int k = vStartFace + 1;
               !isStillSemiSharp && (k < cVertEdges.size()); ++k) {
            isStillSemiSharp = _childLevel.getEdgeTag(cVertEdges[k])._semiSharp;
          }
    ...
    //
    //  Now account for "dependent sharpness" (only matters when we have two
    //  values) -- if one value was dependent/sharpened based on the other,
    //  clear the dependency tag if it is no longer sharp:
    //
...
float FVarRefinement::getFractionalWeight(Index pVert, LocalIndex pSibling,
                                          Index cVert,
                                          LocalIndex /* cSibling */) const {
  ...
  FVarLevel::CreaseEndPair pValueCreaseEnds =
      _parentFVar.getVertexValueCreaseEnds(pVert)[pSibling];
  ...
  if (pEndFace > pStartFace) {
    for (int i = pStartFace + 1; i <= pEndFace; ++i, ++interiorEdgeCount) {
      pEdgeSharpness[interiorEdgeCount] =
          _parentLevel.getEdgeSharpness(pVertEdges[i]);
      cEdgeSharpness[interiorEdgeCount] =
          _childLevel.getEdgeSharpness(cVertEdges[i]);
    }
  } else if (pStartFace > pEndFace) {
    for (int i = pStartFace + 1; i < pVertEdges.size();
         ++i, ++interiorEdgeCount) {
      pEdgeSharpness[interiorEdgeCount] =
          _parentLevel.getEdgeSharpness(pVertEdges[i]);
      cEdgeSharpness[interiorEdgeCount] =
  ...
  return Sdc::Crease(_refinement.getOptions())
      .ComputeFractionalWeightAtVertex(_parentLevel.getVertexSharpness(pVert),
                                       _childLevel.getVertexSharpness(cVert),
                                       interiorEdgeCount, pEdgeSharpness,
                                       cEdgeSharpness);
}
