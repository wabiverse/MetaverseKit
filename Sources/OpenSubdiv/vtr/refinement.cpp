...
//
Refinement::Refinement(Level const &parentArg, Level &childArg,
                       Sdc::Options const &options)
    : _parent(&parentArg), _child(&childArg), _options(options),
      _regFaceSize(-1), _uniform(false), _faceVertsFirst(false),
      _childFaceFromFaceCount(0), _childEdgeFromFaceCount(0),
      _childEdgeFromEdgeCount(0), _childVertFromFaceCount(0),
      _childVertFromEdgeCount(0), _childVertFromVertCount(0),
      _firstChildFaceFromFace(0), _firstChildEdgeFromFace(0),
      _firstChildEdgeFromEdge(0), _firstChildVertFromFace(0),
      _firstChildVertFromEdge(0), _firstChildVertFromVert(0) {
...
void Refinement::initializeChildComponentCounts() {
  ...
  //
  //  Assign the child's component counts/inventory based on the child
  //  components identified:
  //
  _child->_faceCount = _childFaceFromFaceCount;
  _child->_edgeCount = _childEdgeFromFaceCount + _childEdgeFromEdgeCount;
  _child->_vertCount = _childVertFromFaceCount + _childVertFromEdgeCount +
...
//      - determine a mapping from parent components to their potential child
//      components
//          - for sparse refinement this mapping will be partial
//      - determine the reverse mapping from chosen child components back to
//      their parents
//          - previously this was optional -- not strictly necessary and comes
//          at added cost
//          - does simplify iteration of child components when refinement is
//          sparse
//      - propagate/initialize component Tags from parents to their children
//          - knowing these Tags for a child component simplifies dealing with
//          it later
//      - subdivide the topology, i.e. populate all topology relations for the
//      child Level
...
void Refinement::refine(Options refineOptions) {
  ...
  bool optionallyRefineFVar =
      (_parent->getNumFVarChannels() > 0) && !refineOptions_ignoreFVarChannels;

  //
  //  Initialize the parent-to-child and reverse child-to-parent mappings and
  //  propagate component tags to the new child components:
  //
  ...
  //
  //  Subdivide the topology -- populating only those of the 6 relations
  //  specified (though we do require the vertex-face relation for refining FVar
  //  channels):
  //
  ...
  //    - note there is some dependency of the vertex tag/Rule for semi-sharp
  //    vertices
  //
...
void Refinement::populateParentToChildMapping() {
  ...
  //
  //  If sparse refinement, mark indices of any components in addition to those
  //  selected so that we have the full neighborhood for selected components:
  //
  if (!_uniform) {
...
inline int sequenceSparseIndexVector(IndexVector &indexVector,
                                     int baseValue = 0) {
  int validCount = 0;
...
inline int sequenceFullIndexVector(IndexVector &indexVector,
                                   int baseValue = 0) {
...
void Refinement::populateParentChildIndices() {
  ...
  //  from vertices first, or those refined from faces first.  It's possible
  //  this may be extended to more possibilities.  Once the ordering is defined
  //  here, other than analogous initialization in FVarRefinement, the treatment
  //  of vertices in blocks based on origin should make the rest of the code
  //  invariant to ordering changes.
  //
  //  These two blocks now differ only in the utility function that assigns the
  //  sequential values to the index vectors -- so
  //  parameterization/simplification is now possible...
  //
  if (_uniform) {
    //  child faces:
    _firstChildFaceFromFace = 0;
    _childFaceFromFaceCount =
        sequenceFullIndexVector(_faceChildFaceIndices, _firstChildFaceFromFace);

    //  child edges:
    _firstChildEdgeFromFace = 0;
    _childEdgeFromFaceCount =
        sequenceFullIndexVector(_faceChildEdgeIndices, _firstChildEdgeFromFace);

    _firstChildEdgeFromEdge = _childEdgeFromFaceCount;
    _childEdgeFromEdgeCount =
        sequenceFullIndexVector(_edgeChildEdgeIndices, _firstChildEdgeFromEdge);
    ...
    if (_faceVertsFirst) {
      ...
      _firstChildVertFromEdge =
          _firstChildVertFromFace + _childVertFromFaceCount;
      _childVertFromEdgeCount =
          sequenceFullIndexVector(_edgeChildVertIndex, _firstChildVertFromEdge);

      _firstChildVertFromVert =
          _firstChildVertFromEdge + _childVertFromEdgeCount;
      _childVertFromVertCount =
          sequenceFullIndexVector(_vertChildVertIndex, _firstChildVertFromVert);
    } else {
      ...
      _firstChildVertFromFace =
          _firstChildVertFromVert + _childVertFromVertCount;
      _childVertFromFaceCount =
          sequenceFullIndexVector(_faceChildVertIndex, _firstChildVertFromFace);
    ...
    //  child faces:
    _firstChildFaceFromFace = 0;
    _childFaceFromFaceCount = sequenceSparseIndexVector(
        _faceChildFaceIndices, _firstChildFaceFromFace);

    //  child edges:
    _firstChildEdgeFromFace = 0;
    _childEdgeFromFaceCount = sequenceSparseIndexVector(
        _faceChildEdgeIndices, _firstChildEdgeFromFace);

    _firstChildEdgeFromEdge = _childEdgeFromFaceCount;
    _childEdgeFromEdgeCount = sequenceSparseIndexVector(
        _edgeChildEdgeIndices, _firstChildEdgeFromEdge);

    //  child vertices:
    if (_faceVertsFirst) {
      _firstChildVertFromFace = 0;
      _childVertFromFaceCount = sequenceSparseIndexVector(
          _faceChildVertIndex, _firstChildVertFromFace);

      _firstChildVertFromEdge =
          _firstChildVertFromFace + _childVertFromFaceCount;
      _childVertFromEdgeCount = sequenceSparseIndexVector(
          _edgeChildVertIndex, _firstChildVertFromEdge);

      _firstChildVertFromVert =
          _firstChildVertFromEdge + _childVertFromEdgeCount;
      _childVertFromVertCount = sequenceSparseIndexVector(
          _vertChildVertIndex, _firstChildVertFromVert);
    } else {
      _firstChildVertFromVert = 0;
      _childVertFromVertCount = sequenceSparseIndexVector(
          _vertChildVertIndex, _firstChildVertFromVert);
...
void Refinement::populateFaceParentVectors(
    ChildTag const initialChildTags[2][4]) {
...
void Refinement::populateFaceParentFromParentFaces(
    ChildTag const initialChildTags[2][4]) {
      ...
      if (!incomplete && (cFaces.size() == 4)) {
        ...
        for (int i = 0; i < cFaces.size(); ++i) {
          if (IndexIsValid(cFaces[i])) {
            _childFaceTag[cFaces[i]] =
                initialChildTags[incomplete][childTooLarge ? 0 : i];
...
void Refinement::populateEdgeParentVectors(
    ChildTag const initialChildTags[2][4]) {
...
void Refinement::populateEdgeParentFromParentFaces(
    ChildTag const initialChildTags[2][4]) {
...
void Refinement::populateEdgeParentFromParentEdges(
    ChildTag const initialChildTags[2][4]) {
...
void Refinement::populateVertexParentVectors(
    ChildTag const initialChildTags[2][4]) {
...
void Refinement::populateVertexParentFromParentFaces(
  ...
  if (_uniform) {
    ...
    for (Index pFace = 0; pFace < _parent->getNumFaces(); ++pFace, ++cVert) {
      //  Child tag was initialized as the complete and only child when
      //  allocated
...
void Refinement::populateVertexParentFromParentEdges(
    ChildTag const initialChildTags[2][4]) {
...
void Refinement::populateVertexParentFromParentVertices(
    ChildTag const initialChildTags[2][4]) {
  ...
  if (_uniform) {
    ...
    for (Index pVert = 0; pVert < _parent->getNumVertices(); ++pVert) {
      ...
      if (IndexIsValid(cVert)) {
        //  Child tag was initialized as incomplete but these should be
        //  complete:
...
//
//  Methods to propagate/initialize child component tags from their parent
//  component:
//
...
void Refinement::populateVertexTagsFromParentFaces() {
  ...
  //  with the unfortunate exception of refining level 0, where the faces may be
  //  N-sided and so introduce new vertices that need to be tagged as
  //  extra-ordinary:
  //
  if (getNumChildVerticesFromFaces() == 0)
    return;
  ...
  if (_parent->_depth > 0) {
    ...
    for (; cVert < cVertEnd; ++cVert) {
      ...
      if (_parent->getNumFaceVertices(_childVertexParentIndex[cVert]) !=
          _regFaceSize) {
...
void Refinement::populateVertexTagsFromParentEdges() {
  ...
  //
  //  Tags for vertices originating from edges are initialized according to the
  //  tags of the parent edge:
  //
  Level::VTag vTag;
  vTag.clear();
  ...
  for (Index pEdge = 0; pEdge < _parent->getNumEdges(); ++pEdge) {
    ...
    vTag._rule = (Level::VTag::VTagSize)(
        (pEdgeTag._semiSharp || pEdgeTag._infSharp) ? Sdc::Crease::RULE_CREASE
...
void Refinement::populateVertexTagsFromParentVertices() {
  ...
  //
  //  Tags for vertices originating from vertices are inherited from the parent
  //  vertex:
  //
  Index cVert = getFirstChildVertexFromVertices();
  Index cVertEnd = cVert + getNumChildVerticesFromVertices();
  for (; cVert < cVertEnd; ++cVert) {
    _child->_vertTags[cVert] =
        _parent->_vertTags[_childVertexParentIndex[cVert]];
...
//
//  The main method to subdivide topology is fairly simple -- given a set of
//  relations to populate it simply tests and populates each relation
//  separately.  The method for each relation is responsible for appropriate
//  allocation and initialization of all data involved, and these are virtual --
//  provided by a quad- or tri-split subclass.
//
void Refinement::subdivideTopology(Relations const &applyTo) {
  ...
  //  than the parent if that maximal parent vertex was not included in the
  //  sparse refinement (possible when sparse refinement is more general).
  ...
  //      - NOTE that when/if we support N-gons for tri-splitting, that the
  //      valence
  //  of edge-vertices introduced on the N-gon may be 7 rather than 6, while N
  //  may be less than both.
  //
  ...
  if (_splitType == Sdc::SPLIT_TO_QUADS) {
    ...
    _child->_maxValence =
        std::max(_child->_maxValence, 2 + _parent->_maxEdgeFaces);
  } else {
    _child->_maxValence = std::max(_parent->_maxValence, 6);
    _child->_maxValence =
        std::max(_child->_maxValence, 2 + _parent->_maxEdgeFaces * 2);
  }
}
...
void Refinement::subdivideSharpnessValues() {
  ...
  //  to maintain proper classification/tagging of components as semi-sharp,
  //  both must be computed and the neighborhood inspected to properly update
  //  the status.
  //
  //  It is possible to clear the semi-sharp status when propagating the tags
  //  and to reset it (potentially multiple times) when updating the sharpness
  //  values. The vertex subdivision Rule is also affected by this, which
  //  complicates the process.  So for now we apply a post-process to explicitly
  //  handle all semi-sharp vertices.
  //
  ...
  //  This method uses local sharpness tags (set above) to update vertex tags
  //  that reflect the neighborhood of the vertex (e.g. its rule):
...
void Refinement::subdivideEdgeSharpness() {
  ...
  _child->_edgeSharpness.resize(_child->getNumEdges(),
                                Sdc::Crease::SHARPNESS_SMOOTH);
...
void Refinement::subdivideVertexSharpness() {
  ...
  _child->_vertSharpness.resize(_child->getNumVertices(),
                                Sdc::Crease::SHARPNESS_SMOOTH);
...
void Refinement::reclassifySemisharpVertices() {
  ...
  //
  //  Inspect all vertices derived from edges -- for those whose parent edges
  //  were semisharp, reset the semisharp tag and the associated Rule according
  //  to the sharpness pair for the subdivided edges (note this may be better
  //  handled when the edge sharpness is computed):
  //
  ...
  for (Index cVert = vertFromEdgeBegin; cVert < vertFromEdgeEnd; ++cVert) {
    ...
    if (_childVertexTag[cVert]._incomplete) {
      //  One child edge likely missing -- assume Crease if remaining edge
      //  semi-sharp:
      cVertTag._semiSharpEdges =
          (IndexIsValid(cEdges[0]) &&
           _child->_edgeTags[cEdges[0]]._semiSharp) ||
          (IndexIsValid(cEdges[1]) && _child->_edgeTags[cEdges[1]]._semiSharp);
      cVertTag._rule =
          (VTagSize)(cVertTag._semiSharpEdges ? Sdc::Crease::RULE_CREASE
                                              : Sdc::Crease::RULE_SMOOTH);
    } else {
      int sharpEdgeCount = _child->_edgeTags[cEdges[0]]._semiSharp +
                           _child->_edgeTags[cEdges[1]]._semiSharp;

      cVertTag._semiSharpEdges = (sharpEdgeCount > 0);
      cVertTag._rule =
          (VTagSize)(creasing.DetermineVertexVertexRule(0.0, sharpEdgeCount));
    }
  }

  //
  //  Inspect all vertices derived from vertices -- for those whose parent
  //  vertices were semisharp (inherited in the child vert's tag), inspect and
  //  reset the semisharp tag and the associated Rule (based on neighboring
  //  child edges around the child vertex).
  //
  //  We should never find such a vertex "incomplete" in a sparse refinement as
  //  a parent vertex is either selected or not, but never neighboring.  So the
  //  only complication here is whether the local topology of child edges exists
  //  -- it may have been pruned from the last level to reduce memory.  If so,
  //  we use the parent to identify the child edges.
  //
  //  In both cases, we count the number of sharp and semisharp child edges
  //  incident the child vertex and adjust the "semisharp" and "rule" tags
  //  accordingly.
  //
  ...
  for (Index cVert = vertFromVertBegin; cVert < vertFromVertEnd; ++cVert) {
    ...
    //
    //  We need to inspect the child neighborhood's sharpness when either
    //  semi-sharp edges were present around the parent vertex, or the parent
    //  vertex sharpness decayed:
    //
    Level::VTag &cVertTag = _child->_vertTags[cVert];
    ...
    if (pVertTag._semiSharpEdges || sharpVertexDecayed) {
      ...
      if (cVertEdgesPresent) {
        ...
        ConstLocalIndexArray pVertInEdge =
            _parent->getVertexEdgeLocalIndices(pVert);
      ...
      if (!cVertTag._semiSharp && !cVertTag._infSharp) {
        cVertTag._rule = (VTagSize)(creasing.DetermineVertexVertexRule(
            0.0, infSharpEdgeCount + semiSharpEdgeCount));
      }
    }
  }
}
...
void Refinement::subdivideFVarChannels() {
  ...
  for (int channel = 0; channel < channelCount; ++channel) {
    ...
    FVarRefinement *refineFVar =
        new FVarRefinement(*this, *parentFVar, *childFVar);
...
//
//  Marking of sparse child components -- including those selected and those
//  neighboring...
//
//      For schemes requiring neighboring support, this is the equivalent of the
//      "guarantee
//  neighbors" in Hbr -- it ensures that all components required to define the
//  limit of those "selected" are also generated in the refinement.
//
//  The difference with Hbr is that we do this in a single pass for all
//  components once "selection" of components of interest has been completed.
//
...
//              - test and mark local components
//
//  Given a typical quad mesh with N verts, N faces and 2*N edges, determine
//  which is more efficient...
//
//  Going with (2) initially for simplicity -- certain aspects of (1) are
//  awkward, i.e. the identification of child-edges to be marked (trivial in
//  (2).  We are also guaranteed with (2) that we only visit each component
//  once, i.e. each edge and each face.
//
//  Revising the above assessment... (2) has gotten WAY more complicated once
//  the ability to select child faces is provided.  Given that feature is
//  important to Manuel for support of the FarStencilTables we have to assume it
//  will be needed.  So we'll try (1) out as it will be simpler to get it
//  correct -- we can work on improving performance later.
//
//  Complexity added by child component selection:
//      - the child vertex of the component can now be selected as part of a
//      child face or
//  edge, and so the parent face or edge is not fully selected.  So we've had to
//  add another bit to the marking masks to indicate when a parent component is
//  "fully selected".
//      - selecting a child face creates the situation where child edges of
//      parent edges do
//  not have any selected vertex at their ends -- both can be neighboring.  This
//  complicated the marking of neighboring child edges, which was otherwise
//  trivial -- if any end vertex of a child edge (of a parent edge) was
//  selected, the child edge was at least neighboring.
//
//  Final note on the marking technique:
//      There are currently two values to the marking of child components, which
//      are no
//  longer that useful.  It is now sufficient, and not likely to be necessary,
//  to distinguish between what was selected or added to support it.  Ultimately
//  that will be determined by inspecting the selected flag on the parent
//  component once the child-to-parent map is in place.
//
namespace {
Index const IndexSparseMaskNeighboring = (1 << 0);
Index const IndexSparseMaskSelected = (1 << 1);

inline void markSparseIndexNeighbor(Index &index) {
  index = IndexSparseMaskNeighboring;
}
inline void markSparseIndexSelected(Index &index) {
  index = IndexSparseMaskSelected;
}
} // namespace
...
void Refinement::markSparseChildComponentIndices() {
  ...
  //
  //  There is an explicit ordering here as the work done for vertices is a
  //  subset of what is required for edges, which in turn is a subset of what is
  //  required for faces.  This ordering and their related implementations tries
  //  to avoid doing redundant work and accomplishing everything necessary in a
  //  single iteration through each component type.
  //
...
void Refinement::markSparseEdgeChildren() {
  ...
  //      - set transitional bit for all edges based on selection of incident
  //      faces
  //
  //  Note that no edges have been marked "fully selected" -- only their
  //  vertices have been marked and marking of their child edges deferred to
  //  visiting each edge only once here.
  //
  for (Index pEdge = 0; pEdge < parent().getNumEdges(); ++pEdge) {
    ...
    //
    //  TAG the parent edges as "transitional" here if only one was selected (or
    //  in the more general non-manifold case, they are not all selected the
    //  same way). We use the transitional tags on the edges to TAG the parent
    //  face below.
    //
    //  Note -- this is best done now rather than as a post-process as we have
    //  more explicit information about the selected components.  Unless we also
    //  tag the parent faces as selected, we can't easily tell from the
    //  child-faces of the edge's incident faces which were generated by
    //  selection or neighboring...
    //
