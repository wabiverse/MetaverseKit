...
//
QuadRefinement::QuadRefinement(Level const &parentArg, Level &childArg,
                               Sdc::Options const &optionsArg)
...
void QuadRefinement::allocateParentChildIndices() {
  ...
  //  components that will originate from each.
  //
  ...
  //
  //  First reference the parent Level's face-vertex counts/offsets -- they can
  //  be used here for both the face-child-faces and face-child-edges as they
  //  both have one per face-vertex.
  //
  //  Given we will be ignoring initial values with uniform refinement and
  //  assigning all directly, initializing here is a waste...
  //
  Index initValue = 0;
...
void QuadRefinement::populateFaceVertexRelation() {
  ...
  //  Both face-vertex and face-edge share the face-vertex counts/offsets within
  //  a Level, so be sure not to re-initialize it if already done:
  //
...
void QuadRefinement::populateFaceEdgeRelation() {
  ...
  //  Both face-vertex and face-edge share the face-vertex counts/offsets, so be
  //  sure not to re-initialize it if already done:
  //
  if (_child->_faceVertCountsAndOffsets.size() == 0) {
    populateFaceVertexCountsAndOffsets();
  }
...
void QuadRefinement::populateFaceEdgesFromParentFaces() {
  ...
  //
  //  This is fairly straightforward, but since we are dealing with edges here,
  //  we occasionally have to deal with the limitation of them being undirected.
  //  Since child faces from the same parent face share much in common, we
  //  iterate through the parent faces.
  //
  //  Each child face of the parent is based on a corner vertex from which we
  //  denote a "previous" and "next" edge, which are child edges of the parent
  //  face's edges. The two remaining edges per child faces are perpendicular to
  //  these prev/next edges and share the child vertex of the parent face.
  //
  for (Index pFace = 0; pFace < _parent->getNumFaces(); ++pFace) {
    ConstIndexArray pFaceVerts = _parent->getFaceVertices(pFace),
                    pFaceEdges = _parent->getFaceEdges(pFace),
    ...
    for (int j = 0; j < pFaceSize; ++j) {
      ...
      if (IndexIsValid(cFace)) {
        ...
        int cornerInPrevEdge = (pPrevEdgeVerts[0] != pPrevEdgeVerts[1])
                                   ? (pPrevEdgeVerts[0] != pCornerVert)
                                   : 1;

        int cornerInNextEdge = (pNextEdgeVerts[0] != pNextEdgeVerts[1])
                                   ? (pNextEdgeVerts[0] != pCornerVert)
                                   : 0;
...
void QuadRefinement::populateEdgeFaceRelation() {
  ...
  //      - could at least make a quick traversal of components and use the
  //      above
  //        two points to get much closer estimate than what is used for uniform
  //
  int childEdgeFaceIndexSizeEstimate =
      (int)_parent->_faceVertIndices.size() * 2 +
  ...
  //  Revise the over-allocated estimate based on what is used (as indicated in
  //  the count/offset for the last vertex) and trim the index vector
  //  accordingly:
  childEdgeFaceIndexSizeEstimate =
      _child->getNumEdgeFaces(_child->getNumEdges() - 1) +
    ...
    for (int j = 0; j < pFaceSize; ++j) {
      ...
      if (IndexIsValid(cEdge)) {
        ...
        if (IndexIsValid(pFaceChildFaces[j])) {
          ...
          cEdgeInFace[cEdgeFaceCount] =
              (LocalIndex)((pFaceSize == 4) ? jNext : 1);
          cEdgeFaceCount++;
        }
        if (IndexIsValid(pFaceChildFaces[jNext])) {
          ...
          cEdgeInFace[cEdgeFaceCount] =
              (LocalIndex)((pFaceSize == 4) ? ((jNext + 2) & 3) : 2);
          cEdgeFaceCount++;
        }
  ...
  for (Index pEdge = 0; pEdge < _parent->getNumEdges(); ++pEdge) {
    ...
    for (int j = 0; j < 2; ++j) {
      ...
      for (int i = 0; i < pEdgeFaces.size(); ++i) {
        ...
        //
        //  We need to first identify the potentially incident child-face and
        //  see if it exists before we can assign it.  Beware a degenerate edge
        //  here when inspecting the undirected edge.
        //
        int childOfEdge = (pEdgeVerts[0] == pEdgeVerts[1])
                              ? j
        ...
        if (IndexIsValid(pFaceChildren[childInFace])) {
          ...
          cEdgeInFace[cEdgeFaceCount] =
              (LocalIndex)((pFaceVerts.size() == 4) ? edgeInFace
                                                    : (childOfEdge ? 3 : 0));
          cEdgeFaceCount++;
        }
      }
...
void QuadRefinement::populateVertexFaceRelation() {
  ...
  int childVertFaceIndexSizeEstimate =
      (int)_parent->_faceVertIndices.size() +
      (int)_parent->_edgeFaceIndices.size() * 2 +
  ...
  //  Revise the over-allocated estimate based on what is used (as indicated in
  //  the count/offset for the last vertex) and trim the index vectors
  //  accordingly:
  childVertFaceIndexSizeEstimate =
      _child->getNumVertexFaces(_child->getNumVertices() - 1) +
  ...
  for (int pFace = 0; pFace < _parent->getNumFaces(); ++pFace) {
    ...
    for (int j = 0; j < pFaceSize; ++j) {
      if (IndexIsValid(pFaceChildren[j])) {
        ...
        cVertInFace[cVertFaceCount] =
            (LocalIndex)((pFaceSize == 4) ? ((j + 2) & 3) : 2);
        cVertFaceCount++;
      }
    }
...
void QuadRefinement::populateVertexFacesFromParentEdges() {
  ...
  for (int pEdge = 0; pEdge < _parent->getNumEdges(); ++pEdge) {
    ...
    //
    //  For each face incident the parent edge, identify its corresponding two
    //  child faces and assign those of the two that exist.  The second face is
    //  considered and added first to preserve CC-wise ordering of faces wrt the
    //  vertex.
    //
    int cVertFaceCount = 0;
    for (int i = 0; i < pEdgeFaces.size(); ++i) {
      ...
      if (IndexIsValid(pFaceChildren[faceChild1])) {
        ...
        cVertInFace[cVertFaceCount] =
            (LocalIndex)((pFaceSize == 4) ? faceChild0 : 3);
        cVertFaceCount++;
      }
      if (IndexIsValid(pFaceChildren[faceChild0])) {
        ...
        cVertInFace[cVertFaceCount] =
            (LocalIndex)((pFaceSize == 4) ? faceChild1 : 1);
        cVertFaceCount++;
      }
    }
    _child->trimVertexFaces(cVert, cVertFaceCount);
...
void QuadRefinement::populateVertexFacesFromParentVertices() {
  ...
  for (int pVert = 0; pVert < _parent->getNumVertices(); ++pVert) {
    ...
    ConstLocalIndexArray pVertInFace =
        _parent->getVertexFaceLocalIndices(pVert);
    ...
    //  spawned a child face corresponding to (and so incident) this child
    //  vertex:
    //
    int cVertFaceCount = 0;
    for (int i = 0; i < pVertFaces.size(); ++i) {
      ...
      if (IndexIsValid(pFaceChildren[vertInFace])) {
        ...
        cVertInFace[cVertFaceCount] =
            (LocalIndex)((pFaceSize == 4) ? vertInFace : 0);
        cVertFaceCount++;
      }
    }
    _child->trimVertexFaces(cVert, cVertFaceCount);
...
void QuadRefinement::populateVertexEdgeRelation() {
  ...
  //      - 4 or 0 for verts from parent faces (for catmark)
  //      - 2 + N or 2 + 2*N faces incident parent edge for verts from parent
  //      edges
  //      - same as parent vert for verts from parent verts
  ...
  int childVertEdgeIndexSizeEstimate = (int)_parent->_faceVertIndices.size() +
                                       (int)_parent->_edgeFaceIndices.size() +
                                       _parent->getNumEdges() * 2 +
  ...
  //  the count/offset for the last vertex) and trim the index vectors
  //  accordingly:
  childVertEdgeIndexSizeEstimate =
      _child->getNumVertexEdges(_child->getNumVertices() - 1) +
...
void QuadRefinement::populateVertexEdgesFromParentFaces() {
  ...
  for (int pFace = 0; pFace < _parent->getNumFaces(); ++pFace) {
    ...
    //
    //  Need to ensure correct ordering here when complete -- we want the
    //  "leading" edge of each child face first.  The child vert is in the
    //  center of a new face so new "boundaries" will only occur when the vertex
    //  is incomplete.
    //
...
void QuadRefinement::populateVertexEdgesFromParentEdges() {
  ...
  //
  //  This relation turns out to be awkward to populate given the mixed
  //  parentage of the incident edges of the child vertex of an edge -- two
  //  child edges originate from the parent edge while one or more will
  //  originate from the faces incident the parent edge.  The need to interleave
  //  these for proper CC-wise orientation is what really complicates this.
  //
  ...
  for (int pEdge = 0; pEdge < _parent->getNumEdges(); ++pEdge) {
    ...
    //
    //  Identify and assign the first two child edges of the parent edge --
    //  until we look more closely at the orientation of the parent edge in the
    //  first face we don't know what order these two should be in, so just
    //  assign them for now and swap them later if necessary:
    //
    int cVertEdgeCount = 0;
...
//
//  Need to find a better place for these anon helper methods now that they are
//  required both in the base class and the two subclasses for quad- and
//  tri-splitting...
//
namespace {
...
void QuadRefinement::markSparseFaceChildren() {
  ...
  //      All boundary edges will be adequately marked as a result of the pass
  //      over the
  //  edges above and boundary vertices marked by selection.  So all that
  //  remains is to identify the child faces and interior child edges for a face
  //  requiring neighboring child faces.
  //      For each corner vertex selected, we need to mark the corresponding
  //      child face,
  ...
  for (Index pFace = 0; pFace < parent().getNumFaces(); ++pFace) {
    //
    //  Mark all descending child components of a selected face.  Otherwise
    //  inspect its incident vertices to see if anything neighboring has been
    //  selected -- requiring partial refinement of this face.
    //
    ...
    if (pFaceTag._selected) {
      ...
      if (marked) {
        ...
        //
        //  Only non-selected faces may be "transitional", and we need to
        //  inspect all tags on its boundary edges to be sure.  Since we're
        //  inspecting each now (and may need to later) retain the transitional
        //  state of each in a 4-bit mask that reflects the full transitional
        //  topology for later.
        //
        ConstIndexArray fEdges = parent().getFaceEdges(pFace);
        if (fEdges.size() == 4) {
          pFaceTag._transitional =
              (unsigned char)((_parentEdgeTag[fEdges[0]]._transitional << 0) |
