...
//
//  Level:
//      This is intended to be a fairly simple container of topology, sharpness
//      and
//  other information that is useful to retain for subdivision.  It is intended
//  to be constructed by other friend classes, i.e. factories and class
//  specialized to construct topology based on various splitting schemes.  So
//  its interface consists of simple methods for inspection, and low-level
//  protected methods for populating it rather than high-level modifiers.
//
...
//
Level::Level()
    : _faceCount(0), _edgeCount(0), _vertCount(0), _depth(0), _maxEdgeFaces(0),
      _maxValence(0) {}
...
//      - verifying orientation of components, particularly vert-edges and
//      faces:
...
//              - it does ensure our work was not corrupted by client
//              assignments
//
//  Possibilities:
...
bool Level::validateTopology(ValidationCallback callback,
  ...
  //      - currently this requires the relations exactly match those that we
  //      construct from
  //        the ordering method, i.e. we do not allow rotations for interior
  //        vertices.
  ...
  //      - note we have to validate orientation of vertex neighbors to do this
  //      rigorously
  for (int eIndex = 0; eIndex < getNumEdges(); ++eIndex) {
    ...
    if (eVerts[0] == eVerts[1]) {
      REPORT(TOPOLOGY_DEGENERATE_EDGE,
             "Error in eIndex = %d:  degenerate edge not tagged marked "
             "non-manifold",
             eIndex);
      if (returnOnFirstError)
        return false;
    ...
    if ((eFaces.size() < 1) || (eFaces.size() > 2)) {
      REPORT(TOPOLOGY_NON_MANIFOLD_EDGE,
             "edge %d with %d incident faces not tagged non-manifold", eIndex,
             eFaces.size());
      if (returnOnFirstError)
        return false;
      isValid = false;
...
//
//  Anonymous helper functions for debugging output -- yes, using printf(), this
//  is not intended to serve anyone other than myself for now and I favor its
//  formatting control
//
namespace {
template <typename INT_TYPE>
void printIndexArray(ConstArray<INT_TYPE> const &array) {
...
void Level::print(const Refinement *pRefinement) const {
  ...
  if (pRefinement) {
    ...
    for (int i = 0; printFaceChildVerts &&
                    i < (int)pRefinement->_faceChildVertIndex.size();
         ++i) {
      printf("        face %4d child vert:  %d\n", i,
             pRefinement->_faceChildVertIndex[i]);
    }
  }
  ...
  if (pRefinement) {
    ...
    for (int i = 0; printEdgeChildVerts &&
                    i < (int)pRefinement->_edgeChildVertIndex.size();
         ++i) {
      printf("        edge %4d child vert:  %d\n", i,
             pRefinement->_edgeChildVertIndex[i]);
    }
  }
  ...
  if (pRefinement) {
    ...
    for (int i = 0; printVertChildVerts &&
                    i < (int)pRefinement->_vertChildVertIndex.size();
         ++i) {
      printf("        vert %4d child vert:  %d\n", i,
             pRefinement->_vertChildVertIndex[i]);
    }
  }
...
//
bool Level::doesVertexFVarTopologyMatch(Index vIndex, int fvarChannel) const {

  return getFVarLevel(fvarChannel)
      .valueTopologyMatches(
          getFVarLevel(fvarChannel).getVertexValueOffset(vIndex));
}
bool Level::doesEdgeFVarTopologyMatch(Index eIndex, int fvarChannel) const {

  return getFVarLevel(fvarChannel).edgeTopologyMatches(eIndex);
}
bool Level::doesFaceFVarTopologyMatch(Index fIndex, int fvarChannel) const {

  return !getFVarLevel(fvarChannel)
              .getFaceCompositeValueTag(fIndex)
              .isMismatch();
}
...
Level::VTag Level::getFaceCompositeVTag(Index fIndex, int fvarChannel) const {
  ...
  if (fvarChannel < 0) {
    ...
    VTag::VTagSize tagBits =
        fvarTags[0].combineWithLevelVTag(_vertTags[fVerts[0]]).getBits();
    for (int i = 1; i < fVerts.size(); ++i) {
      tagBits |=
          fvarTags[i].combineWithLevelVTag(_vertTags[fVerts[i]]).getBits();
    }
    return VTag(tagBits);
  }
}

Level::VTag Level::getVertexCompositeFVarVTag(Index vIndex,
...
//
//  High-level topology gathering functions -- used mainly in patch
//  construction.  These may eventually be moved elsewhere, possibly to classes
//  specialized for quad- and tri- patch identification and construction, but
//  for now somewhere more accessible than the patch tables factory is
//  preferable.
//
//  Note a couple of nuisances...
//      - debatable whether we should include the face's face-verts in the face
//      functions
//          - we refer to the result as a "patch" when we do
//          - otherwise a "ring" of vertices is more appropriate
//
namespace {
template <typename INT_TYPE> inline INT_TYPE fastMod4(INT_TYPE value) {
...
//      The surrounding one-ring follows the ordering of the incident faces. For
//      each
//  incident quad, the two vertices in CCW order within that quad are added.  If
//  the vertex is on a boundary, a third vertex on the boundary edge will be
//  contributed from the last face.
//
int Level::gatherQuadRegularRingAroundVertex(Index vIndex, int ringPoints[],
                                             int fvarChannel) const {
  ...
  for (int i = 0; i < vFaces.size(); ++i) {
    ...
    //  opposite this one:
    //
    ConstIndexArray fPoints =
        (fvarChannel < 0) ? level.getFaceVertices(vFaces[i])
...
int Level::gatherQuadRegularPartialRingAroundVertex(Index vIndex,
                                                    VSpan const &span,
                                                    int ringPoints[],
                                                    int fvarChannel) const {
...
//        |     |
//
int Level::gatherQuadLinearPatchPoints(Index thisFace, Index patchPoints[],
                                       int rotation, int fvarChannel) const {
  ...
  ConstIndexArray facePoints =
      (fvarChannel < 0) ? level.getFaceVertices(thisFace)
...
//
//  Ordering of resulting vertices:
//      It was debatable whether to include the vertices of the original face
//      for a complete
//  "patch" or just the surrounding ring -- clearly we ended up with a function
//  for the entire patch, but that may change.
//      The latter ring of vertices around the face (potentially returned on its
//      own) was
//  oriented with respect to the face.  The ring of 12 vertices is gathered as 4
//  groups of 3 vertices -- one for each corner vertex, and each group forming
//  the quad opposite each corner vertex when combined with that corner vertex.
//  The four vertices of the face begin the patch.
//
//         |     |     |     |
//      ---5-----4-----15----14---
//         |     |     |     |
...
//         |     |     |     |
//
int Level::gatherQuadRegularInteriorPatchPoints(Index thisFace,
                                                Index patchPoints[],
                                                int rotation,
                                                int fvarChannel) const {
...
//      The latter ring of vertices around the face (potentially returned on its
//      own) was
//  oriented beginning from the leading CCW boundary edge and ending at the
//  trailing edge. The four vertices of the face begin the patch and are
//  oriented similarly to this outer ring -- forming an inner ring that begins
...
//         |     |     |     |
//
int Level::gatherQuadRegularBoundaryPatchPoints(Index face, Index patchPoints[],
  ...
  //
  //  V0 and V1 are the two interior vertices (opposite the boundary edge)
  //  around which we will gather most of the ring:
  //
  ...
  //
  ConstIndexArray thisFacePoints, prevFacePoints, intV0FacePoints,
      intV1FacePoints, nextFacePoints;
...
//  "patch" or just the surrounding ring -- clearly we ended up with a function
//  for the entire patch, but that may change.
//      Like the boundary case, the latter ring of vertices around the face was
//      oriented
//  beginning from the leading CCW boundary edge and ending at the trailing
//  edge.  The four face vertices begin the patch, and begin with the corner
//  vertex.
//
//      0-----3-----8---
//      |x   x|     |
...
//      |     |     |
//
int Level::gatherQuadRegularCornerPatchPoints(Index face, Index patchPoints[],
  ...
  ConstLocalIndexArray intVertInFaces =
      level.getVertexFaceLocalIndices(intVert);
...
//
//  Ordering of resulting vertices:
//      The three vertices of the triangle begin the sequence, followed by
//      counter-clockwise
//  traversal of the outer ring of vertices -- beginning with the vertex
//  incident V0 such that the ring is symmetric about the triangle.
/*
//                   3           11
//                   X - - - - - X
//                 /   \       /   \
...
//             6           7           8
*/
int Level::gatherTriRegularInteriorPatchPoints(Index fIndex, Index points[],
                                               int rotation) const {
  ConstIndexArray fVerts = getFaceVertices(fIndex);
  ConstIndexArray fEdges = getFaceEdges(fIndex);
...
//      - the neighborhood of the face is assumed to be tri-regular
//      - referred to as "boundary edge" as the boundary occurs on the edge of
//      the triangle
//
//  Boundary edge:
//
...
//                   0           1
*/
int Level::gatherTriRegularBoundaryEdgePatchPoints(Index fIndex, Index points[],
                                                   int boundaryFaceEdge) const {
  ConstIndexArray fVerts = getFaceVertices(fIndex);
...
//      - the neighborhood of the face is assumed to be tri-regular
//      - referred to as "boundary vertex" as the boundary occurs on the vertex
//      of the triangle
//
//  Boundary vertex:
/*
...
//             5           6           7
*/
int Level::gatherTriRegularBoundaryVertexPatchPoints(
    Index fIndex, Index points[], int boundaryFaceVert) const {
  ConstIndexArray fVerts = getFaceVertices(fIndex);
  ConstIndexArray fEdges = getFaceEdges(fIndex);
...
//      - the neighborhood of the face is assumed to be tri-regular
//      - referred to as "corner vertex" to disambiguate it from another corner
//      case
...
//             3           4           5
*/
int Level::gatherTriRegularCornerVertexPatchPoints(Index fIndex, Index points[],
                                                   int cornerFaceVert) const {
  ConstIndexArray fVerts = getFaceVertices(fIndex);
...
//      - the neighborhood of the face is assumed to be tri-regular
//      - referred to as "corner edge" to disambiguate it from the vertex corner
//      case
...
//                         3
*/
int Level::gatherTriRegularCornerEdgePatchPoints(Index fIndex, Index points[],
                                                 int cornerFaceEdge) const {
  ConstIndexArray fVerts = getFaceVertices(fIndex);
...
bool Level::isSingleCreasePatch(Index face, float *sharpnessOut,
                                int *sharpEdgeInFaceOut) const {

  //  Using the composite tag for all face vertices, first make sure that some
  //  face-vertices are Crease vertices, and quickly reject this case based on
  //  the presence of other features.  Ultimately we want a regular interior
  //  face with two (adjacent) Crease vertics and two Smooth vertices.  This
  //  first test quickly ensures a regular interior face with some number of
  //  Crease vertices and any remaining as Smooth.
  //
  ...
  if (!(allCornersTag._rule & Sdc::Crease::RULE_CREASE) ||
      ...
      allCornersTag._boundary || allCornersTag._xordinary ||
  ...
  //
  int creaseCornerMask =
      ((getVertexTag(fVerts[0])._rule == Sdc::Crease::RULE_CREASE) << 0) |
  ...
  static const int sharpEdgeFromCreaseMask[16] = {
  ...
  //  Reject if the crease at the two crease vertices A and B is not regular,
  //  i.e. any pair of opposing edges does not have the same sharpness value
  //  (one pair sharp, the other smooth).  The resulting two regular creases
  //  must be "colinear" (sharing the edge between them, and so its common
  //  sharpness value) otherwise we would have more than two crease vertices.
  //
  ConstIndexArray vAEdges = getVertexEdges(fVerts[sharpEdgeInFace]);
  ConstIndexArray vBEdges =
      getVertexEdges(fVerts[fastMod4(sharpEdgeInFace + 1)]);

  if (!isSharpnessEqual(getEdgeSharpness(vAEdges[0]),
                        getEdgeSharpness(vAEdges[2])) ||
      !isSharpnessEqual(getEdgeSharpness(vAEdges[1]),
                        getEdgeSharpness(vAEdges[3])) ||
      !isSharpnessEqual(getEdgeSharpness(vBEdges[0]),
                        getEdgeSharpness(vBEdges[2])) ||
      !isSharpnessEqual(getEdgeSharpness(vBEdges[1]),
                        getEdgeSharpness(vBEdges[3]))) {
    return false;
  }
  if (sharpnessOut) {
...
//
//  What follows is an internal/anonymous class and protected methods to
//  complete all topological relations when only the face-vertex relations are
//  defined.
//
//  In keeping with the original idea that Level is just data and relies on
//  other classes to construct it, this functionality may be warranted
//  elsewhere, but we are collectively unclear as to where that should be at
//  present.  In the meantime, the implementation is provided here so that we
//  can test and make use of it.
//
namespace {
//
//  This is an internal helper class to manage the assembly of the topological
//  relations that do not have a predictable size, i.e. faces-per-edge,
//  faces-per-vertex and edges-per-vertex.  Level manages these with two
//  vectors:
//
//      - a vector of integer pairs for the "counts" and "offsets"
//      - a vector of incident members accessed by the "offset" of each
//
//  The "dynamic relation" allocates the latter vector of members based on a
//  typical number of members per component, e.g. we expect valence 4 vertices
//  in a typical quad-mesh, and so an "expected" number might be 6 to
//  accommodate a few x-ordinary vertices.  The member vector is allocated with
//  this number per component and the counts and offsets initialized to refer to
//  them -- but with the counts set to 0. The count will be incremented as
//  members are identified and entered, and if any component "overflows" the
//  expected number of members, the members are moved to a separate vector in an
//  std::map for the component.
//
//  Once all incident members have been added, the main vector is compressed and
//  may need to merge entries from the map in the process.
//
typedef std::map<Index, IndexVector> IrregIndexMap;

class DynamicRelation {
public:
  DynamicRelation(IndexVector &countAndOffsets, IndexVector &indices,
                  int membersPerComp);
  ~DynamicRelation() {}
...
inline DynamicRelation::DynamicRelation(IndexVector &countAndOffsets,
                                        IndexVector &indices,
                                        int membersPerComp)
    : _compCount(0), _memberCountPerComp(membersPerComp),
      _countsAndOffsets(countAndOffsets), _regIndices(indices) {
...
inline void DynamicRelation::appendCompMember(Index compIndex,
                                              Index memberValue) {
  ...
  if (count < _memberCountPerComp) {
    ...
    if (count > _memberCountPerComp) {
      ...
      std::memcpy(&irregMembers[0], &_regIndices[offset],
                  sizeof(Index) * _memberCountPerComp);
...
int DynamicRelation::compressMemberIndices() {
  ...
  if (_irregIndices.size() == 0) {
    ...
    for (int i = 1; i < _compCount; ++i) {
      ...
      memmove(&_regIndices[memberCount], &_regIndices[offset],
              count * sizeof(Index));
    ...
    //  Assign new offsets-per-component while determining if we can trivially
    //  compress in place:
    ...
    cannotBeCompressedInPlace |=
        (memberCount > (_memberCountPerComp * _compCount));
    ...
    IndexVector &dstIndices =
        cannotBeCompressedInPlace ? tmpIndices : _regIndices;
...
//
inline Index Level::findEdge(Index v0Index, Index v1Index,
                             ConstIndexArray v0Edges) const {
...
Index Level::findEdge(Index v0Index, Index v1Index) const {
...
bool Level::completeTopologyFromFaceVertices() {
  ...
  //
  //  It's assumed (a pre-condition) that face-vertices have been fully
  //  specified and that we are to construct the remaining relations:  including
  //  the edge list.  We may want to support the existence of the edge list too
  //  in future:
  //
  ...
  //  May be unnecessary depending on how the vertices and faces were defined,
  //  but worth a call to ensure all data related to verts and faces is
  //  available -- this will be a harmless call if all has been taken care of.
  //
  //  Remember to resize edges similarly after the edge list has been
  //  assembled...
  ...
  //
  //  Resize face-edges to match face-verts and reserve for edges based on an
  //  estimate:
  //
  ...
  //
  //  Create the dynamic relations to be populated (edge-faces will remain empty
  //  as reserved above since there are currently no edges) and iterate through
  //  the faces to do so:
  //
  const int avgSize = 6;

  DynamicRelation dynEdgeFaces(this->_edgeFaceCountsAndOffsets,
                               this->_edgeFaceIndices, 2);
  DynamicRelation dynVertFaces(this->_vertFaceCountsAndOffsets,
                               this->_vertFaceIndices, avgSize);
  DynamicRelation dynVertEdges(this->_vertEdgeCountsAndOffsets,
                               this->_vertEdgeIndices, avgSize);

  //  Inspect each edge created and identify those that are non-manifold as we
  //  go:
  ...
  for (Index fIndex = 0; fIndex < fCount; ++fIndex) {
    ...
    for (int i = 0; i < fVerts.size(); ++i) {
      ...
      //
      //  If not degenerate, search for a previous occurrence of this edge
      //  [v0,v1] in v0's incident edge members.  Otherwise, set the edge index
      //  as invalid to trigger creation of a new/unique instance of the
      //  degenerate edge:
      //
      Index eIndex;
      if (v0Index != v1Index) {
        eIndex = this->findEdge(v0Index, v1Index,
                                dynVertEdges.getCompMembers(v0Index));
      } else {
      ...
      //
      //  If the edge already exists, see if it is non-manifold, i.e. it has
      //  already been added to two faces, or this face has the edge in the same
      //  orientation as the first face (indicating opposite winding orders
      //  between the two faces).
      //
      //  Otherwise, create a new edge, append the new vertex pair [v0,v1] and
      //  update the incidence relations for the edge and its end vertices and
      //  this face.
      //
      //  Regardless of whether or not the edge was new, update the edge-faces,
      //  the face-edges and the vertex-faces for this vertex.
      //
  ...
  //
  //  Compress the incident member vectors while determining the maximum for
  //  each. Use these to set maximum relation count members and to test for
  //  valence or other incident member overflow:  max edge-faces is simple, but
  //  for max-valence, remember it was first initialized with the maximum of
  //  face-verts, so use its existing value -- and some non-manifold cases can
  //  have #faces > #edges, so be sure to consider both.
  //
  ...
  //  If max-edge-faces too large, max-valence must also be, so just need the
  //  one:
  ...
  //
  //  At this point all incident members are associated with each component.  We
  //  still need to populate the "local indices" for each and orient manifold
  //  components in counter-clockwise order.  First tag non-manifold edges and
  //  their incident vertices so that we can trivially skip orienting these --
  //  though some vertices will be determined non-manifold as a result of a
  //  failure to orient them (and will be marked accordingly when so detected).
  //
  //  Finally, the local indices are assigned.  This is trivial for manifold
  //  components as if component V is in component F, V will only occur once in
  //  F.  For non-manifold cases V may occur multiple times in F -- we rely on
  //  such instances being successive based on their original assignment above,
  //  which simplifies the task.
  //
  //  First resize edges to the new count to ensure anything related to edges is
  //  created:
...
void Level::populateLocalIndices() {
  ...
  //
  //  We have three sets of local indices -- edge-faces, vert-faces and
  //  vert-edges:
  //
  int eCount = this->getNumEdges();
  int vCount = this->getNumVertices();
  ...
  for (Index vIndex = 0; vIndex < vCount; ++vIndex) {
    ...
    for (int i = 0; i < vFaces.size(); ++i) {
      ...
      int vInFaceIndex =
          (int)(std::find(fVerts.begin() + vStart, fVerts.end(), vIndex) -
                fVerts.begin());
  ...
  for (Index eIndex = 0; eIndex < eCount; ++eIndex) {
    ...
    for (int i = 0; i < eFaces.size(); ++i) {
      ...
      int eInFaceIndex =
          (int)(std::find(fEdges.begin() + eStart, fEdges.end(), eIndex) -
                fEdges.begin());
...
namespace {
inline int findInArray(ConstIndexArray array, Index value) {
  return (int)(std::find(array.begin(), array.end(), value) - array.begin());
}
} // namespace

bool Level::orderVertexFacesAndEdges(Index vIndex, Index *vFacesOrdered,
                                     Index *vEdgesOrdered) const {
  ...
  //
  //  Note we have already eliminated the possibility of incident degenerate
  //  edges and other bad edges earlier -- marking its vertices non-manifold as
  //  a result and explicitly avoiding this method:
  //
  ...
  if (eCount == fCount) {
    ...
    //  Boundary case -- start with (identify) the leading of two boundary
    //  edges:
...
//
int Level::createFVarChannel(int fvarValueCount,
...
int Level::getNumFVarValues(int channel) const {
  return _fvarChannels[channel]->getNumValues();
}

Sdc::Options Level::getFVarOptions(int channel) const {
  return _fvarChannels[channel]->getOptions();
}

ConstIndexArray Level::getFaceFVarValues(Index faceIndex, int channel) const {
  return _fvarChannels[channel]->getFaceValues(faceIndex);
}

IndexArray Level::getFaceFVarValues(Index faceIndex, int channel) {
  return _fvarChannels[channel]->getFaceValues(faceIndex);
}

void Level::completeFVarChannelTopology(int channel, int regBoundaryValence) {
