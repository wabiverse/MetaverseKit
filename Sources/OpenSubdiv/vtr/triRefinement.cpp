...
//
TriRefinement::TriRefinement(Level const &parentArg, Level &childArg,
...
void TriRefinement::allocateParentChildIndices() {
  ...
  //
  //
  //  Beware these child-counts when Loop subdivision supports N-sided faces in
  //  the cage
  //      - there will 2*(N-2) additional face-child-faces for each N-sided face
  //      - there will 2*(N-2)+1 additional face-child-edges for each N-sided
  //      face
  //      - there will 1 face-child-vertex for each N-sided face
  //  Can consider these reasonable estimates and grow as needed later -- but be
  //  clear about it if so.
  //
  ...
  //
  //  First initialize the count/offset vectors for the child-faces and
  //  child-edges of parent faces.  For now we can use the parent's face-vert
  //  counts for the child-edges of faces, but we must use a local vector for
  //  the child-faces.
  //
  //  This will be more necessary (and need adjustment) when N-sided faces are
  //  supported.
  //
  ...
  _faceChildFaceCountsAndOffsets =
      IndexArray(&_localFaceChildFaceCountsAndOffsets[0],
...
void TriRefinement::populateFaceEdgesFromParentFaces() {
  ...
  for (Index pFace = 0; pFace < _parent->getNumFaces(); ++pFace) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      bool edgeReversedWrtFace =
          (pEdgeVerts[0] != pEdgeVerts[1]) && (pFaceVerts[i] != pEdgeVerts[0]);
...
void TriRefinement::populateEdgeFaceRelation() {
  ...
  int childEdgeFaceIndexSizeEstimate =
      (int)_faceChildEdgeIndices.size() * 2 +
...
void TriRefinement::populateVertexFaceRelation() {
  ...
  //  faces.  We also have to consider 3 faces for every incident face for
  //  vertices originating from edges.
  //
  int childVertFaceIndexSizeEstimate =
      (int)_parent->_edgeFaceIndices.size() * 3 +
...
void TriRefinement::populateVertexFacesFromParentEdges() {
  ...
  for (Index pEdge = 0; pEdge < _parent->getNumEdges(); ++pEdge) {
    ...
    for (int i = 0; i < pEdgeFaces.size(); ++i) {
      ...
      //  since we have the desired ordering of child faces from the parent
      //  face, we don't care about the orientation of the parent edge.
      //
...
void TriRefinement::populateVertexEdgeRelation() {
  ...
  int childVertEdgeIndexSizeEstimate =
      (int)_parent->_edgeFaceIndices.size() * 2 + _parent->getNumEdges() * 2 +
