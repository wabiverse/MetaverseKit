...
//  Length Arrays) which most compilers support, but are not strictly standard
//  C++. Portability concerns forces us to make use of either alloca() or some
//  other mechanism to create small arrays on the stack that are typically based
//  on the valence of a vertex -- small in general, but occasionally large.
//
//  Note also that since the intent of this is to replace VLAs -- not general
//  std::vectors -- support for std::vector functionality is intentionally
//  limited and STL-like naming is avoided.  Like a VLA there is no incremental
//  growth. Support for resizing is available to reuse an instance at the
//  beginning of a loop with a new size, but resizing in this case reinitializes
//  all elements.
//
...
private:
  ...
  StackBuffer &operator=(const StackBuffer<TYPE, SIZE, POD_TYPE> &) {
    return *this;
  }
...
private:
  ...
  //  Is alignment an issue here?  The staticData arena will at least be
  //  double-word aligned within this struct, which meets current and most
  //  anticipated needs.
...
inline void StackBuffer<TYPE, SIZE, POD_TYPE>::allocate(size_type capacity) {

  //  Again, is alignment an issue here?  C++ spec says new will return pointer
  //  "suitably aligned" for conversion to pointers of other types, which
  //  implies at least an alignment of 16.
...
template <typename TYPE, unsigned int SIZE, bool POD_TYPE>
inline void StackBuffer<TYPE, SIZE, POD_TYPE>::deallocate() {
...
//
template <typename TYPE, unsigned int SIZE, bool POD_TYPE>
inline void StackBuffer<TYPE, SIZE, POD_TYPE>::construct() {
...
template <typename TYPE, unsigned int SIZE, bool POD_TYPE>
inline void StackBuffer<TYPE, SIZE, POD_TYPE>::destruct() {
...
//
template <typename TYPE, unsigned int SIZE, bool POD_TYPE>
inline StackBuffer<TYPE, SIZE, POD_TYPE>::StackBuffer()
    : _data(reinterpret_cast<TYPE *>(_staticData)), _size(0), _capacity(SIZE),
      _dynamicData(0) {}

template <typename TYPE, unsigned int SIZE, bool POD_TYPE>
inline StackBuffer<TYPE, SIZE, POD_TYPE>::StackBuffer(size_type size)
    : _data(reinterpret_cast<TYPE *>(_staticData)), _size(size),
      _capacity(SIZE), _dynamicData(0) {
...
//
template <typename TYPE, unsigned int SIZE, bool POD_TYPE>
inline void StackBuffer<TYPE, SIZE, POD_TYPE>::Reserve(size_type capacity) {
...
template <typename TYPE, unsigned int SIZE, bool POD_TYPE>
inline void StackBuffer<TYPE, SIZE, POD_TYPE>::SetSize(size_type size) {
  if (!POD_TYPE) {
    destruct();
  }
  if (size == 0) {
