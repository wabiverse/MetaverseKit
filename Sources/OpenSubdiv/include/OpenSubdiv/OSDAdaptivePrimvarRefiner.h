...
public:
  ...
  ///       <br><br>
  ///       See the <a
  ///       href=http://graphics.pixar.com/opensubdiv/docs/tutorials.html> Far
  ///       tutorials</a> for code examples.
  ///
  ...
  ///
  /// @param dst    Destination primvar buffer (\ref templating refined vertex
  /// data)
  ///
  ...
  /// involve any weighting.  Setting the source primvar data for the base level
  /// to be the index of each face allows the propagation of the base face to
  /// primvar data for child faces in all levels.
  ///
  /// The destination buffer must allocate an array of data for all the refined
  /// faces, i.e. at least refiner.GetLevel(level).GetNumFaces()
  ///
  /// @param level  The refinement level
  ///
  ...
  /// correspondence between vertices and face-varying values -- typically there
  /// are more face-varying values than vertices.  Each face-varying channel is
  /// also independent in how its values relate to the vertices.
  ///
  /// The destination buffer must allocate an array of data for all the refined
  /// values, i.e. at least refiner.GetLevel(level).GetNumFVarValues(channel).
  ///
  template <class T, class U>
  void InterpolateFaceVarying(int level, T const &src, U &dst,
                              int channel = 0) const;
...
private:
  ...
  void interpFromFaces(int, T const &, U &) const;
  template <Sdc::SchemeType SCHEME, class T, class U>
  void interpFromEdges(int, T const &, U &) const;
  template <Sdc::SchemeType SCHEME, class T, class U>
  void interpFromVerts(int, T const &, U &) const;

  template <Sdc::SchemeType SCHEME, class T, class U>
  void interpFVarFromFaces(int, T const &, U &, int) const;
  template <Sdc::SchemeType SCHEME, class T, class U>
  void interpFVarFromEdges(int, T const &, U &, int) const;
  template <Sdc::SchemeType SCHEME, class T, class U>
  void interpFVarFromVerts(int, T const &, U &, int) const;
...
private:
  //
  //  Local class to fulfill interface for <typename MASK> in the Scheme mask
  //  queries:
  //
  class Mask {
  public:
  ...
  public:
    Mask(Weight *v, Weight *e, Weight *f)
        : _vertWeights(v), _edgeWeights(e), _faceWeights(f), _vertCount(0),
          _edgeCount(0), _faceCount(0), _faceWeightsForFaceCenters(false) {}
...
template <class T, class U>
inline void PrimvarRefinerReal<REAL>::Interpolate(int level, T const &src,
                                                  U &dst) const {
...
template <class T, class U>
inline void
PrimvarRefinerReal<REAL>::InterpolateFaceVarying(int level, T const &src,
                                                 U &dst, int channel) const {
...
template <typename REAL>
template <class T, class U>
inline void PrimvarRefinerReal<REAL>::Limit(T const &src, U &dst) const {
...
template <typename REAL>
template <class T, class U, class U1, class U2>
inline void PrimvarRefinerReal<REAL>::Limit(T const &src, U &dstPos,
                                            U1 &dstTan1, U2 &dstTan2) const {
...
template <typename REAL>
template <class T, class U>
inline void PrimvarRefinerReal<REAL>::LimitFaceVarying(T const &src, U &dst,
                                                       int channel) const {
...
template <typename REAL>
template <class T, class U>
inline void PrimvarRefinerReal<REAL>::InterpolateFaceUniform(int level,
                                                             T const &src,
                                                             U &dst) const {
  ...
  Vtr::internal::Refinement const &refinement =
      _refiner.getRefinement(level - 1);
...
template <typename REAL>
template <class T, class U>
inline void PrimvarRefinerReal<REAL>::InterpolateVarying(int level,
                                                         T const &src,
                                                         U &dst) const {
...
template <typename REAL>
template <Sdc::SchemeType SCHEME, class T, class U>
inline void PrimvarRefinerReal<REAL>::interpFromFaces(int level, T const &src,
                                                      U &dst) const {
  ...
  for (int face = 0; face < parent.getNumFaces(); ++face) {
    ...
    //  Declare and compute mask weights for this vertex relative to its parent
    //  face:
    ConstIndexArray fVerts = parent.getFaceVertices(face);
...
template <typename REAL>
template <Sdc::SchemeType SCHEME, class T, class U>
inline void PrimvarRefinerReal<REAL>::interpFromEdges(int level, T const &src,
                                                      U &dst) const {
  ...
  for (int edge = 0; edge < parent.getNumEdges(); ++edge) {
    ...
    Sdc::Crease::Rule pRule = (parent.getEdgeSharpness(edge) > 0.0f)
...
template <typename REAL>
template <Sdc::SchemeType SCHEME, class T, class U>
inline void PrimvarRefinerReal<REAL>::interpFromVerts(int level, T const &src,
                                                      U &dst) const {
  ...
  Vtr::internal::StackBuffer<Weight, 32> weightBuffer(2 *
                                                      parent.getMaxValence());
  ...
  for (int vert = 0; vert < parent.getNumVertices(); ++vert) {
    ...
    Weight vVertWeight, *vEdgeWeights = weightBuffer,
    ...
    //  Apply the weights to the parent vertex, the vertices opposite its
    //  incident edges, and the child vertices of its incident faces:
    //
    //  In order to improve numerical precision, it's better to apply smaller
    //  weights first, so begin with the face-weights followed by the
    //  edge-weights and the vertex weight last.
    dst[cVert].Clear();
    ...
    if (vMask.GetNumEdgeWeights() > 0) {
      ...
      for (int i = 0; i < vEdges.size(); ++i) {
        ...
        Vtr::Index pVertOppositeEdge =
            (eVerts[0] == vert) ? eVerts[1] : eVerts[0];
...
template <typename REAL>
template <Sdc::SchemeType SCHEME, class T, class U>
inline void PrimvarRefinerReal<REAL>::interpFVarFromFaces(int level,
                                                          T const &src, U &dst,
                                                          int channel) const {
  ...
  Vtr::internal::FVarLevel const &parentFVar =
      parentLevel.getFVarLevel(channel);
  Vtr::internal::FVarLevel const &childFVar = childLevel.getFVarLevel(channel);

  Vtr::internal::StackBuffer<Weight, 16> fValueWeights(
  ...
  for (int face = 0; face < parentLevel.getNumFaces(); ++face) {
    ...
    //  The only difference for face-varying here is that we get the values
    //  associated with each face-vertex directly from the FVarLevel, rather
    //  than using the parent face-vertices directly.  If any face-vertex has
    //  any sibling values, then we may get the wrong one using the face-vertex
    //  index directly.
...
template <typename REAL>
template <Sdc::SchemeType SCHEME, class T, class U>
inline void PrimvarRefinerReal<REAL>::interpFVarFromEdges(int level,
                                                          T const &src, U &dst,
                                                          int channel) const {
  ...
  Vtr::internal::FVarRefinement const &refineFVar =
  ...
  //
  //  Allocate and initialize (if linearly interpolated) interpolation weights
  //  for the edge mask:
  //
  Weight eVertWeights[2];
  Vtr::internal::StackBuffer<Weight, 8> eFaceWeights(
      parentLevel.getMaxEdgeFaces());

  Mask eMask(eVertWeights, 0, eFaceWeights);

  bool isLinearFVar =
      parentFVar.isLinear() || (_refiner._subdivType == Sdc::SCHEME_BILINEAR);
  if (isLinearFVar) {
  ...
  for (int edge = 0; edge < parentLevel.getNumEdges(); ++edge) {
    ...
    bool fvarEdgeVertMatchesVertex =
        childFVar.valueTopologyMatches(cVertValues[0]);
    if (fvarEdgeVertMatchesVertex) {
      ...
      //  the child vertices of its incident faces:
      //
      //  Even though the face-varying topology matches the vertex topology, we
      //  need to be careful here when getting values corresponding to the two
      //  end-vertices. While the edge may be continuous, the vertices at their
      //  ends may have discontinuities elsewhere in their neighborhood (i.e. on
      //  the "other side" of the end-vertex) and so have sibling values
      //  associated with them.  In most cases the topology for an end-vertex
      //  will match and we can use it directly, but we must still check and
      //  retrieve as needed.
      //
      //  Indices for values corresponding to face-vertices are guaranteed to
      //  match, so we can use the child-vertex indices directly.
      //
      //  And by "directly", we always use getVertexValue(vertexIndex) to
      //  reference values in the "src" to account for the possible indirection
      //  that may exist at level 0 -- where there may be fewer values than
      //  vertices and an additional indirection is necessary.  We can use a
      //  vertex index directly for "dst" when it matches.
      //
      ...
      if (eMask.GetNumFaceWeights() > 0) {
        ...
        for (int i = 0; i < eFaces.size(); ++i) {
          if (eMask.AreFaceWeightsForFaceCenters()) {
            ...
            Vtr::Index cValueOfFace =
                childFVar.getVertexValueOffset(cVertOfFace);
      ...
      //
      //  Mismatched edge-verts should just be linearly interpolated between the
      //  pairs of values for each sibling of the child edge-vertex -- the
      //  question is:  which face holds that pair of values for a given
      //  sibling?
      //
      //  In the manifold case, the sibling and edge-face indices will
      //  correspond.  We will eventually need to update this to account for > 3
      //  incident faces.
      //
...
template <typename REAL>
template <Sdc::SchemeType SCHEME, class T, class U>
inline void PrimvarRefinerReal<REAL>::interpFVarFromVerts(int level,
                                                          T const &src, U &dst,
                                                          int channel) const {
  ...
  Vtr::internal::StackBuffer<Weight, 32> weightBuffer(
      2 * parentLevel.getMaxValence());

  Vtr::internal::StackBuffer<Vtr::Index, 16> vEdgeValues(
      parentLevel.getMaxValence());
  ...
  for (int vert = 0; vert < parentLevel.getNumVertices(); ++vert) {
    ...
    bool fvarVertVertMatchesVertex =
        childFVar.valueTopologyMatches(cVertValues[0]);
    ...
    if (fvarVertVertMatchesVertex) {
      //
      //  Declare and compute mask weights for this vertex relative to its
      //  parent edge:
      //
      //  (We really need to encapsulate this somewhere else for use here and in
      //  the general case)
      //
      ...
      //
      //  Even though the face-varying topology matches the vertex topology, we
      //  need to be careful here when getting values corresponding to vertices
      //  at the ends of edges.  While the edge may be continuous, the end
      //  vertex may have discontinuities elsewhere in their neighborhood (i.e.
      //  on the "other side" of the end-vertex) and so have sibling values
      //  associated with them.  In most cases the topology for an end-vertex
      //  will match and we can use it directly, but we must still check and
      //  retrieve as needed.
      //
      ...
      //  vertex index directly for "dst" when it matches.
      //
      //  As with applying the mask to vertex data, in order to improve
      //  numerical precision, it's better to apply smaller weights first, so
      //  begin with the face-weights followed by the edge-weights and the
      //  vertex weight last.
      //
      ...
      //
      //  Each FVar value associated with a vertex will be either a corner or a
      //  crease, or potentially in transition from corner to crease:
      //      - if the CHILD is a corner, there can be no transition so we have
      //      a corner
      //      - otherwise if the PARENT is a crease, both will be creases (no
      //      transition)
      //      - otherwise the parent must be a corner and the child a crease
      //      (transition)
      //
      Vtr::internal::FVarLevel::ConstValueTagArray pValueTags =
          parentFVar.getVertexValueTags(vert);
      Vtr::internal::FVarLevel::ConstValueTagArray cValueTags =
          childFVar.getVertexValueTags(cVert);

      for (int cSiblingIndex = 0; cSiblingIndex < cVertValues.size();
           ++cSiblingIndex) {
        int pSiblingIndex =
            refineFVar.getChildValueParentSource(cVert, cSiblingIndex);
        ...
        if (isLinearFVar || cValueTags[cSibling].isCorner()) {
          ...
          //
          //  We have either a crease or a transition from corner to crease --
          //  in either case, we need the end values for the full/fractional
          //  crease:
          //
          Index pEndValues[2];
          ...
          //  sharp because of the other sibling (dependent-sharp) use the
          //  fractional weight from that other sibling (should only occur when
          //  there are 2):
          //
          if (pValueTags[pSibling].isSemiSharp()) {
            Weight wCorner = pValueTags[pSibling].isDepSharp()
                                 ? refineFVar.getFractionalWeight(
                                       vert, !pSibling, cVert, !cSibling)
                                 : refineFVar.getFractionalWeight(
                                       vert, pSibling, cVert, cSibling);
...
inline void PrimvarRefinerReal<REAL>::limit(T const &src, U &dstPos,
  ...
  Vtr::internal::StackBuffer<Weight, 99> weightBuffer(numMasks *
                                                      maxWeightsPerMask);

  Weight *vPosWeights = weightBuffer, *ePosWeights = vPosWeights + 1,
  ...
  //  This is a bit obscure -- assigning both parent and child as last level --
  //  but this mask type was intended for another purpose.  Consider one for the
  //  limit:
  ...
  for (int vert = 0; vert < level.getNumVertices(); ++vert) {
    ...
    //  Incomplete vertices (present in sparse refinement) do not have their
    //  full topological neighborhood to determine a proper limit -- just leave
    //  the vertex at the refined location and continue to the next:
    ...
    //  with infinitely sharp features correctly -- including boundaries and
    //  corners. The vertex neighborhood is minimally defined with vertex and
    //  edge counts.
    //
    ...
    //
    //  Gather the neighboring vertices of this vertex -- the vertices opposite
    //  its incident edges, and the opposite vertices of its incident faces:
    //
    ...
    if (posMask.GetNumFaceWeights() ||
        (hasTangents && tan1Mask.GetNumFaceWeights())) {
    ...
    //
    //  Combine the weights and indices for position and tangents.  As with
    //  applying refinement masks to vertex data, in order to improve numerical
    ...
    //  indices (one tangent may be "padded" to accommodate the other), but
    //  these may differ from those of the position:
    //
    if (hasTangents) {
...
template <typename REAL>
template <Sdc::SchemeType SCHEME, class T, class U>
inline void PrimvarRefinerReal<REAL>::limitFVar(T const &src, U &dst,
                                                int channel) const {
  ...
  for (int vert = 0; vert < level.getNumVertices(); ++vert) {
    ...
    //  Incomplete vertices (present in sparse refinement) do not have their
    //  full topological neighborhood to determine a proper limit -- just leave
    //  the values (perhaps more than one per vertex) at the refined location.
    //
    //  The same can be done if the face-varying channel is purely linear.
    //
    bool isIncomplete =
        (level.getVertexTag(vert)._incomplete || (vEdges.size() == 0));
    ...
    if (fvarVertMatchesVertex) {
      ...
      //
      Weight *vWeights = weightBuffer, *eWeights = vWeights + 1,
      ...
      //
      //  Sibling FVar values associated with a vertex will be either a corner
      //  or a crease:
      //
      for (int i = 0; i < vValues.size(); ++i) {
        Vtr::Index vValue = vValues[i];
