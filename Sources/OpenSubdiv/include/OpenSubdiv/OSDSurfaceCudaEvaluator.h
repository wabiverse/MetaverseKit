...
private:
  void *_sizes, *_offsets, *_indices, *_weights, *_duWeights, *_dvWeights,
      *_duuWeights, *_duvWeights, *_dvvWeights;
  int _numStencils;
};
...
public:
  ...
  EvalStencils(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
               ...
               STENCIL_TABLE const *stencilTable, const void *instance = NULL,
               void *deviceContext = NULL) {
  ...
  EvalStencils(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalStencils(
        srcBuffer->BindCudaBuffer(), srcDesc, dstBuffer->BindCudaBuffer(),
        dstDesc, duBuffer->BindCudaBuffer(), duDesc, dvBuffer->BindCudaBuffer(),
        dvDesc, (int const *)stencilTable->GetSizesBuffer(),
        (int const *)stencilTable->GetOffsetsBuffer(),
        (int const *)stencilTable->GetIndicesBuffer(),
        (float const *)stencilTable->GetWeightsBuffer(),
  ...
  EvalStencils(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalStencils(
        srcBuffer->BindCudaBuffer(), srcDesc, dstBuffer->BindCudaBuffer(),
        dstDesc, duBuffer->BindCudaBuffer(), duDesc, dvBuffer->BindCudaBuffer(),
        dvDesc, duuBuffer->BindCudaBuffer(), duuDesc,
        duvBuffer->BindCudaBuffer(), duvDesc, dvvBuffer->BindCudaBuffer(),
        dvvDesc, (int const *)stencilTable->GetSizesBuffer(),
        (int const *)stencilTable->GetOffsetsBuffer(),
        (int const *)stencilTable->GetIndicesBuffer(),
        (float const *)stencilTable->GetWeightsBuffer(),
        (float const *)stencilTable->GetDuWeightsBuffer(),
  ...
  EvalPatches(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
              ...
              PATCH_TABLE *patchTable, CudaEvaluator const *instance,
              void *deviceContext = NULL) {
    ...
    return EvalPatches(srcBuffer->BindCudaBuffer(), srcDesc,
                       dstBuffer->BindCudaBuffer(), dstDesc, numPatchCoords,
  ...
  EvalPatches(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalPatches(srcBuffer->BindCudaBuffer(), srcDesc,
                       ...
                       dvBuffer->BindCudaBuffer(), dvDesc, numPatchCoords,
                       (const PatchCoord *)patchCoords->BindCudaBuffer(),
                       (const PatchArray *)patchTable->GetPatchArrayBuffer(),
                       (const int *)patchTable->GetPatchIndexBuffer(),
                       (const PatchParam *)patchTable->GetPatchParamBuffer());
