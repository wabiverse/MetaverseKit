...
//
//  Declaration for the main refinement class (Refinement) and its
//  pre-requisites:
//
...
//
//  Refinement:
//      A refinement is a mapping between two levels -- relating the components
//      in the original
//  (parent) level to the one refined (child).  The refinement may be complete
//  (uniform) or sparse (adaptive or otherwise selective), so not all components
//  in the parent level will spawn components in the child level.
//
//  Refinement is an abstract class and expects subclasses corresponding to the
//  different types of topological splits that the supported subdivision schemes
//  collectively require, i.e. those listed in Sdc::SplitType.  Note the virtual
//  requirements expected of the subclasses in the list of protected methods --
//  they differ mainly in the topology that is created in the child Level and
//  not the propagation of tags through refinement, subdivision of sharpness
//  values or the treatment of face-varying data.  The primary subclasses are
//  QuadRefinement and TriRefinement.
//
//  At a high level, all that is necessary in terms of interface is to
//  construct, initialize (linking the two levels), optionally select components
//  for sparse refinement (via use of the SparseSelector) and call the refine()
//  method.  This usage is expected of Far::TopologyRefiner.
//
//  Since we really want this class to be restricted from public access
//  eventually, all methods begin with lower case (as is the convention for
//  protected methods) and the list of friends will be maintained more strictly.
//
class Refinement {

public:
  Refinement(Level const &parent, Level &child,
  ...
  //          -- which requires at least the face-vertices for faces, but also
  //          the vertex-faces for any face-varying channels present.  So it
  //          will generate one or two of the six possible topological
  //          relations.
  //
  //  These are strictly controlled right now, e.g. for sparse refinement, we
  //  currently enforce full topology at the finest level to allow for
  //  subsequent patch construction.
  //
  struct Options {
    Options()
        : _sparse(false), _faceVertsFirst(false), _minimalTopology(false) {}
...
public:
  //
  //  Tags have now been added per-component in Level, but there is additional
  //  need to tag components within Refinement -- we can't tag the parent level
  //  components for any refinement (in order to keep it const) and tags
  //  associated with children that are specific to the child-to-parent mapping
  //  may not be warranted in the child level.
  //
  //  Parent tags are only required for sparse refinement.  The main property to
  //  tag is whether a component was selected, and so a single SparseTag is used
  //  for all three component types.  Tagging if a component is "transitional"
  //  is also useful.  This may only be necessary for edges but is currently
  //  packed into a mask per-edge for faces, which could be deferred, in which
  //  case "transitional" could be a single bit.
  //
  //  Child tags are part of the child-to-parent mapping, which consists of the
  //  parent component index for each child component, plus a tag for the child
  //  indicating more about its relationship to its parent, e.g. is it
  //  completely defined, what the parent component type is, what is the index
  //  of the child within its parent, etc.
  //
  struct SparseTag {
    SparseTag() : _selected(0), _transitional(0) {}

    unsigned char
        _selected : 1; // component specifically selected for refinement
    unsigned char
        _transitional : 4; // adjacent to a refined component (4-bits for face)
  };
  ...
  struct ChildTag {
    ...
    unsigned char
        _incomplete : 1; // incomplete neighborhood to represent limit of parent
    unsigned char
        _parentType : 2; // type of parent component:  vertex, edge or face
    unsigned char _indexInParent : 2; // index of child wrt parent:  0-3, or
                                      // iterative if N > 4
  };
...
public:
  ...
  //  refinement is sparse, additional methods are needed to identify the
  //  selection:
  //
  ...
  void
  populateVertexParentFromParentFaces(ChildTag const initialChildTags[2][4]);
  void
  populateVertexParentFromParentEdges(ChildTag const initialChildTags[2][4]);
  void
  populateVertexParentFromParentVertices(ChildTag const initialChildTags[2][4]);
...
protected:
  ...
  //
  //  Data members -- the logical grouping of some of these (and methods that
  //  make use of them) may lead to grouping them into a few utility classes or
  //  structs...
  //
  ...
  //
  int _childFaceFromFaceCount; // arguably redundant (all faces originate from
                               // faces)
  ...
  int _firstChildFaceFromFace; // arguably redundant (all faces originate from
                               // faces)
  ...
  //      These are vectors sized according to the number of parent components
  //      (and
  //  their topology) that contain references/indices to the child components
  //  that result from them by refinement.  When refinement is sparse, parent
  //  components that have not spawned all child components will have their
  //  missing children marked as invalid.
  //
  //  NOTE the "Array" members here.  Often vectors within the Level can be
  //  shared with the Refinement, and an Array instance is used to do so.  If
  //  not shared the subclass just initializes the Array members after
  //  allocating its own local vector members.
  //
  IndexArray _faceChildFaceCountsAndOffsets;
  IndexArray _faceChildEdgeCountsAndOffsets;

  IndexVector
      _faceChildFaceIndices; // *cannot* always use face-vert counts/offsets
  ...
  //
  //  Refinement data for face-varying channels present in the Levels being
  //  refined:
  //
  std::vector<FVarRefinement *> _fvarChannels;
};

inline ConstIndexArray Refinement::getFaceChildFaces(Index parentFace) const {

  return ConstIndexArray(
      &_faceChildFaceIndices[_faceChildFaceCountsAndOffsets[2 * parentFace +
                                                            1]],
      _faceChildFaceCountsAndOffsets[2 * parentFace]);
}

inline IndexArray Refinement::getFaceChildFaces(Index parentFace) {
...
inline ConstIndexArray Refinement::getFaceChildEdges(Index parentFace) const {

  return ConstIndexArray(
      &_faceChildEdgeIndices[_faceChildEdgeCountsAndOffsets[2 * parentFace +
                                                            1]],
      _faceChildEdgeCountsAndOffsets[2 * parentFace]);
}
inline IndexArray Refinement::getFaceChildEdges(Index parentFace) {
...
inline ConstIndexArray Refinement::getEdgeChildEdges(Index parentEdge) const {

  return ConstIndexArray(&_edgeChildEdgeIndices[parentEdge * 2], 2);
}

inline IndexArray Refinement::getEdgeChildEdges(Index parentEdge) {
