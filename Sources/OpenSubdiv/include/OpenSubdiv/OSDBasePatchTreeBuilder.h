...
public:
  ...
  struct Options {
    ...
    Options(int depth = 4)
        : irregularBasis((unsigned char)GREGORY),
          maxPatchDepthSharp((unsigned char)depth), maxPatchDepthSmooth(15),
          includeInteriorPatches(false), useDoublePrecision(false) {}
...
private:
  ...
  template <typename REAL>
  void
  appendConversionStencilsToMatrix(int stencilIndexBase,
