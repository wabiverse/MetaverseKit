...
public:
  ...
  void SubdivideCreaseWeight(HbrHalfedge<T> *edge, HbrVertex<T> *vertex,
...
protected:
  ...
  void AddSurroundingVerticesWithWeight(HbrMesh<T> *mesh, HbrVertex<T> *vertex,
                                        float weight, T *data);

  // Helper routine for subclasses: for a given vertex with a crease
  // mask, adds contributions from the two crease edges
  void AddCreaseEdgesWithWeight(HbrMesh<T> *mesh, HbrVertex<T> *vertex,
                                bool next, float weight, T *data);
...
private:
  ...
  public:
    SmoothSubdivisionVertexOperator(T *data, bool meshHasEdits, float weight)
        : m_data(data), m_meshHasEdits(meshHasEdits), m_weight(weight) {}
  ...
  public:
    CreaseSubdivisionHalfedgeOperator(HbrVertex<T> *vertex, T *data,
                                      bool meshHasEdits, bool next,
                                      float weight)
        : m_vertex(vertex), m_data(data), m_meshHasEdits(meshHasEdits),
          m_next(next), m_weight(weight), m_count(0) {}
...
private:
  ...
  public:
    RefineFaceAtVertexOperator(HbrSubdivision<T> *subdivision, HbrMesh<T> *mesh,
                               HbrVertex<T> *vertex)
        : m_subdivision(subdivision), m_mesh(mesh), m_vertex(vertex) {}
...
template <class T>
void HbrSubdivision<T>::RefineAtVertex(HbrMesh<T> *mesh, HbrVertex<T> *vertex) {
...
template <class T>
void HbrSubdivision<T>::SubdivideCreaseWeight(HbrHalfedge<T> *edge,
                                              HbrVertex<T> *vertex,
                                              HbrHalfedge<T> *subedge) {
  ...
  // plus 1/4 of crease sharpnesses incident to vertex
    ...
    public:
      ChaikinEdgeCreaseOperator(HbrHalfedge<T> const *edge, float &childsharp,
                                int &count)
...
template <class T>
void HbrSubdivision<T>::AddSurroundingVerticesWithWeight(HbrMesh<T> *mesh,
                                                         HbrVertex<T> *vertex,
                                                         float weight,
                                                         T *data) {
...
template <class T>
void HbrSubdivision<T>::AddCreaseEdgesWithWeight(HbrMesh<T> *mesh,
                                                 HbrVertex<T> *vertex,
                                                 bool next, float weight,
                                                 T *data) {
  CreaseSubdivisionHalfedgeOperator op(vertex, data, mesh->HasVertexEdits(),
                                       next, weight);
  vertex->ApplyOperatorSurroundingEdges(op);
}
