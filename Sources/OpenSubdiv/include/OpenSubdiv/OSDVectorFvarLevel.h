...
//
//  Analogous to a set of vertices and face-vertices that define the topology
//  for the geometry, a channel requires a set of "values" and "face-values".
//  The "values" are indices of entries in a set of face-varying data, just as
//  vertices are indices into a set of vertex data.  The face-values identify a
//  value for each vertex of the face, and so define topology for the values
//  that may be unique to each channel.
//
//  In addition to the value size and the vector of face-values (which matches
//  the size of the geometry's face-vertices), tags are associated with each
//  component to identify deviations of the face-varying topology from the
//  vertex topology. And since there may be a one-to-many mapping between
//  vertices and face-varying values, that mapping is also allocated.
//
...
//  become a concern, we do not need to store them.  The only reason we do so
//  now is that the face-value interface for specifying base topology and
//  inspecting subsequent levels is very familiar to that of face-vertices for
//  clients.  So having them available for such access is convenient.
//
//  Regarding scope and access...
//      Unclear at this early state, but leaning towards nesting this class
//      within
//  Level, given the intimate dependency between the two.
//      Everything is being declared public for now to facilitate access until
//      it's
...
public:
  ...
  struct ValueTag {
    ...
    ValueTagSize
        _infIrregular : 1; // value span includes inf-sharp irregularity
...
public:
  ...
  //  Tag queries related to faces (use Level methods for those returning
  //  Level::VTag/ETag)
  ...
  void getEdgeFaceValues(Index eIndex, int fIncToEdge,
                         Index valuesPerVert[2]) const;
  void getVertexEdgeValues(Index vIndex, Index valuesPerEdge[]) const;
  void getVertexCreaseEndValues(Index vIndex, Sibling sibling,
  ...
  bool validate() const;
  void print() const;
  void buildFaceVertexSiblingsFromVertexFaceSiblings(
...
private:
  ...
  //  data through the refinement hierarchy.  Vectors are not sparse but most
  //  use 8-bit values relative to the local topology.
  //
  //  The vector of face-values is actually redundant here, but is constructed
  //  as it is most convenient for clients.  It represents almost half the
  //  memory of the topology (4 32-bit integers per face) and not surprisingly,
  //  populating it takes a considerable amount of the refinement time (1/3). We
  //  can reduce both if we are willing to compute these on demand for clients.
  //
...
inline Index FVarLevel::findVertexValueIndex(Index vertexIndex,
