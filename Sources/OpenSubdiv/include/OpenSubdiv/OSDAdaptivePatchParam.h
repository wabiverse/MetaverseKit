...
struct PatchParam {
  ...
  unsigned short GetBoundary() const {
    return (unsigned short)unpack(field1, 5, 7);
  }
  ...
  unsigned short GetDepth() const {
    return (unsigned short)unpack(field1, 4, 0);
  }
  ...
  /// \brief A (u,v) pair in a normalized parametric space is mapped back into
  /// the fraction of parametric space covered by this face.
  ///
  /// @param u  u parameter
  /// @param v  v parameter
  ///
...
inline void PatchParam::Set(Index faceid, short u, short v,
                            unsigned short depth, bool nonquad,
                            unsigned short boundary, unsigned short transition,
                            bool regular) {
  field0 = pack(faceid, 28, 0) | pack(transition, 4, 28);

  field1 = pack(u, 10, 22) | pack(v, 10, 12) | pack(boundary, 5, 7) |
           pack(regular, 1, 5) | pack(nonquad, 1, 4) | pack(depth, 4, 0);
}

inline float PatchParam::GetParamFraction() const {
  return 1.0f / (float)(1 << (GetDepth() - NonQuadRoot()));
}

template <typename REAL>
inline void PatchParam::Normalize(REAL &u, REAL &v) const {
...
template <typename REAL>
inline void PatchParam::Unnormalize(REAL &u, REAL &v) const {
...
inline bool PatchParam::IsTriangleRotated() const {

  return (GetU() + GetV()) >= (1 << GetDepth());
}

template <typename REAL>
inline void PatchParam::NormalizeTriangle(REAL &u, REAL &v) const {
...
template <typename REAL>
inline void PatchParam::UnnormalizeTriangle(REAL &u, REAL &v) const {
