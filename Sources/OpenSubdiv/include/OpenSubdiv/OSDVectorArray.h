...
//
//  This class provides a simple array-like interface -- a subset std::vector's
//  interface -- for a sequence of elements stored in contiguous memory.  It
//  provides a unified representation for referencing data on the stack, all or
//  a subset of std::vector<>, or anywhere else in memory.
//
//  Note that its members are head/size rather than begin/end as in std::vector
//  -- we frequently need only the size for many queries, and that is most often
//  what is stored elsewhere in other classes, so we hope to reduce unnecessary
//  address arithmetic constructing the interface and accessing the size.  The
//  size type is also specifically 32-bit (rather than size_t) to match internal
//  usage and avoid unnecessary conversion to/from 64-bit.
//
//  Question:
//      Naming is at issue here...  formerly called ArrayInterface until that
//      was shot down it has
//  been simplified to Array but needs to be distanced from std::array as it
//  DOES NOT store its own memory and is simply an interface to memory stored
//  elsewhere.
//
