...
public:
  StencilBuilder(int coarseVertCount, bool genCtrlVertStencils = true,
  ...
  // Mapping from stencil[i] to its starting offset in the sources[] and
  // weights[] arrays;
  ...
  public:
    ...
    void AddWithWeight(StencilReal<REAL> const &src, REAL weight, REAL du,
                       REAL dv);

    // Add with first and second derivatives.
    void AddWithWeight(StencilReal<REAL> const &src, REAL weight, REAL du,
                       REAL dv, REAL duu, REAL duv, REAL dvv);
