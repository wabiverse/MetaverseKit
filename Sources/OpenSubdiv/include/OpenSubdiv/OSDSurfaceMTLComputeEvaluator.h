...
public:
  ...
  ///
  /// @param context        used to obtain the MTLDevice object and command
  /// queue
  ...
  EvalStencils(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    if (instance) {
      return instance->EvalStencils(srcBuffer, srcDesc, dstBuffer, dstDesc,
                                    ...
                                    dvvBuffer, dvvDesc, stencilTable, context);
    } else {
      // Create an instance on demand (slow)
  ...
  bool EvalStencils(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalStencils(
        srcBuffer->BindMTLBuffer(context), srcDesc,
        dstBuffer->BindMTLBuffer(context), dstDesc, 0, BufferDescriptor(), 0,
        BufferDescriptor(), stencilTable->GetSizesBuffer(),
  ...
  EvalPatches(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
              ...
              PATCH_TABLE *patchTable, MTLComputeEvaluator const *instance,
              MTLContext *context) {
  ...
  EvalPatches(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
              ...
              DST_BUFFER *dvBuffer, BufferDescriptor const &dvDesc,
              int numPatchCoords, PATCHCOORD_BUFFER *patchCoords,
              PATCH_TABLE *patchTable, MTLComputeEvaluator *instance,
              MTLContext *context) {

    if (instance) {
      return instance->EvalPatches(
          srcBuffer, srcDesc, dstBuffer, dstDesc, duBuffer, duDesc, dvBuffer,
          dvDesc, numPatchCoords, patchCoords, patchTable, context);
    } else {
      // Create an instance on demand (slow)
      instance = Create(srcDesc, dstDesc, duDesc, dvDesc, context);
      if (instance) {
  ...
  EvalPatches(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    if (instance) {
      return instance->EvalPatches(
          srcBuffer, srcDesc, dstBuffer, dstDesc, duBuffer, duDesc, dvBuffer,
          dvDesc, duuBuffer, duuDesc, duvBuffer, duvDesc, dvvBuffer, dvvDesc,
          numPatchCoords, patchCoords, patchTable, context);
    } else {
      // Create an instance on demand (slow)
      instance = Create(srcDesc, dstDesc, duDesc, dvDesc, duuDesc, duvDesc,
                        dvvDesc, context);
  ...
  bool EvalPatches(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalPatches(
        srcBuffer->BindMTLBuffer(context), srcDesc,
        dstBuffer->BindMTLBuffer(context), dstDesc, 0, BufferDescriptor(), 0,
        BufferDescriptor(), numPatchCoords, patchCoords->BindMTLBuffer(context),
  ...
  bool EvalPatches(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalPatches(srcBuffer->BindMTLBuffer(context), srcDesc,
                       dstBuffer->BindMTLBuffer(context), dstDesc,
                       duBuffer->BindMTLBuffer(context), duDesc,
                       dvBuffer->BindMTLBuffer(context), dvDesc, numPatchCoords,
  ...
  bool EvalPatches(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalPatches(srcBuffer->BindMTLBuffer(context), srcDesc,
                       ...
                       duvBuffer->BindMTLBuffer(context), duvDesc,
                       dvvBuffer->BindMTLBuffer(context), dvvDesc,
                       numPatchCoords, patchCoords->BindMTLBuffer(context),
                       patchTable->GetPatchArrays(),
                       patchTable->GetPatchIndexBuffer(),
                       patchTable->GetPatchParamBuffer(), context);
  }
  ...
  ///                       it may cause a performance problem.
  ///
  /// @param deviceContext  used to obtain the MTLDevice object and command
  /// queue
  ///                       to obtain command buffers from.
  ///
  template <typename SRC_BUFFER, typename DST_BUFFER,
  ...
  EvalPatchesVarying(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalPatches(srcBuffer->BindMTLBuffer(deviceContext), srcDesc,
                       dstBuffer->BindMTLBuffer(deviceContext), dstDesc, 0,
                       ...
                       patchTable->GetPatchParamBuffer(), deviceContext);
  }
  ...
  static bool EvalPatchesVarying(
    ...
    if (instance) {
      return instance->EvalPatchesVarying(
          srcBuffer, srcDesc, dstBuffer, dstDesc, duBuffer, duDesc, dvBuffer,
          dvDesc, numPatchCoords, patchCoords, patchTable, deviceContext);
    } else {
      // Create an instance on demand (slow)
  ...
  EvalPatchesVarying(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    if (instance) {
      return instance->EvalPatchesVarying(
          srcBuffer, srcDesc, dstBuffer, dstDesc, duBuffer, duDesc, dvBuffer,
          dvDesc, duuBuffer, duuDesc, duvBuffer, duvDesc, dvvBuffer, dvvDesc,
          numPatchCoords, patchCoords, patchTable, deviceContext);
    } else {
      // Create an instance on demand (slow)
      instance = Create(srcDesc, dstDesc, duDesc, dvDesc, duuDesc, duvDesc,
  ...
  EvalPatchesFaceVarying(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    if (instance) {
      return instance->EvalPatchesFaceVarying(
          srcBuffer, srcDesc, dstBuffer, dstDesc, numPatchCoords, patchCoords,
          patchTable, fvarChannel, deviceContext);
    } else {
      // Create an instance on demand (slow)
      instance = Create(srcDesc, dstDesc, BufferDescriptor(),
                        BufferDescriptor(), deviceContext);
  ...
  EvalPatchesFaceVarying(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalPatches(
        srcBuffer->BindMTLBuffer(deviceContext), srcDesc,
        dstBuffer->BindMTLBuffer(deviceContext), dstDesc, 0, BufferDescriptor(),
        0, BufferDescriptor(), numPatchCoords,
        patchCoords->BindMTLBuffer(deviceContext),
        patchTable->GetFVarPatchArrays(fvarChannel),
        patchTable->GetFVarPatchIndexBuffer(fvarChannel),
        patchTable->GetFVarPatchParamBuffer(fvarChannel), deviceContext);
  }
  ...
  EvalPatchesFaceVarying(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalPatches(
        ...
        dstBuffer->BindMTLBuffer(deviceContext), dstDesc,
        duBuffer->BindMTLBuffer(deviceContext), duDesc,
        dvBuffer->BindMTLBuffer(deviceContext), dvDesc, numPatchCoords,
        patchCoords->BindMTLBuffer(deviceContext),
        patchTable->GetFVarPatchArrays(fvarChannel),
        patchTable->GetFVarPatchIndexBuffer(fvarChannel),
        patchTable->GetFVarPatchParamBuffer(fvarChannel), deviceContext);
  ...
  EvalPatchesFaceVarying(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    if (instance) {
      return instance->EvalPatchesFaceVarying(
          srcBuffer, srcDesc, dstBuffer, dstDesc, duBuffer, duDesc, dvBuffer,
          dvDesc, duuBuffer, duuDesc, duvBuffer, duvDesc, dvvBuffer, dvvDesc,
          numPatchCoords, patchCoords, patchTable, fvarChannel, deviceContext);
    } else {
      // Create an instance on demand (slow)
      instance = Create(srcDesc, dstDesc, duDesc, dvDesc, duuDesc, duvDesc,
                        dvvDesc, deviceContext);
