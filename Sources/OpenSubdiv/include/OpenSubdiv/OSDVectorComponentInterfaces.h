...
//
//  For <typename FACE>, which provides information in the neighborhood of a
//  face:
//
class FaceInterface {
public:
...
//
//  For <typename EDGE>, which provides information in the neighborhood of an
//  edge:
//
class EdgeInterface {
public:
...
//
//  For <typename VERTEX>, which provides information in the neighborhood of a
//  vertex:
//
...
public: //  Generic interface expected of <typename VERT>:
  ...
  float *GetChildSharpnessPerEdge(Sdc::Crease const &crease,
                                  float cSharpness[]) const {
    internal::StackBuffer<float, 16> pSharpness(_eCount);
    GetSharpnessPerEdge(pSharpness);
    crease.SubdivideEdgeSharpnessesAroundVertex(_eCount, pSharpness,
                                                cSharpness);
    return cSharpness;
  }
