...
///
/// \brief An interface for accessing data in a specific level of a refined
/// topology hierarchy.
///
...
public:
  ...
  /// available for immediate inspection.  All are accessed by methods that
  /// return an array of fixed size containing the indices of the incident
  /// components.
  ///
  /// For some of the relations, i.e. those for which the incident components
  /// are of higher order or 'contain' the component itself (e.g. a vertex has
  /// incident faces that contain it), an additional 'local index' is available
  /// that identifies the component within each of its neighbors.  For example,
  /// if vertex V is the k'th vertex in some face F, then when F occurs in the
  /// set of incident vertices of V, the local index corresponding to F will be
  /// k.  The ordering of local indices matches the ordering of the incident
  ...
  ConstLocalIndexArray GetVertexFaceLocalIndices(Index v) const {
    return _level->getVertexFaceLocalIndices(v);
  }

  /// \brief Access the local indices of a vertex with respect to its incident
  /// edges
  ConstLocalIndexArray GetVertexEdgeLocalIndices(Index v) const {
    return _level->getVertexEdgeLocalIndices(v);
  }
  ...
  ConstLocalIndexArray GetEdgeFaceLocalIndices(Index e) const {
    return _level->getEdgeFaceLocalIndices(e);
  }
  ...
  bool IsVertexNonManifold(Index v) const {
    return _level->isVertexNonManifold(v);
  }
  ...
  /// \brief Return if the vertex is on a boundary (at least one incident
  /// boundary edge)
  bool IsVertexBoundary(Index v) const {
    return _level->getVertexTag(v)._boundary;
  }

  /// \brief Return if the vertex is a corner (only one incident face)
  bool IsVertexCorner(Index v) const {
    return (_level->getNumVertexFaces(v) == 1);
  }

  /// \brief Return if the valence of the vertex is regular (must be manifold)
  ///
  /// Note that this test only determines if the valence of the vertex is
  /// regular with respect to the assigned subdivision scheme -- not if the
  /// neighborhood around the vertex is regular. The latter depends on a number
  /// of factors including the incident faces of the vertex (they must all be
  /// regular) and the presence of sharpness at the vertex itself or its
  /// incident edges.
  ///
  /// The regularity of the valence is a necessary but not a sufficient
  /// condition in determining the regularity of the neighborhood. For example,
  /// while the valence of an interior vertex may be regular, its neighborhood
  /// is not if the vertex was made infinitely sharp.  Conversely, a corner
  /// vertex is considered regular by its valence but its neighborhood is not if
  /// the vertex was not made infinitely sharp.
  ///
  /// Whether the valence of the vertex is regular is also a property that
  /// remains the same for the vertex in all subdivision levels. In contrast,
  /// the regularity of the region around the vertex may change as the presence
  /// of irregular faces or semi-sharp features is reduced by subdivision.
  ///
  bool IsVertexValenceRegular(Index v) const {
    return !_level->getVertexTag(v)._xordinary || IsVertexCorner(v);
  }
  //@}
  ...
  float GetVertexSharpness(Index v) const {
    return _level->getVertexSharpness(v);
  }
  ...
  int GetNumFVarValues(int channel = 0) const {
    return _level->getNumFVarValues(channel);
  }
  ...
  void PrintTopology(bool children = true) const {
    _level->print((children && _refToChild) ? _refToChild : 0);
  }
  //@}
