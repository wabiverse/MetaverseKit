...
public:
  ...
  public:
    Options()
        : _preserveQuads(false), _facetSize4(false), _coordStride(0),
          _facetStride(0) {}
...
private:
  ...
  bool validateArguments(Parameterization const &p, int nRates,
...
template <typename REAL>
inline int Tessellation::GetVertexCoord(int vertex, REAL coord[]) const {
  _param.GetVertexCoord(vertex, coord);
  return 1;
}

template <typename REAL>
inline int Tessellation::GetCoords(REAL coordTuples[]) const {
