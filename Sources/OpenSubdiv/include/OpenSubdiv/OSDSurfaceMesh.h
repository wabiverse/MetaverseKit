...
public:
  ...
  virtual void UpdateVertexBuffer(float const *vertexData, int startVertex,
                                  int numVerts) = 0;

  virtual void UpdateVaryingBuffer(float const *varyingData, int startVertex,
                                   int numVerts) = 0;
...
protected:
  static inline void refineMesh(Far::TopologyRefiner &refiner, int level,
                                bool adaptive, bool singleCreasePatch) {
    if (adaptive) {
...
STENCIL_TABLE const *
convertToCompatibleStencilTable(SRC_STENCIL_TABLE const *table,
...
convertToCompatibleStencilTable<Far::LimitStencilTable, Far::LimitStencilTable,
                                void>(Far::LimitStencilTable const *table,
                                      void * /*context*/) {
  // no need for conversion
  // XXX: We don't want to even copy.
  if (!table)
...
template <>
inline Far::StencilTable const *
convertToCompatibleStencilTable<Far::StencilTable, Far::StencilTable,
                                ID3D11DeviceContext>(
...
public:
  ...
  struct Entry {
    Entry(BufferDescriptor const &srcDescArg,
          BufferDescriptor const &dstDescArg, BufferDescriptor const &duDescArg,
          BufferDescriptor const &dvDescArg, EVALUATOR *evalArg)
        : srcDesc(srcDescArg), dstDesc(dstDescArg), duDesc(duDescArg),
          dvDesc(dvDescArg), duuDesc(BufferDescriptor()),
          duvDesc(BufferDescriptor()), dvvDesc(BufferDescriptor()),
          evaluator(evalArg) {}
    Entry(BufferDescriptor const &srcDescArg,
          BufferDescriptor const &dstDescArg, BufferDescriptor const &duDescArg,
          BufferDescriptor const &dvDescArg, BufferDescriptor const &duuDescArg,
          BufferDescriptor const &duvDescArg,
          BufferDescriptor const &dvvDescArg, EVALUATOR *evalArg)
        : srcDesc(srcDescArg), dstDesc(dstDescArg), duDesc(duDescArg),
          dvDesc(dvDescArg), duuDesc(duuDescArg), duvDesc(duvDescArg),
          dvvDesc(dvvDescArg), evaluator(evalArg) {}
  ...
  EVALUATOR *GetEvaluator(BufferDescriptor const &srcDesc,
    ...
    return GetEvaluator(srcDesc, dstDesc, BufferDescriptor(),
                        BufferDescriptor(), BufferDescriptor(),
                        BufferDescriptor(), BufferDescriptor(), deviceContext);
  }

  template <typename DEVICE_CONTEXT>
  EVALUATOR *
  GetEvaluator(BufferDescriptor const &srcDesc, BufferDescriptor const &dstDesc,
               BufferDescriptor const &duDesc, BufferDescriptor const &dvDesc,
               DEVICE_CONTEXT *deviceContext) {
    return GetEvaluator(srcDesc, dstDesc, duDesc, dvDesc, BufferDescriptor(),
                        BufferDescriptor(), BufferDescriptor(), deviceContext);
  }
  ...
  GetEvaluator(BufferDescriptor const &srcDesc, BufferDescriptor const &dstDesc,
    ...
    for (typename Evaluators::iterator it = _evaluators.begin();
         it != _evaluators.end(); ++it) {
      if (isEqual(srcDesc, it->srcDesc) && isEqual(dstDesc, it->dstDesc) &&
          isEqual(duDesc, it->duDesc) && isEqual(dvDesc, it->dvDesc) &&
          isEqual(duuDesc, it->duuDesc) && isEqual(duvDesc, it->duvDesc) &&
          isEqual(dvvDesc, it->dvvDesc)) {
        return it->evaluator;
      }
    }
    EVALUATOR *e = EVALUATOR::Create(srcDesc, dstDesc, duDesc, dvDesc, duuDesc,
                                     duvDesc, dvvDesc, deviceContext);
    _evaluators.push_back(
        Entry(srcDesc, dstDesc, duDesc, dvDesc, duuDesc, duvDesc, dvvDesc, e));
    return e;
  }
...
static EVALUATOR *GetEvaluator(
    EvaluatorCacheT<EVALUATOR> *cache, BufferDescriptor const &srcDesc,
  ...
  if (cache == NULL)
    return NULL;
  return cache->GetEvaluator(srcDesc, dstDesc, duDesc, dvDesc, duuDesc, duvDesc,
...
static EVALUATOR *GetEvaluator(
    EvaluatorCacheT<EVALUATOR> *cache, BufferDescriptor const &srcDesc,
    BufferDescriptor const &dstDesc, DEVICE_CONTEXT deviceContext,
    typename enable_if<instantiatable<EVALUATOR>::value, void>::type *t = 0) {
  (void)t;
  if (cache == NULL)
    return NULL;
  return cache->GetEvaluator(srcDesc, dstDesc, BufferDescriptor(),
...
template <typename EVALUATOR, typename DEVICE_CONTEXT>
static EVALUATOR *GetEvaluator(
    EvaluatorCacheT<EVALUATOR> *, BufferDescriptor const &,
    BufferDescriptor const &, BufferDescriptor const &,
    BufferDescriptor const &, BufferDescriptor const &,
    BufferDescriptor const &, BufferDescriptor const &, DEVICE_CONTEXT,
    typename enable_if<!instantiatable<EVALUATOR>::value, void>::type *t = 0) {
  (void)t;
  return NULL;
}
...
static EVALUATOR *GetEvaluator(
    EvaluatorCacheT<EVALUATOR> *, BufferDescriptor const &,
    BufferDescriptor const &, BufferDescriptor const &,
    BufferDescriptor const &, DEVICE_CONTEXT,
    typename enable_if<!instantiatable<EVALUATOR>::value, void>::type *t = 0) {
  (void)t;
  return NULL;
}
...
template <typename VERTEX_BUFFER, typename STENCIL_TABLE, typename EVALUATOR,
          typename PATCH_TABLE, typename DEVICE_CONTEXT = void>
...
public:
  ...
  Mesh(Far::TopologyRefiner *refiner, int numVertexElements,
       int numVaryingElements, int level, MeshBitset bits = MeshBitset(),
       EvaluatorCache *evaluatorCache = NULL,
       DeviceContext *deviceContext = NULL)
      :

        _refiner(refiner), _farPatchTable(NULL), _numVertices(0),
        _maxValence(0), _vertexBuffer(NULL), _varyingBuffer(NULL),
        _vertexStencilTable(NULL), _varyingStencilTable(NULL),
        _evaluatorCache(evaluatorCache), _patchTable(NULL),
    ...
    initializeVertexBuffers(_numVertices, vertexBufferStride,
                            varyingBufferStride);
    ...
    if (bits.test(MeshInterleaveVarying)) {
      _varyingDesc = BufferDescriptor(numVertexElements, numVaryingElements,
                                      vertexBufferStride);
    } else {
      _varyingDesc =
          BufferDescriptor(0, numVaryingElements, varyingBufferStride);
    }
  }
  ...
  virtual void Refine() {
    ...
    Evaluator::EvalStencils(_vertexBuffer, srcDesc, _vertexBuffer, dstDesc,
                            _vertexStencilTable, instance, _deviceContext);
    ...
    if (_varyingDesc.length > 0) {
      ...
      instance = GetEvaluator<Evaluator>(_evaluatorCache, vSrcDesc, vDstDesc,
                                         _deviceContext);

      if (_varyingBuffer) {
        // non-interleaved
        Evaluator::EvalStencils(_varyingBuffer, vSrcDesc, _varyingBuffer,
                                vDstDesc, _varyingStencilTable, instance,
                                _deviceContext);
      } else {
        // interleaved
        Evaluator::EvalStencils(_vertexBuffer, vSrcDesc, _vertexBuffer,
                                vDstDesc, _varyingStencilTable, instance,
                                _deviceContext);
      }
    }
...
private:
  void initializeContext(int numVertexElements, int numVaryingElements,
    ...
    poptions.generateLegacySharpCornerPatches =
        !bits.test(MeshUseSmoothCornerPatch);
    ...
    _vertexStencilTable = convertToCompatibleStencilTable<StencilTable>(
        vertexStencils, _deviceContext);
    _varyingStencilTable = convertToCompatibleStencilTable<StencilTable>(
  ...
  void initializeVertexBuffers(int numVertices, int numVertexElements,
                               int numVaryingElements) {

    if (numVertexElements) {
      _vertexBuffer =
          VertexBuffer::Create(numVertexElements, numVertices, _deviceContext);
    }

    if (numVaryingElements) {
      _varyingBuffer =
          VertexBuffer::Create(numVaryingElements, numVertices, _deviceContext);
    }
  }
