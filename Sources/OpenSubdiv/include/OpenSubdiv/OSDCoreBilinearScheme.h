...
//  Bilinear traits:
//
template <> inline Split Scheme<SCHEME_BILINEAR>::GetTopologicalSplitType() {
  return SPLIT_TO_QUADS;
}

template <> inline int Scheme<SCHEME_BILINEAR>::GetRegularFaceSize() {
  return 4;
}

template <> inline int Scheme<SCHEME_BILINEAR>::GetRegularVertexValence() {
  return 4;
}

template <> inline int Scheme<SCHEME_BILINEAR>::GetLocalNeighborhoodSize() {
  return 0;
}
...
void Scheme<SCHEME_BILINEAR>::ComputeEdgeVertexMask(EDGE const &edge,
                                                    MASK &mask, Crease::Rule,
                                                    Crease::Rule) const {
...
void Scheme<SCHEME_BILINEAR>::ComputeVertexVertexMask(VERTEX const &vertex,
                                                      MASK &mask, Crease::Rule,
                                                      Crease::Rule) const {
  //  This should be inline, otherwise trivially replicate it:
...
//
//  Limit masks for position -- the limit position of all vertices is the
//  refined vertex.
//
template <>
template <typename VERTEX, typename MASK>
inline void
Scheme<SCHEME_BILINEAR>::assignCornerLimitMask(VERTEX const & /* vertex */,
                                               MASK &posMask) const {
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_BILINEAR>::assignCreaseLimitMask(
...
template <typename VERTEX, typename MASK>
inline void
Scheme<SCHEME_BILINEAR>::assignSmoothLimitMask(VERTEX const &vertex,
                                               MASK &posMask) const {
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_BILINEAR>::assignCornerLimitTangentMasks(
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_BILINEAR>::assignCreaseLimitTangentMasks(
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_BILINEAR>::assignSmoothLimitTangentMasks(
    VERTEX const &vertex, MASK &tan1Mask, MASK &tan2Mask) const {
