...
public:
  ...
  struct Options {
    ...
    enum EndCapType {
      ...
      ENDCAP_BSPLINE_BASIS,  ///< use BSpline-like patches (same patch type as
                             ///< regular)
      ENDCAP_GREGORY_BASIS,  ///< use Gregory patches (highest quality,
                             ///< recommended default)
      ENDCAP_LEGACY_GREGORY  ///< legacy option for 2.x style Gregory patches
                             ///< (Catmark only)
    };

    Options(unsigned int maxIsolation = 10)
        : generateAllLevels(false), includeBaseLevelIndices(true),
          includeFVarBaseLevelIndices(false), triangulateQuads(false),
          useSingleCreasePatch(false), useInfSharpPatch(false),
          maxIsolationLevel(maxIsolation & 0xf),
          endCapType(ENDCAP_GREGORY_BASIS), shareEndCapPatchPoints(true),
          generateVaryingTables(true), generateVaryingLocalPoints(true),
          generateFVarTables(false), patchPrecisionDouble(false),
          fvarPatchPrecisionDouble(false),
          generateFVarLegacyLinearPatches(true),
          generateLegacySharpCornerPatches(true), numFVarChannels(-1),
    ...
    /// \brief Determine adaptive refinement options to match assigned patch
    /// options
    TopologyRefiner::AdaptiveOptions GetRefineAdaptiveOptions() const {
      ...
      adaptiveOptions.considerFVarChannels =
          generateFVarTables && !generateFVarLegacyLinearPatches;
      return adaptiveOptions;
    }

    unsigned int generateAllLevels : 1,  ///< Generate levels from 'firstLevel'
                                         ///< to 'maxLevel' (Uniform mode only)
        includeBaseLevelIndices : 1,     ///< Include base level in patch point
                                         ///< indices (Uniform mode only)
        includeFVarBaseLevelIndices : 1, ///< Include base level in face-varying
                                         ///< patch point indices (Uniform mode
                                         ///< only)
        triangulateQuads : 1, ///< Triangulate 'QUADS' primitives (Uniform mode
                              ///< only)

        useSingleCreasePatch : 1, ///< Use single crease patch
        useInfSharpPatch : 1,     ///< Use infinitely-sharp patch
        maxIsolationLevel : 4, ///< Cap adaptive feature isolation to the given
                               ///< level (max. 10)

        // end-capping
        endCapType : 3,             ///< EndCapType
        shareEndCapPatchPoints : 1, ///< Share endcap patch points among
                                    ///< adjacent endcap patches. currently only
                                    ///< work with GregoryBasis.

        // varying
        generateVaryingTables : 1,      ///< Generate varying patch tables
        generateVaryingLocalPoints : 1, ///< Generate local points with varying
                                        ///< patches
        ...
        // precision
        patchPrecisionDouble : 1, ///< Generate double-precision stencils for
                                  ///< vertex patches
        fvarPatchPrecisionDouble : 1, ///< Generate double-precision stencils
                                      ///< for face-varying patches

        // legacy behaviors (default to true)
        generateFVarLegacyLinearPatches : 1,  ///< Generate all linear
                                              ///< face-varying patches (legacy)
        generateLegacySharpCornerPatches : 1; ///< Generate sharp regular
                                              ///< patches at smooth corners
                                              ///< (legacy)

    int numFVarChannels; ///< Number of channel indices and interpolation modes
                         ///< passed
    int const *fvarChannelIndices; ///< List containing the indices of the
                                   ///< channels selected for the factory
  };
  ...
  ///  vertex and varying patches.  Indices for vertex and varying patches
  ///  include the base level in addition to the last level while indices for
  ///  face-varying patches include only the last level.
  ///
...
public:
  //  PatchFaceTag
  //
  //  This simple struct was previously used within the factory to take
  //  inventory of various kinds of patches to fully allocate buffers prior to
  //  populating them.  It was not intended to be exposed as part of the public
  //  interface.
  //
  //  It is no longer used internally and is being kept here to respect
  //  preservation of the public interface, but it will be deprecated at the
