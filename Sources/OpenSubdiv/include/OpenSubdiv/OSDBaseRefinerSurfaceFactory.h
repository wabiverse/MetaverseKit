...
protected:
  ...
  int getFaceVertexIncidentFaceVertexIndices(
      Index faceIndex, int faceVertex, Index vertexIndices[]) const override;
  ...
  bool getFaceNeighborhoodVertexIndicesIfRegular(
      Index faceIndex, Index vertexIndices[]) const override;

  bool getFaceNeighborhoodFVarValueIndicesIfRegular(
      Index faceIndex, FVarID fvarID, Index fvarValueIndices[]) const override;
  /// @endcond
...
private:
  ...
  int getFacePatchPointIndices(Index faceIndex, Index indices[],
...
public:
  RefinerSurfaceFactory(Far::TopologyRefiner const &mesh,
      ...
      : RefinerSurfaceFactoryBase(mesh, options), _localCache() {
