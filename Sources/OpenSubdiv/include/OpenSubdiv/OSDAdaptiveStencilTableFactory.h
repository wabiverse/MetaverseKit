...
public:
  ...
  struct Options {
    ...
    Options()
        : interpolationMode(INTERPOLATE_VERTEX), generateOffsets(false),
          generateControlVerts(false), generateIntermediateLevels(true),
          factorizeIntermediateLevels(true), maxLevel(10), fvarChannel(0) {}
    ...
    unsigned int interpolationMode : 2, ///< interpolation mode
        ...
        generateIntermediateLevels : 1,  ///< vertices at all levels or highest
                                         ///< only
        factorizeIntermediateLevels : 1, ///< accumulate stencil weights from
                                         ///< control
                                         ///  vertices or from the stencils of
                                         ///  the previous level
  ...
  ///
  static StencilTableReal<REAL> const *
  ...
  ///                             StencilTable for the change of basis patch
  ///                             points.
  ///
  /// @param factorize            If factorize is set to true, endcap stencils
  /// will be
  ///                             factorized with supporting vertices from
  ///                             baseStencil table so that the endcap points
  ///                             can be computed directly from control
  ///                             vertices.
  ///
  ...
  /// \brief Utility function for stencil splicing for local point varying
  /// stencils.
  ///
  /// @param refiner              The TopologyRefiner containing the topology
  ///
  ...
  static StencilTableReal<REAL> const *AppendLocalPointStencilTableVarying(
    ...
    return AppendLocalPointStencilTable(refiner, baseStencilTable,
                                        localPointStencilTable, factorize);
  }
  ...
  static StencilTableReal<REAL> const *AppendLocalPointStencilTableFaceVarying(
      TopologyRefiner const &refiner,
      StencilTableReal<REAL> const *baseStencilTable,
      StencilTableReal<REAL> const *localPointStencilTable, int channel = 0,
      bool factorize = true);

private:
  // Generate stencils for the coarse control-vertices (single weight = 1.0f)
  static void generateControlVertStencils(int numControlVerts,
...
public:
  ...
  struct Options {
    ...
    Options()
        : interpolationMode(INTERPOLATE_VERTEX), generate1stDerivatives(true),
          generate2ndDerivatives(false), fvarChannel(0) {}
  ...
  /// @param options          Options controlling the creation of the table
  ///
  static LimitStencilTableReal<REAL> const *
  Create(TopologyRefiner const &refiner, LocationArrayVec const &locationArrays,
...
public:
  ...
  static StencilTable const *Create(int numTables,
    ...
    return static_cast<StencilTable const *>(BaseFactory::Create(
        numTables, reinterpret_cast<BaseTable const **>(tables)));
  }

  static StencilTable const *AppendLocalPointStencilTable(
      TopologyRefiner const &refiner, StencilTable const *baseStencilTable,
      StencilTable const *localPointStencilTable, bool factorize = true) {

    return static_cast<StencilTable const *>(
        BaseFactory::AppendLocalPointStencilTable(
            refiner, static_cast<BaseTable const *>(baseStencilTable),
            static_cast<BaseTable const *>(localPointStencilTable), factorize));
  }
  ...
  static StencilTable const *AppendLocalPointStencilTableVarying(
    ...
    return static_cast<StencilTable const *>(
        BaseFactory::AppendLocalPointStencilTableVarying(
            refiner, static_cast<BaseTable const *>(baseStencilTable),
            static_cast<BaseTable const *>(localPointStencilTable), factorize));
  }
  ...
  static StencilTable const *AppendLocalPointStencilTableFaceVarying(
      TopologyRefiner const &refiner, StencilTable const *baseStencilTable,
      StencilTable const *localPointStencilTable, int channel = 0,
      bool factorize = true) {

    return static_cast<StencilTable const *>(
        BaseFactory::AppendLocalPointStencilTableFaceVarying(
            refiner, static_cast<BaseTable const *>(baseStencilTable),
            static_cast<BaseTable const *>(localPointStencilTable), channel,
            factorize));
  }
};
...
public:
  static LimitStencilTable const *Create(TopologyRefiner const &refiner,
    ...
    return static_cast<LimitStencilTable const *>(BaseFactory::Create(
        refiner, locationArrays, static_cast<BaseTable const *>(cvStencils),
        patchTable, options));
  }
};
