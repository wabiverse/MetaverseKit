...
public:
  ...
  EvalStencils(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalStencils(
        srcBuffer->BindCpuBuffer(), srcDesc, dstBuffer->BindCpuBuffer(),
        dstDesc, &stencilTable->GetSizes()[0], &stencilTable->GetOffsets()[0],
        &stencilTable->GetControlIndices()[0], &stencilTable->GetWeights()[0],
        /*start = */ 0,
  ...
  EvalStencils(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalStencils(
        srcBuffer->BindCpuBuffer(), srcDesc, dstBuffer->BindCpuBuffer(),
        dstDesc, duBuffer->BindCpuBuffer(), duDesc, dvBuffer->BindCpuBuffer(),
        dvDesc, &stencilTable->GetSizes()[0], &stencilTable->GetOffsets()[0],
        &stencilTable->GetControlIndices()[0], &stencilTable->GetWeights()[0],
        &stencilTable->GetDuWeights()[0], &stencilTable->GetDvWeights()[0],
        /*start = */ 0,
        /*end   = */ stencilTable->GetNumStencils());
  }
  ...
  /// @param weights        pointer to the weights buffer of the stencil table
  ///
  /// @param duWeights      pointer to the du-weights buffer of the stencil
  /// table
  ///
  /// @param dvWeights      pointer to the dv-weights buffer of the stencil
  /// table
  ///
  /// @param start          start index of stencil table
  ///
  ...
  static bool EvalStencils(const float *src, BufferDescriptor const &srcDesc,
                           float *dst, BufferDescriptor const &dstDesc,
                           float *du, BufferDescriptor const &duDesc, float *dv,
                           BufferDescriptor const &dvDesc, const int *sizes,
                           const int *offsets, const int *indices,
                           const float *weights, const float *duWeights,
  ...
  EvalStencils(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalStencils(
        srcBuffer->BindCpuBuffer(), srcDesc, dstBuffer->BindCpuBuffer(),
        dstDesc, duBuffer->BindCpuBuffer(), duDesc, dvBuffer->BindCpuBuffer(),
        dvDesc, duuBuffer->BindCpuBuffer(), duuDesc, duvBuffer->BindCpuBuffer(),
        duvDesc, dvvBuffer->BindCpuBuffer(), dvvDesc,
        &stencilTable->GetSizes()[0], &stencilTable->GetOffsets()[0],
        &stencilTable->GetControlIndices()[0], &stencilTable->GetWeights()[0],
        &stencilTable->GetDuWeights()[0], &stencilTable->GetDvWeights()[0],
        &stencilTable->GetDuuWeights()[0], &stencilTable->GetDuvWeights()[0],
  ...
  /// @param duuDesc        vertex buffer descriptor for the duuBuffer
  ///
  /// @param duv            Output pointer 2nd derivative wrt u and v. An offset
  /// of
  ...
  /// table
  ///
  /// @param duuWeights     pointer to the duu-weights buffer of the stencil
  /// table
  ///
  /// @param duvWeights     pointer to the duv-weights buffer of the stencil
  /// table
  ///
  /// @param dvvWeights     pointer to the dvv-weights buffer of the stencil
  /// table
  ///
  /// @param start          start index of stencil table
  ///
  ...
  static bool EvalStencils(const float *src, BufferDescriptor const &srcDesc,
                           ...
                           BufferDescriptor const &dvvDesc, const int *sizes,
                           const int *offsets, const int *indices,
                           const float *weights, const float *duWeights,
                           const float *dvWeights, const float *duuWeights,
                           const float *duvWeights, const float *dvvWeights,
                           int start, int end);
  ...
  EvalPatches(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
              ...
              PATCH_TABLE *patchTable, CpuEvaluator const *instance = NULL,
              void *deviceContext = NULL) {
    ...
    return EvalPatches(srcBuffer->BindCpuBuffer(), srcDesc,
                       dstBuffer->BindCpuBuffer(), dstDesc, numPatchCoords,
  ...
  EvalPatches(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalPatches(
        ...
        dstDesc, duBuffer->BindCpuBuffer(), duDesc, dvBuffer->BindCpuBuffer(),
        dvDesc, duuBuffer->BindCpuBuffer(), duuDesc, duvBuffer->BindCpuBuffer(),
        duvDesc, dvvBuffer->BindCpuBuffer(), dvvDesc, numPatchCoords,
        (const PatchCoord *)patchCoords->BindCpuBuffer(),
        patchTable->GetPatchArrayBuffer(), patchTable->GetPatchIndexBuffer(),
        patchTable->GetPatchParamBuffer());
  }
  ...
  /// @param duuDesc          vertex buffer descriptor for the duuBuffer
  ///
  /// @param duv              Output pointer 2nd derivative wrt u and v. An
  /// offset of
  ///                         duvDesc will be applied internally.
  ///
  /// @param duvDesc          vertex buffer descriptor for the duvBuffer
  ...
  static bool EvalPatchesVarying(
    ...
    return EvalPatches(srcBuffer->BindCpuBuffer(), srcDesc,
                       ...
                       dvBuffer->BindCpuBuffer(), dvDesc, numPatchCoords,
                       (const PatchCoord *)patchCoords->BindCpuBuffer(),
                       patchTable->GetVaryingPatchArrayBuffer(),
                       patchTable->GetVaryingPatchIndexBuffer(),
                       patchTable->GetPatchParamBuffer());
