...
public:
  ...
  struct UniformOptions {
    ...
    UniformOptions(int level)
        : refinementLevel(level & 0xf), orderVerticesFromFacesFirst(false),
    ...
    unsigned int refinementLevel : 4,    ///< Number of refinement iterations
        orderVerticesFromFacesFirst : 1, ///< Order child vertices from faces
                                         ///< first instead of child vertices of
                                         ///< vertices
        fullTopologyInLastLevel : 1; ///< Skip topological relationships in the
                                     ///< last level of refinement that are not
                                     ///< needed for interpolation (keep false
                                     ///< if using limit).
  };
  ...
  /// and be sure it is assigned to satisfy the needs of the resulting
  /// refinement.
  ///
  ...
  struct AdaptiveOptions {
    ...
    AdaptiveOptions(int level)
        : isolationLevel(level & 0xf), secondaryLevel(0xf),
          useSingleCreasePatch(false), useInfSharpPatch(false),
          considerFVarChannels(false), orderVerticesFromFacesFirst(false) {}
    ...
    unsigned int
        isolationLevel : 4; ///< Number of iterations applied to isolate
    ...
    unsigned int useSingleCreasePatch : 1; ///< Use 'single-crease' patch and
                                           ///< stop isolation where applicable
    unsigned int useInfSharpPatch : 1;     ///< Use infinitely sharp patches and
                                           ///< stop isolation where applicable
    unsigned int
        considerFVarChannels : 1; ///< Inspect face-varying channels and
  ...
  GetFVarLinearInterpolation(int channel = 0) const;
...
inline int TopologyRefiner::GetNumFVarChannels() const {
