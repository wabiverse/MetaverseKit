...
/// This is an implementation of the functionality from
/// OpenSubdiv/OSDAdaptivePatchBasis.h which can be used from client shader and
/// kernel code, i.e for GLSL, HLSL, CUDA, OpenCL, Metal, and C++ shaders and
/// kernels.
///
...
#if !defined(OSD_PATCH_BASIS_GLSL) && !defined(OSD_PATCH_BASIS_HLSL) &&        \
    !defined(OSD_PATCH_BASIS_CUDA) && !defined(OSD_PATCH_BASIS_OPENCL) &&      \
...
int Osd_EvalBasisLinear(OSD_REAL s, OSD_REAL t, OSD_OUT_ARRAY(OSD_REAL, wP, 4),
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
void Osd_evalBSplineCurve(OSD_REAL t, OSD_OUT_ARRAY(OSD_REAL, wP, 4),
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
void Osd_adjustBSplineBoundaryWeights(int boundary,
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
void Osd_boundBasisBSpline(int boundary, OSD_INOUT_ARRAY(OSD_REAL, wP, 16),
...
OSD_FUNCTION_STORAGE_CLASS
int Osd_EvalBasisBSpline(OSD_REAL s, OSD_REAL t,
  ...
  OSD_REAL sWeights[4], tWeights[4], dsWeights[4], dtWeights[4], dssWeights[4],
      dttWeights[4];

  Osd_evalBSplineCurve(s, sWeights, OSD_OPTIONAL_INIT(wDs, dsWeights),
                       OSD_OPTIONAL_INIT(wDss, dssWeights));
  Osd_evalBSplineCurve(t, tWeights, OSD_OPTIONAL_INIT(wDt, dtWeights),
                       OSD_OPTIONAL_INIT(wDtt, dttWeights));
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
void Osd_evalBezierCurve(OSD_REAL t, OSD_OUT_ARRAY(OSD_REAL, wP, 4),
                         OSD_OUT_ARRAY(OSD_REAL, wDP, 4),
                         OSD_OUT_ARRAY(OSD_REAL, wDP2, 4)) {
...
OSD_FUNCTION_STORAGE_CLASS
int Osd_EvalBasisBezier(OSD_REAL s, OSD_REAL t, OSD_OUT_ARRAY(OSD_REAL, wP, 16),
                        OSD_OUT_ARRAY(OSD_REAL, wDs, 16),
                        OSD_OUT_ARRAY(OSD_REAL, wDt, 16),
                        OSD_OUT_ARRAY(OSD_REAL, wDss, 16),
                        OSD_OUT_ARRAY(OSD_REAL, wDst, 16),
  ...
  Osd_evalBezierCurve(s, OSD_OPTIONAL_INIT(wP, sWeights),
                      OSD_OPTIONAL_INIT(wDs, dsWeights),
                      OSD_OPTIONAL_INIT(wDss, dssWeights));
  Osd_evalBezierCurve(t, OSD_OPTIONAL_INIT(wP, tWeights),
                      OSD_OPTIONAL_INIT(wDt, dtWeights),
                      OSD_OPTIONAL_INIT(wDtt, dttWeights));
...
OSD_FUNCTION_STORAGE_CLASS
int Osd_EvalBasisGregory(OSD_REAL s, OSD_REAL t,
  ...
  //  Indices of boundary and interior points and their corresponding Bezier
  //  points (this can be reduced with more direct indexing and unrolling of
  //  loops):
  //
  OSD_DATA_STORAGE_CLASS const int boundaryGregory[12] =
      OSD_ARRAY_12(int, 0, 1, 7, 5, 2, 6, 16, 12, 15, 17, 11, 10);
  OSD_DATA_STORAGE_CLASS const int boundaryBezSCol[12] =
      OSD_ARRAY_12(int, 0, 1, 2, 3, 0, 3, 0, 3, 0, 1, 2, 3);
  OSD_DATA_STORAGE_CLASS const int boundaryBezTRow[12] =
      OSD_ARRAY_12(int, 0, 0, 0, 0, 1, 1, 2, 2, 3, 3, 3, 3);

  OSD_DATA_STORAGE_CLASS const int interiorGregory[8] =
      OSD_ARRAY_8(int, 3, 4, 8, 9, 13, 14, 18, 19);
  OSD_DATA_STORAGE_CLASS const int interiorBezSCol[8] =
      OSD_ARRAY_8(int, 1, 1, 2, 2, 2, 2, 1, 1);
  OSD_DATA_STORAGE_CLASS const int interiorBezTRow[8] =
      OSD_ARRAY_8(int, 1, 1, 1, 1, 2, 2, 2, 2);

  //
  //  Bezier basis functions are denoted with B while the rational multipliers
  //  for the interior points will be denoted G -- so we have B(s), B(t) and
  //  G(s,t):
  //
  ...
  Osd_evalBezierCurve(s, Bs, OSD_OPTIONAL_INIT(wDs, Bds),
                      OSD_OPTIONAL_INIT(wDss, Bdss));
  Osd_evalBezierCurve(t, Bt, OSD_OPTIONAL_INIT(wDt, Bdt),
                      OSD_OPTIONAL_INIT(wDtt, Bdtt));

  //  Rational multipliers G at s and t:
  OSD_REAL sC = 1.0f - s;
  OSD_REAL tC = 1.0f - t;

  //  Use <= here to avoid compiler warnings -- the sums should always be
  //  non-negative:
  OSD_REAL df0 = s + t;
  ...
  OSD_REAL G[8] = OSD_ARRAY_8(OSD_REAL, s * df0, (1.0f - s * df0), t * df1,
                              (1.0f - t * df1), sC * df2, (1.0f - sC * df2),
                              tC * df3, (1.0f - tC * df3));
  ...
  for (int j = 0; j < 8; ++j) {
    wP[interiorGregory[j]] =
        Bs[interiorBezSCol[j]] * Bt[interiorBezTRow[j]] * G[j];
  }

  //
  //  For derivatives, the basis functions for the interior points are rational
  //  and ideally require appropriate differentiation, i.e. product rule for the
  //  combination of B and G and the quotient rule for the rational G itself. As
  //  initially proposed by Loop et al though, the approximation using the 16
  //  Bezier points arising from the G(s,t) has proved adequate (and is what the
  //  GPU shaders use) so we continue to use that here.
  //
  //  An implementation of the true derivatives is provided and conditionally
  //  compiled for those that require it, e.g.:
  //
  //    dclyde's note: skipping half of the product rule like this does seem to
  //    change the result a lot in my tests.  This is not a runtime bottleneck
  //    for cloth sims anyway so I'm just using the accurate version.
  //
  if (OSD_OPTIONAL(wDs && wDt)) {
    ...
    //  Approximation to the true Gregory derivatives by differentiating the
    //  Bezier patch unique to the given (s,t), i.e. having F = (g^+ * f^+) +
    //  (g^- * f^-) as its four interior points:
    //
...
#else
    //  True Gregory derivatives using appropriate differentiation of composite
    //  functions:
    //
    //  Note that for G(s,t) = N(s,t) / D(s,t), all N' and D' are trivial
    //  constants (which simplifies things for higher order derivatives).  And
    //  while each pair of functions G (i.e. the G+ and G- corresponding to
    //  points f+ and f-) must sum to 1 to ensure Bezier equivalence (when f+ =
    //  f-), the pairs of G' must similarly sum to 0.  So we can potentially
    //  compute only one of the pair and negate the result for the other (and
    //  with 4 or 8 computations involving these constants, this is all very
    //  SIMD friendly...) but for now we treat all 8 independently for
    //  simplicity.
    //
    // float N[8] = OSD_ARRAY_8(float,    s,     t,      t,     sC,      sC, tC,
    // tC,     s );
    OSD_REAL D[8] =
        OSD_ARRAY_8(OSD_REAL, df0, df0, df1, df1, df2, df2, df3, df3);

    OSD_DATA_STORAGE_CLASS const OSD_REAL Nds[8] =
        OSD_ARRAY_8(OSD_REAL, 1.0f, 0.0f, 0.0f, -1.0f, -1.0f, 0.0f, 0.0f, 1.0f);
    OSD_DATA_STORAGE_CLASS const OSD_REAL Ndt[8] =
        OSD_ARRAY_8(OSD_REAL, 0.0f, 1.0f, 1.0f, 0.0f, 0.0f, -1.0f, -1.0f, 0.0f);

    OSD_DATA_STORAGE_CLASS const OSD_REAL Dds[8] = OSD_ARRAY_8(
        OSD_REAL, 1.0f, 1.0f, -1.0f, -1.0f, -1.0f, -1.0f, 1.0f, 1.0f);
    OSD_DATA_STORAGE_CLASS const OSD_REAL Ddt[8] = OSD_ARRAY_8(
        OSD_REAL, 1.0f, 1.0f, 1.0f, 1.0f, -1.0f, -1.0f, -1.0f, -1.0f);
    //  Combined weights for interior points -- (scaled) combinations of B, B',
    //  G and G':
    for (int k = 0; k < 8; ++k) {
      ...
      //  Quotient rule for G' (re-expressed in terms of G to simplify (and D =
      //  1/D)):
      ...
      if (find_second_partials) {
        ...
        OSD_REAL Gdst = Dsqr_inv * (2.0f * G[k] * Dds[k] * Ddt[k] -
                                    Nds[k] * Ddt[k] - Ndt[k] * Dds[k]);
        OSD_REAL Gdtt = 2.0f * Ddt[k] * Dsqr_inv * (G[k] * Ddt[k] - Ndt[k]);

        wDss[iDst] =
            (Bdss[sCol] * G[k] + 2.0f * Bds[sCol] * Gds + Bs[sCol] * Gdss) *
            Bt[tRow];
        wDst[iDst] = Bt[tRow] * (Bs[sCol] * Gdst + Bds[sCol] * Gdt) +
                     Bdt[tRow] * (Bds[sCol] * G[k] + Bs[sCol] * Gds);
        wDtt[iDst] =
            (Bdtt[tRow] * G[k] + 2.0f * Bdt[tRow] * Gdt + Bt[tRow] * Gdtt) *
            Bs[sCol];
      }
    }
#endif
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
int Osd_EvalBasisLinearTri(OSD_REAL s, OSD_REAL t,
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
void Osd_evalBivariateMonomialsQuartic(OSD_REAL s, OSD_REAL t,
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
void Osd_evalBoxSplineTriDerivWeights(OSD_INOUT_ARRAY(OSD_REAL,
                                                      /*stMonomials*/ M, 15),
                                      int ds, int dt,
  ...
  if (totalOrder == 0) {
    ...
    w[0] = S * (1 - 2 * M[1] - 4 * M[2] + 6 * M[4] + 6 * M[5] + 2 * M[6] -
                6 * M[8] - 4 * M[9] - M[10] - 2 * M[11] + 2 * M[13] + M[14]);
    w[1] = S * (1 + 2 * M[1] - 2 * M[2] - 6 * M[4] - 4 * M[6] + 6 * M[8] +
                2 * M[9] + 2 * M[10] + 4 * M[11] - 2 * M[13] - M[14]);
    w[2] = S * (2 * M[6] - M[10] - 2 * M[11]);
    w[3] = S * (1 - 4 * M[1] - 2 * M[2] + 6 * M[3] + 6 * M[4] - 4 * M[6] -
                6 * M[7] + 2 * M[9] + M[10] + 2 * M[11] - 2 * M[13] - M[14]);
    w[4] = S * (6 - 12 * M[3] - 12 * M[4] - 12 * M[5] + 8 * M[6] + 12 * M[7] +
                12 * M[8] + 8 * M[9] - M[10] - 2 * M[11] - 2 * M[13] - M[14]);
    w[5] = S * (1 + 4 * M[1] + 2 * M[2] + 6 * M[3] + 6 * M[4] - 4 * M[6] -
                6 * M[7] - 12 * M[8] - 4 * M[9] - M[10] - 2 * M[11] +
                4 * M[13] + 2 * M[14]);
    w[6] = S * (M[10] + 2 * M[11]);
    w[7] = S * (1 - 2 * M[1] + 2 * M[2] - 6 * M[4] + 2 * M[6] + 6 * M[7] -
                4 * M[9] - M[10] - 2 * M[11] + 4 * M[13] + 2 * M[14]);
    w[8] = S * (1 + 2 * M[1] + 4 * M[2] + 6 * M[4] + 6 * M[5] - 4 * M[6] -
                12 * M[7] - 6 * M[8] - 4 * M[9] + 2 * M[10] + 4 * M[11] -
                2 * M[13] - M[14]);
    w[9] = S * (2 * M[6] + 6 * M[7] + 6 * M[8] + 2 * M[9] - M[10] - 2 * M[11] -
                2 * M[13] - M[14]);
    ...
    if (ds != 0) {
      w[0] = S *
             (-1 + 3 * M[2] + 3 * M[3] - 3 * M[5] - 2 * M[6] - 3 * M[7] + M[9]);
      w[1] =
          S * (1 - 3 * M[2] - 6 * M[3] + 3 * M[5] + 4 * M[6] + 6 * M[7] - M[9]);
      w[2] = S * (3 * M[3] - 2 * M[6] - 3 * M[7]);
      w[3] = S * (-2 + 6 * M[1] + 3 * M[2] - 6 * M[3] - 6 * M[4] + 2 * M[6] +
                  3 * M[7] - M[9]);
      w[4] = S * (-12 * M[1] - 6 * M[2] + 12 * M[3] + 12 * M[4] + 6 * M[5] -
                  2 * M[6] - 3 * M[7] - M[9]);
      w[5] = S * (2 + 6 * M[1] + 3 * M[2] - 6 * M[3] - 6 * M[4] - 6 * M[5] -
                  2 * M[6] - 3 * M[7] + 2 * M[9]);
      w[6] = S * (2 * M[6] + 3 * M[7]);
      w[7] = S * (-1 - 3 * M[2] + 3 * M[3] + 6 * M[4] - 2 * M[6] - 3 * M[7] +
                  2 * M[9]);
      w[8] = S * (1 + 3 * M[2] - 6 * M[3] - 12 * M[4] - 3 * M[5] + 4 * M[6] +
                  6 * M[7] - M[9]);
      w[9] = S * (3 * M[3] + 6 * M[4] + 3 * M[5] - 2 * M[6] - 3 * M[7] - M[9]);
      w[10] = S * (-M[9]);
      w[11] = S * (M[9]);
    } else {
      w[0] = S * (-2 + 3 * M[1] + 6 * M[2] - 6 * M[4] - 6 * M[5] - M[6] +
                  3 * M[8] + 2 * M[9]);
      w[1] = S * (-1 - 3 * M[1] + 6 * M[4] + 3 * M[5] + 2 * M[6] - 3 * M[8] -
                  2 * M[9]);
      w[2] = S * (-M[6]);
      w[3] = S *
             (-1 + 3 * M[1] - 3 * M[3] + 3 * M[5] + M[6] - 3 * M[8] - 2 * M[9]);
      w[4] = S * (-6 * M[1] - 12 * M[2] + 6 * M[3] + 12 * M[4] + 12 * M[5] -
                  M[6] - 3 * M[8] - 2 * M[9]);
      w[5] = S * (1 + 3 * M[1] - 3 * M[3] - 12 * M[4] - 6 * M[5] - M[6] +
                  6 * M[8] + 4 * M[9]);
      w[6] = S * (+M[6]);
      w[7] =
          S * (1 - 3 * M[1] + 3 * M[3] - 6 * M[5] - M[6] + 6 * M[8] + 4 * M[9]);
      w[8] = S * (2 + 3 * M[1] + 6 * M[2] - 6 * M[3] - 6 * M[4] - 6 * M[5] +
                  2 * M[6] - 3 * M[8] - 2 * M[9]);
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
void Osd_adjustBoxSplineTriBoundaryWeights(int boundaryMask,
                                           OSD_INOUT_ARRAY(OSD_REAL, weights,
                                                           12)) {
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
void Osd_boundBasisBoxSplineTri(int boundary, OSD_INOUT_ARRAY(OSD_REAL, wP, 12),
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
int Osd_EvalBasisBoxSplineTri(OSD_REAL s, OSD_REAL t,
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
void Osd_evalBezierTriDerivWeights(OSD_REAL s, OSD_REAL t, int ds, int dt,
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
int Osd_EvalBasisBezierTri(OSD_REAL s, OSD_REAL t,
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
void Osd_convertBezierWeightsToGregory(OSD_INOUT_ARRAY(OSD_REAL, wB, 15),
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
int Osd_EvalBasisGregoryTri(OSD_REAL s, OSD_REAL t,
  ...
  //
  //  Bezier basis functions are denoted with B while the rational multipliers
  //  for the interior points will be denoted G -- so we have B(s,t) and G(s,t)
  //  (though we switch to barycentric (u,v,w) briefly to compute G)
  //
...
OSD_FUNCTION_STORAGE_CLASS
void OsdGetBezierWeights(OSD_REAL t, OSD_OUT_ARRAY(OSD_REAL, wP, 4),
                         OSD_OUT_ARRAY(OSD_REAL, wDP, 4),
                         OSD_OUT_ARRAY(OSD_REAL, wDP2, 4)) {
...
//  OsdEvaluatePatchBasisNormalized() methods.
OSD_FUNCTION_STORAGE_CLASS
void OsdGetBSplineWeights(OSD_REAL t, OSD_OUT_ARRAY(OSD_REAL, wP, 4),
                          OSD_OUT_ARRAY(OSD_REAL, wDP, 4),
                          OSD_OUT_ARRAY(OSD_REAL, wDP2, 4)) {
...
//  OsdEvaluatePatchBasisNormalized() methods.
OSD_FUNCTION_STORAGE_CLASS
void OsdAdjustBoundaryWeights(int boundary,
...
//  OsdEvaluatePatchBasisNormalized() methods.
OSD_FUNCTION_STORAGE_CLASS
void OsdComputeTensorProductPatchWeights(
    float dScale, int boundary, OSD_IN_ARRAY(float, sWeights, 4),
    OSD_IN_ARRAY(float, tWeights, 4), OSD_IN_ARRAY(float, dsWeights, 4),
    OSD_IN_ARRAY(float, dtWeights, 4), OSD_IN_ARRAY(float, dssWeights, 4),
    OSD_IN_ARRAY(float, dttWeights, 4), OSD_OUT_ARRAY(float, wP, 16),
    OSD_OUT_ARRAY(float, wDs, 16), OSD_OUT_ARRAY(float, wDt, 16),
    OSD_OUT_ARRAY(float, wDss, 16), OSD_OUT_ARRAY(float, wDst, 16),
...
//  OsdEvaluatePatchBasisNormalized() methods.
OSD_FUNCTION_STORAGE_CLASS
void OsdGetBilinearPatchWeights(OSD_REAL s, OSD_REAL t, OSD_REAL d1Scale,
...
//  OsdEvaluatePatchBasisNormalized() methods.
OSD_FUNCTION_STORAGE_CLASS
void OsdGetBSplinePatchWeights(OSD_REAL s, OSD_REAL t, OSD_REAL d1Scale,
                               int boundary, OSD_OUT_ARRAY(OSD_REAL, wP, 16),
                               OSD_OUT_ARRAY(OSD_REAL, wDs, 16),
                               OSD_OUT_ARRAY(OSD_REAL, wDt, 16),
                               OSD_OUT_ARRAY(OSD_REAL, wDss, 16),
                               OSD_OUT_ARRAY(OSD_REAL, wDst, 16),
...
//  OsdEvaluatePatchBasisNormalized() methods.
OSD_FUNCTION_STORAGE_CLASS
void OsdGetBezierPatchWeights(OSD_REAL s, OSD_REAL t, OSD_REAL d1Scale,
                              OSD_OUT_ARRAY(OSD_REAL, wP, 16),
                              OSD_OUT_ARRAY(OSD_REAL, wDs, 16),
                              OSD_OUT_ARRAY(OSD_REAL, wDt, 16),
                              OSD_OUT_ARRAY(OSD_REAL, wDss, 16),
...
//  OsdEvaluatePatchBasisNormalized() methods.
OSD_FUNCTION_STORAGE_CLASS
void OsdGetGregoryPatchWeights(OSD_REAL s, OSD_REAL t, OSD_REAL d1Scale,
                               OSD_OUT_ARRAY(OSD_REAL, wP, 20),
                               OSD_OUT_ARRAY(OSD_REAL, wDs, 20),
                               OSD_OUT_ARRAY(OSD_REAL, wDt, 20),
                               OSD_OUT_ARRAY(OSD_REAL, wDss, 20),
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
int OsdEvaluatePatchBasisNormalized(
    int patchType, OsdPatchParam param, OSD_REAL s, OSD_REAL t,
    OSD_OUT_ARRAY(OSD_REAL, wP, 20), OSD_OUT_ARRAY(OSD_REAL, wDs, 20),
    OSD_OUT_ARRAY(OSD_REAL, wDt, 20), OSD_OUT_ARRAY(OSD_REAL, wDss, 20),
  ...
  if (patchType == OSD_PATCH_DESCRIPTOR_REGULAR) {
    ...
    nPoints =
        Osd_EvalBasisBSpline(s, t, wP16, wDs16, wDt16, wDss16, wDst16, wDtt16);
    if (boundaryMask != 0) {
      Osd_boundBasisBSpline(boundaryMask, wP16, wDs16, wDt16, wDss16, wDst16,
                            wDtt16);
    }
    for (int i = 0; i < nPoints; ++i) {
      wP[i] = wP16[i];
    ...
    nPoints = Osd_EvalBasisBoxSplineTri(s, t, wP12, wDs12, wDt12, wDss12,
                                        wDst12, wDtt12);
    if (boundaryMask != 0) {
      Osd_boundBasisBoxSplineTri(boundaryMask, wP12, wDs12, wDt12, wDss12,
                                 wDst12, wDtt12);
    }
    for (int i = 0; i < nPoints; ++i) {
      wP[i] = wP12[i];
    ...
    nPoints = Osd_EvalBasisGregoryTri(s, t, wP18, wDs18, wDt18, wDss18, wDst18,
                                      wDtt18);
    for (int i = 0; i < nPoints; ++i) {
      wP[i] = wP18[i];
      wDs[i] = wDs18[i];
    ...
    nPoints =
        Osd_EvalBasisLinearTri(s, t, wP3, wDs3, wDt3, wDss3, wDst3, wDtt3);
    for (int i = 0; i < nPoints; ++i) {
      wP[i] = wP3[i];
      wDs[i] = wDs3[i];
      wDt[i] = wDt3[i];
...
OSD_FUNCTION_STORAGE_CLASS
// template <typename REAL>
int OsdEvaluatePatchBasis(int patchType, OsdPatchParam param, OSD_REAL s,
                          OSD_REAL t, OSD_OUT_ARRAY(OSD_REAL, wP, 20),
                          OSD_OUT_ARRAY(OSD_REAL, wDs, 20),
                          OSD_OUT_ARRAY(OSD_REAL, wDt, 20),
                          OSD_OUT_ARRAY(OSD_REAL, wDss, 20),
                          OSD_OUT_ARRAY(OSD_REAL, wDst, 20),
  ...
  int nPoints = OsdEvaluatePatchBasisNormalized(patchType, param, s, t, wP, wDs,
                                                wDt, wDss, wDst, wDtt);
