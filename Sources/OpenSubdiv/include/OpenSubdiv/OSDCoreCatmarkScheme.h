...
//  Catmark traits:
//
template <> inline Split Scheme<SCHEME_CATMARK>::GetTopologicalSplitType() {
  return SPLIT_TO_QUADS;
}

template <> inline int Scheme<SCHEME_CATMARK>::GetRegularFaceSize() {
  return 4;
}

template <> inline int Scheme<SCHEME_CATMARK>::GetRegularVertexValence() {
  return 4;
}

template <> inline int Scheme<SCHEME_CATMARK>::GetLocalNeighborhoodSize() {
  return 1;
}

//
//  Masks for edge-vertices:  the hard Crease mask does not need to be
//  specialized (simply the midpoint), so all that is left is the Smooth case:
//
//  The Smooth mask is complicated by the need to support the "triangle
//  subdivision" option, which applies different weighting in the presence of
//  triangles.  It is up for debate as to whether this is useful or not -- we
//  may be able to deprecate this option.
//
template <>
template <typename EDGE, typename MASK>
inline void Scheme<SCHEME_CATMARK>::assignSmoothMaskForEdge(EDGE const &edge,
                                                            MASK &mask) const {
  ...
  //
  //  Determine if we need to inspect incident faces and apply alternate
  //  weighting for triangles -- and if so, determine which of the two are
  //  triangles.
  //
  bool face0IsTri = false;
  bool face1IsTri = false;
  bool useTriangleOption =
      (_options.GetTriangleSubdivision() == Options::TRI_SUB_SMOOTH);
  if (useTriangleOption) {
    if (faceCount == 2) {
      //
      //  Ideally we want to avoid this inspection when we have already
      //  subdivided at least once -- need something in the Edge interface to
      //  help avoid this, e.g. an IsRegular() query, the subdivision level...
      //
      int vertsPerFace[2];
...
//
//  Masks for vertex-vertices:  the hard Corner mask does not need to be
//  specialized (simply the vertex itself), leaving the Crease and Smooth cases
//  (Dart is smooth):
//
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_CATMARK>::assignCreaseMaskForVertex(
    VERTEX const &vertex, MASK &mask, int const creaseEnds[2]) const {
  typedef typename MASK::Weight Weight;
...
template <typename VERTEX, typename MASK>
inline void
Scheme<SCHEME_CATMARK>::assignSmoothMaskForVertex(VERTEX const &vertex,
                                                  MASK &mask) const {

  typedef typename MASK::Weight Weight;

  //
  //  A Smooth vertex must be manifold and interior -- manifold boundary
  //  vertices will be Creases and non-manifold vertices of any kind will be
  //  Corners or Creases.  If smooth rules for non-manifold vertices are ever
  //  defined, this will need adjusting:
  //
...
template <typename VERTEX, typename MASK>
inline void
Scheme<SCHEME_CATMARK>::assignCornerLimitMask(VERTEX const & /* vertex */,
                                              MASK &posMask) const {
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_CATMARK>::assignCreaseLimitMask(
    VERTEX const &vertex, MASK &posMask, int const creaseEnds[2]) const {
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_CATMARK>::assignSmoothLimitMask(VERTEX const &vertex,
                                                          MASK &posMask) const {
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_CATMARK>::assignCornerLimitTangentMasks(
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_CATMARK>::assignCreaseLimitTangentMasks(
  ...
  if (interiorEdgeCount == 1) {
    ...
    for (int i = 1; i < k; ++i) {
      ...
      tan2Mask.EdgeWeight(creaseEnds[0] + i) =
          (Weight)((4.0f * sinThetaI) * commonDenom);
      tan2Mask.FaceWeight(creaseEnds[0] + i) =
          (Weight)((sinThetaI + sinThetaIplus1) * commonDenom);
    }
  } else {
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_CATMARK>::assignSmoothLimitTangentMasks(
    VERTEX const &vertex, MASK &tan1Mask, MASK &tan2Mask) const {
  ...
  if (valence == 4) {
    ...
    for (int i = 0; i < valence; ++i) {
      ...
      tan1Mask.FaceWeight(i) =
          (Weight)(faceWeightScale * (cosThetaI + cosThetaIplus1));
    }
  }
