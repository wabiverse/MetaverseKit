...
public:
  ...
  public:
    Options()
        : _dfltFVarID(-1), _externCache(0), _enableCache(true),
...
public:
  ...
  bool InitSurfaces(Index faceIndex, Surface<REAL> *vtxSurface,
                    Surface<REAL> *fvarSurfaces, FVarID const fvarIDs[] = 0,
                    int fvarCount = 0, Surface<REAL> *varSurface = 0) const;
  //@}
...
private:
  ...
  bool initSurfaces(Index faceIndex, internal::SurfaceData *vtxSurface,
                    ...
                    internal::SurfaceData *fvarSurfaces, int fvarCount,
                    FVarID const fvarIDs[]) const;

  //  Methods to assemble topology and corresponding indices for entire face:
  bool isFaceNeighborhoodRegular(Index faceIndex, FVarID const *fvarPtrOrVtx,
                                 Index indices[]) const;
  ...
  void assignLinearSurface(SurfaceType *surfacePtr, Index faceIndex,
...
//
template <typename REAL>
inline bool SurfaceFactory::InitVertexSurface(Index face,
                                              Surface<REAL> *s) const {

  return initSurfaces(face, &s->getSurfaceData(), 0, 0, 0, 0);
}
template <typename REAL>
inline bool SurfaceFactory::InitVaryingSurface(Index face,
                                               Surface<REAL> *s) const {

  return initSurfaces(face, 0, &s->getSurfaceData(), 0, 0, 0);
}
template <typename REAL>
inline bool SurfaceFactory::InitFaceVaryingSurface(Index face, Surface<REAL> *s,
                                                   FVarID fvarID) const {
  return initSurfaces(face, 0, 0, &s->getSurfaceData(), 1, &fvarID);
}
template <typename REAL>
inline bool SurfaceFactory::InitFaceVaryingSurface(Index face,
                                                   Surface<REAL> *s) const {
...
template <typename REAL>
inline bool SurfaceFactory::InitSurfaces(Index faceIndex,
                                         Surface<REAL> *vtxSurface,
                                         Surface<REAL> *fvarSurfaces,
                                         FVarID const fvarIDs[], int fvarCount,
  ...
  return initSurfaces(faceIndex, vtxSurface ? &vtxSurface->getSurfaceData() : 0,
