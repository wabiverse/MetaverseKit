...
/// efficiently recomputed simply by applying the blending weights to the series
/// of coarse control vertices.
///
template <typename REAL> class StencilTableReal {
protected:
  StencilTableReal(int numControlVerts, std::vector<int> const &offsets,
                   std::vector<int> const &sizes,
                   std::vector<int> const &sources,
                   std::vector<REAL> const &weights, bool includeCoarseVerts,
                   size_t firstOffset);
...
public:
  ...
  void UpdateValues(T const &srcValues, U &dstValues, Index start = -1,
                    Index end = -1) const {
  ...
  template <class T, class U>
  void UpdateValues(T const *src, U *dst, Index start = -1,
                    Index end = -1) const {
...
protected:
  ...
  void update(T const &srcValues, U &dstValues,
              std::vector<REAL> const &valueWeights, Index start,
              Index end) const;
  template <class T1, class T2, class U>
  ...
  // Populate the offsets table from the stencil sizes in _sizes (factory
  // helper)
...
protected:
  ...
  StencilTable(int numControlVerts, std::vector<int> const &offsets,
               std::vector<int> const &sizes, std::vector<int> const &sources,
               std::vector<float> const &weights, bool includeCoarseVerts,
               size_t firstOffset)
      : BaseTable(numControlVerts, offsets, sizes, sources, weights,
...
public:
  ...
  ///
  LimitStencilReal(int *size, Index *indices, REAL *weights,
                   REAL *duWeights = 0, REAL *dvWeights = 0,
                   REAL *duuWeights = 0, REAL *duvWeights = 0,
                   REAL *dvvWeights = 0)
      : StencilReal<REAL>(size, indices, weights), _duWeights(duWeights),
        _dvWeights(dvWeights), _duuWeights(duuWeights), _duvWeights(duvWeights),
...
public:
  ...
  LimitStencil(int *size, Index *indices, float *weights, float *duWeights = 0,
               float *dvWeights = 0, float *duuWeights = 0,
               float *duvWeights = 0, float *dvvWeights = 0)
      : BaseStencil(size, indices, weights, duWeights, dvWeights, duuWeights,
                    duvWeights, dvvWeights) {}
};
...
protected:
  LimitStencilTableReal(
      int numControlVerts, std::vector<int> const &offsets,
      std::vector<int> const &sizes, std::vector<int> const &sources,
      std::vector<REAL> const &weights, std::vector<REAL> const &duWeights,
      std::vector<REAL> const &dvWeights, std::vector<REAL> const &duuWeights,
      std::vector<REAL> const &duvWeights, std::vector<REAL> const &dvvWeights,
      bool includeCoarseVerts, size_t firstOffset);
...
public:
  ...
  ///
  template <class T, class U>
  void UpdateDerivs(T const &srcValues, U &uderivs, U &vderivs, int start = -1,
                    int end = -1) const {
  ...
  void UpdateDerivs(T1 const &srcBase, int numBase, T2 const &srcRef,
                    U &uderivs, U &vderivs, int start = -1,
                    int end = -1) const {
  ...
  //  Pointer interface for backward compatibility
  template <class T, class U>
  void UpdateDerivs(T const *src, U *uderivs, U *vderivs, int start = -1,
                    int end = -1) const {
  ...
  ///
  template <class T, class U>
  void Update2ndDerivs(T const &srcValues, U &uuderivs, U &uvderivs,
                       U &vvderivs, int start = -1, int end = -1) const {
  ...
  void Update2ndDerivs(T1 const &srcBase, int numBase, T2 const &srcRef,
                       U &uuderivs, U &uvderivs, U &vvderivs, int start = -1,
                       int end = -1) const {
...
protected:
  LimitStencilTable(int numControlVerts, std::vector<int> const &offsets,
                    std::vector<int> const &sizes,
                    std::vector<int> const &sources,
                    ...
                    bool includeCoarseVerts, size_t firstOffset)
      : BaseTable(numControlVerts, offsets, sizes, sources, weights, duWeights,
...
template <typename REAL>
template <class T1, class T2, class U>
void StencilTableReal<REAL>::update(T1 const &srcBase, int numBase,
                                    T2 const &srcRef, U &dstValues,
                                    std::vector<REAL> const &valueWeights,
                                    Index start, Index end) const {
  ...
  for (int i = 0; i < nstencils; ++i, ++sizes) {
    ...
    for (int j = 0; j < *sizes; ++j, ++indices, ++weights) {
      if (*indices < numBase) {
        ...
        dstValues[start + i].AddWithWeight(srcRef[*indices - numBase],
                                           *weights);
      }
    }
  }
...
template <typename REAL>
template <class T, class U>
void StencilTableReal<REAL>::update(T const &srcValues, U &dstValues,
                                    std::vector<REAL> const &valueWeights,
                                    Index start, Index end) const {
...
template <typename REAL>
inline void StencilTableReal<REAL>::resize(int nstencils, int nelems) {
...
template <typename REAL>
inline void StencilTableReal<REAL>::reserve(int nstencils, int nelems) {
...
template <typename REAL>
inline StencilReal<REAL> StencilTableReal<REAL>::GetStencil(Index i) const {
...
template <typename REAL>
inline StencilReal<REAL> StencilTableReal<REAL>::operator[](Index index) const {
  return GetStencil(index);
}

template <typename REAL>
inline void LimitStencilTableReal<REAL>::resize(int nstencils, int nelems) {
...
LimitStencilTableReal<REAL>::GetLimitStencil(Index i) const {
  ...
  if (!_duWeights.empty() && !_dvWeights.empty() && !_duuWeights.empty() &&
      !_duvWeights.empty() && !_dvvWeights.empty()) {
