...
public:
  ...
  void BoundControlPoints(REAL const controlPoints[],
                          ...
                          REAL minExtent[], REAL maxExtent[]) const;
  ...
  void Evaluate(REAL const uv[2], REAL const patchPoints[],
                PointDescriptor const &pointDesc, REAL P[]) const;

  /// @brief Overload of evaluation for 1st derivatives
  void Evaluate(REAL const uv[2], REAL const patchPoints[],
                PointDescriptor const &pointDesc, REAL P[], REAL Du[],
                REAL Dv[]) const;

  /// @brief Overload of evaluation for 2nd derivatives
  void Evaluate(REAL const uv[2], REAL const patchPoints[],
                PointDescriptor const &pointDesc, REAL P[], REAL Du[],
                REAL Dv[], REAL Duu[], REAL Duv[], REAL Dvv[]) const;
  //@}
  ...
  int EvaluateStencil(REAL const uv[2], REAL sP[], REAL sDu[],
                      REAL sDv[]) const;

  /// @brief Overload of limit stencil evaluation for 2nd derivatives
  int EvaluateStencil(REAL const uv[2], REAL sP[], REAL sDu[], REAL sDv[],
...
template <typename REAL>
inline void Surface<REAL>::PreparePatchPoints(
...
//
template <typename REAL>
inline void Surface<REAL>::evaluateDerivs(REAL const uv[2],
...
template <typename REAL>
inline void Surface<REAL>::Evaluate(REAL const uv[2], REAL const patchPoints[],
                                    PointDescriptor const &pointDesc,
                                    REAL P[]) const {
...
template <typename REAL>
inline void Surface<REAL>::Evaluate(REAL const uv[2], REAL const patchPoints[],
                                    PointDescriptor const &pointDesc, REAL P[],
...
inline void Surface<REAL>::Evaluate(REAL const uv[2], REAL const patchPoints[],
                                    PointDescriptor const &pointDesc, REAL P[],
                                    REAL Du[], REAL Dv[], REAL Duu[],
                                    REAL Duv[], REAL Dvv[]) const {
...
template <typename REAL>
inline int Surface<REAL>::evaluateStencils(REAL const uv[2],
                                           REAL *sDeriv[]) const {
...
template <typename REAL>
inline int Surface<REAL>::EvaluateStencil(REAL const uv[2], REAL sP[]) const {

  REAL *derivativeStencils[6] = {sP, 0, 0, 0, 0, 0};
  return evaluateStencils(uv, derivativeStencils);
}
template <typename REAL>
inline int Surface<REAL>::EvaluateStencil(REAL const uv[2], REAL sP[],
