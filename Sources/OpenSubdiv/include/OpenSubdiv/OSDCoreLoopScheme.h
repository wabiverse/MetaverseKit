...
//  Loop traits:
//
template <> inline Split Scheme<SCHEME_LOOP>::GetTopologicalSplitType() {
  return SPLIT_TO_TRIS;
}

template <> inline int Scheme<SCHEME_LOOP>::GetRegularFaceSize() { return 3; }

template <> inline int Scheme<SCHEME_LOOP>::GetRegularVertexValence() {
  return 6;
}

template <> inline int Scheme<SCHEME_LOOP>::GetLocalNeighborhoodSize() {
  return 1;
}
...
inline void Scheme<SCHEME_LOOP>::assignCreaseMaskForEdge(EDGE const &,
                                                         MASK &mask) const {
...
template <>
template <typename EDGE, typename MASK>
inline void Scheme<SCHEME_LOOP>::assignSmoothMaskForEdge(EDGE const &edge,
                                                         MASK &mask) const {
  ...
  //
  //  This is where we run into the issue of "face weights" -- we want to weight
  //  the face-centers for Catmark, but face-centers are not generated for Loop.
  //  So do we make assumptions on how the mask is used, assign some property to
  //  the mask to indicate how they were assigned, or take input from the mask
  //  itself?
  //
...
inline void Scheme<SCHEME_LOOP>::assignCornerMaskForVertex(VERTEX const &,
                                                           MASK &mask) const {
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_LOOP>::assignSmoothMaskForVertex(VERTEX const &vertex,
                                                           MASK &mask) const {
  typedef typename MASK::Weight Weight;
  ...
  //  Specialize for the regular case:  1/16 per edge-vert, 5/8 for the vert
  //  itself:
  ...
  if (valence != 6) {
    ...
    //     - could use some lookup tables here for common irregular valence (5,
    //     7, 8)
...
template <typename VERTEX, typename MASK>
inline void
Scheme<SCHEME_LOOP>::assignCornerLimitMask(VERTEX const & /* vertex */,
                                           MASK &posMask) const {
...
Scheme<SCHEME_LOOP>::assignCreaseLimitMask(VERTEX const &vertex, MASK &posMask,
  ...
  //
  //  The refinement mask for a crease vertex is (1/8, 3/4, 1/8) and for a
  //  crease edge is (1/2, 1/2) -- producing a uniform B-spline curve along the
  //  crease (boundary) whether the vertex or its crease is regular or not.  The
  //  limit mask is therefore (1/6, 2/3, 1/6) for ALL cases.
  //
  //  An alternative limit mask (1/5, 3/5, 1/5) is often published for use
  //  either for irregular crease vertices or for all crease/boundary vertices,
  //  but this is based on an alternate refinement mask for the edge -- (3/8,
  //  5/8) versus the usual (1/2, 1/2) -- and will not produce the B-spline
  //  curve desired.
  //
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_LOOP>::assignSmoothLimitMask(VERTEX const &vertex,
                                                       MASK &posMask) const {
  ...
  //  Specialize for the regular case:  1/12 per edge-vert, 1/2 for the vert
  //  itself:
  if (valence == 6) {
...
//  Ideally a regular mesh of uniformly shaped triangles with similar edge
lengths
//  should produce tangents of similar magnitudes throughout -- including
corners
...
//  2.0 to compensate for the fact that the difference of only side of the
vertex
//  is considered here.  The resulting scale factor of 3.0 for the regular
corner
...
//  from the above construction of the boundary, but the commonly used weights
of
//  +/- 1 and 2 result from omitting the common factor of sqrt(3)/2 (arising
from
//  the sines of increments of pi/3).  With that scale factor close to one, it
has
...
//  scaling of 3.0 is similarly understandable for the valence 2 and 3 cases, it
is
//  less obvious in the irregular formula for valence > 4, but similarly
effective.
//
//  The end result of these adjustments should be a set of limit tangents that
are
//  of similar magnitude over a regular mesh including boundaries and corners.
*/
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_LOOP>::assignCornerLimitTangentMasks(
    VERTEX const &vertex, MASK &tan1Mask, MASK &tan2Mask) const {
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_LOOP>::assignCreaseLimitTangentMasks(
    VERTEX const &vertex, MASK &tan1Mask, MASK &tan2Mask,
    int const creaseEnds[2]) const {
  ...
  if (interiorEdgeCount == 2) {
    ...
    for (int i = 1; i <= interiorEdgeCount; ++i) {
      tan2Mask.EdgeWeight(creaseEnds[0] + i) =
          (Weight)(eCoeff * std::sin(i * theta));
    }
...
template <>
template <typename VERTEX, typename MASK>
inline void Scheme<SCHEME_LOOP>::assignSmoothLimitTangentMasks(
    VERTEX const &vertex, MASK &tan1Mask, MASK &tan2Mask) const {
