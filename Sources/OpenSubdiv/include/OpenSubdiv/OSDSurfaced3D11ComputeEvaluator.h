...
public:
  ...
  bool EvalStencils(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc,
    ...
    return EvalStencils(
        srcBuffer->BindD3D11UAV(deviceContext), srcDesc,
        dstBuffer->BindD3D11UAV(deviceContext), dstDesc,
        stencilTable->GetSizesSRV(), stencilTable->GetOffsetsSRV(),
        stencilTable->GetIndicesSRV(), stencilTable->GetWeightsSRV(),
        /* start = */ 0,
  ...
  bool EvalStencils(ID3D11UnorderedAccessView *srcSRV,
                    ...
                    ID3D11ShaderResourceView *weightsSRV, int start, int end,
                    ID3D11DeviceContext *deviceContext) const;
