...
///
/// The PatchMap provides a quad-tree based lookup structure that, given a
/// singular parametric location, can efficiently return a handle to the
/// sub-patch that contains this location.
///
class PatchMap {
public:
...
private:
  ...
  struct QuadNode {
    ...
    // sets the child in "quadrant" to point to the node or patch of the given
    // index
  ...
  QuadNode *assignLeafOrChildNode(QuadNode *node, bool isLeaf, int quad,
                                  int index);

  template <class T>
  static int transformUVToQuadQuadrant(T const &median, T &u, T &v);
  template <class T>
  static int transformUVToTriQuadrant(T const &median, T &u, T &v,
                                      bool &rotated);

private:
  bool _patchesAreTriangular; // tri and quad assembly and search requirements
                              // differ
...
//
template <class T>
inline int PatchMap::transformUVToQuadQuadrant(T const &median, T &u, T &v) {
...
template <class T>
int inline PatchMap::transformUVToTriQuadrant(T const &median, T &u, T &v,
                                              bool &rotated) {
...
inline PatchMap::Handle const *PatchMap::FindPatch(int faceid, double u,
                                                   double v) const {
