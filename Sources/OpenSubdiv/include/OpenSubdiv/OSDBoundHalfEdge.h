...
private:
  ...
  // called by HbrFace
  void Initialize(HbrHalfedge<T> *opposite, int index, HbrVertex<T> *origin,
...
public:
  ...
  HbrHalfedge<T> *GetNext() const {
    if (m_index == 4) {
      ...
      if (lastedge) {
        return (HbrHalfedge<T> *)((char *)this -
                                  (GetFace()->GetNumVertices() - 1) * edgesize);
      } else {
  ...
  HbrHalfedge<T> *GetPrev() const {
    const size_t edgesize =
        (m_index == 4) ? (sizeof(HbrHalfedge<T>) + sizeof(HbrFace<T> *))
  ...
  HbrVertex<T> *GetDestVertex(HbrMesh<T> *mesh) const {
    return GetNext()->GetOrgVertex(mesh);
  }
  ...
  HbrFace<T> *GetRightFace() const {
    return opposite ? opposite->GetLeftFace() : NULL;
  }
  ...
  void CopyFVarInfiniteSharpness(HbrHalfedge<T> *edge) {
    ...
    if (fvarinfsharp) {
      ...
      if (edge->IsSharp(true)) {
        memset(fvarinfsharp, 0x55555555,
               fvarbitsSizePerEdge * sizeof(unsigned int));
      } else {
        memcpy(fvarinfsharp, edge->getFVarInfSharp(),
               fvarbitsSizePerEdge * sizeof(unsigned int));
      }
    }
  }
  ...
  bool GetStitchCCW(bool raytraced) const {
  ...
  int getIndex() const {
    if (m_index < 4) {
      ...
      HbrFace<T> *incidentFace =
          *(HbrFace<T> **)((char *)this + sizeof(HbrHalfedge<T>));
...
public:
  struct adaptiveFlags {
    ...
    adaptiveFlags()
        : isTransition(0), isTriangleHead(0), isWatertightCritical(0) {}
  };
  ...
  bool IsWatertightCritical() const {
...
template <class T>
void HbrHalfedge<T>::Initialize(HbrHalfedge<T> *opposite, int index,
                                HbrVertex<T> *origin, unsigned int *fvarbits,
                                HbrFace<T> *face) {
  ...
  if (opposite) {
    ...
    StitchEdge **stitchEdges =
        face->stitchEdges + mesh->GetStitchCount() * index;
    ...
    if (fvarbits) {
      ...
      int fvarbitsSizePerEdge = ((fvarcount + 15) / 16);
      memcpy(fvarbits, opposite->getFVarInfSharp(),
             fvarbitsSizePerEdge * sizeof(unsigned int));
    }
  } else {
    sharpness = 0.0f;
...
template <class T> bool HbrHalfedge<T>::GetFVarInfiniteSharp(int datum) {
  ...
  if (!right->GetFVarData(rorg).Compare(left->GetFVarData(lorg), startindex,
                                        width, 0.001f) ||
      !right->GetFVarData(rdst).Compare(left->GetFVarData(ldst), startindex,
                                        width, 0.001f)) {
    bits = ~(0x2 << shift);
    fvarinfsharp[intindex] &= bits;
    if (opposite)
...
template <class T>
float HbrHalfedge<T>::GetFVarSharpness(int datum, bool ignoreGeometry) {
