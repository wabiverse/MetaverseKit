...
struct PatchArray {
  // 4-ints struct.
  PatchArray(Far::PatchDescriptor desc_in, int numPatches_in, int indexBase_in,
             int primitiveIdBase_in)
      : regDesc(desc_in), desc(desc_in), numPatches(numPatches_in),
        indexBase(indexBase_in), stride(desc_in.GetNumControlVertices()),
