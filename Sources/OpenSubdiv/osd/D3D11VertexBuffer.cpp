...
void D3D11VertexBuffer::UpdateData(const float *src, int startVertex,
                                   int numVertices,
                                   ID3D11DeviceContext *deviceContext) {
  ...
  HRESULT hr = deviceContext->Map(_uploadBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                                  &resource);
  ...
  deviceContext->CopySubresourceRegion(_buffer, 0, 0, 0, 0, _uploadBuffer, 0,
                                       &srcBox);
}
