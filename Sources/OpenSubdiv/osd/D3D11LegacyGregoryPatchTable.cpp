...
D3D11LegacyGregoryPatchTable::D3D11LegacyGregoryPatchTable()
    : _vertexValenceBuffer(0), _quadOffsetsBuffer(0), _vertexSRV(0),
      _vertexValenceSRV(0), _quadOffsetsSRV(0) {
...
D3D11LegacyGregoryPatchTable::Create(Far::PatchTable const *farPatchTable,
  ...
  if (!valenceTable.empty()) {
    ...
    hr = pd3d11Device->CreateShaderResourceView(
        result->_vertexValenceBuffer, &srvd, &result->_vertexValenceSRV);
    if (FAILED(hr)) {
      delete result;
      return NULL;
    }
  ...
  if (!quadOffsetsTable.empty()) {
    ...
    HRESULT hr =
        pd3d11Device->CreateBuffer(&bd, &initData, &result->_quadOffsetsBuffer);
    if (FAILED(hr)) {
      delete result;
      return NULL;
    }
...
void D3D11LegacyGregoryPatchTable::UpdateVertexBuffer(
