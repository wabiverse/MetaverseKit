...
static void resolveInteropFunctions() {
  ...
  if (!clCreateFromD3D11Buffer) {
    clCreateFromD3D11Buffer =
        (clCreateFromD3D11Buffer_fn)clGetExtensionFunctionAddress(
  ...
  if (!clEnqueueAcquireD3D11Objects) {
    clEnqueueAcquireD3D11Objects =
        (clEnqueueAcquireD3D11Objects_fn)clGetExtensionFunctionAddress(
  ...
  if (!clEnqueueReleaseD3D11Objects) {
    clEnqueueReleaseD3D11Objects =
        (clEnqueueReleaseD3D11Objects_fn)clGetExtensionFunctionAddress(
...
CLD3D11VertexBuffer::CLD3D11VertexBuffer(int numElements, int numVertices)
    : _numElements(numElements), _numVertices(numVertices), _d3d11Buffer(NULL),
      _clMemory(NULL), _clQueue(NULL), _clMapped(false) {}
...
void CLD3D11VertexBuffer::UpdateData(const float *src, int startVertex,
  ...
  map(queue);
  clEnqueueWriteBuffer(queue, _clMemory, true, offset, size, src, 0, NULL,
                       NULL);
}
...
bool CLD3D11VertexBuffer::allocate(cl_context clContext, ID3D11Device *device) {
  ...
  _clMemory =
      clCreateFromD3D11Buffer(clContext, CL_MEM_READ_WRITE, _d3d11Buffer, &err);
