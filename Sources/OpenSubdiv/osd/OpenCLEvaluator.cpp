...
template <class T>
cl_mem createCLBuffer(std::vector<T> const &src, cl_context clContext) {
  if (src.empty()) {
    return NULL;
  }
...
CLStencilTable::CLStencilTable(Far::LimitStencilTable const *limitStencilTable,
  ...
  if (_numStencils > 0) {
    ...
    _indices =
        createCLBuffer(limitStencilTable->GetControlIndices(), clContext);
...
CLEvaluator::CLEvaluator(cl_context context, cl_command_queue queue)
    : _clContext(context), _clCommandQueue(queue), _program(NULL),
      _stencilKernel(NULL), _stencilDerivKernel(NULL), _patchKernel(NULL) {}
...
bool CLEvaluator::Compile(BufferDescriptor const &srcDesc,
  ...
  const char *sources[] = {defineStr.c_str(), patchBasisTypesSource,
                           patchBasisSource, clSource};
  _program = clCreateProgramWithSource(_clContext, 4, sources, 0, &errNum);
  if (errNum != CL_SUCCESS) {
    Far::Error(Far::FAR_RUNTIME_ERROR, "clCreateProgramWithSource (%d)",
               errNum);
  }
  ...
  if (errNum != CL_SUCCESS) {
    ...
    cl_int numDevices = 0;
    clGetContextInfo(_clContext, CL_CONTEXT_NUM_DEVICES, sizeof(cl_uint),
                     &numDevices, NULL);
    ...
    for (int i = 0; i < numDevices; ++i) {
      ...
      clGetProgramBuildInfo(_program, devices[i], CL_PROGRAM_BUILD_LOG,
  ...
  _stencilDerivKernel =
      clCreateKernel(_program, "computeStencilsDerivatives", &errNum);
  if (errNum != CL_SUCCESS) {
    Far::Error(Far::FAR_RUNTIME_ERROR, "buildKernel (%d)\n", errNum);
    return false;
  }
...
bool CLEvaluator::EvalStencils(cl_mem src, BufferDescriptor const &srcDesc,
                               cl_mem dst, BufferDescriptor const &dstDesc,
                               cl_mem sizes, cl_mem offsets, cl_mem indices,
                               cl_mem weights, int start, int end,
  ...
  cl_int errNum = clEnqueueNDRangeKernel(_clCommandQueue, _stencilKernel, 1,
...
bool CLEvaluator::EvalStencils(
    cl_mem src, BufferDescriptor const &srcDesc, cl_mem dst,
    BufferDescriptor const &dstDesc, cl_mem du, BufferDescriptor const &duDesc,
    cl_mem dv, BufferDescriptor const &dvDesc, cl_mem sizes, cl_mem offsets,
    cl_mem indices, cl_mem weights, cl_mem duWeights, cl_mem dvWeights,
    int start, int end, unsigned int numStartEvents,
  ...
  cl_int errNum = clEnqueueNDRangeKernel(_clCommandQueue, _stencilDerivKernel,
...
bool CLEvaluator::EvalStencils(
    cl_mem src, BufferDescriptor const &srcDesc, cl_mem dst,
    BufferDescriptor const &dstDesc, cl_mem du, BufferDescriptor const &duDesc,
    cl_mem dv, BufferDescriptor const &dvDesc, cl_mem duu,
    BufferDescriptor const &duuDesc, cl_mem duv,
    BufferDescriptor const &duvDesc, cl_mem dvv,
    BufferDescriptor const &dvvDesc, cl_mem sizes, cl_mem offsets,
    cl_mem indices, cl_mem weights, cl_mem duWeights, cl_mem dvWeights,
    cl_mem duuWeights, cl_mem duvWeights, cl_mem dvvWeights, int start, int end,
    unsigned int numStartEvents, const cl_event *startEvents,
    cl_event *endEvent) const {
  if (end <= start)
    return true;
...
bool CLEvaluator::EvalPatches(
    cl_mem src, BufferDescriptor const &srcDesc, cl_mem dst,
    BufferDescriptor const &dstDesc, cl_mem du, BufferDescriptor const &duDesc,
    cl_mem dv, BufferDescriptor const &dvDesc, int numPatchCoords,
    cl_mem patchCoordsBuffer, cl_mem patchArrayBuffer, cl_mem patchIndexBuffer,
    cl_mem patchParamBuffer, unsigned int numStartEvents,
    const cl_event *startEvents, cl_event *endEvent) const {
  ...
  cl_int errNum = clEnqueueNDRangeKernel(_clCommandQueue, _patchKernel, 1, NULL,
                                         &globalWorkSize, NULL, numStartEvents,
...
/* static */
void CLEvaluator::Synchronize(cl_command_queue clCommandQueue) {
