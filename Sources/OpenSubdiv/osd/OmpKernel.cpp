...
#pragma omp parallel for
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < (int)sizes[index]; ++j) {
      addWithWeight(threadResult, src, threadIndices[j], threadWeights[j],
                    srcDesc);
    }
...
void OmpEvalStencils(float const *src, BufferDescriptor const &srcDesc,
                     ...
                     BufferDescriptor const &dstDvDesc, int const *sizes,
  ...
  float *result = (float *)alloca(srcDesc.length * numThreads * sizeof(float));
  float *resultDu =
      (float *)alloca(srcDesc.length * numThreads * sizeof(float));
  float *resultDv =
      (float *)alloca(srcDesc.length * numThreads * sizeof(float));
...
#pragma omp parallel for
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < (int)sizes[index]; ++j) {
      addWithWeight(threadResult, src, threadIndices[j], threadWeights[j],
                    srcDesc);
      addWithWeight(threadResultDu, src, threadIndices[j], threadWeightsDu[j],
                    srcDesc);
      addWithWeight(threadResultDv, src, threadIndices[j], threadWeightsDv[j],
                    srcDesc);
    }
...
void OmpEvalStencils(float const *src, BufferDescriptor const &srcDesc,
                     ...
                     BufferDescriptor const &dstDvvDesc, int const *sizes,
                     int const *offsets, int const *indices,
                     float const *weights, float const *duWeights,
...
#pragma omp parallel for
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < (int)sizes[index]; ++j) {
      ...
      addWithWeight(threadResultDv, src, threadIndices[j], threadWeightsDv[j],
                    srcDesc);
      addWithWeight(threadResultDuu, src, threadIndices[j], threadWeightsDuu[j],
                    srcDesc);
      addWithWeight(threadResultDuv, src, threadIndices[j], threadWeightsDuv[j],
                    srcDesc);
      addWithWeight(threadResultDvv, src, threadIndices[j], threadWeightsDvv[j],
                    srcDesc);
    }
