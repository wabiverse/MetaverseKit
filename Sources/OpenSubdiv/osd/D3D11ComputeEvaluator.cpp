...
#include "OpenSubdiv/OSDSurfaceHLSLComputeKernel.gen.h"
    ;
...
D3D11ComputeEvaluator::D3D11ComputeEvaluator()
    : _computeShader(NULL), _classLinkage(NULL), _singleBufferKernel(NULL),
      _separateBufferKernel(NULL), _uniformArgs(NULL), _workGroupSize(64) {}

D3D11ComputeEvaluator *D3D11ComputeEvaluator::Create(
...
bool D3D11ComputeEvaluator::Compile(BufferDescriptor const &srcDesc,
  ...
  D3D_SHADER_MACRO defines[] = {{"LENGTH", lengthValue.c_str()},
  ...
  HRESULT hr = D3DCompile(shaderSource, strlen(shaderSource), NULL, &defines[0],
                          NULL, "cs_main", "cs_5_0", dwShaderFlags, 0,
  ...
  device->CreateComputeShader(computeShaderBuffer->GetBufferPointer(),
                              ...
                              _classLinkage, &_computeShader);
  assert(_computeShader);

  ID3D11ShaderReflection *reflector;
  D3DReflect(computeShaderBuffer->GetBufferPointer(),
             computeShaderBuffer->GetBufferSize(), IID_ID3D11ShaderReflection,
             (void **)&reflector);
  assert(reflector);
  ...
  _classLinkage->GetClassInstance("singleBufferCompute", 0,
                                  &_singleBufferKernel);
  assert(_singleBufferKernel);
  _classLinkage->GetClassInstance("separateBufferCompute", 0,
                                  &_separateBufferKernel);
...
bool D3D11ComputeEvaluator::EvalStencils(
    ID3D11UnorderedAccessView *srcUAV, BufferDescriptor const &srcDesc,
    ID3D11UnorderedAccessView *dstUAV, BufferDescriptor const &dstDesc,
    ID3D11ShaderResourceView *sizesSRV, ID3D11ShaderResourceView *offsetsSRV,
    ID3D11ShaderResourceView *indicesSRV, ID3D11ShaderResourceView *weightsSRV,
    int start, int end, ID3D11DeviceContext *deviceContext) const {
  ...
  deviceContext->Map(_uniformArgs, 0, D3D11_MAP_WRITE_DISCARD, 0,
                     &mappedResource);
  ...
  ID3D11ShaderResourceView *SRViews[] = {sizesSRV, offsetsSRV, indicesSRV,
                                         weightsSRV};
  ...
  if (srcUAV == dstUAV) {
    ...
    deviceContext->Dispatch((count + _workGroupSize - 1) / _workGroupSize, 1,
                            1);
  } else {
