...
CpuPatchTable::CpuPatchTable(const Far::PatchTable *farPatchTable) {
  ...
  _varyingIndexBuffer.reserve(
      numPatches *
      farPatchTable->GetVaryingPatchDescriptor().GetNumControlVertices());
  ...
  for (int fvc = 0; fvc < farPatchTable->GetNumFVarChannels(); ++fvc) {
    ...
    _fvarIndexBuffers[fvc].reserve(numPatches *
                                   farPatchTable->GetFVarValueStride(fvc));
  ...
  for (int j = 0; j < nPatchArrays; ++j) {
    ...
    PatchArray patchArray(farPatchTable->GetPatchArrayDescriptor(j),
                          numPatchesThisArray, (int)_indexBuffer.size(),
    ...
    PatchArray varyingPatchArray(
        farPatchTable->GetVaryingPatchDescriptor(), numPatchesThisArray,
        (int)_varyingIndexBuffer.size(), (int)_patchParamBuffer.size());
    _varyingPatchArrays.push_back(varyingPatchArray);

    Far::ConstIndexArray varyingIndices =
        farPatchTable->GetPatchArrayVaryingVertices(j);
    ...
    for (int fvc = 0; fvc < farPatchTable->GetNumFVarChannels(); ++fvc) {
      ...
      PatchArray fvarPatchArray(
          ...
          numPatchesThisArray, (int)_fvarIndexBuffers[fvc].size(),
          (int)_fvarParamBuffers[fvc].size());
      _fvarPatchArrays[fvc].push_back(fvarPatchArray);

      Far::ConstIndexArray fvarIndices =
          farPatchTable->GetPatchArrayFVarValues(j, fvc);
      ...
      Far::ConstPatchParamArray fvarParam =
          farPatchTable->GetPatchArrayFVarPatchParams(j, fvc);
