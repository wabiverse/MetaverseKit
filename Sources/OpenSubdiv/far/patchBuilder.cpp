...
inline bool isEdgeSingular(Level const &level, FVarLevel const *fvarLevel,
...
void identifyManifoldCornerSpan(Level const &level, Index fIndex, int fCorner,
                                Level::ETag eTagMask, Level::VSpan &vSpan,
                                int fvc = -1) {
...
void identifyNonManifoldCornerSpan(Level const &level, Index fIndex,
  ...
  if (eTrailing != eLeading) {
    while (!isEdgeSingular(level, fvarLevel, eTrailing, eTagMask)) {
      ...
      eTrailing = fEdges[(fEdges.FindIndex(eTrailing) + fEdges.size() - 1) %
                         fEdges.size()];
      if (eTrailing == eLeadingStart) {
        vSpan._periodic =
            !isEdgeSingular(level, fvarLevel, eTrailing, eTagMask);
        break;
      }
    }
  }
...
inline int countManifoldCornerSpan(Level const &level, Index fIndex,
                                   int fCorner, Level::ETag eTagMask,
                                   int fvc = -1) {
  Level::VSpan vSpan;
...
inline int countNonManifoldCornerSpan(Level const &level, Index fIndex,
                                      int fCorner, Level::ETag eTagMask,
                                      int fvc = -1) {
  Level::VSpan vSpan;
...
//      each
//  incident tri, the vertex opposite its leading edge is added.  If the vertex
//  is on a boundary, a second vertex on the boundary edge will be contributed
//  from the last face.
//
int gatherTriRegularRingAroundVertex(Level const &level, Index vIndex,
                                     int ringPoints[], int fvarChannel) {
...
int gatherRegularPartialRingAroundVertex(Level const &level, Index vIndex,
                                         Level::VSpan const &span,
                                         int ringPoints[], int fvarChannel) {
  ...
  for (int i = 0; i < nFaces; ++i) {
    ...
    if (i == (nFaces - 1)) {
      ...
      int nextInThisFace =
          fastModN(vInThisFace + fPoints.size() - 1, fPoints.size());
...
inline int packTriBoundaryMask(int upper, int lower) {
...
inline Index getNextFaceInVertFaces(Level const &level, int thisFaceInVFaces,
  ...
  if (manifold) {
    ...
    Index nextEdge =
        fEdges[fastModN((vInThisFace + fEdges.size() - 1), fEdges.size())];
...
inline Index getPrevFaceInVertFaces(Level const &level, int thisFaceInVFaces,
                                    ConstIndexArray const &vFaces,
                                    ConstLocalIndexArray const &vInFaces,
  ...
  Index prevFace;
  if (manifold) {
    int prevFaceInVFaces =
        (thisFaceInVFaces ? thisFaceInVFaces : vFaces.size()) - 1;
...
inline ConstIndexArray getFacePoints(Level const &level, Index faceIndex,
                                     int fvarChannel) {
...
//
PatchBuilder *PatchBuilder::Create(TopologyRefiner const &refiner,
...
bool PatchBuilder::IsFaceALeaf(int levelIndex, Index faceIndex) const {
  ...
  if (levelIndex < _refiner.GetMaxLevel()) {
    ...
    if (_refiner.getRefinement(levelIndex)
            .getParentFaceSparseTag(faceIndex)
            ._selected) {
      return false;
    }
  }
...
bool PatchBuilder::IsPatchRegular(int levelIndex, Index faceIndex,
                                  int fvc) const {
  ...
  if (featureIsIsolated) {
    bool featureRequiresFurtherInspection =
        fCompVTag._nonManifold ||
        (_options.approxSmoothCornerWithSharp && fCompVTag._xordinary &&
         fCompVTag._boundary) ||
        (testInfSharpFeatures && fCompVTag._infIrregular &&
         fCompVTag._infSharpEdges);
  ...
  for (int i = 0; i < _schemeRegFaceSize; ++i) {
    ...
    if (vTag._nonManifold) {
      ...
      int nSpanFaces = countNonManifoldCornerSpan(
          level, faceIndex, i, getSingularEdgeMask(testInfSharpFeatures), fvc);
      if (vTag._infSharp) {
        if (nSpanFaces != 1)
          return false;
      } else {
    ...
    if (vTag._xordinary) {
      ...
      if (_options.approxSmoothCornerWithSharp && vTag._boundary &&
          !vTag._infSharp) {
        Level::ETag eTags[4];
...
int PatchBuilder::GetRegularPatchBoundaryMask(int levelIndex, Index faceIndex,
...
void PatchBuilder::GetIrregularPatchCornerSpans(int levelIndex, Index faceIndex,
  ...
  for (int i = 0; i < _schemeRegFaceSize; ++i) {
    ...
    bool testInfSharpEdges = testInfSharpFeatures && vTag._infSharpEdges &&
                             (vTag._rule != Sdc::Crease::RULE_DART);
    ...
    if (testInfSharpEdges || isFVarMisMatch || isNonManifold) {
      if (isNonManifold) {
        identifyNonManifoldCornerSpan(level, faceIndex, i, singularEdgeMask,
                                      cornerSpans[i], fvarChannel);
      } else {
        identifyManifoldCornerSpan(level, faceIndex, i, singularEdgeMask,
                                   cornerSpans[i], fvarChannel);
      }
    } else {
      cornerSpans[i].clear();
    ...
    if (vTag._corner) {
      ...
      if (isFVarMisMatch) {
        cornerSpans[i]._sharp =
            (cornerSpans[i]._numFaces == 1) || isNonManifold;
      } else {
      ...
      cornerSpans[i]._sharp =
          testInfSharpEdges ? !vTag._infSharpCrease : vTag._infSharp;
    }

    //  Legacy option -- reinterpret a smooth corner as sharp:
    bool smoothCorner = !cornerSpans[i]._sharp;
    if (smoothCorner && _options.approxSmoothCornerWithSharp &&
        vTag._xordinary && vTag._boundary && !vTag._infSharp &&
        !vTag._nonManifold) {
      int nFaces =
          cornerSpans[i].isAssigned()
              ? cornerSpans[i]._numFaces
              : level.getVertexFaces(level.getFaceVertices(faceIndex)[i])
                    .size();
...
int PatchBuilder::getRegularFacePoints(int levelIndex, Index faceIndex,
                                       Index patchPoints[],
                                       int fvarChannel) const {
  ...
  ConstIndexArray facePoints =
      (fvarChannel < 0) ? level.getFaceVertices(faceIndex)
                        : level.getFaceFVarValues(faceIndex, fvarChannel);
...
int PatchBuilder::getQuadRegularPatchPoints(int levelIndex, Index faceIndex,
                                            int regBoundaryMask,
                                            Index patchPoints[],
                                            int fvarChannel) const {
  ...
  static int const patchPointsPerCorner[4][4] = {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    bool interiorCorner =
        interiorPatch ||
        (((eMask & (1 << i)) | (eMask & (1 << fastMod4(i + 3)))) == 0);
    if (interiorCorner) {
      ...
      int vInFNext;
      Index fNext =
          getNextFaceInVertFaces(level, fInVFaces, vFaces, vInFaces,
      ...
      int vInFPrev;
      Index fPrev =
          getPrevFaceInVertFaces(level, fInVFaces, vFaces, vInFaces,
...
int PatchBuilder::getTriRegularPatchPoints(int levelIndex, Index faceIndex,
                                           int regBoundaryMask,
                                           Index patchPoints[],
                                           int fvarChannel) const {
  ...
  static int const patchPointsPerCorner[3][4] = {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (interiorCorner) {
      ...
      if (level.getVertexTag(v)._nonManifold) {
        Index f1 = getNextFaceInVertFaces(level, fInVFaces, vFaces, vInFaces,
                                          false, vInf2);
        f2 = getNextFaceInVertFaces(level, vFaces.FindIndex(f1), vFaces,
                                    vInFaces, false, vInf2);
      }
      ConstIndexArray f2Points = getFacePoints(level, f2, fvarChannel);
      ...
      if (level.getVertexTag(v)._nonManifold) {
        Index f1 = getPrevFaceInVertFaces(level, fInVFaces, vFaces, vInFaces,
                                          false, vInf0);
        f0 = getPrevFaceInVertFaces(level, vFaces.FindIndex(f1), vFaces,
                                    vInFaces, false, vInf0);
      }
...
int PatchBuilder::GetRegularPatchPoints(int levelIndex, Index faceIndex,
                                        int regBoundaryMask,
                                        Index patchPoints[],
                                        int fvarChannel) const {

  if (_schemeIsLinear) {
    return getRegularFacePoints(levelIndex, faceIndex, patchPoints,
                                fvarChannel);
  } else if (_schemeRegFaceSize == 4) {
    return getQuadRegularPatchPoints(levelIndex, faceIndex, regBoundaryMask,
                                     patchPoints, fvarChannel);
  } else {
    return getTriRegularPatchPoints(levelIndex, faceIndex, regBoundaryMask,
                                    patchPoints, fvarChannel);
  }
  return 0;
}

int PatchBuilder::assembleIrregularSourcePatch(int levelIndex, Index faceIndex,
  ...
  for (int corner = 0; corner < fVerts.size(); ++corner) {
    ...
    patchCorner._dart =
        (vTag._rule == Sdc::Crease::RULE_DART) && vTag._infSharpEdges;
  }
...
//
int PatchBuilder::gatherIrregularSourcePoints(int levelIndex, Index faceIndex,
                                              Level::VSpan const cornerSpans[4],
  ...
  for (int corner = 0; corner < sourcePatch._numCorners; ++corner) {
    ...
    if (cornerSpans[corner].isAssigned()) {
      sourceRingSize = gatherRegularPartialRingAroundVertex(
          level, cornerVertex, cornerSpans[corner], sourceRingVertices,
          fvarChannel);
    } else if (sourcePatch._numCorners == 4) {
      sourceRingSize = level.gatherQuadRegularRingAroundVertex(
          cornerVertex, sourceRingVertices, fvarChannel);
    } else {
      sourceRingSize = gatherTriRegularRingAroundVertex(
          level, cornerVertex, sourceRingVertices, fvarChannel);
    }

    //  Gather the ring of local points from the patch:
    int patchRingSize =
        sourcePatch.GetCornerRingPoints(corner, patchRingPoints);
...
int PatchBuilder::GetIrregularPatchSourcePoints(
  ...
  return gatherIrregularSourcePoints(levelIndex, faceIndex, cornerSpans,
                                     sourcePatch, sourcePoints, fvarChannel);
}
...
int PatchBuilder::GetIrregularPatchConversionMatrix(
    int levelIndex, Index faceIndex, Level::VSpan const cornerSpans[],
  ...
  return convertToPatchType(sourcePatch, GetIrregularPatchType(),
                            conversionMatrix);
}
...
bool PatchBuilder::IsRegularSingleCreasePatch(
    int levelIndex, Index faceIndex, SingleCreaseInfo &creaseInfo) const {
  ...
  return level.isSingleCreasePatch(faceIndex, &creaseInfo.creaseSharpness,
                                   &creaseInfo.creaseEdgeInFace);
}

PatchParam PatchBuilder::ComputePatchParam(int levelIndex, Index faceIndex,
                                           PtexIndices const &ptexIndices,
                                           bool isRegular, int boundaryMask,
                                           bool computeTransitionMask) const {
  ...
  bool irregBase =
      _refiner.GetLevel(depth).GetFaceVertices(faceIndex).size() != regFaceSize;
  ...
  for (int i = depth; i > 0; --i) {
    ...
    irregBase =
        parentLevel.getFaceVertices(parentFaceIndex).size() != regFaceSize;
...
void SourcePatch::Finalize(int size) {
  ...
  for (int cIndex = 0; cIndex < _numCorners; ++cIndex) {
    ...
    bool prevIsVal2Interior =
        ((_corners[cPrev]._numFaces == 2) && !_corners[cPrev]._boundary);
    bool thisIsVal2Interior =
        ((_corners[cIndex]._numFaces == 2) && !_corners[cIndex]._boundary);
    bool nextIsVal2Interior =
        ((_corners[cNext]._numFaces == 2) && !_corners[cNext]._boundary);
    ...
    if ((corner._numFaces + corner._boundary) > 2) {
      ...
      //
      if (corner._boundary) {
        corner._sharesWithPrev =
            isQuad && (corner._patchFace != (corner._numFaces - 1));
        ...
        bool cNextOnDartEdge =
            cN._boundary && (cN._patchFace == cN._numFaces - 1);
...
int SourcePatch::GetCornerRingPoints(int corner, int ringPoints[]) const {
  ...
  if (_corners[corner]._sharesWithPrev) {
    ringPoints[ringSize++] =
        _localRingOffsets[cPrev] + _localRingSizes[cPrev] - 1;
  }
