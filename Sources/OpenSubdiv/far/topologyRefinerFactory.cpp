...
//
//
bool TopologyRefinerFactoryBase::prepareComponentTopologySizing(
...
bool TopologyRefinerFactoryBase::prepareComponentTopologyAssignment(
    TopologyRefiner &refiner, bool fullValidation, TopologyCallback callback,
...
bool TopologyRefinerFactoryBase::prepareComponentTagsAndSharpness(
    TopologyRefiner &refiner) {
  ...
  bool sharpenCornerVerts = (options.GetVtxBoundaryInterpolation() ==
  ...
  if (makeBoundaryFacesHoles) {
    for (Vtr::Index vIndex = 0; vIndex < baseLevel.getNumVertices(); ++vIndex) {
      ...
      for (int i = 0; !excludeFaces && (i < vEdges.size()); ++i) {
        excludeFaces =
            (baseLevel.getNumEdgeFaces(vEdges[i]) == 1) &&
  ...
  for (Vtr::Index vIndex = 0; vIndex < baseLevel.getNumVertices(); ++vIndex) {
    ...
    vTag._rule =
        (Vtr::internal::Level::VTag::VTagSize)
...
bool TopologyRefinerFactoryBase::prepareFaceVaryingChannels(
    TopologyRefiner &refiner) {
