...
template <typename REAL> inline bool isWeightZero(REAL w) {
  return (w == (REAL)0.0);
}
...
void StencilTableFactoryReal<REAL>::generateControlVertStencils(
    int numControlVerts, StencilReal<REAL> &dst) {
...
StencilTableFactoryReal<REAL>::Create(TopologyRefiner const &refiner,
  ...
  bool interpolateFaceVarying =
      options.interpolationMode == INTERPOLATE_FACE_VARYING;
  ...
  for (int level = 1; level <= maxlevel; ++level) {
    if (interpolateVertex) {
      ...
      primvarRefiner.InterpolateFaceVarying(level, srcIndex, dstIndex,
                                            options.fvarChannel);
    }
  ...
  StencilTableReal<REAL> *result = new StencilTableReal<REAL>(
      numControlVertices, builder.GetStencilOffsets(),
      builder.GetStencilSizes(), builder.GetStencilSources(),
      builder.GetStencilWeights(), options.generateControlVerts, firstOffset);
  return result;
}
...
StencilTableFactoryReal<REAL>::AppendLocalPointStencilTable(
    ...
    StencilTableReal<REAL> const *localPointStencilTable, bool factorize) {

  return appendLocalPointStencilTable(refiner, baseStencilTable,
...
StencilTableFactoryReal<REAL>::AppendLocalPointStencilTableFaceVarying(
    TopologyRefiner const &refiner,
    StencilTableReal<REAL> const *baseStencilTable,
    StencilTableReal<REAL> const *localPointStencilTable, int channel,
    bool factorize) {

  return appendLocalPointStencilTable(
      refiner, baseStencilTable, localPointStencilTable, channel, factorize);
}
...
StencilTableFactoryReal<REAL>::appendLocalPointStencilTable(
  ...
  {
    int nverts = channel < 0 ? refiner.GetNumVerticesTotal()
...
//------------------------------------------------------------------------------
template <typename REAL>
LimitStencilTableReal<REAL> const *LimitStencilTableFactoryReal<REAL>::Create(
    TopologyRefiner const &refiner, LocationArrayVec const &locationArrays,
    StencilTableReal<REAL> const *cvStencilsIn, PatchTable const *patchTableIn,
    Options options) {
  ...
  // If an appropriate StencilTable was given, use it, otherwise, create a new
  // one
  ...
  if (!cvstencils) {
    ...
    cvstencils =
        StencilTableFactoryReal<REAL>::Create(refiner, stencilTableOptions);
  }
  ...
  if (!patchtable) {
    ...
    if (interpolateFaceVarying) {
      ...
      patchTableOptions.generateFVarLegacyLinearPatches =
          uniform || !refiner.GetAdaptiveOptions().considerFVarChannels;
    }
    patchTableOptions.SetEndCapType(
        Far::PatchTableFactory::Options::ENDCAP_GREGORY_BASIS);
    patchTableOptions.useInfSharpPatch =
        !uniform && refiner.GetAdaptiveOptions().useInfSharpPatch;
  ...
  if (interpolateVertex) {
    ...
    localstencils =
        patchtable->GetLocalPointFaceVaryingStencilTable<REAL>(fvarChannel);
  } else {
  ...
  if (localstencils && (cvstencils->GetNumStencils() == nRefinedStencils)) {
    ...
    if (interpolateFaceVarying) {
      cvstencils = StencilTableFactoryReal<
          REAL>::AppendLocalPointStencilTableFaceVarying(refiner,
                                                         refinedstencils,
                                                         localstencils,
                                                         fvarChannel);
    } else {
  ...
  //
  //  Generally use the patches corresponding to the interpolation mode, but
  //  Uniform PatchTables do not have varying patches -- use the equivalent
  //  linear vertex patches in this case:
  //
  ...
  for (size_t i = 0; i < locationArrays.size(); ++i) {
    ...
    for (int j = 0; j < array.numLocations;
         ++j) { // for each face we're working on
      REAL s = array.s[j],
      ...
      if (handle) {
        ...
        if (options.generate2ndDerivatives) {
          if (useVertexPatches) {
            patchtable->EvaluateBasis<REAL>(*handle, s, t, wP, wDs, wDt, wDss,
                                            wDst, wDtt);
          } else if (useFVarPatches) {
          ...
          for (int k = 0; k < cvs.size(); ++k) {
            dst.AddWithWeight(src[cvs[k]], wP[k], wDs[k], wDt[k], wDss[k],
                              wDst[k], wDtt[k]);
          }
          ...
          if (useVertexPatches) {
            ...
            patchtable->EvaluateBasisFaceVarying<REAL>(*handle, s, t, wP, 0, 0,
                                                       0, 0, 0, fvarChannel);
          } else {
  ...
  LimitStencilTableReal<REAL> *result = new LimitStencilTableReal<REAL>(
      nControlVertices, builder.GetStencilOffsets(), builder.GetStencilSizes(),
      builder.GetStencilSources(), builder.GetStencilWeights(),
      builder.GetStencilDuWeights(), builder.GetStencilDvWeights(),
      builder.GetStencilDuuWeights(), builder.GetStencilDuvWeights(),
