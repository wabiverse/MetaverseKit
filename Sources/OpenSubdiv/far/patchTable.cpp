...
PatchTable::PatchTable(int maxvalence)
    : _maxValence(maxvalence), _localPointStencils(),
      _localPointVaryingStencils(), _varyingDesc(Far::PatchDescriptor::QUADS),
      _isUniformLinear(false), _vertexPrecisionIsDouble(false),
      _varyingPrecisionIsDouble(false), _faceVaryingPrecisionIsDouble(false) {}

// Copy constructor
// XXXX manuelk we need to eliminate this constructor (C++11 smart pointers)
PatchTable::PatchTable(PatchTable const &src)
    : _maxValence(src._maxValence), _numPtexFaces(src._numPtexFaces),
      _patchArrays(src._patchArrays), _patchVerts(src._patchVerts),
      _paramTable(src._paramTable), _quadOffsetsTable(src._quadOffsetsTable),
      ...
      _varyingDesc(src._varyingDesc), _fvarChannels(src._fvarChannels),
  ...
  if (src._localPointVaryingStencils) {
    if (src._varyingPrecisionIsDouble) {
      _localPointVaryingStencils.Set(new StencilTableReal<double>(
          *src._localPointVaryingStencils.Get<double>()));
    } else {
      _localPointVaryingStencils.Set(new StencilTableReal<float>(
          *src._localPointVaryingStencils.Get<float>()));
    }
  }
  if (!src._localPointFaceVaryingStencils.empty()) {
    _localPointFaceVaryingStencils.resize(
        src._localPointFaceVaryingStencils.size());
...
void PatchTable::reservePatchArrays(int numPatchArrays) {
...
void PatchTable::allocateFVarPatchChannels(int numChannels) {
  _fvarChannels.resize(numChannels);
}
void PatchTable::allocateFVarPatchChannelValues(PatchDescriptor regDesc,
...
void PatchTable::setFVarPatchChannelLinearInterpolation(
...
inline int getPatchSize(PatchDescriptor desc) {
  return desc.GetNumControlVertices();
}

void PatchTable::pushPatchArray(PatchDescriptor desc, int npatches, Index *vidx,
                                Index *pidx, Index *qoidx) {

  if (npatches > 0) {
    _patchArrays.push_back(
        PatchArray(desc, npatches, *vidx, *pidx, qoidx ? *qoidx : 0));
    int nverts = getPatchSize(desc);
    *vidx += npatches * nverts;
    *pidx += npatches;
    if (qoidx) {
      *qoidx +=
          (desc.GetType() == PatchDescriptor::GREGORY) ? npatches * nverts : 0;
    }
  }
}
...
Index *PatchTable::getSharpnessIndices(int arrayIndex) {
  return &_sharpnessIndices[getPatchArray(arrayIndex).patchIndex];
}

float *PatchTable::getSharpnessValues(int arrayIndex) {
...
PatchDescriptor PatchTable::GetPatchArrayDescriptor(int arrayIndex) const {
  return getPatchArray(arrayIndex).desc;
}

int PatchTable::GetNumPatchArrays() const { return (int)_patchArrays.size(); }
int PatchTable::GetNumPatches(int arrayIndex) const {
...
ConstIndexArray PatchTable::GetPatchVertices(int arrayIndex,
...
float PatchTable::GetSingleCreasePatchSharpnessValue(
    PatchHandle const &handle) const {
...
float PatchTable::GetSingleCreasePatchSharpnessValue(int arrayIndex,
                                                     int patchIndex) const {
  PatchArray const &pa = getPatchArray(arrayIndex);
...
PatchTable::GetPatchQuadOffsets(PatchHandle const &handle) const {
  PatchArray const &pa = getPatchArray(handle.arrayIndex);
  return Vtr::ConstArray<unsigned int>(
      &_quadOffsetsTable[pa.quadOffsetIndex + handle.vertIndex], 4);
}
bool PatchTable::IsFeatureAdaptive() const { return !_isUniformLinear; }

PatchDescriptor PatchTable::GetVaryingPatchDescriptor() const {
  return _varyingDesc;
}
ConstIndexArray
...
ConstIndexArray PatchTable::GetPatchVaryingVertices(int array,
                                                    int patch) const {
  if (_varyingVerts.empty()) {
    return ConstIndexArray();
  }
...
void PatchTable::populateVaryingVertices() {
  ...
  // triangle patches.
  int numVaryingCVs = _varyingDesc.GetNumControlVertices();
  for (int arrayIndex = 0; arrayIndex < (int)_patchArrays.size();
       ++arrayIndex) {
    PatchArray const &pa = getPatchArray(arrayIndex);
...
ConstIndexArray PatchTable::GetPatchFVarValues(PatchHandle const &handle,
                                               int channel) const {
  return getPatchFVarValues(handle.patchIndex, channel);
}
ConstIndexArray PatchTable::GetPatchFVarValues(int arrayIndex, int patchIndex,
                                               int channel) const {
  return getPatchFVarValues(getPatchIndex(arrayIndex, patchIndex), channel);
}
ConstIndexArray PatchTable::GetPatchArrayFVarValues(int array,
                                                    int channel) const {
  PatchArray const &pa = getPatchArray(array);
  FVarPatchChannel const &c = getFVarPatchChannel(channel);
  int ncvs = c.stride;
...
PatchParam PatchTable::GetPatchFVarPatchParam(PatchHandle const &handle,
                                              int channel) const {
  return getPatchFVarPatchParam(handle.patchIndex, channel);
}
PatchParam PatchTable::GetPatchFVarPatchParam(int arrayIndex, int patchIndex,
                                              int channel) const {
...
void PatchTable::EvaluateBasis(PatchHandle const &handle, REAL s, REAL t,
                               REAL wP[], REAL wDs[], REAL wDt[], REAL wDss[],
                               REAL wDst[], REAL wDtt[]) const {
  ...
  internal::EvaluatePatchBasis(patchType, param, s, t, wP, wDs, wDt, wDss, wDst,
                               wDtt);
}
...
//
template <typename REAL>
void PatchTable::EvaluateBasisVarying(PatchHandle const &handle, REAL s, REAL t,
...
//
template <typename REAL>
void PatchTable::EvaluateBasisFaceVarying(PatchHandle const &handle, REAL s,
                                          REAL t, REAL wP[], REAL wDs[],
                                          REAL wDt[], REAL wDss[], REAL wDst[],
                                          REAL wDtt[], int channel) const {

  PatchParam param = getPatchFVarPatchParam(handle.patchIndex, channel);
  PatchDescriptor::Type patchType =
      param.IsRegular() ? GetFVarPatchDescriptorRegular(channel).GetType()
...
template void PatchTable::EvaluateBasis<float>(PatchHandle const &handle,
                                               ...
                                               float wDs[], float wDt[],
                                               float wDss[], float wDst[],
                                               float wDtt[]) const;
template void PatchTable::EvaluateBasisVarying<float>(
    PatchHandle const &handle, float s, float t, float wP[], float wDs[],
    float wDt[], float wDss[], float wDst[], float wDtt[]) const;
template void PatchTable::EvaluateBasisFaceVarying<float>(
    PatchHandle const &handle, float s, float t, float wP[], float wDs[],
    float wDt[], float wDss[], float wDst[], float wDtt[], int channel) const;
...
template void PatchTable::EvaluateBasis<double>(PatchHandle const &handle,
                                                ...
                                                double wDtt[]) const;
template void PatchTable::EvaluateBasisVarying<double>(
    PatchHandle const &handle, double s, double t, double wP[], double wDs[],
    double wDt[], double wDss[], double wDst[], double wDtt[]) const;
template void PatchTable::EvaluateBasisFaceVarying<double>(
    PatchHandle const &handle, double s, double t, double wP[], double wDs[],
    double wDt[], double wDss[], double wDst[], double wDtt[],
    int channel) const;
