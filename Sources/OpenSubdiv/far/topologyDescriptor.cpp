...
//
template <>
bool TopologyRefinerFactory<TopologyDescriptor>::resizeComponentTopology(
...
template <>
bool TopologyRefinerFactory<TopologyDescriptor>::assignComponentTopology(
    TopologyRefiner &refiner, TopologyDescriptor const &desc) {
...
template <>
bool TopologyRefinerFactory<TopologyDescriptor>::assignComponentTags(
    TopologyRefiner &refiner, TopologyDescriptor const &desc) {

  if ((desc.numCreases > 0) && desc.creaseVertexIndexPairs &&
      desc.creaseWeights) {
    ...
    for (int edge = 0; edge < desc.numCreases; ++edge, vertIndexPairs += 2) {
      ...
      if (idx != INDEX_INVALID) {
        ...
        char msg[1024];
        snprintf(msg, 1024,
                 "Edge %d specified to be sharp does not exist (%d, %d)", edge,
                 vertIndexPairs[0], vertIndexPairs[1]);
        reportInvalidTopology(
            Vtr::internal::Level::TOPOLOGY_INVALID_CREASE_EDGE, msg, desc);
      }
    }
  }
  ...
  if ((desc.numCorners > 0) && desc.cornerVertexIndices && desc.cornerWeights) {
    ...
    for (int vert = 0; vert < desc.numCorners; ++vert) {
      ...
      if (idx >= 0 && idx < getNumBaseVertices(refiner)) {
        ...
        char msg[1024];
        snprintf(msg, 1024, "Vertex %d specified to be sharp does not exist",
                 idx);
        reportInvalidTopology(
            Vtr::internal::Level::TOPOLOGY_INVALID_CREASE_VERT, msg, desc);
      }
    }
  }
  if (desc.numHoles > 0) {
...
template <>
bool TopologyRefinerFactory<TopologyDescriptor>::assignFaceVaryingTopology(
    TopologyRefiner &refiner, TopologyDescriptor const &desc) {
...
template <>
void TopologyRefinerFactory<TopologyDescriptor>::reportInvalidTopology(
    TopologyError /* errCode */, char const *msg,
    TopologyDescriptor const & /* mesh */) {
  Warning(msg);
}
