...
public:
  ...
  static void ComputeInteriorPointWeights(int valence, int faceInRing,
                                          Weight *pWeights, Weight *epWeights,
                                          Weight *emWeights);
...
inline double CatmarkLimits<REAL>::computeCoefficient(int valence) {
...
template <typename REAL>
void CatmarkLimits<REAL>::ComputeInteriorPointWeights(int valence,
                                                      int faceInRing,
                                                      Weight *pWeights,
                                                      Weight *epWeights,
                                                      Weight *emWeights) {
...
template <typename REAL>
void CatmarkLimits<REAL>::ComputeBoundaryPointWeights(int valence,
                                                      int faceInRing,
                                                      Weight *pWeights,
                                                      Weight *epWeights,
                                                      Weight *emWeights) {
...
//  Gregory patch.
//
//  Historically the source topology was specified as a Vtr::Level and face
//  index, from which contributions of all 1-ring vertices that support the 20
//  points of the patch are determined.  The source topology is now specified
//  via a simple SourcePatch, so a matrix can be determined for a particular
//  configuration and re-used for any similar instance.
//
...
//
template <typename REAL>
GregoryConverter<REAL>::GregoryConverter(SourcePatch const &sourcePatch) {
...
template <typename REAL>
void GregoryConverter<REAL>::Initialize(SourcePatch const &sourcePatch) {
  ...
  for (int cIndex = 0; cIndex < 4; ++cIndex) {
    ...
    corner.isRegular =
        ((corner.numFaces << corner.isBoundary) == 4) && !corner.isSharp;
    if (corner.isRegular) {
...
template <typename REAL>
void GregoryConverter<REAL>::Convert(Matrix &matrix) const {
...
template <typename REAL>
void GregoryConverter<REAL>::resizeMatrixIsolatedIrregular(
...
template <typename REAL>
void GregoryConverter<REAL>::resizeMatrixUnisolated(Matrix &matrix) const {
...
template <typename REAL>
void GregoryConverter<REAL>::assignRegularEdgePoints(int cIndex,
                                                     Matrix &matrix) const {
...
template <typename REAL>
void GregoryConverter<REAL>::computeIrregularEdgePoints(
...
template <typename REAL>
void GregoryConverter<REAL>::computeIrregularInteriorEdgePoints(
  ...
  CatmarkLimits<REAL>::ComputeInteriorPointWeights(
      valence, corner.faceInRing, pWeights, epWeights, emWeights);
...
template <typename REAL>
void GregoryConverter<REAL>::computeIrregularBoundaryEdgePoints(
    int cIndex, Point &p, Point &ep, Point &em, Weight *ringWeights) const {
...
template <typename REAL>
int GregoryConverter<REAL>::getIrregularFacePointSize(int cIndexNear,
                                                      int cIndexFar) const {
...
template <typename REAL>
void GregoryConverter<REAL>::computeIrregularFacePoint(
  ...
  rowWeights[cornerNear.ringPoints[2 * iEdgePrev + 1]] +=
      -signForSideOfEdge / 18.0f;
  rowWeights[cornerNear.ringPoints[2 * iEdgeInterior + 1]] +=
      signForSideOfEdge / 18.0f;
  ...
  //  Complete the expected row size when val-2 interior corners induce
  //  duplicates:
...
template <typename REAL>
void GregoryConverter<REAL>::assignRegularFacePoints(int cIndex,
                                                     Matrix &matrix) const {
...
template <typename REAL>
void GregoryConverter<REAL>::computeIrregularFacePoints(int cIndex,
                                                        Matrix &matrix,
                                                        Weight *rowWeights,
...
template <typename REAL>
void BSplineConverter<REAL>::Initialize(SourcePatch const &sourcePatch) {
...
template <typename REAL>
void BSplineConverter<REAL>::Convert(Matrix &matrix) const {
...
template <typename REAL>
void BSplineConverter<REAL>::convertFromGregory(Matrix const &G,
                                                Matrix &B) const {
  ...
  //  points:
  //
  static REAL const wCorner[9] = {49.f,  -42.f, -42.f, 36.f, -14.f,
                                  -14.f, 12.f,  12.f,  4.f};
  ...
  //          {  0,  1,  2,  3 }         {  0,  1,  7,  5 }
  //
  //  With four symmetric quadrants the dependencies of the BSpline points on
  //  the Gregory/Bezier points are as follows -- using the "p", "ep", "em" and
  //  "f" naming from the Gregory points:
  //
  ...
  static int const fIndices[4][4] = {
      {3, 8, 18, 13}, {8, 13, 3, 18}, {13, 18, 8, 3}, {18, 3, 13, 8}};
...
void BSplineConverter<REAL>::buildIrregularCornerMatrix(
    int irregularCornerValence, int numSourcePoints,
    int const rowsForXPoints[7], Matrix &matrix) const {
...
template <typename REAL>
void BSplineConverter<REAL>::convertIrregularCorner(int irregularCorner,
                                                    Matrix &matrix) const {
  //
  ...
  //
  //  Compute limit points P, Ep and Em in terms of weights of the 1-ring for
  //  the corner and identify the indices of relevant points within the ring:
  //
  ...
  assert(valence > 2);
  CatmarkLimits<REAL>::ComputeInteriorPointWeights(valence, faceInRing, wP, wEp,
                                                   wEm);
  ...
  for (int i = 0; i < ringSizePlusCorner; ++i) {
    ...
    wX0[i] =
        wP[i] * 36.0f - wX0[i] - (wX2[i] + wX1[i]) * 4.0f - (wX3[i] + wX4[i]);
  }
  ...
  for (int i = 8; i < 16; ++i, ++pNext) {
    pPoints[i] =
        (pNext < numSourcePoints) ? pNext : (pNext - numSourcePoints + 4);
  }
...
//  not much more to it than a single conversion method -- it follows the
...
template <typename REAL>
void LinearConverter<REAL>::Initialize(SourcePatch const &sourcePatch) {

  _sourcePatch = &sourcePatch;
}

template <typename REAL>
void LinearConverter<REAL>::Convert(Matrix &matrix) const {

  StackBuffer<Index, 64, true> indexBuffer(1 + _sourcePatch->GetMaxRingSize());
  StackBuffer<Weight, 64, true> weightBuffer(1 +
                                             _sourcePatch->GetMaxRingSize());
  ...
  for (int cIndex = 0; cIndex < 4; ++cIndex) {
    ...
    if (sourceCorner._boundary) {
      CatmarkLimits<REAL>::ComputeBoundaryPointWeights(
          1 + sourceCorner._numFaces, sourceCorner._patchFace, &weightBuffer[0],
          0, 0);
...
template <typename REAL>
int CatmarkPatchBuilder::convertSourcePatch(SourcePatch const &sourcePatch,
...
int CatmarkPatchBuilder::convertToPatchType(SourcePatch const &sourcePatch,
                                            PatchDescriptor::Type patchType,
                                            SparseMatrix<float> &matrix) const {
  return convertSourcePatch(sourcePatch, patchType, matrix);
}
int CatmarkPatchBuilder::convertToPatchType(
    SourcePatch const &sourcePatch, PatchDescriptor::Type patchType,
    SparseMatrix<double> &matrix) const {
  return convertSourcePatch(sourcePatch, patchType, matrix);
}

CatmarkPatchBuilder::CatmarkPatchBuilder(TopologyRefiner const &refiner,
