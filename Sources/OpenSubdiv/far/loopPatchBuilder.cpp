...
void LoopLimits<REAL>::ComputeInteriorPointWeights(int valence, int faceInRing,
                                                   REAL *pWeights,
                                                   REAL *epWeights,
                                                   REAL *emWeights) {
...
template <typename REAL>
void LoopLimits<REAL>::ComputeBoundaryPointWeights(int valence, int faceInRing,
                                                   REAL *pWeights,
                                                   REAL *epWeights,
                                                   REAL *emWeights) {
...
#endif
template <typename REAL> inline bool _isZero(REAL w) {
  return (w == (REAL)0.0);
}
#ifdef __INTEL_COMPILER
#pragma warning(pop)
...
template <typename REAL>
void _initializeFullMatrix(SparseMatrix<REAL> &M, int nRows, int nColumns) {
...
template <typename REAL>
void _resizeMatrix(SparseMatrix<REAL> &matrix, int numRows, int numColumns,
...
template <typename REAL>
void _addSparsePointToFullRow(REAL *fullRow, SparseMatrixRow<REAL> const &p,
...
template <typename REAL>
void _combineSparsePointsInFullRow(SparseMatrixRow<REAL> &p, REAL aCoeff,
                                   SparseMatrixRow<REAL> const &a, REAL bCoeff,
                                   SparseMatrixRow<REAL> const &b, int rowSize,
                                   REAL *rowBuffer, int *maskBuffer) {
...
template <typename REAL>
void _addSparseRowToFull(REAL *fullRow, SparseMatrix<REAL> const &M,
                         int sparseRow, REAL s) {
...
template <typename REAL>
void _combineSparseMatrixRowsInFull(SparseMatrix<REAL> &dstMatrix,
                                    int dstRowIndex,
                                    SparseMatrix<REAL> const &srcMatrix,
                                    int numSrcRows, int const srcRowIndices[],
                                    REAL const *srcRowWeights) {
  ...
  for (int i = 0; i < numSrcRows; ++i) {
    if (!_isZero(srcRowWeights[i])) {
      _addSparseRowToFull(dstRow, srcMatrix, srcRowIndices[i],
                          srcRowWeights[i]);
    }
  }
}
...
template <typename REAL>
void _matrixPrintDensity(const char *prefix, SparseMatrix<REAL> const &M) {
  ...
  printf("%s(%dx%d = %d):  elements = %d, non-zero = %d, density = %.1f\n",
         prefix, M.GetNumRows(), M.GetNumColumns(), fullSize, sparseSize,
         nonZeroSize, (REAL)nonZeroSize * 100.0f / (REAL)fullSize);
}
...
//  vertices in a Loop mesh to the 18 control points of a quartic Gregory
//  triangle.
//
...
private:
  ...
  void computeIrregularInteriorEdgePoints(int cornerIndex, Point &P, Point &Ep,
                                          Point &Em,
                                          Weight *weightBuffer) const;
  void computeIrregularBoundaryEdgePoints(int cornerIndex, Point &P, Point &Ep,
                                          Point &Em,
                                          Weight *weightBuffer) const;

  int getIrregularFacePointSize(int cornerNear, int cornerFar) const;
  void computeIrregularFacePoint(int cornerNear, int edgeInNearRing,
                                 int cornerFar, Point const &p,
                                 Point const &eNear, Point const &eFar,
                                 Point &fNear,
                                 REAL signForSideOfEdge /* -1.0 or 1.0 */,
  ...
  void promoteCubicEdgePointsToQuartic(Matrix &matrix, Weight *rowWeights,
                                       int *columnMask) const;
...
//
template <typename REAL>
GregoryTriConverter<REAL>::GregoryTriConverter(SourcePatch const &sourcePatch) {
...
template <typename REAL>
void GregoryTriConverter<REAL>::Initialize(SourcePatch const &sourcePatch) {
  ...
  for (int cIndex = 0; cIndex < 3; ++cIndex) {
    ...
    corner.isRegular =
        ((corner.numFaces << corner.isBoundary) == 6) && !corner.isSharp;
    if (corner.isRegular) {
      corner.faceAngle = (REAL)(M_PI / 3.0);
      corner.cosFaceAngle = 0.5f;
    } else {
      corner.faceAngle = (corner.isBoundary ? REAL(M_PI) : REAL(2.0 * M_PI)) /
...
template <typename REAL>
void GregoryTriConverter<REAL>::Convert(Matrix &matrix) const {
...
template <typename REAL>
void GregoryTriConverter<REAL>::resizeMatrixIsolatedIrregular(
...
template <typename REAL>
void GregoryTriConverter<REAL>::resizeMatrixUnisolated(Matrix &matrix) const {
  ...
  for (int cIndex = 0; cIndex < 3; ++cIndex) {
    ...
    if (!corner.fpIsRegular || !corner.fmIsRegular) {
      ...
      if (!corner.fpIsRegular) {
        rowSize[3] = getIrregularFacePointSize(
            cIndex, corner.fpIsCopied ? cPrev : cNext);
      }
      if (!corner.fmIsRegular) {
        rowSize[4] = getIrregularFacePointSize(
            cIndex, corner.fmIsCopied ? cNext : cPrev);
      }
    }
...
template <typename REAL>
void GregoryTriConverter<REAL>::assignRegularEdgePoints(int cIndex,
                                                        Matrix &matrix) const {
...
template <typename REAL>
void GregoryTriConverter<REAL>::computeIrregularEdgePoints(
    int cIndex, Matrix &matrix, Weight *weightBuffer) const {
...
template <typename REAL>
void GregoryTriConverter<REAL>::computeIrregularInteriorEdgePoints(
    int cIndex, Point &p, Point &ep, Point &em, Weight *ringWeights) const {
...
template <typename REAL>
void GregoryTriConverter<REAL>::computeIrregularBoundaryEdgePoints(
    int cIndex, Point &p, Point &ep, Point &em, Weight *ringWeights) const {
...
template <typename REAL>
int GregoryTriConverter<REAL>::getIrregularFacePointSize(int cIndexNear,
                                                         int cIndexFar) const {
...
template <typename REAL>
void GregoryTriConverter<REAL>::computeIrregularFacePoint(
    int cIndexNear, int edgeInNearCornerRing, int cIndexFar, Point const &p,
    Point const &eNear, Point const &eFar, Point &fNear, REAL signForSideOfEdge,
    Weight *rowWeights, int *columnMask) const {
...
template <typename REAL>
void GregoryTriConverter<REAL>::assignRegularFacePoints(int cIndex,
                                                        Matrix &matrix) const {
...
template <typename REAL>
void GregoryTriConverter<REAL>::computeIrregularFacePoints(
    int cIndex, Matrix &matrix, Weight *rowWeights, int *columnMask) const {
  ...
  if (!corner.fpIsRegular && !corner.fpIsCopied) {
    ...
    computeIrregularFacePoint(cIndex, iEdgeP, cNext, p, ep, emNext, fp, 1.0,
                              rowWeights, columnMask);
  }
  if (!corner.fmIsRegular && !corner.fmIsCopied) {
    int iEdgeM = (corner.faceInRing + 1) % corner.valence;
    computeIrregularFacePoint(cIndex, iEdgeM, cPrev, p, em, epPrev, fm, -1.0,
                              rowWeights, columnMask);
  }
...
template <typename REAL>
void GregoryTriConverter<REAL>::assignRegularMidEdgePoint(
    int edgeIndex, Matrix &matrix) const {
  ...
  if (corner.epOnBoundary) {
    ...
    int oppositeInRing = corner.isBoundary ? (corner.faceInRing - 1)
                                           : ((corner.faceInRing + 5) % 6);
...
template <typename REAL>
void GregoryTriConverter<REAL>::computeIrregularMidEdgePoint(
    int edgeIndex, Matrix &matrix, Weight *rowWeights, int *columnMask) const {
  //
...
template <typename REAL>
void GregoryTriConverter<REAL>::promoteCubicEdgePointsToQuartic(
...
namespace {
template <typename REAL>
void _printPoint(SparseMatrixRow<REAL> const &p, bool printIndices = true,
...
template <typename REAL>
void _printMatrix(SparseMatrix<REAL> const &matrix, bool printIndices = true,
                  bool printWeights = true) {

  printf("Matrix %d x %d, %d elements:\n", matrix.GetNumRows(),
         matrix.GetNumColumns(), matrix.GetNumElements());
...
void _printSourcePatch(SourcePatch const &patch, bool printCornerInfo = true,
                       bool printRingPoints = true) {

  printf("SoucePatch - %d corners, %d points:\n", patch._numCorners,
         patch._numSourcePoints);

  if (printCornerInfo) {
    printf("  Corner info:\n");
    for (int i = 0; i < patch._numCorners; ++i) {
      printf("%6d:  boundary = %d, sharp = %d, numFaces = %d, in-ring = %d, "
             "ringSize = %d\n",
             i, patch._corners[i]._boundary, patch._corners[i]._sharp,
             patch._corners[i]._numFaces, patch._corners[i]._patchFace,
             patch._ringSizes[i]);
    }
  }
  if (printRingPoints) {
...
//
template <typename REAL>
void convertToLinear(SourcePatch const &sourcePatch,
  ...
  for (int cIndex = 0; cIndex < 3; ++cIndex) {
    ...
    if (sourceCorner._sharp) {
      ...
      LoopLimits<REAL>::ComputeBoundaryPointWeights(1 + sourceCorner._numFaces,
                                                    sourceCorner._patchFace,
                                                    &weightBuffer[0], 0, 0);
      ...
      LoopLimits<REAL>::ComputeInteriorPointWeights(sourceCorner._numFaces,
                                                    sourceCorner._patchFace,
                                                    &weightBuffer[0], 0, 0);

      std::memcpy(&rowIndices[0], indexBuffer,
                  rowIndices.size() * sizeof(Index));
      std::memcpy(&rowWeights[0], weightBuffer,
                  rowWeights.size() * sizeof(Weight));
    }
...
template <typename REAL>
void convertToGregory(SourcePatch const &sourcePatch,
                      SparseMatrix<REAL> &matrix) {
...
template <typename REAL>
void convertToLoop(SourcePatch const &sourcePatch, SparseMatrix<REAL> &matrix) {

  //
  //  Unlike quads, there are not enough degrees of freedom in the regular patch
  //  to enforce interpolation of the limit point and tangent at the corner
  //  while preserving the two adjoining boundary curves.  Since we end up
  //  destroying neighboring conintuity in doing so, we use a fully constructed
  //  Gregory patch here for the isolated corner case as well as the general
  //  case.
  //
  ...
  REAL const gregoryToLoopMatrix[12][15] = {
      {8.214411f, 7.571190f, -7.690082f, 2.237840f, -1.118922f, -16.428828f,
       0.666666f, 0.666666f, 2.237835f, 6.309870f, 0.666666f, -1.690100f,
       -0.428812f, -0.428805f, 0.214407f},
      {-0.304687f, 0.609374f, 6.752593f, 0.609374f, -0.304687f, 0.609378f,
       -3.333333f, -3.333333f, 0.609378f, -1.247389f, -3.333333f, -1.247389f,
       3.276037f, 3.276037f, -1.638020f},
      {-1.118922f, 2.237840f, -7.690082f, 7.571190f, 8.214411f, 2.237835f,
       0.666666f, 0.666666f, -16.428828f, -1.690100f, 0.666666f, 6.309870f,
       -0.428805f, -0.428812f, 0.214407f},
      {8.214411f, -16.428828f, 6.309870f, -0.428812f, 0.214407f, 7.571190f,
       0.666666f, 0.666666f, -0.428805f, -7.690082f, 0.666666f, -1.690100f,
       2.237840f, 2.237835f, -1.118922f},
      {-0.813368f, 1.626735f, -0.773435f, -1.039929f, 0.519965f, 1.626735f,
       0.666666f, 0.666666f, -1.039930f, -0.773435f, 0.666666f, 1.226558f,
       -1.039929f, -1.039930f, 0.519965f},
      {0.519965f, -1.039929f, -0.773435f, 1.626735f, -0.813368f, -1.039930f,
       0.666666f, 0.666666f, 1.626735f, 1.226558f, 0.666666f, -0.773435f,
       -1.039930f, -1.039929f, 0.519965f},
      {0.214407f, -0.428812f, 6.309870f, -16.428828f, 8.214411f, -0.428805f,
       0.666666f, 0.666666f, 7.571190f, -1.690100f, 0.666666f, -7.690082f,
       2.237835f, 2.237840f, -1.118922f},
      {-0.304687f, 0.609378f, -1.247389f, 3.276037f, -1.638020f, 0.609374f,
       -3.333333f, -3.333333f, 3.276037f, 6.752593f, -3.333333f, -1.247389f,
       0.609374f, 0.609378f, -0.304687f},
      {0.519965f, -1.039930f, 1.226558f, -1.039930f, 0.519965f, -1.039929f,
       0.666666f, 0.666666f, -1.039929f, -0.773435f, 0.666666f, -0.773435f,
       1.626735f, 1.626735f, -0.813368f},
      {-1.638020f, 3.276037f, -1.247389f, 0.609378f, -0.304687f, 3.276037f,
       -3.333333f, -3.333333f, 0.609374f, -1.247389f, -3.333333f, 6.752593f,
       0.609378f, 0.609374f, -0.304687f},
      {-1.118922f, 2.237835f, -1.690100f, -0.428805f, 0.214407f, 2.237840f,
       0.666666f, 0.666666f, -0.428812f, -7.690082f, 0.666666f, 6.309870f,
       7.571190f, -16.428828f, 8.214411f},
      {0.214407f, -0.428805f, -1.690100f, 2.237835f, -1.118922f, -0.428812f,
       0.666666f, 0.666666f, 2.237840f, 6.309870f, 0.666666f, -7.690082f,
       -16.428828f, 7.571190f, 8.214411f}};
  int const gRowIndices[15] = {0, 1,  15, 7,  5,  2,  4, 8,
                               6, 17, 14, 16, 11, 12, 10};
...
LoopPatchBuilder::LoopPatchBuilder(TopologyRefiner const &refiner,
...
template <typename REAL>
int LoopPatchBuilder::convertSourcePatch(SourcePatch const &sourcePatch,
...
int LoopPatchBuilder::convertToPatchType(SourcePatch const &sourcePatch,
                                         PatchDescriptor::Type patchType,
