...
//
Crease::Rule Crease::DetermineVertexVertexRule(float vertexSharpness,
                                               int sharpEdgeCount) const {

  if (IsSharp(vertexSharpness))
    return Crease::RULE_CORNER;

  return (sharpEdgeCount > 2) ? Crease::RULE_CORNER
                              : (Crease::Rule)(1 << sharpEdgeCount);
}

Crease::Rule
Crease::DetermineVertexVertexRule(float vertexSharpness, int incidentEdgeCount,
...
float Crease::ComputeFractionalWeightAtVertex(
    float parentVertexSharpness, float childVertexSharpness,
    int incidentEdgeCount, float const *parentSharpness,
...
//
float Crease::SubdivideEdgeSharpnessAtVertex(
    float edgeSharpness, int incEdgeCountAtVertex,
  ...
  if (sharpCount > 1) {
    ...
    float avgSharpnessAtVertex =
        (sharpSum - edgeSharpness) / (float)(sharpCount - 1);
...
void Crease::SubdivideEdgeSharpnessesAroundVertex(int edgeCount,
  ...
  //
  //  Chaikin creasing is most efficiently computed for all edges around a
  //  vertex at once as the subdivided value for each creased edge depends on
  //  the average of the other edges around the vertex.  So we can sum up the
  //  sharpness around the vertex once and use that for each edge, rather than
  //  iterating around the vertex for each incident edge.
  //
