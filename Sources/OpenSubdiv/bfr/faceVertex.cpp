...
void FaceVertex::finalizeOrderedTags() {
  ...
  if (_vDesc.HasEdgeSharpness()) {
    ...
    if (isBoundary) {
      ...
      _tag._boundaryNonSharp = !Sdc::Crease::IsInfinite(sharpness[0]) ||
...
bool FaceVertex::HasImplicitVertexSharpness() const {
...
int FaceVertex::findFVarSubsetExtent(Subset const &vtxSub,
...
int FaceVertex::FindFaceVaryingSubset(Subset *fvarSubsetPtr,
...
void FaceVertex::adjustSubsetTags(Subset *subset,
...
//
int FaceVertex::createUnOrderedEdges(Edge edges[], short feEdges[],
...
void FaceVertex::markDuplicateEdges(Edge edges[], short const feEdges[],
                                    Index const fvIndices[]) const {
...
void FaceVertex::assignUnOrderedFaceNeighbors(Edge const edges[],
                                              short const feEdges[]) {
