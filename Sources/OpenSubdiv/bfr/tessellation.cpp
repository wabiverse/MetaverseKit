...
inline int appendUIsoLine(Coord2Array<REAL> P, int n, REAL u, REAL v, REAL dv) {
...
template <typename REAL>
inline int appendVIsoLine(Coord2Array<REAL> P, int n, REAL u, REAL v, REAL du) {
...
template <typename REAL>
inline int appendUVLine(Coord2Array<REAL> P, int n, REAL u, REAL v, REAL du,
                        REAL dv) {
...
inline int appendQuad(FacetArray facets, int q0, int q1, int q2, int q3,
...
inline int appendTriFan(FacetArray facets, int size, int startIndex) {
  ...
  for (int i = 1; i <= size; ++i) {
    facets[i - 1].Set(startIndex + (i - 1), startIndex + ((i < size) ? i : 0),
                      startIndex + size);
  }
  return size;
}
...
int FacetStrip::connectUniformQuads(FacetArray facets) const {
  ...
  if (splitFirstFace) {
    ...
    nFacets += appendQuad(facets + nFacets, out0, out0 + 1, in0, outerPrev,
                          quadTriangulate);
  }
  ...
  if (splitLastFace) {
    ...
    nFacets += appendQuad(facets + nFacets, outI, outN, outN + 1, inN,
                          -(int)quadTriangulate);
  }
  return nFacets;
}
...
public:
  ...
  template <typename REAL>
  static int GetEdgeCoords(int edge, int edgeRes, Coord2Array<REAL> coords);
  template <typename REAL>
  static int GetBoundaryCoords(int const edgeRates[], Coord2Array<REAL> coords);
  template <typename REAL>
  static int GetInteriorCoords(int const uvRes[2], Coord2Array<REAL> coords);
...
public:
  ...
  static int GetBoundaryCoords(int const edgeRates[], Coord2Array<REAL> coords);
  template <typename REAL>
  static int GetInteriorCoords(int edgeRes, Coord2Array<REAL> coords);
...
private:
  ...
  template <typename REAL> static int getCenterCoord(Coord2Array<REAL> coords);
  template <typename REAL>
  static int getInteriorRingCoords(int edgeRes, REAL uStart, REAL vStart,
                                   REAL tDelta, Coord2Array<REAL> coords);
...
private:
  ...
  static int getRingEdgeCoords(Parameterization P, int edge, int edgeRes,
                               bool incFirst, bool incLast, REAL tStart,
                               REAL tDelta, Coord2Array<REAL> coords);
  template <typename REAL>
  static int getInteriorRingCoords(Parameterization P, int edgeRes, REAL tStart,
                                   REAL tDelta, Coord2Array<REAL> coords);
...
//
inline int quad::CountUniformFacets(int edgeRes, bool triangulate) {
...
int quad::CountNonUniformFacets(int const outerRes[], int const innerRes[],
                                bool triangulate) {
...
inline int quad::countUniformCoords(int edgeRes) {
  return (edgeRes + 1) * (edgeRes + 1);
}

inline int quad::CountInteriorCoords(int edgeRes) {
  return (edgeRes - 1) * (edgeRes - 1);
}

inline int quad::CountInteriorCoords(int const uvRes[]) {
  return (uvRes[0] - 1) * (uvRes[1] - 1);
}

template <typename REAL>
inline int quad::getCenterCoord(Coord2Array<REAL> coords) {
...
template <typename REAL>
int quad::GetEdgeCoords(int edge, int edgeRes, Coord2Array<REAL> coords) {
...
template <typename REAL>
int quad::GetBoundaryCoords(int const edgeRates[], Coord2Array<REAL> coords) {
...
template <typename REAL>
int quad::getInteriorRingCoords(int uRes, int vRes, REAL u0, REAL v0, REAL du,
                                REAL dv, Coord2Array<REAL> coords) {
...
template <typename REAL>
int quad::GetInteriorCoords(int const uvRes[2], Coord2Array<REAL> coords) {
  ...
  for (int i = 0; i < nIntRings; ++i, uRes -= 2, vRes -= 2, u += du, v += dv) {
    nCoords +=
        getInteriorRingCoords(uRes, vRes, u, v, du, dv, coords + nCoords);
  }
  return nCoords;
}

int quad::getSingleStripFacets(int uRes, int vRes, int coord0, bool triangulate,
                               FacetArray facets) {
...
int quad::getInteriorRingFacets(int uRes, int vRes, int coord0,
                                bool triangulate, FacetArray facets) {
...
int quad::getBoundaryRingFacets(int const outerRes[], int uRes, int vRes,
                                int nBoundaryEdges, bool triangulate,
                                FacetArray facets) {
...
int quad::GetSegmentedFacets(int const innerRes[], bool triangulate,
                             FacetArray facets) {
...
int quad::GetNonUniformFacets(int const outerRes[], int const innerRes[],
                              int nBoundaryEdges, bool triangulate,
                              FacetArray facets) {
...
int quad::GetUniformFacets(int res, bool triangulate, FacetArray facets) {
  ...
  for (int ring = 0; ring < nRings; ++ring, res -= 2) {
    nFacets +=
        getInteriorRingFacets(res, res, coord0, triangulate, facets + nFacets);
    coord0 += 4 * res;
  }
  return nFacets;
}
...
int tri::CountNonUniformFacets(int const outerRes[], int innerRes) {
  ...
  //
  int nBoundary = (nInnerEdges + outerRes[0]) + (nInnerEdges + outerRes[1]) +
                  (nInnerEdges + outerRes[2]);

  return nInterior + nBoundary;
}

inline int tri::countUniformCoords(int edgeRes) {
  return edgeRes * (edgeRes + 1) / 2;
}

inline int tri::CountInteriorCoords(int edgeRes) {
  return countUniformCoords(edgeRes - 2);
}

template <typename REAL>
inline int tri::getCenterCoord(Coord2Array<REAL> coords) {
...
template <typename REAL>
int tri::GetEdgeCoords(int edge, int edgeRes, Coord2Array<REAL> coords) {
...
template <typename REAL>
int tri::GetBoundaryCoords(int const edgeRates[], Coord2Array<REAL> coords) {

  int nCoords = 0;
  nCoords += appendVIsoLine<REAL>(coords + nCoords, edgeRates[0], 0.0f, 0.0f,
                                  1.0f / (REAL)edgeRates[0]);
  nCoords +=
      appendUVLine<REAL>(coords + nCoords, edgeRates[1], 1.0f, 0.0f,
                         -1.0f / (REAL)edgeRates[1], 1.0f / (REAL)edgeRates[1]);
...
template <typename REAL>
int tri::getInteriorRingCoords(int edgeRes, REAL u0, REAL v0, REAL dt,
                               Coord2Array<REAL> coords) {
  assert(edgeRes);
...
template <typename REAL>
int tri::GetInteriorCoords(int edgeRes, Coord2Array<REAL> coords) {
...
int tri::getBoundaryRingFacets(int const outerRes[], int innerRes,
                               int nBoundaryEdges, FacetArray facets) {
...
int tri::GetNonUniformFacets(int const outerRes[], int innerRes,
                             int nBoundaryEdges, FacetArray facets) {
  ...
  //  First, generate the ring of boundary facets separately:
  int nFacets =
      getBoundaryRingFacets(outerRes, innerRes, nBoundaryEdges, facets);
...
int qsub::CountNonUniformFacets(int N, int const outerRes[], int innerRes,
                                bool triangulate) {
...
template <typename REAL>
inline int qsub::getCenterCoord(Coord2Array<REAL> coords) {
...
int qsub::getRingEdgeCoords(Parameterization P, int edge, int edgeRes,
                            bool incFirst, bool incLast, REAL tOrigin, REAL dt,
                            Coord2Array<REAL> coords) {
...
template <typename REAL>
int qsub::GetEdgeCoords(Parameterization P, int edge, int edgeRes,
                        Coord2Array<REAL> coords) {

  return getRingEdgeCoords<REAL>(P, edge, edgeRes, false, false, 0.0f,
                                 1.0f / (REAL)edgeRes, coords);
}

template <typename REAL>
int qsub::GetBoundaryCoords(Parameterization P, int const edgeRates[],
                            Coord2Array<REAL> coords) {
  ...
  for (int i = 0; i < N; ++i) {
    nCoords +=
        getRingEdgeCoords<REAL>(P, i, edgeRates[i], true, false, 0.0f,
                                1.0f / (REAL)edgeRates[i], coords + nCoords);
  }
  return nCoords;
}

template <typename REAL>
int qsub::getInteriorRingCoords(Parameterization P, int edgeRes, REAL tOrigin,
                                REAL dt, Coord2Array<REAL> coords) {
  assert(edgeRes > 1);
  ...
  int nCoords = 0;
  for (int i = 0; i < N; ++i) {
    nCoords += getRingEdgeCoords(P, i, edgeRes, true, false, tOrigin, dt,
                                 coords + nCoords);
  }
  return nCoords;
}
...
template <typename REAL>
int qsub::getCenterRingCoords(Parameterization P, REAL tOrigin,
                              Coord2Array<REAL> coords) {
...
template <typename REAL>
int qsub::GetInteriorCoords(Parameterization P, int edgeRes,
                            Coord2Array<REAL> coords) {
...
int qsub::getCenterFacets(int N, int coord0, FacetArray facets) {

  return (N == 3) ? appendTri(facets, coord0, coord0 + 1, coord0 + 2)
                  : appendTriFan(facets, N, coord0);
}

int qsub::getInteriorRingFacets(int N, int edgeRes, int coord0,
                                bool triangulate, FacetArray facets) {
...
int qsub::getBoundaryRingFacets(int N, int const outerRes[], int innerRes,
                                int nBoundaryEdges, bool triangulate,
                                FacetArray facets) {
...
int qsub::GetUniformFacets(int N, int edgeRes, bool triangulate,
                           FacetArray facets) {
...
int qsub::GetNonUniformFacets(int N, int const outerRes[], int innerRes,
                              int nBoundaryEdges, bool triangulate,
                              FacetArray facets) {
...
bool Tessellation::validateArguments(Parameterization const &p, int numRates,
                                     int const rates[],
...
void Tessellation::initialize(Parameterization const &p, int numRates,
                              int const rates[], Options const &options) {
  ...
  _facetStride = options.GetFacetStride() ? options.GetFacetStride()
                                          : options.GetFacetSize();
...
void Tessellation::initializeInventoryForParamQuad(int sumOfEdgeRates) {
  ...
  if (_isUniform) {
    ...
    if ((inner[0] > 1) && (inner[1] > 1)) {
      ...
      _numFacets =
          quad::CountNonUniformFacets(_outerRates, _innerRates, _triangulate);
...
void Tessellation::initializeInventoryForParamQPoly(int sumOfEdgeRates) {
  ...
  if (_isUniform) {
    ...
    if (res > 1) {
      _numInteriorPoints = qsub::CountInteriorCoords(N, res);
      _numFacets =
          qsub::CountNonUniformFacets(N, _outerRates, res, _triangulate);
    } else {
      _numInteriorPoints = 1;
      _numFacets = sumOfEdgeRates;
      _triangleFan = true;
...
//
Tessellation::Tessellation(Parameterization const &p, int uniformRate,
                           Options const &options) {

  initialize(p, 1, &uniformRate, options);
}

Tessellation::Tessellation(Parameterization const &p, int numRates,
                           int const rates[], Options const &options) {
...
//
template <typename REAL>
int Tessellation::GetEdgeCoords(int edge, REAL coordBuffer[]) const {
...
template <typename REAL>
int Tessellation::GetBoundaryCoords(REAL coordBuffer[]) const {
...
template <typename REAL>
int Tessellation::GetInteriorCoords(REAL coordBuffer[]) const {
...
int Tessellation::GetFacets(int facetIndices[]) const {
  ...
  case Parameterization::QUAD:
    if (_isUniform) {
      ...
      nFacets = quad::GetNonUniformFacets(
          _outerRates, _innerRates, _numBoundaryPoints, _triangulate, facets);
    }
    break;
  case Parameterization::TRI:
    if (_isUniform) {
  ...
  case Parameterization::QUAD_SUBFACES:
    if (_isUniform) {
      ...
      nFacets =
          qsub::GetNonUniformFacets(N, _outerRates, _innerRates[0],
...
void Tessellation::TransformFacetCoordIndices(int facetIndices[],
                                              int commonOffset) {
...
void Tessellation::TransformFacetCoordIndices(int facetIndices[],
  ...
  for (int i = 0; i < _numFacets; ++i, facetIndices += _facetStride) {
    for (int j = 0; j < (int)_facetSize; ++j) {
      ...
      if (index >= 0) {
        index = (index < _numBoundaryPoints) ? boundaryIndices[index]
...
template int Tessellation::GetEdgeCoords<float>(int edge,
                                                float coordBuffer[]) const;
template int Tessellation::GetEdgeCoords<double>(int edge,
