...
int RegularPatchBuilder::GetBoundaryMask(int faceSize, Index const cvs[]) {
  ...
  if (isQuad) {
    int eMask = ((cvs[1] < 0) << 0) | ((cvs[7] < 0) << 1) |
                ((cvs[14] < 0) << 2) | ((cvs[8] < 0) << 3);
    return eMask;
  } else {
...
RegularPatchBuilder::RegularPatchBuilder(FaceSurface const &surface)
  ...
  if (!_isBoundary) {
    ...
    int eMask = ((C[0].IsBoundary() & (C[0]._numFacesBefore == 0)) << 0) |
                ((C[1].IsBoundary() & (C[1]._numFacesBefore == 0)) << 1) |
                ((C[2].IsBoundary() & (C[2]._numFacesBefore == 0)) << 2);
    int vMask = (C[0].IsBoundary() << 0) | (C[1].IsBoundary() << 1) |
                (C[2].IsBoundary() << 2);
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    case 0:
      ...
      if (!cSub.IsBoundary()) {
        ...
        P[3] = fvOther[2];
        fvOther =
            fvIndices + cTop.GetFaceIndexOffset(cTop.GetFaceNext(faceOther));
        P[0] = fvOther[2];
      } else {
