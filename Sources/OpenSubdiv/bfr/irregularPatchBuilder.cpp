...
IrregularPatchBuilder::IrregularPatchBuilder(
    ...
    : _surface(surfaceDescription), _options(options) {
...
IrregularPatchBuilder::getCornerIndices(int corner) const {
  ...
  return getSurfaceIndices() + _cornerHullInfo[corner].surfaceIndicesOffset;
}
...
void IrregularPatchBuilder::addMeshControlVertices(Index const fVerts[],
                                                   int fSize) {
...
int IrregularPatchBuilder::GatherControlVertexIndices(Index cvIndices[]) const {
  ...
  for (int corner = 0; corner < faceSize; ++corner) {
    ...
    if (cSub._numFacesAfter && cSub.IsBoundary()) {
      ...
      cvIndices[numIndices++] =
          cTop.GetFaceIndexTrailing(nextFace, getCornerIndices(corner));
    }
    if (cSub._numFacesBefore) {
      int nextFace = cTop.GetFaceFirst(cSub);
...
int IrregularPatchBuilder::gatherControlFaces(int faceSizes[],
  ...
  *faceSizes++ = faceSize;
  ...
  for (int corner = 0; corner < faceSize; ++corner) {
    ...
    if (cHull.singleSharedFace) {
      ...
      getControlFaceVertices(
          faceVerts, faceSize, corner,
...
int IrregularPatchBuilder::gatherControlVertexSharpness(
  ...
  for (int i = 0; i < _surface.GetFaceSize(); ++i) {
    ...
    if (cSub._tag.IsInfSharp()) {
      ...
      vertSharpness[nSharpVerts] =
          (cSub._localSharpness > 0.0f)
...
int IrregularPatchBuilder::gatherControlEdgeSharpness(
...
//
void IrregularPatchBuilder::getControlFaceVertices(
    int fVerts[], int numFVerts, int corner, Index const srcVerts[]) const {
  assert(_useControlVertMap);
...
void IrregularPatchBuilder::getControlFaceVertices(
    int fVerts[], int numFVerts, int corner, int nextPerimeterVert) const {
...
void IrregularPatchBuilder::getControlFaceVertices(int fVerts[], int numFVerts,
                                                   int corner,
...
void IrregularPatchBuilder::removeDuplicateControlFaces(
    int faceSizes[], int faceVerts[], int *numFaces, int *numFaceVerts) const {
...
void IrregularPatchBuilder::sharpenBoundaryControlEdges(
...
internal::IrregularPatchSharedPtr IrregularPatchBuilder::Build() {
  ...
  numCorners = _surface.GetTag().HasSharpVertices()
                   ? gatherControlVertexSharpness(cornerIndices, cornerWeights)
                   : 0;

  numCreases = _surface.GetTag().HasSharpEdges()
                   ? gatherControlEdgeSharpness(creaseIndices, creaseWeights)
                   : 0;

  //  Make some adjustments when control faces may overlap:
  if (controlFacesMayOverlap()) {
    if (numFaces > 2) {
      removeDuplicateControlFaces(faceSizes, faceVerts, &numFaces,
                                  &numFaceVerts);
    }
