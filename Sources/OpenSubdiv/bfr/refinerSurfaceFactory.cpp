...
RefinerSurfaceFactoryBase::RefinerSurfaceFactoryBase(
    ...
    : SurfaceFactory(mesh.GetSchemeType(), mesh.GetSchemeOptions(),
                     factoryOptions),
      _mesh(mesh), _numFaces(mesh.GetLevel(0).GetNumFaces()),
...
//
bool RefinerSurfaceFactoryBase::isFaceHole(Index face) const {

  return _mesh.HasHoles() && _mesh.getLevel(0).isFaceHole(face);
}

int RefinerSurfaceFactoryBase::getFaceSize(Index baseFace) const {
...
//
int RefinerSurfaceFactoryBase::getFaceVertexIndices(Index baseFace,
                                                    Index indices[]) const {
...
int RefinerSurfaceFactoryBase::getFaceFVarValueIndices(Index baseFace,
                                                       FVarID fvarID,
                                                       Index indices[]) const {
...
//
int RefinerSurfaceFactoryBase::populateFaceVertexDescriptor(
    ...
    if (vTag._semiSharpEdges || vTag._infSharpEdges) {
      if (isManifold) {
        ...
        for (int i = 0; i < nFaces; ++i) {
          ...
          vd.SetIncidentFaceEdgeSharpness(
              i, baseLevel.getEdgeSharpness(fEdges[eLeading]),
...
//
int RefinerSurfaceFactoryBase::getFaceVertexPointIndices(
    Index baseFace, int cornerVertex, Index indices[],
    int vtxOrFVarChannel) const {
...
int RefinerSurfaceFactoryBase::getFaceVertexIncidentFaceVertexIndices(
    Index baseFace, int cornerVertex, Index indices[]) const {

  return getFaceVertexPointIndices(baseFace, cornerVertex, indices, -1);
}

int RefinerSurfaceFactoryBase::getFaceVertexIncidentFaceFVarValueIndices(
    Index baseFace, int corner, FVarID fvarID, Index indices[]) const {
...
//
bool RefinerSurfaceFactoryBase::getFaceNeighborhoodVertexIndicesIfRegular(
  ...
  if (fTag._xordinary || fTag._nonManifold || fTag._incidIrregFace ||
      fTag._semiSharp || fTag._semiSharpEdges || fTag._infIrregular) {
    return false;
  }
...
bool RefinerSurfaceFactoryBase::getFaceNeighborhoodFVarValueIndicesIfRegular(
...
int gatherPatchPoints4(Level const &level, Index face, IArray const &fVerts,
                       POINT P[], int fvar) {

  static int const pointsPerCorner[4][4] = {
...
//
template <typename POINT>
int gatherPatchPoints3(Level const &level, Index face, IArray const &fVerts,
                       POINT P[], int fvar) {

  static int const pointsPerCorner[3][4] = {
      {4, 3, 0, 1}, {5, 2, 6, 9}, {8, 11, 10, 7}};
...
//
int RefinerSurfaceFactoryBase::getFacePatchPointIndices(
    Index baseFace, Index indices[], int vtxOrFVarChannel) const {
  ...
  if (baseFaceVerts.size() == 4) {
    return gatherPatchPoints4(baseLevel, baseFace, baseFaceVerts, indices,
                              vtxOrFVarChannel);
  } else {
    return gatherPatchPoints3(baseLevel, baseFace, baseFaceVerts, indices,
                              vtxOrFVarChannel);
  }
}
