...
//
void FaceSurface::preInitialize(FaceTopology const &faceTopology,
...
//
void FaceSurface::Initialize(FaceTopology const &vtxTopology,
...
void FaceSurface::Initialize(FaceSurface const &vtxSurface,
  ...
  for (int corner = 0; corner < GetFaceSize(); ++corner) {
    ...
    if (fvarSub.IsBoundary() && !fvarSub.IsSharp()) {
      sharpenBySdcFVarLinearInterpolation(&fvarSub, fvarIndices, vtxSub,
                                          vtxTop);
    }
...
bool FaceSurface::isRegular() const {
  ...
  //
  if (_combinedTag.HasSharpEdges() || _combinedTag.HasSemiSharpVertices() ||
  ...
  if (!_combinedTag.HasBoundaryVertices()) {
    ...
    if (GetRegFaceSize() == 4) {
      ...
      return (_corners[0].GetNumFaces() | _corners[1].GetNumFaces() |
              _corners[2].GetNumFaces() | _corners[3].GetNumFaces()) == 4;
    } else {
...
//
void FaceSurface::sharpenBySdcVtxBoundaryInterpolation(
    FaceVertexSubset *vtxSub, FaceVertex const &vtxTop) const {
...
//
bool hasMoreThanTwoFVarSubsets(FaceVertex const &top,
                               Index const fvarIndices[]) {
  ...
  for (int face = 0; face < numFaces; ++face) {
    ...
    numOtherEdgesDiscts += (faceNext < 0) || !top.FaceIndicesMatchAcrossEdge(
                                                 face, faceNext, fvarIndices);
...
//
bool hasDependentSharpness(FaceVertex const &topology,
                           FaceVertexSubset const &subset) {

  return ((topology.GetNumFaces() - subset.GetNumFaces()) > 1) &&
         topology.GetTag().HasSharpEdges() && !subset.GetTag().HasSharpEdges();
}
...
//
float getDependentSharpness(FaceVertex const &top,
                            FaceVertexSubset const &subset) {
...
//
void FaceSurface::sharpenBySdcFVarLinearInterpolation(
    FaceVertexSubset *fvarSub, Index const fvarIndices[],
    FaceVertexSubset const &vtxSub, FaceVertex const &vtxTop) const {
    ...
    if (!isSharp) {
      ...
      if (numOtherFaces == 0) {
        ...
        if (fvar_plus::hasDependentSharpness(vtxTop, *fvarSub)) {
          vtxTop.SharpenSubset(
              fvarSub, fvar_plus::getDependentSharpness(vtxTop, *fvarSub));
        }
      }
    }
    break;
