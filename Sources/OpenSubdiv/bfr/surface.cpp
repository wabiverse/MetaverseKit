...
template <typename REAL>
int Surface<REAL>::GetControlPointIndices(Index cvs[]) const {
...
void Surface<REAL>::GatherControlPoints(
...
template <typename REAL>
void Surface<REAL>::computeLinearPatchPoints(
    REAL pointData[], PointDescriptor const &pointDesc) const {
...
//
template <typename REAL>
void Surface<REAL>::BoundControlPoints(REAL const controlPoints[],
...
template <typename REAL>
void Surface<REAL>::BoundControlPointsFromMesh(REAL const meshPoints[],
                                               PointDescriptor const &pointDesc,
                                               REAL boundMin[],
                                               REAL boundMax[]) const {
...
namespace {
template <typename REAL>
inline int assignWeightsPerDeriv(REAL *const deriv[6], int wSize,
                                 REAL wBuffer[], REAL *wDeriv[6]) {
...
//
template <typename REAL>
void Surface<REAL>::evalRegularBasis(REAL const uv[2], REAL *wDeriv[]) const {

  Far::PatchParam patchParam;
  patchParam.Set(0, 0, 0, 0, 0, getRegPatchMask(), 0, true);

  Far::internal::EvaluatePatchBasisNormalized(
      getRegPatchType(), patchParam, uv[0], uv[1], wDeriv[0], wDeriv[1],
      wDeriv[2], wDeriv[3], wDeriv[4], wDeriv[5]);
}

template <typename REAL>
int Surface<REAL>::evalRegularStencils(REAL const uv[2], REAL *sDeriv[]) const {
  ...
  Far::internal::EvaluatePatchBasisNormalized(
      getRegPatchType(), patchParam, uv[0], uv[1], sDeriv[0], sDeriv[1],
      sDeriv[2], sDeriv[3], sDeriv[4], sDeriv[5]);

  return GetNumControlPoints();
}

template <typename REAL>
void Surface<REAL>::evalRegularDerivs(REAL const uv[2],
...
Surface<REAL>::evalIrregularBasis(REAL const UV[2], REAL *wDeriv[]) const {
  ...
  REAL uv[2] = {UV[0], UV[1]};
  int subFace =
      param.HasSubFaces() ? param.ConvertCoordToNormalizedSubFace(uv, uv) : 0;

  internal::IrregularPatchType const &irregPatch = getIrregPatch();
  int subPatchIndex = irregPatch.FindSubPatch(uv[0], uv[1], subFace);
  assert(subPatchIndex >= 0);

  irregPatch.EvalSubPatchBasis(subPatchIndex, uv[0], uv[1], wDeriv[0],
                               wDeriv[1], wDeriv[2], wDeriv[3], wDeriv[4],
                               wDeriv[5]);
...
template <typename REAL>
int Surface<REAL>::evalIrregularStencils(REAL const UV[2],
                                         REAL *sDeriv[]) const {
  ...
  return irregPatch.EvalSubPatchStencils(subPatchIndex, uv[0], uv[1], sDeriv[0],
                                         sDeriv[1], sDeriv[2], sDeriv[3],
                                         sDeriv[4], sDeriv[5]);
}

template <typename REAL>
void Surface<REAL>::evalIrregularDerivs(REAL const uv[2],
                                        REAL const patchPoints[],
                                        PointDescriptor const &pointDesc,
                                        REAL *deriv[]) const {
...
//
template <typename REAL>
inline void transformLinearQuadWeightsToStencil(REAL w[4], int N) {
...
template <typename REAL>
int Surface<REAL>::evalMultiLinearBasis(REAL const UV[2],
                                        REAL *wDeriv[]) const {
  ...
  Far::internal::EvaluatePatchBasisNormalized(
      Far::PatchDescriptor::QUADS, Far::PatchParam(), uv[0], uv[1], wDeriv[0],
      wDeriv[1], wDeriv[2], wDeriv[3], wDeriv[4], wDeriv[5]);
...
template <typename REAL>
int Surface<REAL>::evalMultiLinearStencils(REAL const uv[2],
                                           REAL *sDeriv[]) const {
...
template <typename REAL>
void Surface<REAL>::evalMultiLinearDerivs(REAL const uv[],
                                          REAL const patchPoints[],
                                          PointDescriptor const &pointDesc,
                                          REAL *deriv[]) const {
...
//
template <typename REAL>
void Surface<REAL>::ApplyStencilFromMesh(REAL const stencil[],
...
template <typename REAL>
void Surface<REAL>::ApplyStencil(REAL const stencil[],
...
//
template void
Surface<float>::GatherControlPoints(float const[], PointDescriptor const &,
                                    float[], PointDescriptor const &) const;
template void
Surface<float>::GatherControlPoints(double const[], PointDescriptor const &,
                                    float[], PointDescriptor const &) const;

template void
Surface<double>::GatherControlPoints(double const[], PointDescriptor const &,
                                     double[], PointDescriptor const &) const;
template void
Surface<double>::GatherControlPoints(float const[], PointDescriptor const &,
                                     double[], PointDescriptor const &) const;
