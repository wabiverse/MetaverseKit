...
public:
  SurfaceSet()
      : numSurfs(0), numFVarSurfs(0), vtxSurf(0), varSurf(0), fvarSurfs(0),
        fvarSurfPtrs(0), fvarIDs(0) {}
...
void SurfaceFactory::setSubdivisionOptions(Sdc::SchemeType subdivScheme,
                                           Sdc::Options const &subdivOptions) {
  ...
  _linearFVarInterp =
      _linearScheme || (_subdivOptions.GetFVarLinearInterpolation() ==
                        Sdc::Options::FVAR_LINEAR_ALL);

  //  Initialize members related to the "face has limit" test:
  _rejectSmoothBoundariesForLimit =
      !_linearScheme && (_subdivOptions.GetVtxBoundaryInterpolation() ==
                         Sdc::Options::VTX_BOUNDARY_NONE);

  _rejectIrregularFacesForLimit = !_linearScheme && (_regFaceSize == 3);

  _testNeighborhoodForLimit =
      _rejectSmoothBoundariesForLimit || _rejectIrregularFacesForLimit;
}
...
//
inline bool SurfaceFactory::faceHasLimitSimple(Index faceIndex,
                                               int faceSize) const {
...
bool SurfaceFactory::faceHasLimitNeighborhood(
    FaceTopology const &topology) const {
  ...
  for (int i = 0; i < faceSize; ++i) {
    ...
    if (_rejectSmoothBoundariesForLimit) {
      if (faceVtxTag.IsUnOrdered()) {
        ...
        if (getFaceVertexIncidentFaceVertexIndices(faceIndex, i,
                                                   cFaceVertIndices) < 0)
          return false;
...
//  10% in extreme cases.
//
KeyIntType packTopologyKey(FaceSurface const &surface,
  ...
  if (combinedTag.HasSharpEdges() || combinedTag.HasSemiSharpVertices() ||
...
//
KeyIntType hashTopologyKey(FaceSurface const &surface,
                           IrregularPatchBuilder::Options options) {
  ...
  for (int i = 0; i < faceSize; ++i) {
    ...
    hashBufferSize +=
        cSub._tag.HasUnCommonFaceSizes() ? (sizeof(short) * N) : 0;
    hashBufferSize += cSub._tag.HasSharpEdges()
                          ? (sizeof(float) * (N - cSub.IsBoundary()))
                          : 0;
...
//
void SurfaceFactory::assignLinearSurface(SurfaceType *surfacePtr,
                                         Index faceIndex,
...
void SurfaceFactory::assignRegularSurface(SurfaceType *surfacePtr,
...
void SurfaceFactory::assignIrregularSurface(
    SurfaceType *surfacePtr, FaceSurface const &descriptor) const {
...
void SurfaceFactory::copyNonLinearSurface(SurfaceType *surfaceDstPtr,
...
//
bool SurfaceFactory::initFaceNeighborhoodTopology(
    Index faceIndex, FaceTopology *faceTopologyPtr) const {
...
bool SurfaceFactory::gatherFaceNeighborhoodTopology(
    Index faceIndex, FaceTopology *faceTopologyPtr) const {
...
int SurfaceFactory::gatherFaceNeighborhoodIndices(
    Index faceIndex, FaceTopology const &faceTopology,
    FVarID const *fvarPtrOrVtx, Index controlIndices[]) const {
  ...
  for (int i = 0; i < faceSize; ++i) {
    int numFaceVerts =
        (fvarPtrOrVtx == 0)
            ? getFaceVertexIncidentFaceVertexIndices(faceIndex, i, indices)
...
bool SurfaceFactory::isFaceNeighborhoodRegular(Index faceIndex,
  ...
  return (fvarPtrOrVtx == 0)
             ...
             : getFaceNeighborhoodFVarValueIndicesIfRegular(
                   faceIndex, *fvarPtrOrVtx, indices);
}

//
//  Main internal methods to populate set of limit Surfaces:
//
bool SurfaceFactory::populateAllSurfaces(Index faceIndex,
  ...
  bool hasNonLinearSurfaces = (surfaces.HasVertexSurface() && !_linearScheme) ||
                              (numFVarSurfaces && !_linearFVarInterp);

  bool hasLinearSurfaces = surfaces.HasVaryingSurface() ||
...
bool SurfaceFactory::populateLinearSurfaces(Index faceIndex,
                                            SurfaceSet *surfaceSetPtr) const {
...
bool SurfaceFactory::populateNonLinearSurfaces(
    Index faceIndex, SurfaceSet *surfaceSetPtr) const {
  ...
  if (fvarIsNonLinear) {
    ...
    for (int i = 0; i < numFVarSurfaces; ++i) {
      ...
      bool fvarIsExplicitlyRegular =
          vtxIsExplicitlyRegular &&
      ...
      if (gatherFaceNeighborhoodIndices(faceIndex, faceTopology, &fvarID,
                                        fvIndices) < 0) {
        return false;
      }
