...
PatchTreeBuilder::PatchTreeBuilder(TopologyRefiner &faceRefiner,
    ...
    : _patchTree(new PatchTree), _faceRefiner(faceRefiner), _faceAtRoot(0),
      _patchBuilder(0) {
  ...
  for (int i = 0; i < numLevels; ++i) {
    _levelOffsets[1 + i] =
        _levelOffsets[i] + _faceRefiner.GetLevel(i).GetNumVertices();
  }
  ...
  _patchTree->_numRefinedPoints =
      _faceRefiner.GetNumVerticesTotal() - _patchTree->_numControlPoints;
...
void PatchTreeBuilder::identifyPatches() {
  ...
  _patchTree->_numSubPatchPoints +=
      numIrregPatches * _patchTree->_irregPatchSize;
}

void PatchTreeBuilder::initializePatches() {

  //  Keep track of the growing index of local points in irregular patches:
  int irregPointIndexBase =
      _patchTree->_numControlPoints + _patchTree->_numRefinedPoints;
  ...
  for (size_t i = 0; i < _patchFaces.size(); ++i) {
    ...
    if (pf.isRegular) {
      ...
      patchParam = _patchBuilder->ComputePatchParam(
      ...
      _patchBuilder->GetRegularPatchPoints(pf.level, pf.face, boundaryMask,
                                           patchPoints);
      ...
      patchParam = _patchBuilder->ComputePatchParam(
          pf.level, pf.face, ptexIndices, false /*irreg*/, 0 /*mask*/, false);
...
template <typename REAL> void PatchTreeBuilder::initializeStencilMatrix() {
  ...
  int numPointStencils =
      _patchTree->_numRefinedPoints +
      (_patchTree->_numIrregPatches * _patchTree->_irregPatchSize);
  ...
  if (_patchTree->_numIrregPatches) {
    ...
    for (size_t i = 0; i < _patchFaces.size(); ++i) {
      if (!_patchFaces[i].isRegular) {
        getIrregularPatchConversion(_patchFaces[i], irregConvMatrix,
                                    irregSourcePoints);

        appendConversionStencilsToMatrix(stencilIndexBase, irregConvMatrix,
                                         irregSourcePoints);
...
void PatchTreeBuilder::appendConversionStencilsToMatrix(
    int stencilBaseIndex, SparseMatrix<REAL> const &conversionMatrix,
...
template <typename REAL>
void PatchTreeBuilder::getIrregularPatchConversion(
    PatchFace const &pf, SparseMatrix<REAL> &conversionMatrix,
  ...
  _patchBuilder->GetIrregularPatchConversionMatrix(
      pf.level, pf.face, cornerSpans, conversionMatrix);
  ...
  _patchBuilder->GetIrregularPatchSourcePoints(pf.level, pf.face, cornerSpans,
                                               &sourcePoints[0]);
