...
FileDialog::FileDialog(int flags) : _flags(flags) {}

void FileDialog::setTitle(const std::string &title) { _title = title; }
...
bool FileDialog::isOpened() { return _isOpened; }

bool FileDialog::hasSelected() { return !_selectedFilenames.empty(); }

mx::FilePath FileDialog::getSelected() {
  return _selectedFilenames.empty() ? mx::FilePath() : _selectedFilenames[0];
}

void FileDialog::clearSelected() { _selectedFilenames.clear(); }
