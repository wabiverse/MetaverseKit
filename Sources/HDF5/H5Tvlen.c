...
static h5_posix_io_ret_t H5T_vlen_seq_mem_getlen(const void *_vl);
...
static h5_posix_io_ret_t H5T_vlen_str_mem_getlen(const void *_vl);
...
static h5_posix_io_ret_t H5T_vlen_disk_getlen(const void *_vl);
...
H5T_vlen_seq_mem_getlen(const void *_vl)
{
  ...
  FUNC_LEAVE_NOAPI((h5_posix_io_ret_t)vl->len)
#else
  HDassert(_vl);
...
H5T_vlen_str_mem_getlen(const void *_vl)
{
  ...
  FUNC_LEAVE_NOAPI((h5_posix_io_ret_t)HDstrlen(s))
...
H5T_vlen_disk_getlen(const void *_vl)
{
  ...
  FUNC_LEAVE_NOAPI((h5_posix_io_ret_t)seq_len)
