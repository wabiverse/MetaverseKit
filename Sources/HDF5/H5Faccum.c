                ...
                size_t size, void *buf /*out*/)
{
  ...
  if ((fio_info->f->shared->feature_flags & H5FD_FEAT_ACCUMULATE_METADATA) && map_type != H5FD_MEM_DRAW)
  {
    ...
    if (size < H5F_ACCUM_MAX_SIZE)
    {
      ...
      if (H5F_addr_overlap(addr, size, accum->loc, accum->size) || ((addr + size) == accum->loc) || (accum->loc + accum->size) == addr)
      {
    ...
    else
    {
          ...
          H5F_addr_overlap(addr, size, accum->loc + accum->dirty_off, accum->dirty_len))
      {
        ...
        { /* Read starts after beginning of dirty region */
    ...
    if (new_size > H5F_ACCUM_MAX_SIZE)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            if ((h5_posix_io_ret_t)(H5F_ACCUM_MAX_SIZE - (accum->dirty_off + adjust_size)) >= (h5_posix_io_ret_t)(2 * size))
    ...
    else
    {
      ...
      if (H5F_addr_overlap(addr, size, accum->loc, accum->size))
      {
        ...
        if (H5F_addr_le(addr, accum->loc))
        {
          ...
          if (H5F_addr_le(addr + size, accum->loc + accum->size))
          {
            ...
            if (accum->dirty)
            {
              ...
              else
              {
                ...
                { /* Dirty region overlaps w/written region */
                  accum->dirty_off = 0;
          ...
          { /* Access covers whole accumulator */
        ...
        {                      /* Write starts after beginning of accumulator */
          size_t overlap_size; /* Size of overlapping region */
                ...
                hsize_t size)
{
  ...
  if ((fio_info->f->shared->feature_flags & H5FD_FEAT_ACCUMULATE_METADATA) && H5F_addr_overlap(addr, size, accum->loc, accum->size))
  {
