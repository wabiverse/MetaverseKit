...
static h5_posix_io_ret_t H5E_get_class_name(const H5E_cls_t *cls, char *name, size_t size);
...
static h5_posix_io_ret_t H5E_get_num(const H5E_t *err_stack);
...
{
  h5_posix_io_ret_t len = 0; /* Length of error class's name */
  ...
  len = (h5_posix_io_ret_t)HDstrlen(cls->cls_name);
...
H5E_get_num(const H5E_t *estack)
{
  ...
  FUNC_LEAVE_NOAPI((h5_posix_io_ret_t)estack->nused)
} /* end H5E_get_num() */
