...
H5D__open_oid(H5D_t *dataset, hid_t dapl_id, hid_t dxpl_id)
{
  ...
  else
  {
    ...
    if (fill_prop->size == 0)
      fill_prop->size = (h5_posix_io_ret_t)-1;
  } /* end if */
  alloc_time_state = 0;
  if ((dataset->shared->layout.type == H5D_COMPACT && fill_prop->alloc_time == H5D_ALLOC_TIME_EARLY) || (dataset->shared->layout.type == H5D_CONTIGUOUS && fill_prop->alloc_time == H5D_ALLOC_TIME_LATE) || (dataset->shared->layout.type == H5D_CHUNKED && fill_prop->alloc_time == H5D_ALLOC_TIME_INCR) || (dataset->shared->layout.type == H5D_VIRTUAL && fill_prop->alloc_time == H5D_ALLOC_TIME_INCR))
    alloc_time_state = 1;
...
H5D_oloc(H5D_t *dataset){
    ...
    FUNC_ENTER_NOAPI_NOINIT_NOERR
        ...
        FUNC_LEAVE_NOAPI(dataset ? &(dataset -> oloc) : (H5O_loc_t *)NULL)} /* end H5D_oloc() */
...
H5G_name_t *H5D_nameof(H5D_t *dataset){
    ...
    FUNC_ENTER_NOAPI_NOINIT_NOERR
        ...
        FUNC_LEAVE_NOAPI(dataset ? &(dataset->path) : (H5G_name_t *)NULL)} /* end H5D_nameof() */
 ...
 *-------------------------------------------------------------------------
 */
H5T_t *H5D_typeof(const H5D_t *dset)
{
  /* Use FUNC_ENTER_NOAPI_NOINIT_NOERR here to avoid performance issues */
  FUNC_ENTER_NOAPI_NOINIT_NOERR
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5D_get_create_plist(H5D_t *dset)
{
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5D_get_access_plist(H5D_t *dset)
{
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5D_get_space(H5D_t *dset)
{
  H5S_t *space = NULL;
