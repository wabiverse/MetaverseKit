...
#define H5C__AUX_DLL_SC(head_ptr, tail_ptr, len, Size, fv)                     \
  if (((((head_ptr) == NULL) || ((tail_ptr) == NULL)) &&                       \
      ...
      ((len) < 0) || ((Size) < 0) ||                                           \
      (((len) == 1) &&                                                         \
       (((head_ptr) != (tail_ptr)) || ((Size) <= 0) || ((head_ptr) == NULL) || \
        ((head_ptr)->size != (Size)))) ||                                      \
      (((len) >= 1) &&                                                         \
...
#define H5C__UPDATE_STATS_FOR_ENTRY_SIZE_CHANGE(cache_ptr, entry_ptr,          \
                                                new_size)                      \
  if (cache_ptr->flush_in_progress) {                                          \
...
/* (Keep in sync w/H5C_TEST__POST_SUC_HT_SEARCH_SC macro in test/cache_common.h
 * -QAK) */
...
/* (Keep in sync w/H5C_TEST__POST_HT_SHIFT_TO_FRONT macro in test/cache_common.h
 * -QAK) */
...
#define H5C__PRE_HT_ENTRY_SIZE_CHANGE_SC(cache_ptr, old_size, new_size,        \
  ...
  if (((cache_ptr) == NULL) || ((cache_ptr)->index_len <= 0) ||                \
      ...
      ((old_size) > (cache_ptr)->index_size) || ((new_size) <= 0) ||           \
...
#define H5C__UPDATE_INDEX_FOR_SIZE_CHANGE(cache_ptr, old_size, new_size,       \
                                          entry_ptr, was_clean)                \
  {                                                                            \
    H5C__PRE_HT_ENTRY_SIZE_CHANGE_SC(cache_ptr, old_size, new_size, entry_ptr, \
                                     was_clean)                                \
    (cache_ptr)->index_size -= (old_size);                                     \
    (cache_ptr)->index_size += (new_size);                                     \
...
#define H5C__UPDATE_RP_FOR_EVICTION(cache_ptr, entry_ptr, fail_val)            \
  {                                                                            \
    ...
    if ((entry_ptr)->is_dirty) {                                               \
                                                                               \
      H5C__AUX_DLL_REMOVE(                                                     \
          (entry_ptr), (cache_ptr)->dLRU_head_ptr, (cache_ptr)->dLRU_tail_ptr, \
          (cache_ptr)->dLRU_list_len, (cache_ptr)->dLRU_list_size, (fail_val)) \
    } else {                                                                   \
      H5C__AUX_DLL_REMOVE(                                                     \
          (entry_ptr), (cache_ptr)->cLRU_head_ptr, (cache_ptr)->cLRU_tail_ptr, \
...
#define H5C__COLL_DLL_PRE_REMOVE_SC(entry_ptr, hd_ptr, tail_ptr, len, Size,    \
                                    fv)                                        \
  if (((hd_ptr) == NULL) || ((tail_ptr) == NULL) || ((entry_ptr) == NULL) ||   \
      ((len) <= 0) || ((Size) < (entry_ptr)->size) ||                          \
      (((Size) == (entry_ptr)->size) && (!((len) == 1))) ||                    \
...
#define H5C__COLL_DLL_PRE_INSERT_SC(entry_ptr, hd_ptr, tail_ptr, len, Size,    \
                                    fv)                                        \
...
#define H5C__COLL_DLL_APPEND(entry_ptr, head_ptr, tail_ptr, len, Size,         \
                             fail_val)                                         \
  {                                                                            \
...
#define H5C__COLL_DLL_REMOVE(entry_ptr, head_ptr, tail_ptr, len, Size, fv)     \
  {                                                                            \
    H5C__COLL_DLL_PRE_REMOVE_SC(entry_ptr, head_ptr, tail_ptr, len, Size,      \
                                fv) {                                          \
      if ((head_ptr) == (entry_ptr)) {                                         \
...
#define H5C__INSERT_IN_COLL_LIST(cache_ptr, entry_ptr, fail_val)               \
  {                                                                            \
    ...
    /* insert the entry at the head of the list. */                            \
                                                                               \
    H5C__COLL_DLL_PREPEND(                                                     \
        (entry_ptr), (cache_ptr)->coll_head_ptr, (cache_ptr)->coll_tail_ptr,   \
