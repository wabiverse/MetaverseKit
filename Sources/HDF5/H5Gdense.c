...
typedef struct
{
  ...
  /* upward */
  h5_posix_io_ret_t name_len; /* Full length of name               */
} H5G_bt2_ud_gnbi_t;
                         ...
                         H5_index_t idx_type, H5_iter_order_t order, hsize_t n, H5O_link_t *lnk)
{
  ...
  { /* Otherwise, we need to build a table of the links and sort it */
...
H5G_dense_get_name_by_idx_fh_cb(const void *obj, size_t H5_ATTR_UNUSED obj_len, void *_udata)
{
  ...
  udata->name_len = (h5_posix_io_ret_t)HDstrlen(lnk->name);
                           ...
                           size_t size)
{
  ...
  { /* Otherwise, we need to build a table of the links and sort it */
    ...
    /* Get the length of the name */
    ret_value = (h5_posix_io_ret_t)HDstrlen(ltable.lnks[n].name);
