...
H5S_hyper_is_valid_helper(const H5S_hyper_span_info_t *spans, const hssize_t *offset, const hsize_t *size, hsize_t rank)
{
  ...
  while (curr != NULL && ret_value == TRUE)
  {
    ...
    if ((((hssize_t)curr->low + offset[rank]) >= (hssize_t)size[rank]) || (((hssize_t)curr->low + offset[rank]) < 0) || (((hssize_t)curr->high + offset[rank]) >= (hssize_t)size[rank]) || (((hssize_t)curr->high + offset[rank]) < 0))
    {
      ret_value = FALSE;
      break;
    } /* end if */
...
H5S_hyper_serial_size(const H5S_t *space)
{
  ...
  if (space->select.sel_info.hslab->unlim_dim >= 0)
     ...
     * 17 + (4 * rank * 8) bytes
     */
    ret_value = (hssize_t)17 + ((hssize_t)4 * (hssize_t)space->extent.rank * (hssize_t)8);
  else
  {
    /* Version 1 */
      ...
      if (outcount > 0)
      {
        if (span->down)
        {
          ...
          for (u = 0; u < rank - 1; u++)
          {
            ...
            {
              if (curr_down_span_slab_info->start != canon_down_span_slab_info[u].start || curr_down_span_slab_info->stride != canon_down_span_slab_info[u].stride || curr_down_span_slab_info->block != canon_down_span_slab_info[u].block || curr_down_span_slab_info->count != canon_down_span_slab_info[u].count)
                HGOTO_DONE(FALSE)
            } /* end if */
                     ...
                     const hsize_t *block)
{
  ...
  else
  {
    ...
    for (u = 0; u < space->extent.rank; u++)
    {
      ...
      else
      {
        ...
        {
          HDassert((stride[u] > block[u]) || ((stride[u] == block[u]) && (count[u] == H5S_UNLIMITED)));
  ...
  if ((H5S_GET_SELECT_TYPE(space) == H5S_SEL_HYPERSLABS) && (space->select.sel_info.hslab->unlim_dim >= 0) && (op != H5S_SELECT_SET))
  {
    ...
    if (H5S_hyper_clip_unlim(space,
                             start[space->select.sel_info.hslab->unlim_dim] + ((opt_count[space->select.sel_info.hslab->unlim_dim] - (hsize_t)1) * opt_stride[space->select.sel_info.hslab->unlim_dim]) + opt_block[space->select.sel_info.hslab->unlim_dim]) < 0)
                                ...
                                const H5S_t *src_intersect_space, H5S_t *proj_space)
{
  ...
  while (1)
  {
    ...
    else
    {
      ...
      if ((ss_off[ss_i] + (hsize_t)ss_len[ss_i]) <= (sis_off[sis_i] + (hsize_t)sis_len[sis_i]))
      {
      ...
      while (int_len > (size_t)0)
      {
        ...
        while (proj_len_rem > (size_t)0)
        {
          ...
          for (i = proj_rank - 1; ((i > 0) && ((proj_off / proj_down_dims[i - 1]) != curr_span_up_dim[i - 1])); i--)
          {
            if (curr_span_tree[i])
            {
          ...
          span_len = MIN(proj_len_rem,
                         (size_t)(proj_space->extent.size[proj_rank - 1] - low));
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
void H5S__hyper_get_clip_diminfo(hsize_t start, hsize_t stride, hsize_t *count,
                                ...
                                const H5S_t *match_space, hsize_t match_clip_size, hbool_t incl_trail)
{
  ...
  HDassert(clip_space->select.sel_info.hslab->num_elem_non_unlim == match_space->select.sel_info.hslab->num_elem_non_unlim);
  ...
  else
  {
    ...
    if (((match_diminfo->stride * (count - (hsize_t)1)) + block) > (match_clip_size - match_diminfo->start))
    {
      ...
      HDassert((((match_diminfo->stride * (count - (hsize_t)1)) + block) - (match_clip_size - match_diminfo->start)) < num_slices);
...
H5S_hyper_get_unlim_block(const H5S_t *space, hsize_t block_index)
{
  ...
  for (i = 0; i < space->extent.rank; i++)
  {
    ...
    {
      start[i] = hslab->opt_diminfo[i].start + (block_index * hslab->opt_diminfo[i].stride);
      count[i] = (hsize_t)1;
    } /* end if */
    else
    {
