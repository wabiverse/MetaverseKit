...
Create a build folder in the javascript folder below the _root_ of the repository and navigate to that folder:
...
However, any rendering framework that supports custom shaders should do. In the [Web Viewer sample app](./MXViewsrc/index.js) we use the [RawShaderMaterial](https://threejs.org/docs/index.html?q=RawSh#api/en/materials/RawShaderMaterial) class from [three.js](https://threejs.org/).
...
An example that parses the JSON and feeds the uniform data to a three.js based application can be found in the [Web Viewer Sample App](./MXViewsrc/index.js).
