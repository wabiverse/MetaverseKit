...
public:
  ...
  const StringSet &supportedExtensions() const { return _extensions; }

  /// Save an image to the file system. This method must be implemented by
  /// derived classes.
  ...
  virtual bool saveImage(const FilePath &filePath, ConstImagePtr image,
                         bool verticalFlip = false);

  /// Load an image from the file system. This method must be implemented by
  /// derived classes.
  /// @param filePath The requested image file path.
  /// @return On success, a shared pointer to the loaded image; otherwise an
...
public:
  ...
  /// The first image loader which supports the file name extension will be
  /// used.
  /// @param filePath File path to be written
  /// @param image The image to be saved
  /// @param verticalFlip Whether the image should be flipped in Y during save
  /// @return if save succeeded
  bool saveImage(const FilePath &filePath, ConstImagePtr image,
                 bool verticalFlip = false);
  ...
  virtual bool bindImage(ImagePtr image,
  ...
  void setSearchPath(const FileSearchPath &path) { _searchPath = path; }

  /// Return the image search path.
  const FileSearchPath &getSearchPath() const { return _searchPath; }

  /// Set the filename resolver for images.
  void setFilenameResolver(StringResolverPtr resolver) { _resolver = resolver; }

  /// Return the filename resolver for images.
  StringResolverPtr getFilenameResolver() const { return _resolver; }
  ...
  ImagePtr getZeroImage() const { return _zeroImage; }
