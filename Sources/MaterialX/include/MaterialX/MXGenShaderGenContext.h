...
/// A standard function to allow for handling of application variables for a
/// given node
using ApplicationVariableHandler =
    std::function<void(ShaderNode *, GenContext &)>;
...
public:
  ...
  ShaderGenerator &getShaderGenerator() { return *_sg; }

  /// Return shader generation options.
  GenOptions &getOptions() { return _options; }

  /// Return shader generation options.
  const GenOptions &getOptions() const { return _options; }
  ...
  FilePath resolveSourceFile(const FilePath &filename,
                             const FilePath &localPath) const {
  ...
  template <class T> std::shared_ptr<T> getUserData(const string &name) {
    auto it = _userData.find(name);
    return it != _userData.end() && !it->second.empty()
               ? it->second.back()->asA<T>()
               : nullptr;
  }
...
/// On hardware BSDF closures are evaluated differently in reflection,
/// transmission or environment/indirect contexts. This class represents the
/// context we are in and if extra arguments and function decorators are needed
/// for that context.
...
public:
  ...
  /// For the given node type add an extra argument to be used for the function
  /// in this context.
  void addArgument(const TypeDesc *nodeType, const Argument &arg) {
    _arguments[nodeType].push_back(arg);
  }

  /// Return a list of extra argument to be used for the given node in this
  /// context.
  ...
  /// For the given node type set a function name suffix to be used for the
  /// function in this context.
  void setSuffix(const TypeDesc *nodeType, const string &suffix) {
    _suffix[nodeType] = suffix;
  }

  /// Return the function name suffix to be used for the given node in this
  /// context.
  ...
  void setClosureParams(const ShaderNode *closure,
...
public:
  ...
  ScopedSetClosureParams(const ClosureContext::ClosureParams *params,
                         const ShaderNode *node, ClosureContext *cct);

  /// Constructor for setting parameters from one closure node to another.
  ScopedSetClosureParams(const ShaderNode *fromNode, const ShaderNode *toNode,
                         ClosureContext *cct);
