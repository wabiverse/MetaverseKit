...
protected:
  ...
  /// Derived classes are responsible for returning if a given type is an
  /// acceptable input.
  virtual bool acceptsInputType(const TypeDesc *type) const = 0;

  // Derived classes are responsible for computing offset strings relative to
  // the center sample The sample size and offset type are passed in as
  // arguments.
  virtual void computeSampleOffsetStrings(const string &sampleSizeName,
                                          const string &offsetTypeString,
                                          unsigned int filterWidth,
                                          StringVec &offsetStrings) const = 0;
  ...
  /// Generate upstream / input sampling code in uv space and cache the output
  /// variable names which will hold the sample values after execution.
  void emitInputSamplesUV(const ShaderNode &node, unsigned int sampleCount,
                          unsigned int filterWidth, float filterSize,
                          float filterOffset,
