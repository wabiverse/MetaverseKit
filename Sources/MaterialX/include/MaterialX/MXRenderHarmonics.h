...
/// @param env An environment map in lat-long format.
/// @param envRadiance The radiance to which the environment map should be
/// normalized.
/// @param maxTexelRadiance The maximum radiance allowed for any individual
/// texel of the map.
/// @return A new normalized environment map, in the same format as the
/// original.
...
/// @param lightColor Returns the color of the light from the dominant
/// direction.
MX_RENDER_API void computeDominantLight(ConstImagePtr env, Vector3 &lightDir,
                                        Color3 &lightColor);
