...
public:
  ...
  /// Return a vector of all PropertySetAssign elements that belong to this
  /// look, taking look inheritance into account.
...
public:
  ...
  void setLooks(const string &looks) { setAttribute(LOOKS_ATTRIBUTE, looks); }

  /// Get comma-separated list of looks.
  const string &getLooks() const { return getAttribute(LOOKS_ATTRIBUTE); }
  ...
  const string &getActiveLook() const { return getAttribute(ACTIVE_ATTRIBUTE); }
...
public:
  ...
  bool hasMaterial() const { return hasAttribute(MATERIAL_ATTRIBUTE); }

  /// Return the material string for the MaterialAssign.
  const string &getMaterial() const { return getAttribute(MATERIAL_ATTRIBUTE); }
...
public:
  ...
  bool hasViewerGeom() const { return hasAttribute(VIEWER_GEOM_ATTRIBUTE); }
  ...
  bool getVisible() const { return getTypedAttribute<bool>(VISIBLE_ATTRIBUTE); }
...
MX_CORE_API vector<MaterialAssignPtr>
getGeometryBindings(ConstNodePtr materialNode,
