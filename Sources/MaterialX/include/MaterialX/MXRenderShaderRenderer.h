...
/// Provides a provision for an application to share any renderer specific
/// settings
...
public:
  ...
  /// Return the camera.
  CameraPtr getCamera() const { return _camera; }
  ...
  ImageHandlerPtr getImageHandler() const { return _imageHandler; }
  ...
  LightHandlerPtr getLightHandler() const { return _lightHandler; }
  ...
  GeometryHandlerPtr getGeometryHandler() const { return _geometryHandler; }
  ...
  virtual ImagePtr captureImage(ImagePtr image = nullptr) { return nullptr; }
...
public:
  ExceptionRenderError(const string &msg,
                       const StringVec &errorLog = StringVec())
      : Exception(msg), _errorLog(errorLog) {}

  ExceptionRenderError(const ExceptionRenderError &e)
      : Exception(e), _errorLog(e._errorLog) {}
  ...
  const StringVec &errorLog() const { return _errorLog; }
