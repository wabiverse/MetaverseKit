...
public:
  ...
  /// The hash should correspond to the function signature generated for the
  /// node, and can be used to compare implementations, e.g. to query if an
  /// identical function has already been emitted during shader generation.
  ...
  virtual void setValues(const Node &node, ShaderNode &shaderNode,
  ...
  /// uniforms, inputs and outputs). Used if the node requires input data from
  /// the application.
  virtual void createVariables(const ShaderNode &node, GenContext &context,
                               Shader &shader) const;
  ...
  /// Emit the function call or inline source code for given node instance in
  /// the given context.
  virtual void emitFunctionCall(const ShaderNode &node, GenContext &context,
                                ShaderStage &stage) const;

  /// Emit declaration and initialization of output variables to use in a
  /// function call.
  virtual void emitOutputVariables(const ShaderNode &node, GenContext &context,
                                   ShaderStage &stage) const;
  ...
  virtual bool isEditable(const ShaderInput & /*input*/) const { return true; }
...
/// A no operation node, to be used for organizational nodes that has no code to
/// execute.
