...
public:
  LightHandler()
      : _lightTransform(Matrix44::IDENTITY), _directLighting(true),
        _indirectLighting(true), _envSampleCount(DEFAULT_ENV_SAMPLE_COUNT),
  ...
  void setLightTransform(const Matrix44 &mat) { _lightTransform = mat; }

  /// Return the light transform.
  Matrix44 getLightTransform() const { return _lightTransform; }

  /// Set whether direct lighting is enabled.
  void setDirectLighting(bool enable) { _directLighting = enable; }

  /// Return whether direct lighting is enabled.
  bool getDirectLighting() const { return _directLighting; }

  /// Set whether indirect lighting is enabled.
  void setIndirectLighting(bool enable) { _indirectLighting = enable; }

  /// Return whether indirect lighting is enabled.
  bool getIndirectLighting() const { return _indirectLighting; }
  ...
  void setEnvRadianceMap(ImagePtr map) { _envRadianceMap = map; }

  /// Return the environment radiance map
  ImagePtr getEnvRadianceMap() const { return _envRadianceMap; }

  /// Set the environment irradiance map
  void setEnvIrradianceMap(ImagePtr map) { _envIrradianceMap = map; }

  /// Return the environment irradiance map
  ImagePtr getEnvIrradianceMap() const { return _envIrradianceMap; }

  /// Set the environment lighting sample count.
  void setEnvSampleCount(int count) { _envSampleCount = count; }

  /// Return the environment lighting sample count.
  int getEnvSampleCount() const { return _envSampleCount; }

  /// Set the two-sided refraction property.
  void setRefractionTwoSided(bool enable) { _refractionTwoSided = enable; }

  /// Return the two-sided refraction property.
  int getRefractionTwoSided() const { return _refractionTwoSided; }
  ...
  void setAlbedoTable(ImagePtr table) { _albedoTable = table; }

  /// Return the directional albedo table
  ImagePtr getAlbedoTable() const { return _albedoTable; }
  ...
  const vector<NodePtr> &getLightSources() const { return _lightSources; }
  ...
  void registerLights(DocumentPtr doc, const vector<NodePtr> &lights,
                      GenContext &context);
