...
public:
  ...
  /// for the metadata.
  void addMetadata(const string &name, const TypeDesc *type,
  ...
  const ShaderMetadataVec &getAllMetadata() const { return _entries; }
...
class MX_GENSHADER_API ShaderPort
    : public std::enable_shared_from_this<ShaderPort> {
public:
  /// Constructor.
  ShaderPort(ShaderNode *node, const TypeDesc *type, const string &name,
             ValuePtr value = nullptr);

  /// Return a shared pointer instance of this object.
  ShaderPortPtr getSelf() { return shared_from_this(); }
  ...
  bool getFlag(uint32_t flag) const { return ((_flags & flag) != 0); }
...
public:
  ...
  public:
    ...
    static const uint32_t CLOSURE =
        1 << 1; /// Any node that represents light integration
    static const uint32_t SHADER = 1 << 2; /// Any node that outputs a shader
    static const uint32_t MATERIAL = 1
                                     << 3; /// Any node that outputs a material
    ...
    static const uint32_t LAYER =
        1 << 12; /// A node for vertical layering of other closure nodes
    static const uint32_t THINFILM =
        1 << 13; /// A node for adding thin-film over microfacet BSDF nodes
    ...
    static const uint32_t SAMPLE2D = 1
                                     << 18; /// Can be sampled in 2D (uv space)
    static const uint32_t SAMPLE3D = 1
                                     << 19; /// Can be sampled in 3D (position)
...
public:
  ...
  static ShaderNodePtr create(const ShaderGraph *parent, const string &name,
                              const NodeDef &nodeDef, GenContext &context);

  /// Create a new node from a node implementation.
  static ShaderNodePtr
  create(const ShaderGraph *parent, const string &name, ShaderNodeImplPtr impl,
  ...
  const ShaderGraph *getParent() const { return _parent; }

  /// Set classification bits for this node,
  /// replacing any previous set bits.
  void setClassification(uint32_t c) { _classification = c; }

  /// Get classification bits set for this node.
  uint32_t getClassification() const { return _classification; }

  /// Add classification bits to this node.
  void addClassification(uint32_t c) { _classification |= c; }
  ...
  const ShaderNodeImpl &getImplementation() const { return *_impl; }

  /// Initialize this shader node with all required data
  /// from the given node and nodedef.
  void initialize(const Node &node, const NodeDef &nodeDef,
                  GenContext &context);
