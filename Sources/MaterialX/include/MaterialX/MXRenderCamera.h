...
public:
  Camera()
      : _worldMatrix(Matrix44::IDENTITY), _viewMatrix(Matrix44::IDENTITY),
        _projectionMatrix(Matrix44::IDENTITY), _arcballActive(false),
        _arcballQuat(Quaternion::IDENTITY), _arcballDelta(Quaternion::IDENTITY),
        _arcballSpeed(2.0f) {}
  ~Camera() {}
  ...
  void setWorldMatrix(const Matrix44 &mat) { _worldMatrix = mat; }

  /// Return the world matrix.
  const Matrix44 &getWorldMatrix() const { return _worldMatrix; }

  /// Set the view matrix.
  void setViewMatrix(const Matrix44 &mat) { _viewMatrix = mat; }

  /// Return the view matrix.
  const Matrix44 &getViewMatrix() const { return _viewMatrix; }

  /// Set the projection matrix.
  void setProjectionMatrix(const Matrix44 &mat) { _projectionMatrix = mat; }

  /// Return the projection matrix.
  const Matrix44 &getProjectionMatrix() const { return _projectionMatrix; }
  ...
  void setViewportSize(const Vector2 &size) { _viewportSize = size; }
  ...
  /// Indicates a button state change, with pos being the instantaneous location
  /// of the mouse.
  ...
  /// Create a view matrix given an eye position, a target position and an up
  /// vector.
  static Matrix44 createViewMatrix(const Vector3 &eye, const Vector3 &target,
                                   const Vector3 &up);

  /// Create a perpective projection matrix given a set of clip planes with
  /// [-1,1] projected Z.
  static Matrix44 createPerspectiveMatrix(float left, float right, float bottom,
  ...
  static Matrix44 createOrthographicMatrix(float left, float right,
                                           float bottom, float top, float nearP,
                                           float farP);
  ...
  static Vector3 transformPointPerspective(const Matrix44 &m,
                                           const Vector3 &v) {
