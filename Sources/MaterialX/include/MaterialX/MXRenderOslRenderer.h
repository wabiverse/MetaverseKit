...
///     - Source code validation: Use of "oslc" to compile and test output
///     results
...
///     Assumes source compliation was success
...
public:
  ...
  /// method. setOslCompilerExecutable(), and setOslIncludePath().
  ///
  /// Additionally setOslOutputFilePath() should be set to allow for output of
  /// .osl and .oso files to the appropriate path location to be used as input
  ...
  /// setOslOutputName() method to ensure that the appropriate .oso files can be
  /// located.
  ...
  /// executed. For testrender this value is used to replace the %shader_output%
  /// token in the input scene file.
  ...
  /// Set the path to the OSL rendering tester. Note that it is assumed that
  /// this references the location of the "testrender" executable.
  ...
  ///     - %shader_output% : which will be replace with the name of the shader
  ///     output to use
  ...
  /// validation By default testshade is used.
  /// @param useTestRender Indicate whether to use testrender.
  void useTestRender(bool useTestRender) { _useTestRender = useTestRender; }

  /// Set the number of rays per pixel to be used for lit surfaces.
  void setRaysPerPixelLit(int rays) { _raysPerPixelLit = rays; }

  /// Set the number of rays per pixel to be used for unlit surfaces.
  void setRaysPerPixelUnlit(int rays) { _raysPerPixelUnlit = rays; }

  ///
  /// Compile OSL code stored in a file. Will throw an exception if an error
  /// occurs.
...
protected:
  ...
  /// @param shaderName Name of OSL shader. A corresponding .oso file is assumed
  /// to exist in the output path folder.
  /// @param outputName Name of OSL shader output to use.
  void shadeOSL(const FilePath &dirPath, const string &shaderName,
  ...
  /// to exist in the output path folder.
  /// @param outputName Name of OSL shader output to use.
  void renderOSL(const FilePath &dirPath, const string &shaderName,
                 const string &outputName);
