...
using GlslResourceBindingContextPtr =
    shared_ptr<class GlslResourceBindingContext>;

/// @class GlslResourceBindingContext
/// Class representing a resource binding for Glsl shader resources.
class MX_GENGLSL_API GlslResourceBindingContext
    : public HwResourceBindingContext {
public:
  GlslResourceBindingContext(size_t uniformBindingLocation,
                             size_t samplerBindingLocation);

  static GlslResourceBindingContextPtr
  create(size_t uniformBindingLocation = 0, size_t samplerBindingLocation = 0) {
    return std::make_shared<GlslResourceBindingContext>(uniformBindingLocation,
                                                        samplerBindingLocation);
  }
  ...
  void emitResourceBindings(GenContext &context, const VariableBlock &uniforms,
...
protected:
  ...
  // Indicates whether to use a shared binding counter for samplers and uniforms
  // or separate ones. By default a shader counter is used.
