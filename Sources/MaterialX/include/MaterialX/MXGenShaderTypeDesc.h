...
/// generators to know about the type. A unique type descriptor pointer is the
/// identifier used for types, and can be used for type comparisons as well as
/// getting more information about the type. All standard library data types are
/// registered by default and their type descriptors can be accessed from the
/// Type namespace, e.g. MaterialX::Type::FLOAT. If custom types are used they
/// must be registered by calling TypeDesc::registerType(). Descriptors for
/// registered types can be retreived using TypeDesc::get(), see below.
...
public:
  ...
  registerType(const string &name, unsigned char basetype,
  ...
  /// type. For array types 0 is returned since the number of elements is
  /// undefined until an array is instantiated.
  ...
  bool isFloat2() const {
    return _size == 2 &&
           (_semantic == SEMANTIC_COLOR || _semantic == SEMANTIC_VECTOR);
  }
  ...
  bool isClosure() const {
    return (_semantic == SEMANTIC_CLOSURE || _semantic == SEMANTIC_SHADER ||
            _semantic == SEMANTIC_MATERIAL);
  }

private:
  TypeDesc(const string &name, unsigned char basetype, unsigned char semantic,
           size_t size, bool editable, const ChannelMap &channelMapping);
