...
public:
  ...
  GeomInfoPtr addGeomInfo(const string &name = EMPTY_STRING,
  ...
  void removeGeomInfo(const string &name) { removeChildOfType<GeomInfo>(name); }

  /// Return the value of a geometric property for the given geometry string.
  ValuePtr getGeomPropValue(const string &geomPropName,
  ...
  vector<LookPtr> getLooks() const { return getChildrenOfType<Look>(); }

  /// Remove the Look, if any, with the given name.
  void removeLook(const string &name) { removeChildOfType<Look>(name); }
  ...
  TypeDefPtr addTypeDef(const string &name) { return addChild<TypeDef>(name); }
  ...
  void removeTypeDef(const string &name) { removeChildOfType<TypeDef>(name); }
  ...
  /// @param isDefaultVersion If a version is specified is thie definition the
  /// default version
  /// @param newGraphName Make a copy of this NodeGraph with the given name if a
  /// non-empty name is provided. Otherwise
  ///        modify the existing NodeGraph. Default value is an empty string.
  /// @param nodeGroup Optional node group for the new declaration. The Default
  /// value is an emptry string.
  ...
  void removeNodeDef(const string &name) { removeChildOfType<NodeDef>(name); }
  ...
  getMatchingImplementations(const string &nodeDef) const;
  ...
  void removeUnitDef(const string &name) { removeChildOfType<UnitDef>(name); }
  ...
  bool hasColorManagementSystem() const { return hasAttribute(CMS_ATTRIBUTE); }
