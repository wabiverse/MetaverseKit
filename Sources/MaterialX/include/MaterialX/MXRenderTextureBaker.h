...
public:
  ...
  void setExtension(const string &extension) { _extension = extension; }
  ...
  void setDistanceUnit(const string &unitSpace) { _distanceUnit = unitSpace; }

  /// Return the distance unit to which textures are baked.
  const string &getDistanceUnit() const { return _distanceUnit; }

  /// Set whether images should be averaged to generate constants.  Defaults to
  /// false.
  void setAverageImages(bool enable) { _averageImages = enable; }

  /// Return whether images should be averaged to generate constants.
  bool getAverageImages() const { return _averageImages; }

  /// Set whether uniform textures should be stored as constants.  Defaults to
  /// true.
  void setOptimizeConstants(bool enable) { _optimizeConstants = enable; }

  /// Return whether uniform textures should be stored as constants.
  bool getOptimizeConstants() const { return _optimizeConstants; }

  /// Set the output location for baked texture images.  Defaults to the root
  /// folder of the destination material.
  ...
  const FilePath &getOutputImagePath() { return _outputImagePath; }

  /// Set the name of the baked graph element.
  void setBakedGraphName(const string &name) { _bakedGraphName = name; }

  /// Return the name of the baked graph element.
  const string &getBakedGraphName() const { return _bakedGraphName; }

  /// Set the name of the baked geometry info element.
  void setBakedGeomInfoName(const string &name) { _bakedGeomInfoName = name; }

  /// Return the name of the baked geometry info element.
  const string &getBakedGeomInfoName() const { return _bakedGeomInfoName; }
  ...
  void setTextureFilenameTemplate(const string &filenameTemplate) {
    _textureFilenameTemplate =
        (filenameTemplate.find("$EXTENSION") == string::npos)
            ? filenameTemplate + ".$EXTENSION"
            : filenameTemplate;
  }
  ...
  /// Set the output stream for reporting progress and warnings.  Defaults to
  /// std::cout.
  ...
  std::ostream *getOutputStream() const { return _outputStream; }

  /// Set whether to create a short name for baked images by hashing the baked
  /// image filenames This is useful for file systems which may have a maximum
  /// limit on filename size. By default names are not hashed.
  void setHashImageNames(bool enable) { _hashImageNames = enable; }

  /// Return whether automatic baked texture resolution is set.
  bool getHashImageNames() const { return _hashImageNames; }

  /// Set the minimum texcoords used in texture baking.  Defaults to 0, 0.
  void setTextureSpaceMin(const Vector2 &min) { _textureSpaceMin = min; }

  /// Return the minimum texcoords used in texture baking.
  Vector2 getTextureSpaceMin() const { return _textureSpaceMin; }

  /// Set the maximum texcoords used in texture baking.  Defaults to 1, 1.
  void setTextureSpaceMax(const Vector2 &max) { _textureSpaceMax = max; }

  /// Return the maximum texcoords used in texture baking.
  Vector2 getTextureSpaceMax() const { return _textureSpaceMax; }
  ...
  void bakeShaderInputs(NodePtr material, NodePtr shader, GenContext &context,
                        const string &udim = EMPTY_STRING);

  /// Bake a texture for the given graph output.
  void bakeGraphOutput(OutputPtr output, GenContext &context,
  ...
  DocumentPtr bakeMaterialToDoc(DocumentPtr doc,
                                const FileSearchPath &searchPath,
                                const string &materialPath,
                                const StringVec &udimSet,
                                std::string &documentName);

  /// Bake materials in the given document and write them to disk.  If multiple
  /// documents are written, then the given output filename will be used as a
  /// template.
  void bakeAllMaterials(DocumentPtr doc, const FileSearchPath &searchPath,
                        const FilePath &outputFileName);

  /// Set whether to write a separate document per material when calling
  /// bakeAllMaterials. By default separate documents are written.
...
protected:
  TextureBaker(unsigned int width, unsigned int height,
               Image::BaseType baseType, bool flipSavedImage);

  // Populate file template variable naming map
  StringMap initializeFileTemplateMap(InputPtr input, NodePtr shader,
                                      const string &udim = EMPTY_STRING);

  // Find first occurence of variable in filename from start index onwards
  size_t findVarInTemplate(const string &filename, const string &var,
                           size_t start = 0);
  ...
  DocumentPtr generateNewDocumentFromShader(NodePtr shader,
                                            const StringVec &udimSet);
...
#include <MaterialX/MXRenderTextureBaker.inl>
