...
public:
  ...
  static const string
      THICKNESS;           ///< thickness parameter name of the thin_film_bsdf
  static const string IOR; ///< ior parameter name of the thin_film_bsdf

  static const string
      THIN_FILM_THICKNESS; ///< helper parameter name for transporting thickness
  static const string
      THIN_FILM_IOR; ///< helper parameter name for transporting ior

  static const string EMPTY; ///< the empty string ""
};

/// Helper class to be injected into nodes that need to carry thin film
/// parameters from the thin_film_bsdf through layers and mixers, etc., to the
/// elemental bsdfs that support thin film. Because thin-film can not be layered
/// on any BSDF in MDL, we try to push down the parameters to the nodes that
/// support thin-film.
...
class MX_GENMDL_API ClosureLayerNodeMdl
    : public CarryThinFilmParameters<ShaderNodeImpl> {
public:
  ...
  void emitBsdfOverBsdfFunctionCalls(const ShaderNode &node,
                                     GenContext &context, ShaderStage &stage,
                                     const ShaderGenerator &shadergen,
                                     ShaderNode *top, ShaderNode *base,
                                     ShaderOutput *output) const;

  void emitBsdfOverBsdfFunctionCalls_thinFilm(const ShaderNode &node,
                                              GenContext &context,
                                              ShaderStage &stage,
                                              const ShaderGenerator &shadergen,
                                              ShaderNode *top, ShaderNode *base,
...
/// Layerable BSDF node.
/// Because MDL does not support vertical layering the nodes are transformed in
/// a way that the base node is passed as parameter to the top layer node. Note,
/// not all elemental bsdfs support this kind of transformation.
...
class MX_GENMDL_API ThinFilmReceiverNodeMdl
    : public CarryThinFilmParameters<LayerableNodeMdl> {
...
/// Base class for operators that on bsdfs that need to transport the thin film
/// parameters
class ThinFilmCombineNodeMdl
    : public CarryThinFilmParameters<SourceCodeNodeMdl> {
