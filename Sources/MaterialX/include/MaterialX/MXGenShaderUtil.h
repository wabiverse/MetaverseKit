...
MX_GENSHADER_API vector<TypedElementPtr>
findRenderableMaterialNodes(ConstDocumentPtr doc);

/// Find all renderable elements in the given document, including material nodes
/// if present, or graph outputs of renderable types if no material nodes are
/// found.
/// @param doc Document to examine
/// @return A vector of renderable elements
MX_GENSHADER_API vector<TypedElementPtr>
findRenderableElements(ConstDocumentPtr doc);

/// Given a node input, return the corresponding input within its matching
/// nodedef. The optional target string can be used to guide the selection of
/// nodedef declarations.
...
/// substituation map. Tokens are required to start with '$' and can only
/// consist of alphanumeric characters. The full token name, including '$' and
/// all following alphanumeric character, will be replaced by the corresponding
/// string in the substitution map, if the token exists in the map.
...
MX_GENSHADER_API vector<Vector2>
getUdimCoordinates(const StringVec &udimIdentifiers);

/// Get the UV scale and offset to transform uv coordinates from UDIM uv space
/// to 0..1 space.
MX_GENSHADER_API void
getUdimScaleAndOffset(const vector<Vector2> &udimCoordinates, Vector2 &scaleUV,
                      Vector2 &offsetUV);
...
/// Returns true if there is are any value elements with a given set of
/// attributes either on the starting node or any graph upsstream of that node.
...
//
// These are deprecated wrappers for older versions of the function interfaces
// in this module. Clients using these interfaces should update them to the
// latest API.
//
MX_GENSHADER_API [[deprecated]] void
findRenderableMaterialNodes(ConstDocumentPtr doc,
                            vector<TypedElementPtr> &elements, bool,
                            std::unordered_set<ElementPtr> &);
MX_GENSHADER_API [[deprecated]] void
findRenderableElements(ConstDocumentPtr doc, vector<TypedElementPtr> &elements,
