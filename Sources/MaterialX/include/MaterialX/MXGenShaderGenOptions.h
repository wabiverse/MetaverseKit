...
public:
  GenOptions()
      : shaderInterfaceType(SHADER_INTERFACE_COMPLETE),
        fileTextureVerticalFlip(false), addUpstreamDependencies(true),
        libraryPrefix("libraries"), hwTransparency(false),
        ...
        hwWriteDepthMoments(false), hwShadowMap(false),
        hwAmbientOcclusion(false), hwMaxActiveLightSources(3),
        hwNormalizeUdimTexCoords(false), hwWriteAlbedoTable(false),
        hwImplicitBitangents(true), emitColorTransforms(true) {}
  virtual ~GenOptions() {}
