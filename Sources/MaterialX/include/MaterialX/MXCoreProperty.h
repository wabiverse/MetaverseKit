...
public:
  ...
  bool hasProperty() const { return hasAttribute(PROPERTY_ATTRIBUTE); }

  /// Return the property string of this element.
  const string &getProperty() const { return getAttribute(PROPERTY_ATTRIBUTE); }
...
public:
  ...
  void removeProperty(const string &name) { removeChildOfType<Property>(name); }
  ...
  template <class T>
  PropertyPtr setPropertyValue(const string &name, const T &value,
