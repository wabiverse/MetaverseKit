...
public:
  enum class BaseType { UINT8 = 0, UINT16 = 1, HALF = 2, FLOAT = 3 };

public:
  /// Create an empty image with the given properties.
  static ImagePtr create(unsigned int width, unsigned int height,
  ...
  BaseType getBaseType() const { return _baseType; }
  ...
  void setResourceBuffer(void *buffer) { _resourceBuffer = buffer; }

  /// Return the resource buffer for this image.
  void *getResourceBuffer() const { return _resourceBuffer; }
...
/// Create a horizontal image strip from a vector of images with identical
...
MX_RENDER_API UnsignedIntPair
getMaxDimensions(const vector<ImagePtr> &imageVec);
