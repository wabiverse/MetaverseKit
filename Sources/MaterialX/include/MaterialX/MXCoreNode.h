...
public:
  ...
  size_t getUpstreamEdgeCount() const override { return getInputCount(); }

  /// Given a connecting element (Input or Output) return the NodeDef output
  /// corresponding to the output the element is connected to. This is only
  /// valid if the NodeDef has explicit outputs defined, e.g. multiple outputs
  /// or an explicitly named output. If this is not the case, nullptr is
  /// returned, which implies the node is a standard node with a single implicit
  /// output.
  OutputPtr getNodeDefOutput(ElementPtr connectingElement);

  /// Return a vector of all downstream ports that connect to this node, ordered
  ...
  /// definition. If the input already exists on the node it will just be
  /// returned.
...
public:
  ...
  NodePtr addNode(const string &category, const string &name = EMPTY_STRING,
  ...
  NodePtr addNodeInstance(ConstNodeDefPtr nodeDef,
  ...
  void removeNode(const string &name) { removeChildOfType<Node>(name); }
  ...
  /// shader node.
  NodePtr addMaterialNode(const string &name = EMPTY_STRING,
  ...
  void removeBackdrop(const string &name) { removeChildOfType<Backdrop>(name); }
  ...
  NodePtr addGeomNode(ConstGeomPropDefPtr geomPropDef,
...
public:
  ...
  /// Return a vector of all downstream ports that connect to this graph,
  /// ordered by the names of the port elements.
  vector<PortElementPtr> getDownstreamPorts() const;
...
public:
  ...
  bool hasContainsString() const { return hasAttribute(CONTAINS_ATTRIBUTE); }

  /// Return the contains string for this backdrop.
  string getContainsString() const { return getAttribute(CONTAINS_ATTRIBUTE); }
  ...
  bool hasWidth() const { return hasAttribute(WIDTH_ATTRIBUTE); }

  /// Return the width attribute of the backdrop.
  float getWidth() const { return getTypedAttribute<float>(WIDTH_ATTRIBUTE); }
  ...
  bool hasHeight() const { return hasAttribute(HEIGHT_ATTRIBUTE); }

  /// Return the height attribute of the backdrop.
  float getHeight() const { return getTypedAttribute<float>(HEIGHT_ATTRIBUTE); }
