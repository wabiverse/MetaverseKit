...
string
TextureBaker<Renderer, ShaderGen>::getValueStringFromColor(const Color4 &color,
                                                           const string &type) {
...
template <typename Renderer, typename ShaderGen>
TextureBaker<Renderer, ShaderGen>::TextureBaker(unsigned int width,
                                                unsigned int height,
                                                Image::BaseType baseType,
                                                bool flipSavedImage)
    : Renderer(width, height, baseType), _distanceUnit("meter"),
      _averageImages(false), _optimizeConstants(true),
      _bakedGraphName("NG_baked"), _bakedGeomInfoName("GI_baked"),
      _textureFilenameTemplate(
          "$MATERIAL_$SHADINGMODEL_$INPUT$UDIMPREFIX$UDIM.$EXTENSION"),
      _outputStream(&std::cout), _hashImageNames(false), _textureSpaceMin(0.0f),
      _textureSpaceMax(1.0f), _generator(ShaderGen::create()),
      _permittedOverrides({"$ASSET", "$MATERIAL", "$UDIMPREFIX"}),
      _flipSavedImage(flipSavedImage), _writeDocumentPerMaterial(true),
  ...
  Renderer::_imageHandler =
      Renderer::createImageHandler(StbImageLoader::create());
#if MATERIALX_BUILD_OIIO
...
template <typename Renderer, typename ShaderGen>
size_t TextureBaker<Renderer, ShaderGen>::findVarInTemplate(
    const string &filename, const string &var, size_t start) {
...
template <typename Renderer, typename ShaderGen>
FilePath TextureBaker<Renderer, ShaderGen>::generateTextureFilename(
  ...
  for (auto &pair : filenameTemplateMap) {
    string replacement = (_texTemplateOverrides.count(pair.first))
                             ? _texTemplateOverrides[pair.first]
                             : pair.second;
    replacement =
        (filenameTemplateMap.at("$UDIM").empty() && pair.first == "$UDIMPREFIX")
            ? EMPTY_STRING
            : replacement;

    for (size_t i = 0;
         (i = findVarInTemplate(bakedImageName, pair.first, i)) != string::npos;
         i++) {
...
template <typename Renderer, typename ShaderGen>
StringMap TextureBaker<Renderer, ShaderGen>::initializeFileTemplateMap(
    InputPtr input, NodePtr shader, const string &udim) {
...
template <typename Renderer, typename ShaderGen>
bool TextureBaker<Renderer, ShaderGen>::writeBakedImage(const BakedImage &baked,
                                                        ImagePtr image) {
  if (!Renderer::_imageHandler->saveImage(baked.filename, image,
                                          _flipSavedImage)) {
    if (_outputStream) {
      *_outputStream << "Failed to write baked image: "
                     << baked.filename.asString() << std::endl;
    }
    return false;
  }

  if (_outputStream) {
    *_outputStream << "Wrote baked image: " << baked.filename.asString()
                   << std::endl;
  }
...
void TextureBaker<Renderer, ShaderGen>::bakeShaderInputs(NodePtr material,
  ...
  for (InputPtr input : shader->getInputs()) {
    ...
    if (output && !bakedOutputMap.count(output)) {
      ...
      // When possible, nodes with world-space outputs are applied outside of
      // the baking process.
      ...
      StringMap filenameTemplateMap =
          initializeFileTemplateMap(input, shader, udim);
      bakeGraphOutput(output, context, filenameTemplateMap);
    } else if (bakedOutputMap.count(output)) {
      // When the input shares the same output as a previously baked input, we
      // use the already baked input.
...
template <typename Renderer, typename ShaderGen>
void TextureBaker<Renderer, ShaderGen>::bakeGraphOutput(
    OutputPtr output, GenContext &context,
    const StringMap &filenameTemplateMap) {
  if (!output) {
    return;
  }
...
void TextureBaker<Renderer, ShaderGen>::optimizeBakedTextures(NodePtr shader) {
  ...
  for (auto &pair : _bakedImageMap) {
    ...
    for (BakedImage &baked : pair.second) {
      if (!baked.isUniform ||
          baked.uniformColor != pair.second[0].uniformColor) {
    ...
    for (InputPtr shaderInput : shader->getInputs()) {
      ...
      if (output && _bakedConstantMap.count(output)) {
        ...
        if (input) {
          ...
          string uniformColorString =
              getValueStringFromColor(uniformColor, input->getType());
          string defaultValueString = input->hasValue()
...
template <typename Renderer, typename ShaderGen>
DocumentPtr TextureBaker<Renderer, ShaderGen>::generateNewDocumentFromShader(
    NodePtr shader, const StringVec &udimSet) {
  if (!shader) {
    return nullptr;
  }
  ...
  GeomInfoPtr bakedGeom =
      !udimSet.empty() ? _bakedTextureDoc->addGeomInfo(_bakedGeomInfoName)
                       : nullptr;
  if (bakedGeom) {
  ...
  NodePtr bakedShader = _bakedTextureDoc->addNode(
      shader->getCategory(), shader->getName() + BAKED_POSTFIX,
      shader->getType());

  // Optionally create a material node, connecting it to the new shader node.
  if (_material) {
    string materialName = (_texTemplateOverrides.count("$MATERIAL"))
                              ? _texTemplateOverrides["$MATERIAL"]
                              : _material->getName();
    NodePtr bakedMaterial = _bakedTextureDoc->addNode(
        _material->getCategory(), materialName + BAKED_POSTFIX,
        _material->getType());
    for (auto sourceMaterialInput : _material->getInputs()) {
      ...
      if (upstreamShader &&
          (upstreamShader->getNamePath() == shader->getNamePath())) {
        InputPtr bakedMaterialInput =
            bakedMaterial->getInput(sourceMaterialInputName);
        if (!bakedMaterialInput) {
          bakedMaterialInput = bakedMaterial->addInput(
              sourceMaterialInputName, sourceMaterialInput->getType());
        }
  ...
  for (ValueElementPtr valueElem : shader->getChildrenOfType<ValueElement>()) {
    ...
    if (output && _bakedConstantMap.count(output) &&
        _bakedConstantMap[output].isDefault) {
      continue;
    }
    ...
    if (output) {
      ...
      if (_optimizeConstants && _bakedConstantMap.count(output)) {
        Color4 uniformColor = _bakedConstantMap[output].color;
        string uniformColorString =
            getValueStringFromColor(uniformColor, bakedInput->getType());
        bakedInput->setValueString(uniformColorString);
        if (bakedInput->getType() == "color3" ||
            bakedInput->getType() == "color4") {
      ...
      if (!_bakedImageMap.empty()) {
        // Add the image node.
        NodePtr bakedImage = bakedNodeGraph->addNode(
            "image", sourceName + BAKED_POSTFIX, sourceType);
        InputPtr input = bakedImage->addInput("file", "filename");
        StringMap filenameTemplateMap = initializeFileTemplateMap(
            bakedInput, shader, udimSet.empty() ? EMPTY_STRING : UDIM_TOKEN);
        input->setValueString(generateTextureFilename(filenameTemplateMap));

        // Reconstruct any world-space nodes that were excluded from the baking
        // process.
        ...
        if (worldSpacePair != _worldSpaceNodes.end()) {
          ...
          if (origWorldSpaceNode) {
            NodePtr newWorldSpaceNode = bakedNodeGraph->addNode(
                origWorldSpaceNode->getCategory(),
                sourceName + BAKED_POSTFIX + "_map", sourceType);
        ...
        OutputPtr bakedOutput =
            bakedNodeGraph->addOutput(sourceName + "_output", sourceType);
  ...
  ImagePtr uniformImage =
      createUniformImage(4, 4, 4, Renderer::_baseType, Color4());
...
template <typename Renderer, typename ShaderGen>
DocumentPtr TextureBaker<Renderer, ShaderGen>::bakeMaterialToDoc(
    DocumentPtr doc, const FileSearchPath &searchPath,
    const string &materialPath, const StringVec &udimSet,
    string &documentName) {
  if (_outputStream) {
  ...
  DefaultColorManagementSystemPtr cms = DefaultColorManagementSystem::create(
...
template <typename Renderer, typename ShaderGen>
void TextureBaker<Renderer, ShaderGen>::bakeAllMaterials(
    DocumentPtr doc, const FileSearchPath &searchPath,
  ...
  for (size_t i = 0; i < renderableMaterials.size(); i++) {
    ...
    string documentName;
    DocumentPtr bakedMaterialDoc = bakeMaterialToDoc(
        doc, searchPath, element->getNamePath(), udimSet, documentName);
  ...
  if (_writeDocumentPerMaterial) {
      ...
      if (bakedDocuments[i].second) {
        ...
        if (bakedDocuments.size() > 1) {
          ...
          string filenameSeparator =
              writeFilename.isDirectory() ? EMPTY_STRING : "_";
          writeFilename =
              FilePath(writeFilename.asString() + filenameSeparator +
                       bakedDocuments[i].first + "." + extension);
        }

        writeToXmlFile(bakedDocuments[i].second, writeFilename);
        if (_outputStream) {
          *_outputStream << "Wrote baked document: " << writeFilename.asString()
                         << std::endl;
        }
      }
    }
    ...
    if (_outputStream) {
      *_outputStream << "Wrote baked document: " << outputFilename.asString()
                     << std::endl;
    }
  }
}
...
template <typename Renderer, typename ShaderGen>
void TextureBaker<Renderer, ShaderGen>::setupUnitSystem(
    DocumentPtr unitDefinitions) {
  UnitTypeDefPtr distanceTypeDef =
      unitDefinitions ? unitDefinitions->getUnitTypeDef("distance") : nullptr;
  UnitTypeDefPtr angleTypeDef =
      unitDefinitions ? unitDefinitions->getUnitTypeDef("angle") : nullptr;
  ...
  registry->addUnitConverter(distanceTypeDef,
                             LinearUnitConverter::create(distanceTypeDef));
  registry->addUnitConverter(angleTypeDef,
                             LinearUnitConverter::create(angleTypeDef));
