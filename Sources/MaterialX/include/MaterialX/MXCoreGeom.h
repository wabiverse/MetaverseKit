...
public:
  GeomPath() : _empty(true) {}
  ~GeomPath() {}

  bool operator==(const GeomPath &rhs) const {
    return _vec == rhs._vec && _empty == rhs._empty;
  }
  bool operator!=(const GeomPath &rhs) const { return !(*this == rhs); }

  /// Construct a path from a geometry name string.
  explicit GeomPath(const string &geom)
      : _vec(splitString(geom, GEOM_PATH_SEPARATOR)), _empty(geom.empty()) {}
  ...
  bool isUniversal() const { return _vec.empty() && !_empty; }
...
public:
  ...
  void setGeom(const string &geom) { setAttribute(GEOM_ATTRIBUTE, geom); }

  /// Return true if this element has a geometry string.
  bool hasGeom() const { return hasAttribute(GEOM_ATTRIBUTE); }

  /// Return the geometry string of this element.
  const string &getGeom() const { return getAttribute(GEOM_ATTRIBUTE); }
  ...
  string getActiveGeom() const {
    return hasGeom() ? createStringResolver()->resolve(getGeom(),
...
public:
  ...
  void removeGeomProp(const string &name) { removeChildOfType<GeomProp>(name); }
  ...
  template <class T>
  GeomPropPtr setGeomPropValue(const string &name, const T &value,
...
public:
  ...
  bool hasGeomProp() const { return hasAttribute(GEOM_PROP_ATTRIBUTE); }
  ...
  void setSpace(const string &space) { setAttribute(SPACE_ATTRIBUTE, space); }

  /// Return true if this element has a geometric space string.
  bool hasSpace() const { return hasAttribute(SPACE_ATTRIBUTE); }

  /// Return the geometric space string of this element.
  const string &getSpace() const { return getAttribute(SPACE_ATTRIBUTE); }
  ...
  void setIndex(const string &space) { setAttribute(INDEX_ATTRIBUTE, space); }

  /// Return true if this element has an index string.
  bool hasIndex() const { return hasAttribute(INDEX_ATTRIBUTE); }

  /// Return the index string of this element.
  const string &getIndex() const { return getAttribute(INDEX_ATTRIBUTE); }
...
public:
  ...
  bool hasIncludeGeom() const { return hasAttribute(INCLUDE_GEOM_ATTRIBUTE); }
  ...
  string getActiveIncludeGeom() const {
    return hasIncludeGeom() ? createStringResolver()->resolve(
                                  getIncludeGeom(), GEOMNAME_TYPE_STRING)
                            : EMPTY_STRING;
  }
  ...
  bool hasExcludeGeom() const { return hasAttribute(EXCLUDE_GEOM_ATTRIBUTE); }
  ...
  string getActiveExcludeGeom() const {
    return hasExcludeGeom() ? createStringResolver()->resolve(
                                  getExcludeGeom(), GEOMNAME_TYPE_STRING)
                            : EMPTY_STRING;
  }
...
template <class T>
GeomPropPtr GeomInfo::setGeomPropValue(const string &name, const T &value,
                                       const string &type) {
...
MX_CORE_API bool geomStringsMatch(const string &geom1, const string &geom2,
                                  bool contains = false);
