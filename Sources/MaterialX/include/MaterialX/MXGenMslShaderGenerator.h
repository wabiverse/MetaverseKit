...
public:
  ...
  static ShaderGeneratorPtr create() {
    return std::make_shared<MslShaderGenerator>();
  }

  /// Generate a shader starting from the given element, translating
  /// the element and all dependencies upstream into shader code.
  ShaderPtr generate(const string &name, ElementPtr element,
  ...
  void emitVariableDeclaration(const ShaderPort *variable,
                               const string &qualifier, GenContext &context,
                               ShaderStage &stage,
...
protected:
  virtual void emitVertexStage(const ShaderGraph &graph, GenContext &context,
                               ShaderStage &stage) const;
  virtual void emitPixelStage(const ShaderGraph &graph, GenContext &context,
                              ShaderStage &stage) const;

  virtual void emitMetalTextureClass(GenContext &context,
                                     ShaderStage &stage) const;
  ...
  virtual void emitMathMatrixScalarMathOperators(GenContext &context,
                                                 ShaderStage &stage) const;
  virtual void MetalizeGeneratedShader(ShaderStage &shaderStage) const;

  void
  emitConstantBufferDeclarations(GenContext &context,
  ...
  virtual HwResourceBindingContextPtr
  getResourceBindingContext(GenContext &context) const;
  ...
  /// then lighting is assumed to be required. Derived classes can override this
  /// logic.
  virtual bool requiresLighting(const ShaderGraph &graph) const;

  /// Emit specular environment lookup code
  virtual void emitSpecularEnvironment(GenContext &context,
                                       ShaderStage &stage) const;

  /// Emit transmission rendering code
  virtual void emitTransmissionRender(GenContext &context,
                                      ShaderStage &stage) const;

  /// Emit function definitions for lighting code
  virtual void emitLightFunctionDefinitions(const ShaderGraph &graph,
                                            GenContext &context,
                                            ShaderStage &stage) const;
