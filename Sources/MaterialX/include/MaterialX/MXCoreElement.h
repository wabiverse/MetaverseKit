...
protected:
  Element(ElementPtr parent, const string &category, const string &name)
      : _category(category), _name(name), _parent(parent),
...
public:
  ...
  bool hasFilePrefix() const { return hasAttribute(FILE_PREFIX_ATTRIBUTE); }
  ...
  bool hasGeomPrefix() const { return hasAttribute(GEOM_PREFIX_ATTRIBUTE); }
  ...
  bool hasColorSpace() const { return hasAttribute(COLOR_SPACE_ATTRIBUTE); }
  ...
  bool hasInheritString() const { return hasAttribute(INHERIT_ATTRIBUTE); }
  ...
  ElementPtr getInheritsFrom() const {
    return hasInheritString()
               ? resolveNameReference<Element>(getInheritString())
               : nullptr;
  }
  ...
  bool hasNamespace() const { return hasAttribute(NAMESPACE_ATTRIBUTE); }
  ...
  void setDocString(const string &doc) { setAttribute(DOC_ATTRIBUTE, doc); }

  /// Return the documentation string of this element
  string getDocString() const { return getAttribute(DOC_ATTRIBUTE); }
  ...
  const vector<ElementPtr> &getChildren() const { return _childOrder; }
  ...
  template <class T>
  vector<shared_ptr<T>>
  getChildrenOfType(const string &category = EMPTY_STRING) const {
  ...
  const StringVec &getAttributeNames() const { return _attributeOrder; }
  ...
  ElementPtr getSelf() { return shared_from_this(); }

  /// Return our self pointer.
  ConstElementPtr getSelf() const { return shared_from_this(); }

  /// Return our parent element.
  ElementPtr getParent() { return _parent.lock(); }

  /// Return our parent element.
  ConstElementPtr getParent() const { return _parent.lock(); }
  ...
  /// @code
  /// for (mx::TreeIterator it = inputElem->traverseTree().begin(); it !=
  /// mx::TreeIterator::end(); ++it)
  /// {
  ///     mx::ElementPtr elem = it.getElement();
  ///     cout << elem->asString() << " at depth " << it.getElementDepth() <<
  ///     endl;
  /// }
  /// @endcode
  ...
  ///     cout << upElem->asString() << " lies upstream from " <<
  ///     downElem->asString() << endl;
  /// }
  /// @endcode
  /// Example usage with an explicit iterator:
  /// @code
  /// for (mx::GraphIterator it = inputElem->traverseGraph().begin(); it !=
  /// mx::GraphIterator::end(); ++it)
  /// {
  ...
  virtual size_t getUpstreamEdgeCount() const { return 0; }
  ...
  /// {
  ///     if (derivedElem)
  ///         cout << derivedElem->asString() << " inherits from " <<
  ///         elem->asString() << endl;
  ///     derivedElem = elem;
  /// }
  /// @endcode
  ...
  ///    references.
  void setSourceUri(const string &sourceUri) { _sourceUri = sourceUri; }

  /// Return true if this element has a source URI.
  bool hasSourceUri() const { return !_sourceUri.empty(); }

  /// Return the element's source URI.
  const string &getSourceUri() const { return _sourceUri; }
  ...
  StringResolverPtr
  createStringResolver(const string &geom = EMPTY_STRING) const;
...
protected:
  ...
  // taking the namespace at the scope of this element into account.  If no
  // parent is provided, then the root scope of the document is used.
  template <class T>
  shared_ptr<T> resolveNameReference(const string &name,
                                     ConstElementPtr parent = nullptr) const {
  ...
  void validateRequire(bool expression, bool &res, string *message,
                       const string &errorDesc) const;
...
private:
  template <class T>
  static ElementPtr createElement(ElementPtr parent, const string &name) {
...
public:
  ...
  void setType(const string &type) { setAttribute(TYPE_ATTRIBUTE, type); }

  /// Return true if the given element has a type string.
  bool hasType() const { return hasAttribute(TYPE_ATTRIBUTE); }

  /// Return the element's type string.
  virtual const string &getType() const { return getAttribute(TYPE_ATTRIBUTE); }
...
public:
  ...
  bool hasValueString() const { return hasAttribute(VALUE_ATTRIBUTE); }

  /// Get the value string of a element.
  const string &getValueString() const { return getAttribute(VALUE_ATTRIBUTE); }
  ...
  bool hasValue() const { return hasAttribute(VALUE_ATTRIBUTE); }
  ...
  ValuePtr getResolvedValue(StringResolverPtr resolver = nullptr) const {
    if (!hasValue())
      return ValuePtr();
    return Value::createValueFromStrings(getResolvedValueString(resolver),
                                         getType());
  }
  ...
  void setUnit(const string &unit) { setAttribute(UNIT_ATTRIBUTE, unit); }

  /// Return true if the given element has a unit string.
  bool hasUnit() const { return hasAttribute(UNIT_ATTRIBUTE); }

  /// Return the unit string of an element.
  const string &getUnit() const { return getAttribute(UNIT_ATTRIBUTE); }
  ...
  bool hasUnitType() const { return hasAttribute(UNITTYPE_ATTRIBUTE); }

  /// Return the unit type of an element.
  const string &getUnitType() const { return getAttribute(UNITTYPE_ATTRIBUTE); }
...
/// @class CommentElement
/// An element representing a block of descriptive text within a document, which
/// will be stored a comment when the document is written out.
///
...
///
/// A StringResolver may be constructed through the
/// Element::createStringResolver method, which initializes it in the context of
/// a specific element, geometry, and material.
///
...
public:
  ...
  void setFilePrefix(const string &filePrefix) { _filePrefix = filePrefix; }

  /// Return the file prefix for this context.
  const string &getFilePrefix() const { return _filePrefix; }
  ...
  void setGeomPrefix(const string &geomPrefix) { _geomPrefix = geomPrefix; }

  /// Return the geom prefix for this context.
  const string &getGeomPrefix() const { return _geomPrefix; }
  ...
  const StringMap &getFilenameSubstitutions() const { return _filenameMap; }
  ...
  const StringMap &getGeomNameSubstitutions() const { return _geomNameMap; }
