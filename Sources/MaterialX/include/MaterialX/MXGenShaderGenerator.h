...
/// Derived classes should use DECLARE_SHADER_GENERATOR /
/// DEFINE_SHADER_GENERATOR in their declaration / definition, and register with
/// the Registry class.
...
public:
  ...
  virtual void
  emitScopeBegin(ShaderStage &stage,
                 Syntax::Punctuation punc = Syntax::CURLY_BRACKETS) const;

  /// End the current scope.
  virtual void emitScopeEnd(ShaderStage &stage, bool semicolon = false,
  ...
  virtual void emitLine(const string &str, ShaderStage &stage,
                        bool semicolon = true) const;

  /// Add a single line code comment.
  virtual void emitComment(const string &str, ShaderStage &stage) const;

  /// Add a block of code.
  virtual void emitBlock(const string &str, const FilePath &sourceFilename,
                         GenContext &context, ShaderStage &stage) const;

  /// Add the contents of a standard library include file if not already
  /// present. The library file prefix of the given context, if any, will be
  /// prepended to the given filename.
  virtual void emitLibraryInclude(const FilePath &filename, GenContext &context,
  ...
  virtual void emitFunctionDefinitions(const ShaderGraph &graph,
                                       GenContext &context,
                                       ShaderStage &stage) const;

  /// Add the function call for a single node.
  virtual void emitFunctionCall(const ShaderNode &node, GenContext &context,
                                ShaderStage &stage) const;
  [[deprecated]] virtual void emitFunctionCall(const ShaderNode &node,
                                               GenContext &context,
                                               ShaderStage &stage,
                                               bool checkScope) const;

  /// Add all function calls for a graph. If a classification mask is given only
  /// functions for nodes matching this classification will be emitted.
  virtual void emitFunctionCalls(const ShaderGraph &graph, GenContext &context,
                                 ShaderStage &stage,
                                 uint32_t classification = 0u) const;

  /// Add function calls for nodes connected directly upstream from the given
  /// node. If a classification mask is given only functions for nodes matching
  /// this classification will be emitted.
  virtual void emitDependentFunctionCalls(const ShaderNode &node,
                                          GenContext &context,
                                          ShaderStage &stage,
                                          uint32_t classification = 0u) const;

  /// Emit code for starting a new function body.
  virtual void emitFunctionBodyBegin(
      const ShaderNode &node, GenContext &context, ShaderStage &stage,
      Syntax::Punctuation punc = Syntax::CURLY_BRACKETS) const;

  /// Emit code for ending a function body.
  virtual void emitFunctionBodyEnd(const ShaderNode &node, GenContext &context,
                                   ShaderStage &stage) const;

  /// Emit type definitions for all data types that needs it.
  virtual void emitTypeDefinitions(GenContext &context,
                                   ShaderStage &stage) const;

  /// Emit the connected variable name for an input,
  /// or constant value if the port is not connected
  virtual void emitInput(const ShaderInput *input, GenContext &context,
                         ShaderStage &stage) const;

  /// Emit the output variable name for an output, optionally including it's
  /// type and default value assignment.
  virtual void emitOutput(const ShaderOutput *output, bool includeType,
                          bool assignValue, GenContext &context,
                          ShaderStage &stage) const;

  /// Emit definitions for all shader variables in a block.
  /// @param block Block to emit.
  /// @param qualifier Optional qualifier to add before the variable
  /// declaration.
  ...
  virtual void emitVariableDeclarations(const VariableBlock &block,
  ...
  virtual void emitVariableDeclaration(const ShaderPort *variable,
                                       const string &qualifier,
                                       GenContext &context, ShaderStage &stage,
                                       bool assignValue = true) const;

  /// Return the closure contexts defined for the given node.
  virtual void getClosureContexts(const ShaderNode &node,
                                  vector<ClosureContext *> &cct) const;

  /// Return the result of an upstream connection or value for an input.
  virtual string getUpstreamResult(const ShaderInput *input,
                                   GenContext &context) const;

  /// Return the syntax object for the language used by the code generator
  const Syntax &getSyntax() const { return *_syntax; }

  /// Register a shader node implementation for a given implementation element
  /// name
  void registerImplementation(const string &name,
                              CreatorFunction<ShaderNodeImpl> creator);

  /// Register a shader node implementation for a given set of implementation
  /// element names
  void registerImplementation(const StringVec &nameVec,
                              CreatorFunction<ShaderNodeImpl> creator);

  /// Determine if a shader node implementation has been registered for a given
  ...
  virtual ShaderNodeImplPtr getImplementation(const NodeDef &nodedef,
                                              GenContext &context) const;

  /// Sets the color management system
  void
  setColorManagementSystem(ColorManagementSystemPtr colorManagementSystem) {
  ...
  void setUnitSystem(UnitSystemPtr unitSystem) { _unitSystem = unitSystem; }

  /// Returns the unit system
  UnitSystemPtr getUnitSystem() const { return _unitSystem; }

  /// Return the map of token substitutions used by the generator.
  const StringMap &getTokenSubstitutions() const { return _tokenSubstitutions; }
  ...
  /// "uifolder", "uimin", "uimax", etc. But it is also extendable by defining
  /// custom attributes using AttributeDefs. Any AttributeDef in the given
  /// document with exportable="true" will be exported as shader metadata when
  /// found on nodes during shader generation. Derived shader generators may
  /// override this method to change the registration. Applications must
  /// explicitly call this method before shader generation to enable export of
  /// metadata.
  virtual void registerShaderMetadata(const DocumentPtr &doc,
                                      GenContext &context) const;
...
protected:
  ...
  void createVariables(ShaderGraphPtr graph, GenContext &context,
                       Shader &shader) const;
