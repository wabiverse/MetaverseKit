...
public:
  ...
  /// Register names that are reserved words not to be used by a code generator
  /// when naming variables and functions. Keywords, types, built-in functions
  /// etc. should be added to this set. Multiple calls will add to the internal
  /// set of names.
  ...
  /// Returns a mapping from disallowed tokens to replacement strings for this
  /// language syntax.
  ...
  const string &getDefaultValue(const TypeDesc *type,
                                bool uniform = false) const;

  /// Returns the value string for a given type and value object
  virtual string getValue(const TypeDesc *type, const Value &value,
                          bool uniform = false) const;
  ...
  virtual string getSwizzledVariable(const string &srcName,
                                     const TypeDesc *srcType,
                                     const string &channels,
                                     const TypeDesc *dstType) const;

  /// Get swizzled value
  virtual ValuePtr getSwizzledValue(ValuePtr value, const TypeDesc *srcType,
                                    const string &channels,
                                    const TypeDesc *dstType) const;

  /// Returns a type qualifier to be used when declaring types for input
  /// variables. Default implementation returns empty string and derived syntax
  /// classes should override this method.
  virtual const string &getInputQualifier() const { return EMPTY_STRING; };

  /// Returns a type qualifier to be used when declaring types for output
  /// variables. Default implementation returns empty string and derived syntax
  /// classes should override this method.
  ...
  /// Default implementation returns empty string and derived syntax classes
  ...
  virtual string getArrayVariableSuffix(const TypeDesc *type,
  ...
  /// The method is used for naming variables (inputs and outputs) in generated
  /// code. Derived classes can override this method to have a custom naming
  /// strategy. Default implementation adds a number suffix, or increases an
  /// existing number suffix, on the name string if there is a name collision.
  virtual string getVariableName(const string &name, const TypeDesc *type,
                                 IdentifierMap &identifiers) const;

  /// Given an input specification attempt to remap this to an enumeration which
  /// is accepted by the shader generator. The enumeration may be converted to a
  ...
  virtual bool
  remapEnumeration(const string &value, const TypeDesc *type,
...
public:
  ...
  /// Returns a type definition if needed to define the type in the target
  /// language.
...
protected:
  /// Protected constructor
  TypeSyntax(const string &name, const string &defaultValue,
             const string &uniformDefaultValue, const string &typeAlias,
             const string &typeDefinition, const StringVec &members);
...
public:
  ScalarTypeSyntax(const string &name, const string &defaultValue,
...
public:
  StringTypeSyntax(const string &name, const string &defaultValue,
                   const string &uniformDefaultValue,
                   const string &typeAlias = EMPTY_STRING,
                   const string &typeDefinition = EMPTY_STRING);
...
public:
  AggregateTypeSyntax(const string &name, const string &defaultValue,
                      const string &uniformDefaultValue,
                      const string &typeAlias = EMPTY_STRING,
                      const string &typeDefinition = EMPTY_STRING,
