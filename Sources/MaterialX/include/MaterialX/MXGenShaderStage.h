...
/// A standard function predicate taking an ShaderPort pointer and returning a
/// boolean.
...
public:
  ...
  /// @param shouldWiden When false, an exception is thrown if the type of the
  /// existing port with
  ///   the same name does not match the requested type. When true, the types
  ///   can mismatch, and the type of any existing port is widened to match the
  ///   requested type when necessary.
  /// @return A new shader port, or a pre-existing shader port with the same
  /// name.
  ShaderPort *add(const TypeDesc *type, const string &name,
                  ValuePtr value = nullptr, bool shouldWiden = false);
...
public:
  ...
  VariableBlockPtr createUniformBlock(const string &name,
                                      const string &instance = EMPTY_STRING);

  /// Create a new input variable block.
  VariableBlockPtr createInputBlock(const string &name,
                                    const string &instance = EMPTY_STRING);

  /// Create a new output variable block.
  VariableBlockPtr createOutputBlock(const string &name,
                                     const string &instance = EMPTY_STRING);
  ...
  const VariableBlockMap &getInputBlocks() const { return _inputs; }

  /// Return a map of all output blocks.
  const VariableBlockMap &getOutputBlocks() const { return _outputs; }

  /// Return a set of all include files
  const StringSet &getIncludes() const { return _includes; }

  /// Return a set of all source dependencies
  const StringSet &getSourceDependencies() const { return _sourceDependencies; }
  ...
  /// Add a block of code.
  void addBlock(const string &str, const FilePath &sourceFilename,
                GenContext &context);

  /// Add the contents of an include file if not already present.
  void addInclude(const FilePath &includeFilename,
                  const FilePath &sourceFilename, GenContext &context);
  ...
  /// This will register the function as being called in the current scope, and
  /// code for the function call will be added to the stage. If emitCode is set
  /// to false the code for the function call will be omitted.
  void addFunctionCall(const ShaderNode &node, GenContext &context,
                       bool emitCode = true);

  /// Return true if the function for the given node has been emitted in the
  /// current scope.
...
inline ShaderPort *addStageUniform(const string &block, const TypeDesc *type,
                                   const string &name, ShaderStage &stage) {
...
inline ShaderPort *addStageInput(const string &block, const TypeDesc *type,
                                 const string &name, ShaderStage &stage,
...
inline ShaderPort *addStageOutput(const string &block, const TypeDesc *type,
                                  const string &name, ShaderStage &stage,
                                  bool shouldWiden = false) {
...
inline void addStageConnectorBlock(const string &block, const string &instance,
                                   ShaderStage &from, ShaderStage &to) {
...
inline void addStageConnector(const string &block, const TypeDesc *type,
                              const string &name, ShaderStage &from,
                              ShaderStage &to, bool shouldWiden = false) {
