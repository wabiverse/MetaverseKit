...
public:
  FilePath() : _type(TypeRelative) {}
  ~FilePath() {}

  bool operator==(const FilePath &rhs) const {
    return _vec == rhs._vec && _type == rhs._type;
  }
  bool operator!=(const FilePath &rhs) const { return !(*this == rhs); }

  /// @name Syntactic Operations
  /// @{

  /// Construct a path from a standard string.
  FilePath(const string &str) { assign(str); }

  /// Construct a path from a C-style string.
  FilePath(const char *str) { assign(str ? string(str) : EMPTY_STRING); }
  ...
  bool isAbsolute() const { return _type != TypeRelative; }
  ...
  void addExtension(const string &ext) { assign(asString() + "." + ext); }
  ...
  string operator[](size_t index) { return _vec[index]; }

  /// Return the const string at the given index.
  const string &operator[](size_t index) const { return _vec[index]; }
  ...
  /// Return a vector of all files in the given directory with the given
  /// extension.
...
public:
  ...
  void append(const FilePath &path) { _paths.push_back(path); }
  ...
  void prepend(const FilePath &path) { _paths.insert(_paths.begin(), path); }
  ...
  FilePath &operator[](size_t index) { return _paths[index]; }

  /// Return the const path at the given index.
  const FilePath &operator[](size_t index) const { return _paths[index]; }
...
MX_FORMAT_API FileSearchPath
getEnvironmentPath(const string &sep = PATH_LIST_SEPARATOR);
