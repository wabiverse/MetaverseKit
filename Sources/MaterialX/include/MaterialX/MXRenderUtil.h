...
MX_RENDER_API ShaderPtr createShader(const string &shaderName,
                                     GenContext &context, ElementPtr elem);

/// Create a shader with a constant color output, using the given standard
...
/// Create a blur shader, using the given standard libraries for code
/// generation.
...
MX_RENDER_API unsigned int getUIProperties(InputPtr input, const string &target,
...
/// Utility to group UI properties items based on Element group name from a
/// VariableBlock. Returns a list of named and unnamed groups.
