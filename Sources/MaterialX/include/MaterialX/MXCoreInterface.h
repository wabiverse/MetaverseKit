...
public:
  ...
  bool hasNodeName() const { return hasAttribute(NODE_NAME_ATTRIBUTE); }
  ...
  bool hasNodeGraphString() const { return hasAttribute(NODE_GRAPH_ATTRIBUTE); }
  ...
  bool hasOutputString() const { return hasAttribute(OUTPUT_ATTRIBUTE); }
  ...
  bool hasChannels() const { return hasAttribute(CHANNELS_ATTRIBUTE); }

  /// Return the channels string of this element.
  const string &getChannels() const { return getAttribute(CHANNELS_ATTRIBUTE); }

  /// Return true if the given channels characters are valid for the given
  /// source type string.
  static bool validChannelsCharacters(const string &channels,
  ...
  static bool validChannelsString(const string &channels,
                                  const string &sourceType,
...
public:
  ...
  size_t getUpstreamEdgeCount() const override { return 1; }
...
protected:
  InterfaceElement(ElementPtr parent, const string &category,
                   const string &name)
      : TypedElement(parent, category, name), _inputCount(0), _outputCount(0) {}
...
public:
  ...
  bool hasNodeDefString() const { return hasAttribute(NODE_DEF_ATTRIBUTE); }
  ...
  vector<InputPtr> getInputs() const { return getChildrenOfType<Input>(); }

  /// Return the number of Input elements.
  size_t getInputCount() const { return _inputCount; }

  /// Remove the Input, if any, with the given name.
  void removeInput(const string &name) { removeChildOfType<Input>(name); }
  ...
  vector<OutputPtr> getOutputs() const { return getChildrenOfType<Output>(); }

  /// Return the number of Output elements.
  size_t getOutputCount() const { return _outputCount; }

  /// Remove the Output, if any, with the given name.
  void removeOutput(const string &name) { removeChildOfType<Output>(name); }
  ...
  vector<TokenPtr> getTokens() const { return getChildrenOfType<Token>(); }

  /// Remove the Token, if any, with the given name.
  void removeToken(const string &name) { removeChildOfType<Token>(name); }
  ...
  template <class T>
  InputPtr setInputValue(const string &name, const T &value,
  ...
  ValuePtr getInputValue(const string &name,
                         const string &target = EMPTY_STRING) const;
  ...
  bool hasTarget() const { return hasAttribute(TARGET_ATTRIBUTE); }

  /// Return the target string of this interface.
  const string &getTarget() const { return getAttribute(TARGET_ATTRIBUTE); }
  ...
  bool hasVersionString() const { return hasAttribute(VERSION_ATTRIBUTE); }
  ...
  virtual ConstInterfaceElementPtr
  getDeclaration(const string &target = EMPTY_STRING) const;
  ...
  bool hasExactInputMatch(ConstInterfaceElementPtr declaration,
...
template <class T>
InputPtr InterfaceElement::setInputValue(const string &name, const T &value,
                                         const string &type) {
