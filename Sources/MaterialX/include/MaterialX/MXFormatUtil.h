...
/// Read the given file and return a string containing its contents; if the read
/// is not successful, then the empty string is returned.
...
/// Scans for all documents under a root path and returns documents which can be
/// loaded
MX_FORMAT_API void
loadDocuments(const FilePath &rootPath, const FileSearchPath &searchPath,
...
MX_FORMAT_API void
loadLibrary(const FilePath &file, DocumentPtr doc,
...
MX_FORMAT_API StringSet loadLibraries(
    const FilePathVec &libraryFolders, const FileSearchPath &searchPath,
    DocumentPtr doc, const StringSet &excludeFiles = StringSet(),
    const XmlReadOptions *readOptions = nullptr);
...
MX_FORMAT_API void
flattenFilenames(DocumentPtr doc,
                 const FileSearchPath &searchPath = FileSearchPath(),
                 StringResolverPtr customResolver = nullptr);

/// Return a file search path containing the parent folder of each source URI in
/// the given document.
