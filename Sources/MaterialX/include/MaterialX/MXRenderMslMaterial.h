...
public:
  ...
  static MslMaterialPtr create() { return std::make_shared<MslMaterial>(); }
  ...
  MslProgramPtr getProgram() const { return _glProgram; }
  ...
  /// Bind a single image.
  ImagePtr
  bindImage(const FilePath &filePath, const std::string &uniformName,
  ...
  void modifyUniform(const std::string &path, ConstValuePtr value,
                     std::string valueString = EMPTY_STRING) override;

  void prepareUsedResources(CameraPtr cam, GeometryHandlerPtr geometryHandler,
                            ImageHandlerPtr imageHandler,
