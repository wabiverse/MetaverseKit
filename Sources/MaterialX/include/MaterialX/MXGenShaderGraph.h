...
public:
  /// Constructor.
  ShaderGraph(const ShaderGraph *parent, const string &name,
              ConstDocumentPtr document, const StringSet &reservedWords);
  ...
  static ShaderGraphPtr create(const ShaderGraph *parent, const string &name,
                               ElementPtr element, GenContext &context);

  /// Create a new shader graph from a nodegraph.
  static ShaderGraphPtr create(const ShaderGraph *parent,
                               const NodeGraph &nodeGraph, GenContext &context);
  ...
  ShaderGraphInputSocket *addInputSocket(const string &name,
                                         const TypeDesc *type);
  ShaderGraphOutputSocket *addOutputSocket(const string &name,
                                           const TypeDesc *type);

  /// Add a default geometric node and connect to the given input.
  void addDefaultGeomNode(ShaderInput *input, const GeomPropDef &geomprop,
                          GenContext &context);
...
protected:
  static ShaderGraphPtr createSurfaceShader(const string &name,
                                            const ShaderGraph *parent,
                                            NodePtr node, GenContext &context,
                                            ElementPtr &root);

  /// Create node connections corresponding to the connection between a pair of
  /// elements.
  ...
  /// @param connectingElement If non-null, specifies the element on on the
  /// downstream node to connect to.
  ...
  void createConnectedNodes(const ElementPtr &downstreamElement,
                            ...
                            ElementPtr connectingElement, GenContext &context);
  ...
  void addColorTransformNode(ShaderInput *input,
                             const ColorSpaceTransform &transform,
                             GenContext &context);

  /// Add a color transform node and connect to the given output.
  void addColorTransformNode(ShaderOutput *output,
                             const ColorSpaceTransform &transform,
                             GenContext &context);

  /// Add a unit transform node and connect to the given input.
  void addUnitTransformNode(ShaderInput *input, const UnitTransform &transform,
                            GenContext &context);

  /// Add a unit transform node and connect to the given output.
  void addUnitTransformNode(ShaderOutput *output,
  ...
  void bypass(GenContext &context, ShaderNode *node, size_t inputIndex,
  ...
  /// Populates the input or output color transform map if the provided
  /// input/parameter has a color space attribute and has a type of color3 or
  /// color4.
  string
  populateColorTransformMap(ColorManagementSystemPtr colorManagementSystem,
                            ShaderPort *shaderPort, ValueElementPtr element,
                            const string &targetColorSpace, bool asInput);

  /// Populates the appropriate unit transform map if the provided
  /// input/parameter or output has a unit attribute and is of the supported
  /// type
  void populateUnitTransformMap(UnitSystemPtr unitSystem,
                                ShaderPort *shaderPort, ValueElementPtr element,
                                const string &targetUnitSpace, bool asInput);
  ...
  std::unordered_map<ShaderInput *, ColorSpaceTransform>
  ...
  std::unordered_map<ShaderOutput *, ColorSpaceTransform>
...
public:
  ...
  bool operator==(const ShaderGraphEdgeIterator &rhs) const {
    return _upstream == rhs._upstream && _downstream == rhs._downstream &&
           _stack == rhs._stack;
  }
  bool operator!=(const ShaderGraphEdgeIterator &rhs) const {
    return !(*this == rhs);
  ...
  ShaderGraphEdgeIterator &begin() { return *this; }
