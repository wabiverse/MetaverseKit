...
public:
  ...
  Quaternion operator*(const Quaternion &q) const {
    return {_arr[0] * q._arr[3] + _arr[3] * q._arr[0] + _arr[1] * q._arr[2] -
                _arr[2] * q._arr[1],
            _arr[1] * q._arr[3] + _arr[3] * q._arr[1] + _arr[2] * q._arr[0] -
                _arr[0] * q._arr[2],
            _arr[2] * q._arr[3] + _arr[3] * q._arr[2] + _arr[0] * q._arr[1] -
                _arr[1] * q._arr[0],
            _arr[3] * q._arr[3] - _arr[0] * q._arr[0] - _arr[1] * q._arr[1] -
                _arr[2] * q._arr[2]};
  }
  ...
  Quaternion getNormalized() const {
    float l =
        ...
        (_arr[3] < 0
             ? -1.f
             : 1.f); // after normalization, real part will be non-negative
...
private:
  ...
  static constexpr int32_t const maxN =
      0x477FE000; // max flt16 normal as a flt32
  static constexpr int32_t const minN =
      0x38800000; // min flt16 normal as a flt32
  static constexpr int32_t const signN = (int32_t)0x80000000; // flt32 sign bit

  static constexpr int32_t const infC = infN >> shift;
  static constexpr int32_t const nanN =
      (infC + 1) << shift; // minimum flt16 nan as a flt32
  ...
  static constexpr int32_t const mulC =
      0x33800000; // minN / (1 << (23 - shift))

  static constexpr int32_t const subC =
      0x003FF; // max flt32 subnormal down shifted
  static constexpr int32_t const norC =
      0x00400; // min flt32 normal down shifted

  static constexpr int32_t const maxD = infC - maxC - 1;
  static constexpr int32_t const minD = minC - subC - 1;

  static constexpr int32_t const maxF =
      0x7FFFFFBF; // max int32 expressible as a flt32
  ...
  static uint16_t toFloat16(float value) {
    ...
    s.si = mulN;
    int32_t subN =
        (int32_t)std::min(s.f * v.f, (float)maxF); // correct subnormals
