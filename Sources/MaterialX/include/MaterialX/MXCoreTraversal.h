...
public:
  Edge(ElementPtr elemDown, ElementPtr elemConnect, ElementPtr elemUp)
      : _elemDown(elemDown), _elemConnect(elemConnect), _elemUp(elemUp) {}
  ~Edge() {}

  bool operator==(const Edge &rhs) const {
    return _elemDown == rhs._elemDown && _elemConnect == rhs._elemConnect &&
           _elemUp == rhs._elemUp;
  }
  bool operator!=(const Edge &rhs) const { return !(*this == rhs); }
  bool operator<(const Edge &rhs) const {
    return std::tie(_elemDown, _elemConnect, _elemUp) <
           std::tie(rhs._elemDown, rhs._elemConnect, rhs._elemUp);
  }

  operator bool() const;

  /// Return the downstream element of the edge.
  ElementPtr getDownstreamElement() const { return _elemDown; }

  /// Return the connecting element of the edge, if any.
  ElementPtr getConnectingElement() const { return _elemConnect; }

  /// Return the upstream element of the edge.
  ElementPtr getUpstreamElement() const { return _elemUp; }
...
public:
  explicit TreeIterator(ElementPtr elem)
      : _elem(elem), _prune(false), _holdCount(0) {}
  ~TreeIterator() {}
...
public:
  ...
  ElementPtr getElement() const { return _elem; }
  ...
  size_t getElementDepth() const { return _stack.size(); }
...
public:
  explicit GraphIterator(ElementPtr elem)
      : _upstreamElem(elem), _prune(false), _holdCount(0) {
...
public:
  bool operator==(const GraphIterator &rhs) const {
    return _upstreamElem == rhs._upstreamElem && _stack == rhs._stack &&
           _prune == rhs._prune;
  }
  bool operator!=(const GraphIterator &rhs) const { return !(*this == rhs); }

  /// Dereference this iterator, returning the current edge in the traversal.
  Edge operator*() const {
    return Edge(getDownstreamElement(), getConnectingElement(),
                getUpstreamElement());
  }
  ...
  ElementPtr getConnectingElement() const { return _connectingElem; }

  /// Return the upstream element of the current edge.
  ElementPtr getUpstreamElement() const { return _upstreamElem; }
...
/// An iterator object representing the current state of an inheritance
/// traversal.
///
...
public:
  ...
  /// Dereference this iterator, returning the current element in the
  /// traversal.
  ConstElementPtr operator*() const { return _elem; }
