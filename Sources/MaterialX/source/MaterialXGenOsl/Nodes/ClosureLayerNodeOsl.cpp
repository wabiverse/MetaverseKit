...
void ClosureLayerNodeOsl::emitFunctionCall(const ShaderNode &_node,
                                           GenContext &context,
  ...
  if (top->hasClassification(ShaderNode::Classification::THINFILM)) {
    ...
    if (base->getOutput()->getType() == Type::VDF) {
      // Combining a surface closure with a volumetric closure is simply done
      // with the add operator in OSL.
      shadergen.emitLine(output->getVariable() + ".response = " + topResult +
                             ".response + " + baseResult,
                         stage);
      // Just pass the throughput along.
      shadergen.emitLine(output->getVariable() + ".throughput = " + topResult +
                             ".throughput",
                         stage);
    } else {
      shadergen.emitLine(output->getVariable() + ".response = " + topResult +
  ...
  if (top->hasClassification(ShaderNode::Classification::THINFILM)) {
    ...
    shadergen.emitString(" = layer(" + topResult + ", " + baseResult + ")",
                         stage);
