...
void ClosureLayerNodeMdl::emitFunctionCall(const ShaderNode &_node,
                                           GenContext &context,
  ...
  if (baseInput->getType() == Type::VDF) {
    ...
    if (!topInput->getConnection()) {
      ...
      shadergen.emitLine("material " + output->getVariable() + " = material()",
                         stage);
      return;
    }
    ...
    shadergen.emitLine("material " + output->getVariable() +
                           " = material(surface: " + t +
                           ".surface, backface: " + t + ".backface, ior: " + t +
                           ".ior, volume: " + b + ".volume)",
                       stage);
  ...
  if (top->getParent() != node.getParent()) {
    shadergen.emitComment(
        "Warning: MDL has no support for layering BSDFs through a graph "
        "interface. Only the top BSDF will used.",
        stage);
    shadergen.emitLine("material " + output->getVariable() + " = " +
                           shadergen.getUpstreamResult(topInput, context),
                       stage);
    return;
  }
  ...
  if (top->hasClassification(ShaderNode::Classification::THINFILM)) {
    emitBsdfOverBsdfFunctionCalls_thinFilm(node, context, stage, shadergen, top,
                                           base, output);
    return;
  }
  ...
  ShaderInput *layerNodeThicknessInput =
      node.getInput(StringConstantsMdl::THIN_FILM_THICKNESS);
  ShaderInput *layerNodeIorInput =
      node.getInput(StringConstantsMdl::THIN_FILM_IOR);

  ShaderInput *topNodeThicknessInput =
      top->getInput(StringConstantsMdl::THIN_FILM_THICKNESS);
  ShaderInput *topNodeIorInput =
      top->getInput(StringConstantsMdl::THIN_FILM_IOR);
  bool breakTopConnection = false;
  if (layerNodeThicknessInput && layerNodeIorInput && topNodeThicknessInput &&
      topNodeIorInput) {
    topNodeThicknessInput->makeConnection(
        layerNodeThicknessInput->getConnection());
    topNodeIorInput->makeConnection(layerNodeIorInput->getConnection());
    breakTopConnection = true;
  }
  ShaderInput *baseNodeThicknessInput =
      base->getInput(StringConstantsMdl::THIN_FILM_THICKNESS);
  ShaderInput *baseNodeIorInput =
      base->getInput(StringConstantsMdl::THIN_FILM_IOR);
  bool breakBaseConnection = false;
  if (layerNodeThicknessInput && layerNodeIorInput && baseNodeThicknessInput &&
      baseNodeIorInput) {
    baseNodeThicknessInput->makeConnection(
        layerNodeThicknessInput->getConnection());
  ...
  emitBsdfOverBsdfFunctionCalls(node, context, stage, shadergen, top, base,
                                output);
...
void ClosureLayerNodeMdl::emitBsdfOverBsdfFunctionCalls(
    ...
    const ShaderGenerator &shadergen, ShaderNode *top, ShaderNode *base,
  ...
  while (true) {
    ...
    if (baseReceiverNode->hasClassification(
            ShaderNode::Classification::LAYER)) {
      baseReceiverNode =
          top->getInput(StringConstantsMdl::BASE)->getConnection()->getNode();
    } else {
  ...
  // input, and it's only possible to do this workaround on a subset of the
  // BSDFs. So if the top BSDF doesn't have a base input, we can only emit the
  // top BSDF without any base layering.
  ShaderInput *topNodeBaseInput =
      baseReceiverNode->getInput(StringConstantsMdl::BASE);
  if (!topNodeBaseInput) {
    shadergen.emitComment("Warning: MDL has no support for layering BSDF nodes "
                          "without a base input. Only the top BSDF will used.",
                          stage);
...
void ClosureLayerNodeMdl::emitBsdfOverBsdfFunctionCalls_thinFilm(
    ...
    const ShaderGenerator &shadergen, ShaderNode *top, ShaderNode *base,
    ShaderOutput *output) const {
  ShaderInput *thinFilmThicknessInput =
      top->getInput(StringConstantsMdl::THICKNESS);
  ...
  if (!baseNodeThicknessInput || !baseNodeIorInput) {
    shadergen.emitComment("Warning: The base node does not have parameters to "
                          "transport thin-film thickness and IOR.",
                          stage);
  ...
  baseNodeThicknessInput->makeConnection(
      thinFilmThicknessInput->getConnection());
...
void ThinFilmCombineNodeMdl::emitFunctionCall(const ShaderNode &_node,
                                              GenContext &context,
                                              ShaderStage &stage) const {
  const ShaderGenerator &shadergen = context.getShaderGenerator();
  ShaderNode &node = const_cast<ShaderNode &>(_node);
