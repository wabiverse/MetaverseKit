...
const ShaderInput *findTransmissionIOR(const ShaderNode &node) {
  if (node.hasClassification(ShaderNode::Classification::BSDF_T)) {
    ...
    if (ior) {
      ...
      if (scatterMode && scatterMode->getValue()) {
        const string scatterModeValue =
            scatterMode->getValue()->getValueString();
  ...
  for (const ShaderInput *input : node.getInputs()) {
    if (input->getType() == Type::BSDF && input->getConnection()) {
      const ShaderInput *ior =
          findTransmissionIOR(*input->getConnection()->getNode());
      if (ior) {
        return ior;
      }
    }
...
void SurfaceNodeMdl::emitFunctionCall(const ShaderNode &node,
                                      GenContext &context,
                                      ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    const MdlShaderGenerator &shadergen =
        static_cast<const MdlShaderGenerator &>(context.getShaderGenerator());
