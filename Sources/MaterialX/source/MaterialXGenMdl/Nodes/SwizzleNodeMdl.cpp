...
string SwizzleNodeMdl::getVariableName(const ShaderInput *input) const {
  ...
  // Special handling is required because struct field names follow a special
  // naming scheme in this generator.
  const ShaderNode *upstreamNode = input->getConnection()->getNode();
  // Skip upstream nodes that are shader graphs because they don't have an
  // implementation.
