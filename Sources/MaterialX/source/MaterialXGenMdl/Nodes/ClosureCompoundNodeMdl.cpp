...
void ClosureCompoundNodeMdl::emitFunctionDefinition(const ShaderNode &node,
                                                    GenContext &context,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    if (!_returnStruct.empty() && _unrollReturnStructMembers) {
      ...
      for (const ShaderGraphOutputSocket *outputSocket :
        ...
        // since the emit fuctions are const, the field name to generate a
        // function for is passed via context
        const std::string &fieldName = outputSocket->getName();
        GenUserDataStringPtr fieldNamePtr =
            std::make_shared<GenUserDataString>(fieldName);
        context.pushUserData(
            CompoundNodeMdl::GEN_USER_DATA_RETURN_STRUCT_FIELD_NAME,
            fieldNamePtr);

        // Emit function signature.
        shadergen.emitComment("unrolled structure field: " + _returnStruct +
                                  "." + fieldName + " (name=\"" +
                                  node.getName() + "\")",
                              stage);
...
void ClosureCompoundNodeMdl::emitFunctionCall(const ShaderNode &node,
                                              GenContext &context,
                                              ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    const ShaderGenerator &shadergen = context.getShaderGenerator();
