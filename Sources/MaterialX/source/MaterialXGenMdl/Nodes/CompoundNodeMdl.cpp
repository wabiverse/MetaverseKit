...
void CompoundNodeMdl::initialize(const InterfaceElement &element,
  ...
  // Materials can not be members of structs. Identify this case in order to
  // handle it.
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    if (isMaterialExpr) {
      ...
      if (!_returnStruct.empty()) {
        ...
        for (const ShaderGraphOutputSocket *output :
             _rootGraph->getOutputSockets()) {
          const string result = shadergen.getUpstreamResult(output, context);
          shadergen.emitLine(resultVariableName + ".mxp_" + output->getName() +
                                 " = " + result,
                             stage);
        }
...
void CompoundNodeMdl::emitFunctionCall(const ShaderNode &node,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    // Begin function call.
    if (!_returnStruct.empty()) {
      // when unrolling structure members, the call that creates the struct
      // needs to skipped
      if (_unrollReturnStructMembers) {
        // make sure the upstream definitions are known
        shadergen.emitComment(
            "fill unrolled structure fields: " + _returnStruct + " (name=\"" +
                node.getName() + "\")",
            stage);
        for (const ShaderGraphOutputSocket *outputSocket :
             _rootGraph->getOutputSockets()) {
          ...
          const string &outputType =
              syntax.getTypeName(outputSocket->getType());
          const string resultVariableName = node.getName() + "__" + fieldName;

          shadergen.emitLineBegin(stage);
          shadergen.emitString(outputType + " " + resultVariableName + " = ",
                               stage);
      ...
      shadergen.emitLineBegin(stage);
      shadergen.emitString(_returnStruct + " " + resultVariableName + " = ",
                           stage);
    } else {
...
void CompoundNodeMdl::emitFunctionSignature(const ShaderNode &,
                                            GenContext &context,
                                            ShaderStage &stage) const {
  const ShaderGenerator &shadergen = context.getShaderGenerator();
  const Syntax &syntax = shadergen.getSyntax();
  ...
  if (!_returnStruct.empty()) {
    if (_unrollReturnStructMembers) {
      const auto fieldName = context.getUserData<GenUserDataString>(
          GEN_USER_DATA_RETURN_STRUCT_FIELD_NAME);

      if (fieldName) {
        // Begin function signature.
        const ShaderGraphOutputSocket *outputSocket =
            _rootGraph->getOutputSocket(fieldName->getValue());
        const string &outputType = syntax.getTypeName(outputSocket->getType());
        shadergen.emitLine(outputType + " " + _functionName + "__" +
                               fieldName->getValue(),
                           stage, false);
      } else {
        throw Exception("Error during transformation of struct: " +
                        _returnStruct);
      }
    } else {
      ...
      for (const ShaderGraphOutputSocket *output :
           _rootGraph->getOutputSockets()) {
        shadergen.emitLine(syntax.getTypeName(output->getType()) + " mxp_" +
                               output->getName(),
                           stage);
      }
  ...
  for (ShaderGraphInputSocket *input : _rootGraph->getInputSockets()) {
    ...
    string value = input->getValue() ? syntax.getValue(input->getType(),
                                                       *input->getValue(), true)
                                     : EMPTY_STRING;
    ...
    shadergen.emitLine(qualifier + type + " " + input->getVariable() + " = " +
                           value + delim,
                       stage, false);
  }
