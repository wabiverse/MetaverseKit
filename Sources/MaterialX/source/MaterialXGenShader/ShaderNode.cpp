...
ShaderPort::ShaderPort(ShaderNode *node, const TypeDesc *type,
                       const string &name, ValuePtr value)
    : _node(node), _type(type), _name(name), _variable(name), _value(value),
      _flags(0) {}
...
ShaderInput::ShaderInput(ShaderNode *node, const TypeDesc *type,
                         const string &name)
    : ShaderPort(node, type, name), _connection(nullptr) {}
...
void ShaderInput::breakConnection() {
  if (_connection) {
    ...
    ShaderInputVec::iterator it =
        std::find(connected.begin(), connected.end(), this);
    if (it != connected.end()) {
      connected.erase(it);
    }
...
ShaderNode *ShaderInput::getConnectedSibling() const {
  if (_connection &&
      _connection->getNode()->getParent() == _node->getParent()) {
...
ShaderOutput::ShaderOutput(ShaderNode *node, const TypeDesc *type,
                           const string &name)
...
void ShaderOutput::breakConnection(ShaderInput *dst) {
  if (std::find(_connections.begin(), _connections.end(), dst) ==
      _connections.end()) {
...
void ShaderOutput::breakConnections() {
  ...
  if (!_connections.empty()) {
    throw ExceptionShaderGenError(
        "Number of output connections not broken properly'" +
        std::to_string(_connections.size()) + " for output: " + getFullName());
  }
}
...
ShaderNode::ShaderNode(const ShaderGraph *parent, const string &name)
    : _parent(parent), _name(name), _classification(0), _impl(nullptr) {}

ShaderNodePtr ShaderNode::create(const ShaderGraph *parent, const string &name,
                                 const NodeDef &nodeDef, GenContext &context) {
  ...
  if (!newNode->_impl) {
    throw ExceptionShaderGenError(
        "Could not find a matching implementation for node '" +
        nodeDef.getNodeString() + "' matching target '" +
        shadergen.getTarget() + "'");
  }
  ...
  for (const ValueElementPtr &port : nodeDef.getActiveValueElements()) {
    ...
    if (port->isA<Output>()) {
      ...
      const string &enumNames =
          port->getAttribute(ValueElement::ENUM_ATTRIBUTE);
      if (context.getShaderGenerator().getSyntax().remapEnumeration(
              portValue, portType, enumNames, enumResult)) {
  ...
  if (primaryOutput->getType() == Type::MATERIAL) {
    ...
    if (nodeDefName == "ND_surface_unlit") {
      newNode->_classification = Classification::SHADER |
    ...
    if (bsdfType == BSDF_R) {
      ...
      newNode->_classification |=
          (Classification::BSDF_R | Classification::BSDF_T);
    }
  ...
  // Second, check for specific nodes types
    ...
    newNode->_classification =
        Classification::TEXTURE | Classification::CONSTANT;
  ...
  else if (groupName == TEXTURE2D_GROUPNAME ||
           groupName == TEXTURE3D_GROUPNAME) {
    newNode->_classification =
        Classification::TEXTURE | Classification::FILETEXTURE;
  }
  ...
  } else if (groupName == TEXTURE3D_GROUPNAME ||
             groupName == PROCEDURAL3D_GROUPNAME) {
...
void ShaderNode::initialize(const Node &node, const NodeDef &nodeDef,
                            GenContext &context) {
  ...
  for (InputPtr nodeInput : node.getActiveInputs()) {
    ...
    ValueElementPtr nodeDefInput =
        nodeDef.getActiveValueElement(nodeInput->getName());
  ...
  if (hasClassification(Classification::BSDF)) {
    ...
    const string &scatterMode =
        scatterModeInput ? scatterModeInput->getValueString() : EMPTY_STRING;
    // If scatter mode is only T, set classification to only transmission.
    // Note: For only R we must still keep classification at default value (both
    // reflection/transmission) since reflection needs to attenuate the
    // transmission amount in HW shaders when layering is used.
...
void ShaderNode::createMetadata(const NodeDef &nodeDef, GenContext &context) {
  ...
  for (const string &nodedefAttr : nodeDef.getAttributeNames()) {
    ...
    if (metadataEntry) {
      ...
      if (!attrValue.empty()) {
        ValuePtr value = Value::createValueFromStrings(
            attrValue, metadataEntry->type->getName());
        if (!value) {
        ...
        if (value) {
          ...
          nodeMetadataStorage->push_back(
              ShaderMetadata(metadataEntry->name, metadataEntry->type, value));
        }
      }
    }
  }
  ...
  for (const ValueElementPtr &nodedefPort : nodeDef.getActiveValueElements()) {
    ...
    if (input) {
      ...
      for (const string &nodedefPortAttr : nodedefPort->getAttributeNames()) {
        const ShaderMetadata *metadataEntry =
            registry->findMetadata(nodedefPortAttr);
        if (metadataEntry) {
          const string &attrValue = nodedefPort->getAttribute(nodedefPortAttr);
          if (!attrValue.empty()) {
            const TypeDesc *type =
                metadataEntry->type ? metadataEntry->type : input->getType();
            ValuePtr value =
                Value::createValueFromStrings(attrValue, type->getName());
            if (!value) {
              value = metadataEntry->value;
            }
            if (value) {
              ...
              inputMetadataStorage->push_back(
                  ShaderMetadata(metadataEntry->name, type, value));
            }
          }
        }
      }
...
ShaderInput *ShaderNode::addInput(const string &name, const TypeDesc *type) {
  if (getInput(name)) {
    throw ExceptionShaderGenError("An input named '" + name +
                                  "' already exists on node '" + _name + "'");
  }
...
ShaderOutput *ShaderNode::addOutput(const string &name, const TypeDesc *type) {
  if (getOutput(name)) {
    throw ExceptionShaderGenError("An output named '" + name +
                                  "' already exists on node '" + _name + "'");
  }
