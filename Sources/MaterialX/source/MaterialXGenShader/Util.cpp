...
bool hasTransparentInputs(const OpaqueTestPairList &opaqueInputList,
                          NodePtr node) {
    ...
    for (auto inputPair : inputPairList) {
      ...
      if (checkInput) {
        ...
        if (!interfaceName.empty()) {
          interfaceNames.push_back(
              std::make_pair(interfaceName, inputPair.second));
        }
      }
    }
  ...
  for (auto inputPair : inputPairList) {
    ...
    if (checkInput) {
      ...
      if (inputNode) {
        ...
        if (nodeDef && nodeDef->getAttribute(NodeDef::NODE_GROUP_ATTRIBUTE) !=
                           NodeDef::ADJUSTMENT_NODE_GROUP) {
          return true;
        }
      } else {
...
bool isTransparentShaderGraph(OutputPtr output, const string &target,
  ...
  for (GraphIterator it = output->traverseGraph().begin();
       it != GraphIterator::end(); ++it) {
...
bool isTransparentSurface(ElementPtr element, const string &target) {
  ...
  if (node) {
    ...
    // Handle graph definitions.
    NodeDefPtr nodeDef = node->getNodeDef();
    InterfaceElementPtr impl =
        nodeDef ? nodeDef->getImplementation(target) : nullptr;
    if (impl && impl->isA<NodeGraph>()) {
      NodeGraphPtr graph = impl->asA<NodeGraph>();
...
vector<TypedElementPtr> findRenderableElements(ConstDocumentPtr doc) {
  ...
  if (renderableElements.empty()) {
    ...
    const StringSet UNSUPPORTED_TYPES = {BSDF_TYPE_STRING, EDF_TYPE_STRING,
                                         VDF_TYPE_STRING,
...
vector<Vector2> getUdimCoordinates(const StringVec &udimIdentifiers) {
  ...
  for (const string &udimIdentifier : udimIdentifiers) {
    ...
    udimCoordinates.emplace_back(static_cast<float>(uVal),
                                 static_cast<float>(vVal));
  }

  return udimCoordinates;
}

void getUdimScaleAndOffset(const vector<Vector2> &udimCoordinates,
                           Vector2 &scaleUV, Vector2 &offsetUV) {
...
NodePtr connectsToWorldSpaceNode(OutputPtr output) {
  ...
  if (connectedNode &&
      WORLD_SPACE_NODE_CATEGORIES.count(connectedNode->getCategory())) {
...
bool hasElementAttributes(OutputPtr output, const StringVec &attributes) {
  ...
  for (GraphIterator it = output->traverseGraph().begin();
    ...
    for (ValueElementPtr nodeDefElement : nodeDef->getActiveValueElements()) {
      ValueElementPtr testElement =
          upstreamNode->getActiveValueElement(nodeDefElement->getName());
      if (!testElement) {
...
void findRenderableElements(ConstDocumentPtr doc,
                            vector<TypedElementPtr> &elements, bool) {
