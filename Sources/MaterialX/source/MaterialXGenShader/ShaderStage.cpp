...
ShaderPort *VariableBlock::operator[](const string &name) {
  ...
  if (!v) {
    throw ExceptionShaderGenError("No variable named '" + name +
                                  "' exists for block '" + getName() + "'");
  }
  return v;
}
...
ShaderPort *VariableBlock::add(const TypeDesc *type, const string &name,
                               ValuePtr value, bool shouldWiden) {
  auto it = _variableMap.find(name);
  if (it != _variableMap.end()) {
    if (shouldWiden) {
      // Automatically try to widen the type of the shader port if the requested
      // type differs from the existing port's type.
      if (it->second->getType()->getSize() < type->getSize()) {
        it->second->setType(type);
      }
    } else if (type != it->second->getType()) {
      throw ExceptionShaderGenError(
          "Trying to add shader port '" + name + "' with type '" +
...
ShaderStage::ShaderStage(const string &name, ConstSyntaxPtr syntax)
    : _name(name), _syntax(syntax), _indentations(0),
...
VariableBlock &ShaderStage::getUniformBlock(const string &name) {
  auto it = _uniforms.find(name);
  if (it == _uniforms.end()) {
    throw ExceptionShaderGenError("No uniform block named '" + name +
                                  "' exists for shader stage '" + getName() +
                                  "'");
  }
  return *it->second;
}
...
VariableBlock &ShaderStage::getInputBlock(const string &name) {
  auto it = _inputs.find(name);
  if (it == _inputs.end()) {
    throw ExceptionShaderGenError("No input block named '" + name +
                                  "' exists for shader stage '" + getName() +
                                  "'");
  }
  return *it->second;
...
VariableBlock &ShaderStage::getOutputBlock(const string &name) {
  auto it = _outputs.find(name);
  if (it == _outputs.end()) {
    throw ExceptionShaderGenError("No output block named '" + name +
                                  "' exists for shader stage '" + getName() +
                                  "'");
  }
  return *it->second;
...
VariableBlock &ShaderStage::getConstantBlock() { return _constants; }
...
void ShaderStage::endScope(bool semicolon, bool newline) {
  if (_scopes.empty()) {
    throw ExceptionShaderGenError(
        "End scope called with no scope active, please check your "
        "beginScope/endScope calls");
  }
...
void ShaderStage::newLine() { _code += _syntax->getNewline(); }

void ShaderStage::addString(const string &str) { _code += str; }
...
void ShaderStage::addBlock(const string &str, const FilePath &sourceFilename,
                           GenContext &context) {
...
void ShaderStage::addInclude(const FilePath &includeFilename,
  ...
  tokenSubstitution(context.getShaderGenerator().getTokenSubstitutions(),
                    modifiedFile);
  FilePath resolvedFile =
      context.resolveSourceFile(modifiedFile, sourceFilename.getParentPath());

  if (!_includes.count(resolvedFile)) {
    string content = readFile(resolvedFile);
    if (content.empty()) {
      throw ExceptionShaderGenError("Could not find include file: '" +
                                    includeFilename.asString() + "'");
    }
