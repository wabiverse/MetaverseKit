...
GenContext::GenContext(ShaderGeneratorPtr sg) : _sg(sg) {
  if (!_sg) {
    throw ExceptionShaderGenError(
        "GenContext must have a valid shader generator");
  }
...
void GenContext::clearNodeImplementations() { _nodeImpls.clear(); }

void GenContext::clearUserData() { _userData.clear(); }
...
void GenContext::getInputSuffix(const ShaderInput *input,
                                string &suffix) const {
  suffix.clear();
  std::unordered_map<const ShaderInput *, string>::const_iterator iter =
      _inputSuffix.find(input);
...
void GenContext::getOutputSuffix(const ShaderOutput *output,
                                 string &suffix) const {
  suffix.clear();
  std::unordered_map<const ShaderOutput *, string>::const_iterator iter =
      _outputSuffix.find(output);
...
ScopedSetClosureParams::ScopedSetClosureParams(
    const ClosureContext::ClosureParams *params, const ShaderNode *node,
    ClosureContext *cct)
    : _cct(cct), _node(node), _oldParams(nullptr) {
  if (_cct) {
...
ScopedSetClosureParams::ScopedSetClosureParams(const ShaderNode *fromNode,
                                               ...
                                               ClosureContext *cct)
    : _cct(cct), _node(toNode), _oldParams(nullptr) {
  // The class must be safe for the cases where a context is not set
  // so make sure to check for nullptr here.
  if (_cct) {
    const ClosureContext::ClosureParams *newParams =
        _cct->getClosureParams(fromNode);
    if (newParams) {
      _oldParams = _cct->getClosureParams(_node);
...
ScopedSetVariableName::ScopedSetVariableName(const string &name,
                                             ShaderPort *port)
    : _port(port), _oldName(port->getVariable()) {
