...
const TypeSyntax &Syntax::getTypeSyntax(const TypeDesc *type) const {
  ...
  if (it == _typeSyntaxByType.end()) {
    ...
    throw ExceptionShaderGenError("No syntax is defined for the given type '" +
                                  typeName + "'.");
  }
...
const TypeDesc *
Syntax::getTypeDescription(const TypeSyntaxPtr &typeSyntax) const {
  auto pos = std::find(_typeSyntaxes.begin(), _typeSyntaxes.end(), typeSyntax);
  if (pos == _typeSyntaxes.end()) {
    throw ExceptionShaderGenError("The syntax'" + typeSyntax->getName() +
                                  "' is not registered.");
  }
  const size_t index =
      static_cast<size_t>(std::distance(_typeSyntaxes.begin(), pos));
...
string Syntax::getValue(const TypeDesc *type, const Value &value,
                        bool uniform) const {
...
string Syntax::getOutputTypeName(const TypeDesc *type) const {
  ...
  return outputModifier.size() ? outputModifier + " " + syntax.getName()
                               : syntax.getName();
}
...
string Syntax::getSwizzledVariable(const string &srcName,
...
ValuePtr Syntax::getSwizzledValue(ValuePtr value, const TypeDesc *srcType,
                                  const string &channels,
                                  const TypeDesc *dstType) const {
  const TypeSyntax &srcSyntax = getTypeSyntax(srcType);
...
bool Syntax::typeSupported(const TypeDesc *) const { return true; }
...
static bool isInvalidChar(char c) { return !isalnum(c) && c != '_'; }
...
string Syntax::getVariableName(const string &name, const TypeDesc * /*type*/,
...
bool Syntax::remapEnumeration(const string &, const TypeDesc *, const string &,
                              std::pair<const TypeDesc *, ValuePtr> &) const {
  return false;
}

const StringVec TypeSyntax::EMPTY_MEMBERS;

TypeSyntax::TypeSyntax(const string &name, const string &defaultValue,
    ...
    : _name(name), _defaultValue(defaultValue),
      _uniformDefaultValue(uniformDefaultValue), _typeAlias(typeAlias),
      _typeDefinition(typeDefinition), _members(members) {}
...
ScalarTypeSyntax::ScalarTypeSyntax(const string &name,
    ...
    : TypeSyntax(name, defaultValue, uniformDefaultValue, typeAlias,
                 typeDefinition, EMPTY_MEMBERS) {}
...
StringTypeSyntax::StringTypeSyntax(const string &name,
                                   const string &defaultValue,
                                   const string &uniformDefaultValue,
                                   const string &typeAlias,
                                   const string &typeDefinition)
    : ScalarTypeSyntax(name, defaultValue, uniformDefaultValue, typeAlias,
                       typeDefinition) {}
...
AggregateTypeSyntax::AggregateTypeSyntax(const string &name,
                                         const string &defaultValue,
                                         const string &uniformDefaultValue,
                                         const string &typeAlias,
                                         const string &typeDefinition,
...
string AggregateTypeSyntax::getValue(const Value &value,
                                     bool /*uniform*/) const {
  const string valueString = value.getValueString();
  return valueString.empty() ? valueString
                             : getName() + "(" + valueString + ")";
}

string AggregateTypeSyntax::getValue(const StringVec &values,
                                     bool /*uniform*/) const {
  if (values.empty()) {
    throw ExceptionShaderGenError("No values given to construct a value");
  }
