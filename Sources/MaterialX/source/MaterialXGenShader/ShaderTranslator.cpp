...
void ShaderTranslator::connectTranslationInputs(NodePtr shader,
  ...
  for (InputPtr shaderInput : origInputs) {
    if (translationNodeDef->getInput(shaderInput->getName())) {
      InputPtr input = _translationNode->addInput(shaderInput->getName(),
                                                  shaderInput->getType());
      ...
      if (connectedOutput) {
        ...
        // Nodes with world-space outputs are skipped, with translation being
        // applied to the node directly upstream.
        ...
        throw Exception("Shader input has no associated output or value " +
                        shaderInput->getName());
      }
...
void ShaderTranslator::connectTranslationOutputs(NodePtr shader) {
  ...
  NodeGraphPtr translationGraph =
      implement ? implement->asA<NodeGraph>() : nullptr;
  if (!translationGraph) {
    throw Exception("No graph implementation for " +
                    _translationNode->getCategory() + " was found");
  }

  // Iterate through outputs of the translation graph.
  for (OutputPtr translationGraphOutput : translationGraph->getOutputs()) {
    // Convert output name to input name, using a hardcoded naming convention
    // for now.
    string outputName = translationGraphOutput->getName();
    size_t pos = outputName.find("_out");
    if (pos == string::npos) {
      throw Exception("Translation graph output " + outputName +
                      " does not end with '_out'");
    }
    ...
    // Nodes with world-space outputs are moved outside of their containing
    // graph, providing greater flexibility in texture baking.
    ...
    if (worldSpaceNode) {
      ...
      if (nodeInput && nodeInput->hasInterfaceName()) {
        InputPtr interfaceInput =
            _translationNode->getInput(nodeInput->getInterfaceName());
        NodePtr sourceNode =
            interfaceInput ? interfaceInput->getConnectedNode() : nullptr;
        if (!sourceNode) {
          continue;
        }
        translatedStreamNode = _graph->addNode(worldSpaceNode->getCategory(),
                                               worldSpaceNode->getName(),
                                               worldSpaceNode->getType());
    ...
    if (!translatedOutput) {
      translatedOutput =
          _graph->addOutput(outputName, translationGraphOutput->getType());
    }
    ...
    if (!translatedShaderInput) {
      translatedShaderInput =
          shader->addInput(inputName, translationGraphOutput->getType());
    }
...
void ShaderTranslator::translateShader(NodePtr shader,
  ...
  if (sourceCategory == destCategory) {
    throw Exception("The source shader \"" + shader->getNamePath() +
                    "\" category is already \"" + destCategory + "\"");
  }

  DocumentPtr doc = shader->getDocument();
  vector<OutputPtr> referencedOutputs = getConnectedOutputs(shader);
  ElementPtr referencedParent =
      !referencedOutputs.empty() ? referencedOutputs[0]->getParent() : nullptr;
  NodeGraphPtr referencedGraph =
      referencedParent ? referencedParent->asA<NodeGraph>() : nullptr;
  _graph = referencedGraph ? referencedGraph : doc->addNodeGraph();

  string translateNodeString = sourceCategory + "_to_" + destCategory;
  vector<NodeDefPtr> matchingNodeDefs =
      doc->getMatchingNodeDefs(translateNodeString);
  if (matchingNodeDefs.empty()) {
    throw Exception("Shader translation requires a translation nodedef named " +
                    translateNodeString);
  }
