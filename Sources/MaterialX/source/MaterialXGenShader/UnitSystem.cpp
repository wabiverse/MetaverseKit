...
public:
  explicit ScalarUnitNode(LinearUnitConverterPtr scalarUnitConverter)
      : _scalarUnitConverter(scalarUnitConverter),
        _unitRatioFunctionName("mx_" + _scalarUnitConverter->getUnitType() +
                               "_unit_ratio") {}
...
ShaderNodeImplPtr
ScalarUnitNode::create(LinearUnitConverterPtr scalarUnitConverter) {
  return std::make_shared<ScalarUnitNode>(scalarUnitConverter);
}

void ScalarUnitNode::initialize(const InterfaceElement &element,
                                GenContext & /*context*/) {
...
void ScalarUnitNode::emitFunctionDefinition(const ShaderNode &node,
                                            GenContext &context,
                                            ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    // Emit the helper funtion mx_<unittype>_unit_ratio that embeds a look up
    // table for unit scale
    ...
    // See stdlib/gen*/mx_<unittype>_unit. This helper function is called by
    // these shaders.
    const string VAR_UNIT_SCALE =
        "u_" + _scalarUnitConverter->getUnitType() + "_unit_scales";
    VariableBlock unitLUT("unitLUT", EMPTY_STRING);
    ScopedFloatFormatting fmt(Value::FloatFormatFixed, 15);
    unitLUT.add(Type::FLOATARRAY, VAR_UNIT_SCALE,
                Value::createValue<vector<float>>(unitScales));

    const ShaderGenerator &shadergen = context.getShaderGenerator();
    shadergen.emitLine("float " + _unitRatioFunctionName +
                           "(int unit_from, int unit_to)",
                       stage, false);
    shadergen.emitFunctionBodyBegin(node, context, stage);
    shadergen.emitVariableDeclarations(
        unitLUT, shadergen.getSyntax().getConstantQualifier(), ";", context,
        stage, true);
    shadergen.emitLine("return (" + VAR_UNIT_SCALE + "[unit_from] / " +
                           VAR_UNIT_SCALE + "[unit_to])",
                       stage);
    shadergen.emitFunctionBodyEnd(node, context, stage);
  }
}

void ScalarUnitNode::emitFunctionCall(const ShaderNode &node,
                                      GenContext &context,
                                      ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    const ShaderGenerator &shadergen = context.getShaderGenerator();
...
UnitTransform::UnitTransform(const string &ss, const string &ts,
                             const TypeDesc *t, const string &unittype)
    : sourceUnit(ss), targetUnit(ts), type(t), unitType(unittype) {
  if (type != Type::FLOAT && type != Type::VECTOR2 && type != Type::VECTOR3 &&
      type != Type::VECTOR4) {
    throw ExceptionShaderGenError(
        "Unit space transform can only be a float or vectors");
  }
}
...
void UnitSystem::loadLibrary(DocumentPtr document) { _document = document; }
...
NodeDefPtr UnitSystem::getNodeDef(const UnitTransform &transform) const {
  ...
  for (NodeDefPtr nodeDef :
       _document->getMatchingNodeDefs(MULTIPLY_NODE_NAME)) {
...
ShaderNodePtr UnitSystem::createNode(ShaderGraph *parent,
  ...
  NodeDefPtr nodeDef = getNodeDef(transform);
  if (!nodeDef) {
    throw ExceptionShaderGenError("No nodedef found for transform: ('" +
  ...
  if (!_unitRegistry || !_unitRegistry->getUnitConverter(scalarTypeDef)) {
    throw ExceptionTypeError(
        "Unit registry unavaliable or undefined unit converter for: " +
        transform.unitType);
  }
  LinearUnitConverterPtr scalarConverter =
      std::dynamic_pointer_cast<LinearUnitConverter>(
          _unitRegistry->getUnitConverter(scalarTypeDef));

  // Create the node.
  ShaderNodePtr shaderNode =
      ShaderNode::create(parent, name, *nodeDef, context);

  // Set ports on the node.
  ShaderInput *in2 = shaderNode->getInput("in2");
  if (!in2) {
    throw ExceptionShaderGenError(
        "Invalid node signature for unit transform: ('" + transform.sourceUnit +
        "', '" + transform.targetUnit + "').");
  }
  float conversionRatio = scalarConverter->conversionRatio(
      transform.sourceUnit, transform.targetUnit);
