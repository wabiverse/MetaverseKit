...
ShaderGraph::ShaderGraph(const ShaderGraph *parent, const string &name,
                         ConstDocumentPtr document,
                         const StringSet &reservedWords)
    : ShaderNode(parent, name), _document(document) {
...
void ShaderGraph::addInputSockets(const InterfaceElement &elem,
  ...
  for (InputPtr input : elem.getActiveInputs()) {
    ...
    const string &portValueString =
        portValue ? portValue->getValueString() : EMPTY_STRING;
    ...
    if (context.getShaderGenerator().getSyntax().remapEnumeration(
            portValueString, portType, enumNames, enumResult)) {
...
void ShaderGraph::createConnectedNodes(const ElementPtr &downstreamElement,
  ...
  if (!upstreamNode) {
    throw ExceptionShaderGenError(
        "Upstream element to connect is not a node '" +
  ...
  OutputPtr nodeDefOutput =
      connectingElement ? upstreamNode->getNodeDefOutput(connectingElement)
                        : nullptr;
  ShaderOutput *output = nodeDefOutput
                             ? newNode->getOutput(nodeDefOutput->getName())
                             : newNode->getOutput();
  if (!output) {
    throw ExceptionShaderGenError(
        "Could not find an output named '" +
        (nodeDefOutput ? nodeDefOutput->getName() : string("out")) +
  ...
  if (downstreamNode) {
    ...
    if (downstream && connectingElement) {
      ...
      throw ExceptionShaderGenError("Could not find downstream node ' " +
                                    downstreamNode->getName() + "'");
    }
  } else {
    // Not a node, then it must be an output
    ShaderGraphOutputSocket *outputSocket =
        getOutputSocket(downstreamElement->getName());
    if (outputSocket) {
...
void ShaderGraph::addUpstreamDependencies(const Element &root,
  ...
  for (Edge edge : root.traverseGraph()) {
    ...
    createConnectedNodes(downstreamElement, upstreamElement,
                         edge.getConnectingElement(), context);
  }
}
...
void ShaderGraph::addDefaultGeomNode(ShaderInput *input,
  ...
  if (!node) {
    // Find the nodedef for the geometric node referenced by the geomprop. Use
    // the type of the input here and ignore the type of the geomprop. They are
    // required to have the same type.
    string geomNodeDefName =
        "ND_" + geomprop.getGeomProp() + "_" + input->getType()->getName();
    NodeDefPtr geomNodeDef = _document->getNodeDef(geomNodeDefName);
    if (!geomNodeDef) {
      throw ExceptionShaderGenError(
          "Could not find a nodedef named '" + geomNodeDefName +
          "' for defaultgeomprop on input '" + input->getFullName() + "'");
    }

    ShaderNodePtr geomNode =
        ShaderNode::create(this, geomNodeName, *geomNodeDef, context);
    addNode(geomNode);
    ...
    if (!space.empty()) {
      ShaderInput *spaceInput =
          geomNode->getInput(GeomPropDef::SPACE_ATTRIBUTE);
      ValueElementPtr nodeDefSpaceInput =
          geomNodeDef->getActiveValueElement(GeomPropDef::SPACE_ATTRIBUTE);
      if (spaceInput && nodeDefSpaceInput) {
        std::pair<const TypeDesc *, ValuePtr> enumResult;
        const string &enumNames =
            nodeDefSpaceInput->getAttribute(ValueElement::ENUM_ATTRIBUTE);
        const TypeDesc *portType = TypeDesc::get(nodeDefSpaceInput->getType());
        if (context.getShaderGenerator().getSyntax().remapEnumeration(
                space, portType, enumNames, enumResult)) {
    ...
    if (!geomProp.empty()) {
      ShaderInput *geomPropInput =
          geomNode->getInput(GeomPropDef::GEOM_PROP_ATTRIBUTE);
      if (geomPropInput) {
    ...
    variable =
        syntax.getVariableName(variable, output->getType(), _identifiers);
...
void ShaderGraph::addColorTransformNode(ShaderInput *input,
  ...
  ColorManagementSystemPtr colorManagementSystem =
      context.getShaderGenerator().getColorManagementSystem();
  if (!colorManagementSystem) {
    return;
  }
  const string colorTransformNodeName = input->getFullName() + "_cm";
  ShaderNodePtr colorTransformNodePtr = colorManagementSystem->createNode(
      this, transform, colorTransformNodeName, context);
...
void ShaderGraph::addUnitTransformNode(ShaderInput *input,
  ...
  ShaderNodePtr unitTransformNodePtr =
      unitSystem->createNode(this, transform, unitTransformNodeName, context);
...
ShaderGraphPtr ShaderGraph::create(const ShaderGraph *parent,
  ...
  if (!nodeDef) {
    throw ExceptionShaderGenError(
        "Can't find nodedef '" + nodeGraph.getNodeDefString() +
        "' referenced by nodegraph '" + nodeGraph.getName() + "'");
  }

  string graphName = nodeGraph.getName();
  context.getShaderGenerator().getSyntax().makeValidName(graphName);
  ShaderGraphPtr graph = std::make_shared<ShaderGraph>(
      parent, graphName, nodeGraph.getDocument(), context.getReservedWords());
...
ShaderGraphPtr ShaderGraph::createSurfaceShader(const string &name,
  ...
  if (!nodeDef) {
    throw ExceptionShaderGenError("Could not find a nodedef for shader node '" +
                                  node->getName() + "' with category '" +
                                  node->getCategory() + "'");
  }

  ShaderGraphPtr graph = std::make_shared<ShaderGraph>(
      parent, name, node->getDocument(), context.getReservedWords());
  ...
  ShaderNodePtr newNode =
      ShaderNode::create(graph.get(), newNodeName, *nodeDef, context);
  ...
  string targetColorSpace =
      context.getOptions().targetColorSpaceOverride.empty()
  ...
  for (const InputPtr &nodeDefInput : nodeDef->getActiveInputs()) {
    ShaderGraphInputSocket *inputSocket =
        graph->getInputSocket(nodeDefInput->getName());
    ShaderInput *input = newNode->getInput(nodeDefInput->getName());
    if (!inputSocket || !input) {
      throw ExceptionShaderGenError(
          "Shader input '" + nodeDefInput->getName() +
          "' doesn't match an existing input on graph '" + graph->getName() +
          "'");
    }
    ...
    if (nodeInput) {
      ...
      if (nodeInputValue) {
        ...
        input->setBindInput();
        graph->populateColorTransformMap(colorManagementSystem, input,
                                         nodeInput, targetColorSpace, true);
        graph->populateUnitTransformMap(unitSystem, input, nodeInput,
                                        targetDistanceUnit, true);
      }
    ...
    if (isUniform) {
      ...
      // If no explicit connection, connect to geometric node if a geomprop is
      // used or otherwise to the graph interface.
      const string &connection =
          nodeInput ? nodeInput->getOutputString() : EMPTY_STRING;
      if (connection.empty()) {
        if (geomprop) {
...
ShaderGraphPtr ShaderGraph::create(const ShaderGraph *parent,
                                   const string &name, ElementPtr element,
                                   GenContext &context) {
  ...
  if (element->isA<Output>()) {
    ...
    if (outputParent->isA<NodeGraph>()) {
      ...
      interface =
          outputParent ? outputParent->asA<InterfaceElement>() : nullptr;
    }
    if (!interface) {
      throw ExceptionShaderGenError(
    ...
    ShaderGraphOutputSocket *outputSocket = graph->addOutputSocket(
        output->getName(), TypeDesc::get(output->getType()));
    ...
    if (node->getType() == SURFACE_SHADER_TYPE_STRING) {
      ...
      if (!nodeDef) {
        throw ExceptionShaderGenError("Could not find a nodedef for node '" +
                                      node->getName() + "'");
      }

      graph = std::make_shared<ShaderGraph>(
          parent, name, element->getDocument(), context.getReservedWords());
      ...
      // Create this shader node in the graph.
      ShaderNodePtr newNode =
          ShaderNode::create(graph.get(), node->getName(), *nodeDef, context);
      graph->addNode(newNode);
      ...
      for (const InputPtr &nodedefInput : nodeDef->getActiveInputs()) {
        ...
        if (!inputSocket || !input) {
          throw ExceptionShaderGenError(
              ...
              "' doesn't match an existing input on graph '" +
              graph->getName() + "'");
        }
  ...
  if (!graph) {
    throw ExceptionShaderGenError(
        "Shader generation from element '" + element->getName() +
        "' of type '" + element->getCategory() + "' is not supported");
  }
...
ShaderNode *ShaderGraph::createNode(const Node &node, GenContext &context) {
  ...
  for (const InputPtr &nodeDefInput : nodeDef->getActiveInputs()) {
    ...
    const string &connection =
        nodeInput ? nodeInput->getNodeName() : EMPTY_STRING;
  ...
  for (InputPtr input : node.getInputs()) {
    if (input->getType() == FILENAME_TYPE_STRING) {
      ...
      if (shaderOutput) {
        string colorSpace =
            populateColorTransformMap(colorManagementSystem, shaderOutput,
                                      input, targetColorSpace, false);
        ...
        populateUnitTransformMap(unitSystem, shaderOutput, input,
                                 targetDistanceUnit, false);
      }
    } else {
      ShaderInput *shaderInput = newNode->getInput(input->getName());
      populateColorTransformMap(colorManagementSystem, shaderInput, input,
                                targetColorSpace, true);
      populateUnitTransformMap(unitSystem, shaderInput, input,
                               targetDistanceUnit, true);
    }
  }
...
void ShaderGraph::finalize(GenContext &context) {
  ...
  if (outputSocket->getConnection()) {
    addClassification(
        outputSocket->getConnection()->getNode()->getClassification());
  }
  ...
  if (numEdits > 0) {
    ...
    for (ShaderGraphOutputSocket *outputSocket : getOutputSockets()) {
      ...
      if (upstreamPort && upstreamPort->getNode() != this) {
        for (ShaderGraphEdge edge :
             ShaderGraph::traverseUpstream(upstreamPort)) {
...
void ShaderGraph::bypass(GenContext &context, ShaderNode *node,
  ...
  if (upstream) {
    ...
    for (ShaderInput *downstream : downstreamConnections) {
      ...
      if (!channels.empty()) {
        downstream->setValue(
            context.getShaderGenerator().getSyntax().getSwizzledValue(
                input->getValue(), input->getType(), channels,
                downstream->getType()));
...
void ShaderGraph::setVariableNames(GenContext &context) {
  ...
  for (ShaderGraphInputSocket *inputSocket : getInputSockets()) {
    const string variable = syntax.getVariableName(
        inputSocket->getName(), inputSocket->getType(), _identifiers);
    inputSocket->setVariable(variable);
  }
  for (ShaderGraphOutputSocket *outputSocket : getOutputSockets()) {
    const string variable = syntax.getVariableName(
        outputSocket->getName(), outputSocket->getType(), _identifiers);
  ...
  for (ShaderNode *node : getNodes()) {
    for (ShaderInput *input : node->getInputs()) {
      ...
      variable =
          syntax.getVariableName(variable, input->getType(), _identifiers);
...
string ShaderGraph::populateColorTransformMap(
    ColorManagementSystemPtr colorManagementSystem, ShaderPort *shaderPort,
  ...
  if (shaderPort && !sourceColorSpace.empty()) {
    if (shaderPort->getType() == Type::COLOR3 ||
        shaderPort->getType() == Type::COLOR4) {
      // If we're converting between two identical color spaces than we have no
      // work to do.
      if (sourceColorSpace != targetColorSpace) {
        // Cache colorspace on shader port
        shaderPort->setColorSpace(sourceColorSpace);
        if (colorManagementSystem) {
          ColorSpaceTransform transform(sourceColorSpace, targetColorSpace,
                                        shaderPort->getType());
          if (colorManagementSystem->supportsTransform(transform)) {
            if (asInput) {
              _inputColorTransformMap.emplace(
                  static_cast<ShaderInput *>(shaderPort), transform);
            } else {
              _outputColorTransformMap.emplace(
                  static_cast<ShaderOutput *>(shaderPort), transform);
            }
          } else {
...
void ShaderGraph::populateUnitTransformMap(UnitSystemPtr unitSystem,
  ...
  // Only support convertion for float and vectors. arrays, matrices are not
  // supported.
  ...
  if (supportedType) {
    UnitTransform transform(sourceUnitSpace, targetUnitSpace,
                            shaderPort->getType(), unitType);
    if (unitSystem->supportsTransform(transform)) {
      shaderPort->setUnit(sourceUnitSpace);
      if (asInput) {
        _inputUnitTransformMap.emplace(static_cast<ShaderInput *>(shaderPort),
                                       transform);
      } else {
        _outputUnitTransformMap.emplace(static_cast<ShaderOutput *>(shaderPort),
                                        transform);
      }
    }
  }
...
ShaderGraphEdgeIterator::ShaderGraphEdgeIterator(ShaderOutput *output)
    : _upstream(output), _downstream(nullptr) {}
...
ShaderGraphEdgeIterator &ShaderGraphEdgeIterator::operator++() {
  ...
  while (true) {
    ...
    while (parentFrame.second + 1 < parentFrame.first->getNode()->numInputs()) {
      ShaderInput *input =
          parentFrame.first->getNode()->getInput(++parentFrame.second);
      ShaderOutput *output = input->getConnection();
...
void ShaderGraphEdgeIterator::extendPathUpstream(ShaderOutput *upstream,
                                                 ShaderInput *downstream) {
  // Check for cycles.
  if (_path.count(upstream)) {
    throw ExceptionFoundCycle("Encountered cycle at element: " +
