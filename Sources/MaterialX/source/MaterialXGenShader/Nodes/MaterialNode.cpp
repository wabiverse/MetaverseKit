...
void MaterialNode::addClassification(ShaderNode &node) const {
  const ShaderInput *surfaceshaderInput =
      node.getInput(ShaderNode::SURFACESHADER);
...
void MaterialNode::emitFunctionCall(const ShaderNode &_node,
                                    GenContext &context,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    shadergen.emitLine(syntax.getTypeName(output->getType()) + " " +
                           output->getVariable() + " = " +
