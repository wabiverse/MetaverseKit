...
void ClosureCompoundNode::emitFunctionDefinition(const ShaderNode &node,
                                                 GenContext &context,
...
void ClosureCompoundNode::emitFunctionDefinition(ClosureContext *cct,
                                                 GenContext &context,
                                                 ShaderStage &stage) const {
  const ShaderGenerator &shadergen = context.getShaderGenerator();
  ...
  for (ShaderGraphInputSocket *inputSocket : _rootGraph->getInputSockets()) {
    shadergen.emitString(delim + syntax.getTypeName(inputSocket->getType()) +
  ...
  for (ShaderGraphOutputSocket *outputSocket : _rootGraph->getOutputSockets()) {
    shadergen.emitString(delim +
                             syntax.getOutputTypeName(outputSocket->getType()) +
...
void ClosureCompoundNode::emitFunctionCall(const ShaderNode &node,
                                           GenContext &context,
                                           ShaderStage &stage) const {
  const ShaderGenerator &shadergen = context.getShaderGenerator();
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    if (cct) {
      ...
      // Emit function name.
      shadergen.emitString(_functionName + cct->getSuffix(closureType) + "(",
                           stage);
