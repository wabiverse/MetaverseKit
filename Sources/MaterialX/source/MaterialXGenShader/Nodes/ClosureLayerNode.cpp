...
void ClosureLayerNode::emitFunctionCall(const ShaderNode &_node,
                                        GenContext &context,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    if (top->hasClassification(ShaderNode::Classification::THINFILM)) {
      ...
      // Make sure the connection to base is a sibling node and not the graph
      // interface.
      if (base->getParent() != node.getParent()) {
        throw ExceptionShaderGenError(
            "Thin-film can only be applied to a sibling node, not through a "
            "graph interface");
      }
      ...
      ScopedSetVariableName setVariable(output->getVariable(),
                                        base->getOutput());
      ...
      // Make sure the connections are sibling nodes and not the graph
      // interface.
      ...
      if (base->getOutput()->getType() == Type::VDF) {
        shadergen.emitLine(output->getVariable() + ".response = " + topResult +
                               ".response * " + baseResult + ".throughput",
                           stage);
        shadergen.emitLine(output->getVariable() +
                               ".throughput = " + topResult + ".throughput * " +
                               baseResult + ".throughput",
                           stage);
      } else {
        shadergen.emitLine(output->getVariable() + ".response = " + topResult +
                               ".response + " + baseResult + ".response * " +
                               topResult + ".throughput",
                           stage);
        shadergen.emitLine(output->getVariable() +
                               ".throughput = " + topResult + ".throughput * " +
                               baseResult + ".throughput",
