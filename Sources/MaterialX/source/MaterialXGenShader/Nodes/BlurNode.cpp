...
BlurNode::BlurNode() : ConvolutionNode() {}
...
bool BlurNode::acceptsInputType(const TypeDesc *type) const {
  ...
  return ((type == Type::FLOAT && type->isScalar()) || type->isFloat2() ||
          type->isFloat3() || type->isFloat4());
}

void BlurNode::outputSampleArray(const ShaderGenerator &shadergen,
                                 ShaderStage &stage, const TypeDesc *inputType,
                                 const string &sampleName,
                                 const StringVec &sampleStrings) const {
  ...
  shadergen.emitLine(inputTypeString + " " + sampleName + "[" +
                         MX_MAX_SAMPLE_COUNT_STRING + "]",
                     stage);
  for (size_t i = 0; i < sampleStrings.size(); i++) {
    shadergen.emitLine(sampleName + "[" + std::to_string(i) +
                           "] = " + sampleStrings[i],
                       stage);
  }
}
...
void BlurNode::emitFunctionCall(const ShaderNode &node, GenContext &context,
                                ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    const string &inputTypeString =
        inInput && acceptsInputType(inInput->getType())
    ...
    if (!filterTypeInput || inputTypeString.empty()) {
      throw ExceptionShaderGenError("Node '" + node.getName() +
                                    "' is not a valid Blur node");
    }

    // Compute width of filter. Default is 1 which just means one 1x1 upstream
    // samples
    ...
    // Emit samples
    // Note: The maximum sample count MX_MAX_SAMPLE_COUNT is defined in the
    // shader code and is assumed to be 49 (7x7 kernel). If this changes the
    // filter size logic here needs to be adjusted.
    //
    StringVec sampleStrings;
    emitInputSamplesUV(node, sampleCount, filterWidth, _filterSize,
                       _filterOffset, _sampleSizeFunctionUV, context, stage,
                       sampleStrings);

    // There should always be at least 1 sample
    if (sampleStrings.empty()) {
      throw ExceptionShaderGenError("Node '" + node.getName() +
                                    "' cannot compute upstream samples");
    }
    ...
    if (sampleCount > 1) {
      ...
      outputSampleArray(shadergen, stage, inInput->getType(), sampleName,
                        sampleStrings);
      ...
      if (syntax.remapEnumeration(GAUSSIAN_FILTER, Type::STRING, FILTER_LIST,
                                  result)) {
      ...
      shadergen.emitScopeBegin(stage);
      {
        string filterFunctionName =
            MX_CONVOLUTION_PREFIX_STRING + inputTypeString;
        shadergen.emitLineBegin(stage);
      ...
      shadergen.emitScopeBegin(stage);
      {
        ...
        shadergen.emitString(output->getVariable(), stage);
        shadergen.emitString(" = " + filterFunctionName, stage);
        shadergen.emitString("(" + sampleName + ", " + BOX_WEIGHTS_VARIABLE +
                                 ", " + std::to_string(arrayOffset) + ", " +
                                 std::to_string(sampleCount) + ")",
                             stage);
        shadergen.emitLineEnd(stage);
