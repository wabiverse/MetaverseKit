...
void SwizzleNode::emitFunctionCall(const ShaderNode &node, GenContext &context,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    if (!in || !channels) {
      throw ExceptionShaderGenError("Node '" + node.getName() +
                                    "' is not a valid swizzle node");
    }
    if (!in->getConnection() && !in->getValue()) {
      throw ExceptionShaderGenError(
          "No connection or value found to swizzle on node '" + node.getName() +
          "'");
    }

    const string &swizzle = channels->getValue()
                                ? channels->getValue()->getValueString()
                                : EMPTY_STRING;
    ...
    if (!swizzle.empty()) {
      ...
      variableName = shadergen.getSyntax().getSwizzledVariable(
...
string SwizzleNode::getVariableName(const ShaderInput *input) const {
  return input->getConnection() ? input->getConnection()->getVariable()
                                : input->getVariable();
}
