...
const std::array<float, 7> GAUSSIAN_KERNEL_7 = {
    0.00598f,  0.060626f, 0.241843f, 0.383103f,
    0.241843f, 0.060626f, 0.00598f // Sigma 1
};

ConvolutionNode::ConvolutionNode() {}

void ConvolutionNode::createVariables(const ShaderNode &, GenContext &,
                                      Shader &shader) const {
  ...
  constants.add(Type::FLOATARRAY, "c_box_filter_weights",
                Value::createValue<vector<float>>(boxWeightArray));
  ...
  for (unsigned int y = 0; y < 3; y++) {
    for (unsigned int x = 0; x < 3; x++) {
      gaussianWeightArray.push_back(GAUSSIAN_KERNEL_3[y] *
                                    GAUSSIAN_KERNEL_3[x]);
    }
  }
  for (unsigned int y = 0; y < 5; y++) {
    for (unsigned int x = 0; x < 5; x++) {
      gaussianWeightArray.push_back(GAUSSIAN_KERNEL_5[y] *
                                    GAUSSIAN_KERNEL_5[x]);
    }
  }
  for (unsigned int y = 0; y < 7; y++) {
    for (unsigned int x = 0; x < 7; x++) {
      gaussianWeightArray.push_back(GAUSSIAN_KERNEL_7[y] *
                                    GAUSSIAN_KERNEL_7[x]);
    }
  }
  constants.add(Type::FLOATARRAY, "c_gaussian_filter_weights",
                Value::createValue<vector<float>>(gaussianWeightArray));
}

/// Get input which is used for sampling. If there is none
/// then a null pointer is returned.
const ShaderInput *
ConvolutionNode::getSamplingInput(const ShaderNode &node) const {
...
void ConvolutionNode::emitInputSamplesUV(
    const ShaderNode &node, unsigned int sampleCount, unsigned int filterWidth,
    float filterSize, float filterOffset, const string &sampleSizeFunctionUV,
    GenContext &context, ShaderStage &stage, StringVec &sampleStrings) const {
  sampleStrings.clear();
  ...
  const ShaderOutput *inConnection =
      inInput ? inInput->getConnection() : nullptr;

  if (inConnection && inConnection->getType() &&
      acceptsInputType(inConnection->getType())) {
    const ShaderNode *upstreamNode = inConnection->getNode();
    if (upstreamNode &&
        upstreamNode->hasClassification(ShaderNode::Classification::SAMPLE2D)) {
      ...
      if (upstreamOutput) {
        ...
        // If the sample count is 1 then the sample code has already been
        // emitted
        const ShaderInput *samplingInput =
            (sampleCount > 1) ? getSamplingInput(*upstreamNode) : nullptr;
        ...
        if (samplingInput) {
          ...
          string sampleInputValue =
              shadergen.getUpstreamResult(samplingInput, context);

          const string sampleSizeName(output->getVariable() + "_sample_size");
          const string vec2TypeString =
              shadergen.getSyntax().getTypeName(Type::VECTOR2);
          string sampleCall(vec2TypeString + " " + sampleSizeName + " = " +
                            sampleSizeFunctionUV + "(" + sampleInputValue +
          ...
          for (unsigned int i = 0; i < sampleCount; i++) {
            ...
            string outputSuffix("_" + output->getVariable() +
                                std::to_string(i));
    ...
    if (!inInput || !inInput->getValue()) {
      throw ExceptionShaderGenError("No connection or value found on node: '" +
                                    node.getName() + "'");
    }
  }
