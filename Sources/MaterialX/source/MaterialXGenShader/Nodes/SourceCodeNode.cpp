...
void SourceCodeNode::initialize(const InterfaceElement &element,
                                GenContext &context) {
  ShaderNodeImpl::initialize(element, context);

  if (!element.isA<Implementation>()) {
    throw ExceptionShaderGenError("Element '" + element.getName() +
  ...
  if (_functionSource.empty()) {
    ...
    if (_functionSource.empty()) {
      throw ExceptionShaderGenError(
          "Failed to get source code from file '" + _sourceFilename.asString() +
  ...
  if (!_inlined) {
    ...
    if (_functionName != validFunctionName) {
      throw ExceptionShaderGenError(
          "Function name '" + _functionName + "' used by implementation '" +
          impl.getName() + "' is not a valid identifier.");
    }
  } else {
...
void SourceCodeNode::emitFunctionDefinition(const ShaderNode &,
                                            GenContext &context,
...
void SourceCodeNode::emitFunctionCall(const ShaderNode &node,
                                      GenContext &context,
                                      ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    const ShaderGenerator &shadergen = context.getShaderGenerator();
    if (_inlined) {
      ...
      while (i != string::npos) {
        ...
        if (j == string::npos) {
          throw ExceptionShaderGenError(
              "Malformed inline expression in implementation for node " +
              node.getName());
        }
        ...
        if (!input) {
          throw ExceptionShaderGenError("Could not find an input named '" +
                                        variable + "' on node '" +
                                        node.getName() + "'");
        }
        ...
        if (input->getConnection()) {
          ...
          if (!variableNames.count(variableName)) {
            ShaderPort v(nullptr, input->getType(), variableName,
                         input->getValue());
            shadergen.emitLineBegin(stage);
            const Syntax &syntax = shadergen.getSyntax();
            const string valueStr =
                (v.getValue() ? syntax.getValue(v.getType(), *v.getValue())
