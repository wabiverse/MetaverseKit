...
void HwTexCoordNode::createVariables(const ShaderNode &node, GenContext &,
                                     Shader &shader) const {
  ...
  addStageInput(HW::VERTEX_INPUTS, output->getType(),
                HW::T_IN_TEXCOORD + "_" + index, vs, true);
  addStageConnector(HW::VERTEX_DATA, output->getType(),
                    HW::T_TEXCOORD + "_" + index, vs, ps, true);
}

void HwTexCoordNode::emitFunctionCall(const ShaderNode &node,
                                      GenContext &context,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    // Extract the requested number of components from the texture coordinates
    // (which may be a larger datatype than the requested number of texture
    // coordinates, if several texture coordinate nodes with different width
    // coexist).
    ...
    shadergen.emitString(" = " + prefix + texcoord->getVariable() + suffix,
                         stage);
