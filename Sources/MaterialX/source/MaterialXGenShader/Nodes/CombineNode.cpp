...
void CombineNode::emitFunctionCall(const ShaderNode &node, GenContext &context,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    if (in1->getType() == Type::FLOAT) {
    ...
    } else if (in1->getType() == Type::COLOR3 ||
               in1->getType() == Type::VECTOR3) {
      ...
      string in1Variable = in1->getConnection()
                               ? in1->getConnection()->getVariable()
                               : in1->getVariable();
        ...
        string variableValue =
            in1->getValue()
                ? shadergen.getSyntax().getValue(in1->getType(),
                                                 *in1->getValue())
                : shadergen.getSyntax().getDefaultValue(in1->getType());
      ...
      const StringVec &in1Members =
          shadergen.getSyntax().getTypeSyntax(in1->getType()).getMembers();
      ...
      // for it first, in order to access components from it below.
      string in2Variable = in2->getConnection()
                               ? in2->getConnection()->getVariable()
                               : in2->getVariable();
      if (!in2->getConnection()) {
        string variableValue =
            in2->getValue()
                ? shadergen.getSyntax().getValue(in2->getType(),
                                                 *in2->getValue())
                : shadergen.getSyntax().getDefaultValue(in2->getType());
        shadergen.emitLine(shadergen.getSyntax().getTypeName(in2->getType()) +
                               " " + in2Variable + " = " + variableValue,
                           stage);
      }
      ...
      const StringVec &in2Members =
          shadergen.getSyntax().getTypeSyntax(in2->getType()).getMembers();
    ...
    const TypeSyntax &outTypeSyntax =
        shadergen.getSyntax().getTypeSyntax(out->getType());
