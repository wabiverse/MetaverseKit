...
void ClosureMixNode::emitFunctionCall(const ShaderNode &_node,
                                      GenContext &context,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    if (output->getType() == Type::BSDF) {
      ...
      shadergen.emitLine(output->getVariable() + ".response = mix(" + bgResult +
                             ".response, " + fgResult + ".response, " +
                             mixResult + ")",
                         stage);
      shadergen.emitLine(output->getVariable() + ".throughput = mix(" +
                             bgResult + ".throughput, " + fgResult +
                             ".throughput, " + mixResult + ")",
                         stage);
    } else if (output->getType() == Type::EDF) {
      shadergen.emitLine(shadergen.getSyntax().getTypeName(Type::EDF) + " " +
                             output->getVariable() + " = mix(" + bgResult +
                             ", " + fgResult + ", " + mixResult + ")",
                         stage);
    }
  }
