...
void ConvertNode::emitFunctionCall(const ShaderNode &node, GenContext &context,
                                   ShaderStage &stage) const {
  using ConvertTable =
      std::unordered_map<const TypeDesc *,
                         std::unordered_map<const TypeDesc *, string>>;

  static const ConvertTable CONVERT_TABLE(
      {{Type::COLOR3,
        {{Type::VECTOR3, string("rgb")}, {Type::COLOR4, string("rgb1")}}},
       {Type::COLOR4,
        {{Type::VECTOR4, string("rgba")}, {Type::COLOR3, string("rgb")}}},
       {Type::VECTOR2, {{Type::VECTOR3, string("xy0")}}},
       {Type::VECTOR3,
        {{Type::COLOR3, string("xyz")},
         {Type::VECTOR4, string("xyz1")},
         {Type::VECTOR2, string("xy")}}},
       {Type::VECTOR4,
        {{Type::COLOR4, string("xyzw")}, {Type::VECTOR3, string("xyz")}}},
       {Type::FLOAT,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    if (!in || !out) {
      throw ExceptionShaderGenError("Node '" + node.getName() +
                                    "' is not a valid convert node");
    }
    if (!in->getConnection() && !in->getValue()) {
      throw ExceptionShaderGenError(
          "No connection or value found to convert on node '" + node.getName() +
          "'");
    }
    ...
    if (in->getType()->isScalar() && out->getType()->isScalar()) {
      ...
      result = shadergen.getSyntax().getTypeName(out->getType()) + "(" +
               result + ")";
    }
    // Handle supported vector type conversions.
      ...
      if (!swizzle || swizzle->empty()) {
        throw ExceptionShaderGenError(
            "Conversion from '" + in->getType()->getName() + "' to '" +
            out->getType()->getName() + "' is not supported by convert node");
      }
      ...
      if (!in->getConnection()) {
        ...
        string variableValue =
            in->getValue()
                ? shadergen.getSyntax().getValue(in->getType(), *in->getValue())
                : shadergen.getSyntax().getDefaultValue(in->getType());
        shadergen.emitLine(shadergen.getSyntax().getTypeName(in->getType()) +
      ...
      const TypeDesc *type =
          in->getConnection() ? in->getConnection()->getType() : in->getType();
      result = shadergen.getSyntax().getSwizzledVariable(
          variableName, type, *swizzle, node.getOutput()->getType());
    }
