...
const string
    HwShaderGenerator::CLOSURE_CONTEXT_SUFFIX_REFLECTION("_reflection");
const string
    HwShaderGenerator::CLOSURE_CONTEXT_SUFFIX_TRANSMISSION("_transmission");
...
HwShaderGenerator::HwShaderGenerator(SyntaxPtr syntax)
  ...
  _tokenSubstitutions[HW::T_WORLD_TRANSPOSE_MATRIX] =
      HW::WORLD_TRANSPOSE_MATRIX;
  _tokenSubstitutions[HW::T_WORLD_INVERSE_TRANSPOSE_MATRIX] =
  ...
  _tokenSubstitutions[HW::T_VIEW_INVERSE_TRANSPOSE_MATRIX] =
      HW::VIEW_INVERSE_TRANSPOSE_MATRIX;
  ...
  _tokenSubstitutions[HW::T_PROJ_INVERSE_TRANSPOSE_MATRIX] =
      HW::PROJ_INVERSE_TRANSPOSE_MATRIX;
  _tokenSubstitutions[HW::T_WORLD_VIEW_MATRIX] = HW::WORLD_VIEW_MATRIX;
  _tokenSubstitutions[HW::T_VIEW_PROJECTION_MATRIX] =
      HW::VIEW_PROJECTION_MATRIX;
  _tokenSubstitutions[HW::T_WORLD_VIEW_PROJECTION_MATRIX] =
      HW::WORLD_VIEW_PROJECTION_MATRIX;
  ...
  _tokenSubstitutions[HW::T_NUM_ACTIVE_LIGHT_SOURCES] =
      HW::NUM_ACTIVE_LIGHT_SOURCES;
  ...
  _defTransmission.addArgument(
      Type::BSDF, ClosureContext::Argument(Type::VECTOR3, HW::DIR_V));
  // Environment context
  _defIndirect.setSuffix(Type::BSDF, CLOSURE_CONTEXT_SUFFIX_INDIRECT);
  _defIndirect.addArgument(Type::BSDF,
                           ClosureContext::Argument(Type::VECTOR3, HW::DIR_V));
  // Emission context
  _defEmission.addArgument(Type::EDF,
                           ClosureContext::Argument(Type::VECTOR3, HW::DIR_N));
  _defEmission.addArgument(Type::EDF,
                           ClosureContext::Argument(Type::VECTOR3, HW::DIR_L));
}

ShaderPtr HwShaderGenerator::createShader(const string &name,
                                          ElementPtr element,
  ...
  // Check if there are inputs with default geomprops assigned. In order to bind
  // the corresponding data to these inputs we insert geomprop nodes in the
  // graph.
  ...
  // Public uniforms are inputs that should be published in a user interface for
  // user interaction, while private uniforms are internal variables needed by
  // the system which should not be exposed in UI. So when creating these
  // uniforms for a shader node, if the variable is user-facing it should go
  // into the public block, and otherwise the private block. All texture based
  // objects should be added to Sampler block
  ...
  VariableBlockPtr psPrivateUniforms =
      ps->createUniformBlock(HW::PRIVATE_UNIFORMS, "u_prv");
  VariableBlockPtr psPublicUniforms =
      ps->createUniformBlock(HW::PUBLIC_UNIFORMS, "u_pub");
  VariableBlockPtr lightData =
      ps->createUniformBlock(HW::LIGHT_DATA, HW::T_LIGHT_DATA_INSTANCE);
  ...
  if (context.getOptions().hwTransparency) {
    psPrivateUniforms->add(Type::FLOAT, HW::T_ALPHA_THRESHOLD,
                           Value::createValue(0.001f));
  }
  ...
  if (context.getOptions().hwShadowMap) {
    ...
    psPrivateUniforms->add(Type::MATRIX44, HW::T_SHADOW_MATRIX,
                           Value::createValue(Matrix44::IDENTITY));
  }

  // Add inputs and uniforms for ambient occlusion.
  if (context.getOptions().hwAmbientOcclusion) {
    addStageInput(HW::VERTEX_INPUTS, Type::VECTOR2, HW::T_IN_TEXCOORD + "_0",
                  *vs);
    addStageConnector(HW::VERTEX_DATA, Type::VECTOR2, HW::T_TEXCOORD + "_0",
                      *vs, *ps);
    psPrivateUniforms->add(Type::FILENAME, HW::T_AMB_OCC_MAP);
    psPrivateUniforms->add(Type::FLOAT, HW::T_AMB_OCC_GAIN,
                           Value::createValue(1.0f));
  }
  ...
  if (lighting && context.getOptions().hwSpecularEnvironmentMethod !=
                      SPECULAR_ENVIRONMENT_NONE) {
    const Matrix44 yRotationPI = Matrix44::createScale(Vector3(-1, 1, -1));
    psPrivateUniforms->add(Type::MATRIX44, HW::T_ENV_MATRIX,
                           Value::createValue(yRotationPI));
    psPrivateUniforms->add(Type::FILENAME, HW::T_ENV_RADIANCE);
    psPrivateUniforms->add(Type::INTEGER, HW::T_ENV_RADIANCE_MIPS,
                           Value::createValue<int>(1));
    psPrivateUniforms->add(Type::INTEGER, HW::T_ENV_RADIANCE_SAMPLES,
                           Value::createValue<int>(16));
  ...
  if (context.getOptions().hwDirectionalAlbedoMethod ==
          DIRECTIONAL_ALBEDO_TABLE ||
      context.getOptions().hwWriteAlbedoTable) {
    psPrivateUniforms->add(Type::FILENAME, HW::T_ALBEDO_TABLE);
    psPrivateUniforms->add(Type::INTEGER, HW::T_ALBEDO_TABLE_SIZE,
                           Value::createValue<int>(64));
  }
  ...
  //
  // For image textures we need to convert filenames into uniforms (texture
  // samplers). Any unconnected filename input on file texture nodes needs to
  // have a corresponding uniform.
  //
      ...
      if (node->hasClassification(ShaderNode::Classification::FILETEXTURE)) {
        for (ShaderInput *input : node->getInputs()) {
          if (!input->getConnection() && input->getType() == Type::FILENAME) {
            // Create the uniform using the filename type to make this uniform
            // into a texture sampler.
...
void HwShaderGenerator::emitFunctionCall(const ShaderNode &node,
  ...
  if (stage.isEmitted(node, context)) {
    emitComment("Omitted node '" + node.getName() +
                    "'. Function already called in this scope.",
                stage);
    return;
  }
  ...
  if (node.hasClassification(ShaderNode::Classification::CLOSURE) &&
    ...
    if (cct) {
      match =
          ...
          ((cct->getType() == ClosureContextType::REFLECTION ||
            cct->getType() == ClosureContextType::INDIRECT) &&
          ...
          ((cct->getType() == ClosureContextType::TRANSMISSION) &&
           (node.hasClassification(ShaderNode::Classification::BSDF_T) ||
            node.hasClassification(ShaderNode::Classification::VDF))) ||
...
void HwShaderGenerator::bindLightShader(const NodeDef &nodeDef,
                                        unsigned int lightTypeId,
                                        GenContext &context) {
  if (TypeDesc::get(nodeDef.getType()) != Type::LIGHTSHADER) {
    throw ExceptionShaderGenError(
        "Error binding light shader. Given nodedef '" + nodeDef.getName() +
        "' is not of lightshader type");
  }
  ...
  if (lightShaders->get(lightTypeId)) {
    throw ExceptionShaderGenError(
        "Error binding light shader. Light type id '" +
        std::to_string(lightTypeId) + "' has already been bound");
  }

  ShaderNodePtr shader =
      ShaderNode::create(nullptr, nodeDef.getNodeString(), nodeDef, context);
...
void HwShaderGenerator::unbindLightShader(unsigned int lightTypeId,
                                          GenContext &context) {
  HwLightShadersPtr lightShaders =
      context.getUserData<HwLightShaders>(HW::USER_DATA_LIGHT_SHADERS);
  if (lightShaders) {
...
void HwShaderGenerator::getClosureContexts(
    const ShaderNode &node, vector<ClosureContext *> &ccts) const {
  if (node.hasClassification(ShaderNode::Classification::BSDF)) {
    if (node.hasClassification(ShaderNode::Classification::BSDF_R |
                               ShaderNode::Classification::BSDF_T)) {
...
void HwShaderGenerator::addStageLightingUniforms(GenContext &context,
                                                 ShaderStage &stage) const {
  // Create uniform for number of active light sources
  if (context.getOptions().hwMaxActiveLightSources > 0) {
    ShaderPort *numActiveLights =
        addStageUniform(HW::PRIVATE_UNIFORMS, Type::INTEGER,
                        HW::T_NUM_ACTIVE_LIGHT_SOURCES, stage);
