...
const string LIBRARY_VERSION_STRING =
    std::to_string(MATERIALX_MAJOR_VERSION) + "." +
...
bool invalidNameChar(char c) { return !isalnum(c) && c != '_' && c != ':'; }
...
string getVersionString() { return LIBRARY_VERSION_STRING; }

std::tuple<int, int, int> getVersionIntegers() { return LIBRARY_VERSION_TUPLE; }
...
string trimSpaces(const string &str) {
  ...
  string result =
      (start == std::string::npos) ? EMPTY_STRING : str.substr(start);
  size_t end = result.find_last_not_of(SPACE);
  result =
      (end == std::string::npos) ? EMPTY_STRING : result.substr(0, end + 1);
  return result;
}
