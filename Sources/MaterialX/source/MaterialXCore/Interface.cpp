...
OutputPtr PortElement::getConnectedOutput() const {
  ...
  if (hasNodeGraphString()) {
    NodeGraphPtr nodeGraph =
        resolveNameReference<NodeGraph>(getNodeGraphString(), scope);
    if (!nodeGraph) {
  ...
  if (connectedNode) {
    ...
    if (!outputString.empty()) {
      ...
      if (hasNodeName()) {
        ...
        if (output) {
          validateRequire(connectedNode->getType() == MULTI_OUTPUT_TYPE_STRING,
                          res, message,
                          "Multi-output type expected in port connection");
        }
      } else if (hasNodeGraphString()) {
        NodeGraphPtr nodeGraph =
            resolveNameReference<NodeGraph>(getNodeGraphString());
        if (nodeGraph) {
      ...
      validateRequire(output != nullptr, res, message,
                      "No output found for port connection");

      if (output) {
        if (hasChannels()) {
          bool valid =
              validChannelsString(getChannels(), output->getType(), getType());
          validateRequire(valid, res, message,
                          "Invalid channels string in port connection");
        } else {
          validateRequire(getType() == output->getType(), res, message,
                          "Mismatched types in port connection");
        }
      }
    } else if (hasChannels()) {
      bool valid = validChannelsString(getChannels(), connectedNode->getType(),
                                       getType());
      validateRequire(valid, res, message,
                      "Invalid channels string in port connection");
    } else if (connectedNode->getType() != MULTI_OUTPUT_TYPE_STRING) {
      validateRequire(getType() == connectedNode->getType(), res, message,
                      "Mismatched types in port connection");
    }
  }
...
bool PortElement::validChannelsCharacters(const string &channels,
...
bool PortElement::validChannelsString(const string &channels,
                                      const string &sourceType,
...
NodePtr Input::getConnectedNode() const {
  ...
  if (graphInput &&
      (graphInput->hasNodeName() || graphInput->hasNodeGraphString())) {
...
bool Input::validate(string *message) const {
  ...
  if (hasDefaultGeomPropString()) {
    validateRequire(parent->isA<NodeDef>(), res, message,
                    "Invalid defaultgeomprop on non-definition input");
    validateRequire(getDefaultGeomProp() != nullptr, res, message,
                    "Invalid defaultgeomprop string");
  }
  if (parent->isA<Node>()) {
    bool hasValueBinding = hasValue();
    bool hasConnection = hasNodeName() || hasNodeGraphString() ||
                         hasOutputString() || hasInterfaceName();
    validateRequire(hasValueBinding || hasConnection, res, message,
                    "Node input binds no value or connection");
  } else if (parent->isA<NodeGraph>()) {
    validateRequire(parent->asA<NodeGraph>()->getNodeDef() == nullptr, res,
                    message,
                    "Input element in a functional nodegraph has no effect");
  }
...
ValueElementPtr
InterfaceElement::getActiveValueElement(const string &name) const {
...
vector<ValueElementPtr> InterfaceElement::getActiveValueElements() const {
  ...
  for (ConstElementPtr interface : traverseInheritance()) {
    vector<ValueElementPtr> valueElems =
        interface->getChildrenOfType<ValueElement>();
    for (const ValueElementPtr &valueElem : valueElems) {
      if (valueElem &&
          activeValueElemNamesSet.insert(valueElem->getName()).second) {
...
InterfaceElement::getDeclaration(const string &) const {
...
bool InterfaceElement::hasExactInputMatch(ConstInterfaceElementPtr declaration,
  ...
  for (InputPtr input : getActiveInputs()) {
    ...
    if (!declarationInput || declarationInput->getType() != input->getType()) {
      if (message) {
        *message +=
            "Input '" + input->getName() + "' doesn't match declaration";
      }
      return false;
    }
  }
