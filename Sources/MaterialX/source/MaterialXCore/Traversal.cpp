...
Edge::operator bool() const { return *this != NULL_EDGE; }
...
const TreeIterator &TreeIterator::end() { return NULL_TREE_ITERATOR; }
...
const GraphIterator &GraphIterator::end() { return NULL_GRAPH_ITERATOR; }
...
GraphIterator &GraphIterator::operator++() {
  ...
  if (!_prune && _upstreamElem && _upstreamElem->getUpstreamEdgeCount()) {
    ...
    if (nextEdge && nextEdge.getUpstreamElement()) {
      extendPathUpstream(nextEdge.getUpstreamElement(),
                         nextEdge.getConnectingElement());
      return *this;
    }
  }
  _prune = false;
...
void GraphIterator::extendPathUpstream(ElementPtr upstreamElem,
                                       ElementPtr connectingElem) {
  // Check for cycles.
  if (_pathElems.count(upstreamElem)) {
    throw ExceptionFoundCycle("Encountered cycle at element: " +
                              upstreamElem->asString());
  }
