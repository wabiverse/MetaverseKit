    ...
    for (UnitPtr unit : unitdef->getUnits()) {
      ...
      if (!name.empty()) {
        ...
        if (!scaleString.empty()) {
          ValuePtr scaleValue = Value::createValueFromStrings(
              scaleString, getTypeString<float>());
...
LinearUnitConverterPtr LinearUnitConverter::create(UnitTypeDefPtr unitTypeDef) {
  return std::shared_ptr<LinearUnitConverter>(
      new LinearUnitConverter(unitTypeDef));
}
  ...
  if (!doc->getUnitTypeDef(_unitType)) {
    ...
    if (!doc->getUnitDef(unitdefName)) {
      ...
      for (auto unitScale : _unitScale) {
        ...
        unitPtr->setAttribute(SCALE_ATTRIBUTE,
                              std::to_string(unitScale.second));
      }
    }
  }
}
...
float LinearUnitConverter::convert(float input, const string &inputUnit,
                                   const string &outputUnit) const {
...
void UnitConverterRegistry::clearUnitConverters() { _unitConverters.clear(); }
      ...
      if (value && input->hasUnit() && (input->getUnitType() == unitType)) {
        if (type == getTypeString<float>()) {
          ...
          float convertedValue =
              unitConverter->convert(originalval, input->getUnit(), targetUnit);
