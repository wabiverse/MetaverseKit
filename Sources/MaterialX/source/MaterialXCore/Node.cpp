...
void Node::setConnectedNodeName(const string &inputName,
...
NodeDefPtr Node::getNodeDef(const string &target, bool allowRoughMatch) const {
  ...
  vector<NodeDefPtr> nodeDefs =
      getDocument()->getMatchingNodeDefs(getQualifiedName(getCategory()));
  vector<NodeDefPtr> secondary =
      getDocument()->getMatchingNodeDefs(getCategory());
...
OutputPtr Node::getNodeDefOutput(ElementPtr connectingElement) {
  ...
  if (port) {
    ...
    if (output) {
      if (connectedInput || output->getParent() == output->getDocument()) {
...
vector<PortElementPtr> Node::getDownstreamPorts() const {
  ...
  for (PortElementPtr port :
       getDocument()->getMatchingPorts(getQualifiedName(getName()))) {
    if (port->getConnectedNode() == getSelf()) {
      downstreamPorts.push_back(port);
    }
  }
  std::sort(downstreamPorts.begin(), downstreamPorts.end(),
            [](const ConstElementPtr &a, const ConstElementPtr &b) {
...
bool Node::validate(string *message) const {
  bool res = true;
  validateRequire(!getCategory().empty(), res, message,
  ...
  if (nodeDef) {
    ...
    validateRequire(exactMatch, res, message,
                    "Node interface error: " + matchMessage);
  } else {
    bool categoryDeclared =
        !getDocument()->getMatchingNodeDefs(getCategory()).empty();
    validateRequire(!categoryDeclared, res, message,
                    "Node interface doesn't support this output type");
  }
...
NodePtr GraphElement::addMaterialNode(const string &name,
                                      ConstNodePtr shaderNode) {
  bool isVolumeShader =
      shaderNode && shaderNode->getType() == VOLUME_SHADER_TYPE_STRING;
  string category = isVolumeShader ? VOLUME_MATERIAL_NODE_STRING
                                   : SURFACE_MATERIAL_NODE_STRING;
  NodePtr materialNode = addNode(category, name, MATERIAL_TYPE_STRING);
  if (shaderNode) {
    InputPtr input =
        materialNode->addInput(shaderNode->getType(), shaderNode->getType());
  ...
  while (!nodeQueue.empty()) {
    ...
    for (NodePtr node : nodeQueue) {
      ...
      if (implement && implement->isA<NodeGraph>()) {
        ...
        for (NodePtr sourceSubNode : implement->asA<NodeGraph>()->getNodes()) {
          downstreamPortMap[sourceSubNode] =
              sourceSubNode->getDownstreamPorts();
        }
      }
    }
    nodeQueue.clear();
    ...
    for (NodePtr processNode : processNodeVec) {
      ...
      for (NodePtr sourceSubNode : sourceSubGraph->getNodes()) {
        ...
        setChildIndex(destSubNode->getName(),
                      getChildIndex(processNode->getName()));

        // Store the mapping between subgraphs.
        subNodeMap[sourceSubNode] = destSubNode;

        // Add the subnode to the queue, allowing processing of nested
        // subgraphs.
      ...
      for (const auto &subNodePair : subNodeMap) {
        ...
        for (PortElementPtr sourcePort : downstreamPortMap[sourceSubNode]) {
          if (sourcePort->isA<Input>()) {
            ...
            if (it != subNodeMap.end()) {
              InputPtr processNodeInput =
                  it->second->getInput(sourcePort->getName());
              if (processNodeInput) {
                processNodeInput->setNodeName(destSubNode->getName());
              }
            }
          } else if (sourcePort->isA<Output>()) {
            for (PortElementPtr processNodePort :
                 downstreamPortMap[processNode]) {
        ...
        for (InputPtr destInput : destSubNode->getInputs()) {
          if (destInput->hasInterfaceName()) {
            InputPtr sourceInput =
                processNode->getInput(destInput->getInterfaceName());
            if (sourceInput) {
              destInput->copyContentFrom(sourceInput);
            } else {
              ConstInterfaceElementPtr declaration =
                  declarationMap[processNode];
              InputPtr declInput = declaration
                                       ? declaration->getActiveInput(
                                             destInput->getInterfaceName())
                                       : nullptr;
              if (declInput) {
                ...
                if (declInput->hasDefaultGeomPropString()) {
                  ConstGeomPropDefPtr geomPropDef =
                      getDocument()->getGeomPropDef(
                          declInput->getDefaultGeomPropString());
                  if (geomPropDef) {
                    destInput->setConnectedNode(
                        addGeomNode(geomPropDef, "geomNode"));
                  }
                }
              }
            }
      ...
      for (PortElementPtr downstreamPort : downstreamPortMap[processNode]) {
        if (downstreamPort->hasOutputString()) {
          OutputPtr subGraphOutput =
              sourceSubGraph->getOutput(downstreamPort->getOutputString());
          if (subGraphOutput) {
            string destName = subGraphOutput->getNodeName();
            NodePtr sourceSubNode = sourceSubGraph->getNode(destName);
            NodePtr destNode =
                sourceSubNode ? subNodeMap[sourceSubNode] : nullptr;
            if (destNode) {
  ...
  while (!childQueue.empty()) {
    ...
    if (child->isA<Node>()) {
      for (PortElementPtr port : child->asA<Node>()->getDownstreamPorts()) {
        const ElementPtr downstreamElem =
            port->isA<Output>() ? port : port->getParent();
...
NodePtr GraphElement::addGeomNode(ConstGeomPropDefPtr geomPropDef,
  ...
  if (!geomNode) {
    geomNode = addNode(geomPropDef->getGeomProp(), geomNodeName,
                       geomPropDef->getType());
    if (geomPropDef->hasAttribute(GeomPropDef::SPACE_ATTRIBUTE)) {
      geomNode->setInputValue(
          GeomPropDef::SPACE_ATTRIBUTE,
          geomPropDef->getAttribute(GeomPropDef::SPACE_ATTRIBUTE));
    }
    if (geomPropDef->hasAttribute(GeomPropDef::INDEX_ATTRIBUTE)) {
      geomNode->setInputValue(
          GeomPropDef::INDEX_ATTRIBUTE,
          geomPropDef->getAttribute(GeomPropDef::INDEX_ATTRIBUTE),
          getTypeString<int>());
    }
  }
  return geomNode;
  ...
  for (ElementPtr elem : children) {
    ...
    if (node) {
      ...
      const string &nodeGroup =
          nodeDef ? nodeDef->getNodeGroup() : EMPTY_STRING;
  ...
  for (OutputPtr output : getOutputs()) {
    for (Edge edge : output->traverseGraph()) {
      if (!processedEdges.count(edge)) {
        ...
        if (upstreamNode &&
            !processedInterfaces.count(upstreamNode->getName())) {
...
InputPtr Node::addInputFromNodeDef(const string &inputName) {
  ...
  if (!nodeInput) {
    NodeDefPtr nodeDef = getNodeDef();
    InputPtr nodeDefInput =
        nodeDef ? nodeDef->getActiveInput(inputName) : nullptr;
    if (nodeDefInput) {
...
void NodeGraph::addInterfaceName(const string &inputPath,
                                 const string &interfaceName) {
  NodeDefPtr nodeDef = getNodeDef();
  if (!nodeDef) {
    throw Exception("Cannot declare an interface for a nodegraph which is not "
                    "associated with a node definition: " +
                    getName());
  }
  if (nodeDef->getChild(interfaceName)) {
    throw Exception("Interface: " + interfaceName +
                    " has already been declared on the node definition: " +
                    nodeDef->getName());
  }
...
void NodeGraph::modifyInterfaceName(const string &inputPath,
                                    const string &interfaceName) {
  ElementPtr desc = getDescendant(inputPath);
  InputPtr input = desc ? desc->asA<Input>() : nullptr;
  if (input) {
...
NodeDefPtr NodeGraph::getNodeDef() const {
  ...
  // If not directly defined look for an implementation which has a nodedef
  // association
  if (!nodedef) {
...
bool NodeGraph::validate(string *message) const {
  ...
  validateRequire(!hasVersionString(), res, message,
                  "NodeGraph elements do not support version strings");
  if (hasNodeDefString()) {
    NodeDefPtr nodeDef = getNodeDef();
    validateRequire(nodeDef != nullptr, res, message,
                    "NodeGraph implementation refers to non-existent NodeDef");
    if (nodeDef) {
      vector<OutputPtr> graphOutputs = getOutputs();
      vector<OutputPtr> nodeDefOutputs = nodeDef->getActiveOutputs();
      validateRequire(graphOutputs.size() == nodeDefOutputs.size(), res,
                      message,
                      "NodeGraph implementation has a different number of "
                      "outputs than its NodeDef");
      if (graphOutputs.size() == 1 && nodeDefOutputs.size() == 1) {
        validateRequire(graphOutputs[0]->getType() ==
                            nodeDefOutputs[0]->getType(),
                        res, message,
                        "NodeGraph implementation has a different output type "
                        "than its NodeDef");
      }
    }
  }
...
bool Backdrop::validate(string *message) const {
  ...
  if (hasContainsString()) {
    ...
    validateRequire(stringVec.size() == elemVec.size(), res, message,
                    "Invalid element in contains string");
  }
