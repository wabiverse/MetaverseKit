...
vector<NodePtr> getShaderNodes(NodePtr materialNode, const string &nodeType,
                               const string &target) {
  ...
  for (InputPtr input : inputs) {
    ...
    if (shaderNode && !shaderNodeSet.count(shaderNode)) {
      ...
      // If no explicit output name given then scan all outputs on the
      // nodegraph.
      ElementPtr parent = materialNode->getParent();
      NodeGraphPtr nodeGraph =
          parent->getChildOfType<NodeGraph>(input->getNodeGraphString());
      if (!nodeGraph) {
        continue;
      }
  ...
  if (inputs.empty()) {
    ...
    // If a target is specified the nodedef for the given target is searched
    // for.
      ...
      if (impl && impl->isA<NodeGraph>()) {
        ...
        for (OutputPtr defOutput : materialNodeDef->getOutputs()) {
          if (defOutput->getType() == MATERIAL_TYPE_STRING) {
            OutputPtr implGraphOutput =
                implGraph->getOutput(defOutput->getName());
            for (GraphIterator it = implGraphOutput->traverseGraph().begin();
              ...
              if (upstreamNode &&
                  upstreamNode->getType() == MATERIAL_TYPE_STRING) {
                for (NodePtr shaderNode :
                     getShaderNodes(upstreamNode, nodeType, target)) {
