...
NodeDefPtr getShaderNodeDef(ElementPtr shaderRef) {
  if (shaderRef->hasAttribute(NodeDef::NODE_DEF_ATTRIBUTE)) {
    ...
    NodeDefPtr child =
        doc->getNodeDef(shaderRef->getQualifiedName(nodeDefString));
  ...
  if (shaderRef->hasAttribute(NodeDef::NODE_ATTRIBUTE)) {
    ...
    string version =
        shaderRef->getAttribute(InterfaceElement::VERSION_ATTRIBUTE);
    vector<NodeDefPtr> nodeDefs = shaderRef->getDocument()->getMatchingNodeDefs(
        shaderRef->getQualifiedName(nodeString));
    vector<NodeDefPtr> secondary =
        shaderRef->getDocument()->getMatchingNodeDefs(nodeString);
...
DocumentPtr createDocument() { return Document::createDocument<Document>(); }
...
public:
  ...
  void refresh() {
    // Thread synchronization for multiple concurrent readers of a single
    // document.
    ...
    if (!valid) {
      ...
      for (ElementPtr elem : doc.lock()->traverseTree()) {
        const string &nodeName =
            elem->getAttribute(PortElement::NODE_NAME_ATTRIBUTE);
        const string &nodeGraphName =
            elem->getAttribute(PortElement::NODE_GRAPH_ATTRIBUTE);
        const string &nodeString = elem->getAttribute(NodeDef::NODE_ATTRIBUTE);
        const string &nodeDefString =
            elem->getAttribute(InterfaceElement::NODE_DEF_ATTRIBUTE);

        if (!nodeName.empty()) {
          PortElementPtr portElem = elem->asA<PortElement>();
          if (portElem) {
            portElementMap.emplace(portElem->getQualifiedName(nodeName),
                                   portElem);
          }
        } else {
          if (!nodeGraphName.empty()) {
            PortElementPtr portElem = elem->asA<PortElement>();
            if (portElem) {
              portElementMap.emplace(portElem->getQualifiedName(nodeGraphName),
                                     portElem);
            }
          }
        }
        ...
        if (!nodeDefString.empty()) {
          ...
          if (interface) {
            if (interface->isA<NodeGraph>()) {
              implementationMap.emplace(
                  interface->getQualifiedName(nodeDefString), interface);
            }
            ImplementationPtr impl = interface->asA<Implementation>();
            if (impl) {
              // Check for implementation which specifies a nodegraph as the
              // implementation
              const string &nodeGraphString = impl->getNodeGraph();
              if (!nodeGraphString.empty()) {
                NodeGraphPtr nodeGraph =
                    impl->getDocument()->getNodeGraph(nodeGraphString);
                if (nodeGraph)
                  implementationMap.emplace(
                      interface->getQualifiedName(nodeDefString), nodeGraph);
              } else {
                implementationMap.emplace(
                    interface->getQualifiedName(nodeDefString), interface);
              }
...
Document::Document(ElementPtr parent, const string &name)
    : GraphElement(parent, CATEGORY, name), _cache(std::make_unique<Cache>()) {}
...
NodeDefPtr
Document::addNodeDefFromGraph(const NodeGraphPtr nodeGraph,
                              const string &nodeDefName, const string &node,
                              const string &version, bool isDefaultVersion,
                              const string &group, const string &newGraphName) {
...
void Document::importLibrary(const ConstDocumentPtr &library) {
  ...
  for (auto child : library->getChildren()) {
    if (child->getCategory().empty()) {
      throw Exception("Trying to import child without a category: " +
                      child->getName());
    }
...
vector<PortElementPtr>
Document::getMatchingPorts(const string &nodeName) const {
  // Refresh the cache.
  _cache->refresh();
...
vector<OutputPtr> Document::getMaterialOutputs() const {
  ...
  for (NodeGraphPtr docNodeGraph : getNodeGraphs()) {
    // Skip nodegraphs which are either definitions or are from an included
    // file.
    const string graphUri = docNodeGraph->getSourceUri();
    if (docNodeGraph->getNodeDef() ||
        (!graphUri.empty() && documentUri != graphUri)) {
      continue;
    }

    vector<OutputPtr> docNodeGraphOutputs = docNodeGraph->getMaterialOutputs();
    if (!docNodeGraphOutputs.empty()) {
      materialOutputs.insert(materialOutputs.end(), docNodeGraphOutputs.begin(),
                             docNodeGraphOutputs.end());
    }
  }
...
Document::getMatchingImplementations(const string &nodeDef) const {
  // Refresh the cache.
  _cache->refresh();
...
bool Document::validate(string *message) const {
  bool res = true;
  std::pair<int, int> expectedVersion(MATERIALX_MAJOR_VERSION,
                                      MATERIALX_MINOR_VERSION);
  validateRequire(getVersionIntegers() >= expectedVersion, res, message,
...
void Document::upgradeVersion() {
  ...
  if (majorVersion == 1 && minorVersion == 22) {
    for (ElementPtr elem : traverseTree()) {
      if (elem->getAttribute(TypedElement::TYPE_ATTRIBUTE) == "vector") {
        elem->setAttribute(TypedElement::TYPE_ATTRIBUTE,
                           getTypeString<Vector3>());
      }
    }
    minorVersion = 23;
  }
  ...
  if (majorVersion == 1 && minorVersion == 23) {
    for (ElementPtr elem : traverseTree()) {
      if (elem->getCategory() == "shader" && elem->hasAttribute("shadername")) {
        elem->setAttribute(NodeDef::NODE_ATTRIBUTE,
                           elem->getAttribute("shadername"));
  ...
  if (majorVersion == 1 && minorVersion == 26) {
    ...
    for (ElementPtr elem : traverseTree()) {
      ...
      for (ElementPtr child : origChildren) {
        if (child->getCategory() == "opgraph") {
          ...
          NodeDefPtr nodeDef =
              elem->changeChildCategory(child, "nodedef")->asA<NodeDef>();
          ...
          if (child->hasAttribute("shadertype")) {
            child->setAttribute(TypedElement::TYPE_ATTRIBUTE,
                                SURFACE_SHADER_TYPE_STRING);
            child->removeAttribute("shadertype");
          }
        } else if (child->getCategory() == "parameter") {
          if (child->getAttribute(TypedElement::TYPE_ATTRIBUTE) ==
              "opgraphnode") {
            if (elem->isA<Node>()) {
              InputPtr input =
                  elem->changeChildCategory(child, "input")->asA<Input>();
    ...
    for (ElementPtr mat : getChildrenOfType<Element>("material")) {
      for (ElementPtr shaderRef :
           mat->getChildrenOfType<Element>("shaderref")) {
        if (!getShaderNodeDef(shaderRef)) {
          NodeDefPtr nodeDef = getNodeDef(shaderRef->getName());
          if (nodeDef) {
            shaderRef->setAttribute(NodeDef::NODE_DEF_ATTRIBUTE,
                                    nodeDef->getName());
            shaderRef->setAttribute(NodeDef::NODE_ATTRIBUTE,
    ...
    for (NodeDefPtr nodeDef : getNodeDefs()) {
      for (InputPtr input : nodeDef->getActiveInputs()) {
        if (input->hasAttribute("opgraph") &&
            input->hasAttribute("graphoutput")) {
          for (ElementPtr mat : materials) {
            for (ElementPtr shaderRef :
                 mat->getChildrenOfType<Element>("shaderref")) {
              if (getShaderNodeDef(shaderRef) == nodeDef &&
                  !shaderRef->getChild(input->getName())) {
                ElementPtr bindInput = shaderRef->addChildOfCategory(
                    "bindinput", input->getName());
                bindInput->setAttribute(TypedElement::TYPE_ATTRIBUTE,
                                        input->getType());
                bindInput->setAttribute("nodegraph",
                                        input->getAttribute("opgraph"));
                bindInput->setAttribute("output",
                                        input->getAttribute("graphoutput"));
              }
            }
          }
    ...
    for (GeomInfoPtr geomInfo : getGeomInfos()) {
      for (ElementPtr child :
           geomInfo->getChildrenOfType<Element>("geomattr")) {
    ...
    if (getGeomPropValue("udim") && !getGeomPropValue("udimset")) {
      ...
      udimSetInfo->setGeomPropValue(UDIM_SET_PROPERTY, udimSetString,
                                    getTypeString<StringVec>());
    }
  ...
  if (majorVersion == 1 && minorVersion == 34) {
    for (ElementPtr elem : traverseTree()) {
      if (elem->getAttribute(TypedElement::TYPE_ATTRIBUTE) == "matrix") {
        elem->setAttribute(TypedElement::TYPE_ATTRIBUTE,
                           getTypeString<Matrix44>());
      }
      if (elem->hasAttribute("default") &&
          !elem->hasAttribute(ValueElement::VALUE_ATTRIBUTE)) {
        elem->setAttribute(ValueElement::VALUE_ATTRIBUTE,
                           elem->getAttribute("default"));
  ...
  if (majorVersion == 1 && minorVersion == 35) {
    for (ElementPtr elem : traverseTree()) {
      ...
      if (elem->getAttribute(TypedElement::TYPE_ATTRIBUTE) ==
      ...
      if (elem->getAttribute(TypedElement::TYPE_ATTRIBUTE) ==
        ...
        elem->setAttribute(
            ...
            replaceSubstrings(elem->getAttribute(ValueElement::VALUE_ATTRIBUTE),
                              stringMap));
      }

      vector<ElementPtr> origChildren = elem->getChildren();
      for (ElementPtr child : origChildren) {
        if (elem->getCategory() == "material" &&
            child->getCategory() == "override") {
          for (ElementPtr shaderRef :
               elem->getChildrenOfType<Element>("shaderref")) {
            NodeDefPtr nodeDef = getShaderNodeDef(shaderRef);
            if (nodeDef) {
              for (ValueElementPtr activeValue :
                   nodeDef->getActiveValueElements()) {
                if (activeValue->getAttribute("publicname") ==
                        child->getName() &&
                    !shaderRef->getChild(child->getName())) {
                  if (activeValue->getCategory() == "parameter") {
                    ElementPtr bindParam = shaderRef->addChildOfCategory(
                        "bindparam", activeValue->getName());
                    bindParam->setAttribute(TypedElement::TYPE_ATTRIBUTE,
                                            activeValue->getType());
                    bindParam->setAttribute(ValueElement::VALUE_ATTRIBUTE,
                                            child->getAttribute("value"));
                  } else if (activeValue->isA<Input>()) {
                    ElementPtr bindInput = shaderRef->addChildOfCategory(
                        "bindinput", activeValue->getName());
                    bindInput->setAttribute(TypedElement::TYPE_ATTRIBUTE,
                                            activeValue->getType());
                    bindInput->setAttribute(ValueElement::VALUE_ATTRIBUTE,
                                            child->getAttribute("value"));
                  }
                }
              }
          ...
          elem->removeChild(child->getName());
        } else if (elem->getCategory() == "material" &&
                   child->getCategory() == "materialinherit") {
  ...
  if (majorVersion == 1 && minorVersion == 36) {
    ...
    for (NodeDefPtr nodeDef : getNodeDefs()) {
      InterfaceElementPtr interfaceElem =
          std::static_pointer_cast<InterfaceElement>(nodeDef);
      if (interfaceElem && interfaceElem->hasType()) {
        ...
        const string &defaultInput =
            interfaceElem->getAttribute(Output::DEFAULT_INPUT_ATTRIBUTE);
        if (outputPtr && !defaultInput.empty()) {
          outputPtr->setAttribute(Output::DEFAULT_INPUT_ATTRIBUTE,
                                  defaultInput);
        }
    ...
    for (NodeDefPtr nodeDef : getNodeDefs()) {
      if (nodeDef->hasAttribute("shadertype")) {
        for (ElementPtr mat : getChildrenOfType<Element>("material")) {
          for (ElementPtr shaderRef :
               mat->getChildrenOfType<Element>("shaderref")) {
            if (shaderRef->getAttribute(InterfaceElement::NODE_DEF_ATTRIBUTE) ==
                nodeDef->getName()) {
    ...
    for (ElementPtr elem : traverseTree()) {
      ...
      if (nodeCategory == "invert" &&
          (node->getType() == getTypeString<Matrix33>() ||
           node->getType() == getTypeString<Matrix44>())) {
        node->setCategory("invertmatrix");
      }

      // Change category from "rotate" to "rotate2d" or "rotate3d" nodes
      else if (nodeCategory == "rotate") {
        node->setCategory((node->getType() == getTypeString<Vector2>())
                              ? "rotate2d"
                              : "rotate3d");
      }
      ...
      // Convert "separate" to "separate2", "separate3" or "separate4"
        ...
        if (in) {
          ...
          if (inType == getTypeString<Vector4>() ||
              inType == getTypeString<Color4>()) {
            node->setCategory("separate4");
          } else if (inType == getTypeString<Vector3>() ||
                     inType == getTypeString<Color3>()) {
        ...
        for (ElementPtr child : node->getChildrenOfType<Element>("parameter")) {
          if (child->hasAttribute(ValueElement::VALUE_ATTRIBUTE)) {
            backdrop->setAttribute(
                child->getName(),
                child->getAttribute(ValueElement::VALUE_ATTRIBUTE));
          }
        }
  ...
  if (majorVersion == 1 && minorVersion >= 37) {
    ...
    for (ElementPtr mat : getChildrenOfType<Element>("material")) {
      ...
      for (ElementPtr shaderRef :
        ...
        // Get the shader node type and category, using the shader nodedef if
        // present.
        string shaderNodeType =
            nodeDef ? nodeDef->getType() : SURFACE_SHADER_TYPE_STRING;
        string shaderNodeCategory =
            nodeDef ? nodeDef->getNodeString()
                    : shaderRef->getAttribute(NodeDef::NODE_ATTRIBUTE);

        // Add the shader node.
        string shaderNodeName = createValidChildName(shaderRef->getName());
        NodePtr shaderNode =
            addNode(shaderNodeCategory, shaderNodeName, shaderNodeType);

        // Copy attributes to the shader node.
        string nodeDefString =
            shaderRef->getAttribute(NodeDef::NODE_DEF_ATTRIBUTE);
        string target =
            shaderRef->getAttribute(InterfaceElement::TARGET_ATTRIBUTE);
        string version =
            shaderRef->getAttribute(InterfaceElement::VERSION_ATTRIBUTE);
        if (!nodeDefString.empty()) {
        ...
        // Create a material node if needed, making a connection to the new
        // shader node.
        if (!materialNode) {
        ...
        if (!materialNode->getInput(shaderNodeType)) {
          InputPtr shaderInput =
              materialNode->addInput(shaderNodeType, shaderNodeType);
          shaderInput->setNodeName(shaderNode->getName());
        }
      }

      // Remove the material element, transferring its name and attributes to
      // the material node.
    ...
    const StringPair GENERALIZED_SCHLICK_BRDF = {"generalized_schlick_brdf",
    ...
    const StringPair BURLEY_DIFFUSE_BRDF = {"burley_diffuse_brdf",
                                            "burley_diffuse_bsdf"};
    ...
    auto upgradeBsdfLayering = [](NodePtr node) {
      ...
      if (base) {
        ...
        if (baseNode) {
          ...
          // Rename the top bsdf node, and give its old name to the layer
          // operator so we don't need to update any connection references.
    ...
    for (ElementPtr elem : traverseTree()) {
      ...
      if (nodeCategory == "atan2") {
        ...
        // Create an artistic_ior node to convert from artistic to physical
        // parameterization.
        GraphElementPtr parent = node->getParent()->asA<GraphElement>();
        NodePtr artisticIor = parent->addNode(
            "artistic_ior", node->getName() + "__artistic_ior", "multioutput");
        OutputPtr artisticIor_ior = artisticIor->addOutput("ior", "color3");
        OutputPtr artisticIor_extinction =
            artisticIor->addOutput("extinction", "color3");

        // Copy values and connections from conductor node to artistic_ior node.
        InputPtr reflectivity = node->getInput("reflectivity");
        if (reflectivity) {
          InputPtr artisticIor_reflectivity =
              artisticIor->addInput("reflectivity", "color3");
          copyAttributes(reflectivity, artisticIor_reflectivity);
        }
        InputPtr edge_color = node->getInput("edge_color");
        if (edge_color) {
          InputPtr artisticIor_edge_color =
              artisticIor->addInput("edge_color", "color3");
      ...
      for (InputPtr input : node->getInputs()) {
        ...
        } else if (input->getOutputString() == "extinction" &&
                   input->getType() == "vector3") {
          NodePtr connectedNode = input->getConnectedNode();
          if (connectedNode && connectedNode->getCategory() == "artistic_ior") {
    ...
    for (InputPtr input : artisticIorConnections) {
      ...
      GraphElementPtr parent = node->getParent()->asA<GraphElement>();
      NodePtr convert = parent->addNode(
          "convert", node->getName() + "__convert_ior", "vector3");
    ...
    for (InputPtr input : artisticExtConnections) {
      ...
      GraphElementPtr parent = node->getParent()->asA<GraphElement>();
      NodePtr convert = parent->addNode(
          "convert", node->getName() + "__convert_extinction", "vector3");
      InputPtr convertInput = convert->addInput("in", "color3");
      convertInput->setNodeName(artisticIorNode->getName());
    ...
    // Make it so that interface names and nodes in a nodegraph are not
    // duplicates If they are, rename the nodes.
    for (NodeGraphPtr nodegraph : getNodeGraphs()) {
      ...
      for (string interfaceName : interfaceNames) {
        ...
        if (node) {
          ...
          vector<MaterialX::PortElementPtr> downstreamPorts =
    ...
    // Convert parameters to inputs, applying uniform markings to converted
    // inputs of nodedefs.
    for (ElementPtr elem : traverseTree()) {
      if (elem->isA<InterfaceElement>()) {
        for (ElementPtr param : elem->getChildrenOfType<Element>("parameter")) {
          InputPtr input =
              elem->changeChildCategory(param, "input")->asA<Input>();
          if (elem->isA<NodeDef>()) {
...
void Document::invalidateCache() { _cache->valid = false; }
