...
vector<MaterialAssignPtr> getGeometryBindings(ConstNodePtr materialNode,
                                              const string &geom) {
...
MaterialAssignPtr Look::addMaterialAssign(const string &name,
...
vector<PropertySetAssignPtr> Look::getActivePropertySetAssigns() const {
  ...
  for (ConstElementPtr elem : traverseInheritance()) {
    vector<PropertySetAssignPtr> assigns =
        elem->asA<Look>()->getPropertySetAssigns();
    activeAssigns.insert(activeAssigns.end(), assigns.begin(), assigns.end());
  }
  return activeAssigns;
}
...
vector<VisibilityPtr> Look::getActiveVisibilities() const {
  ...
  for (ConstElementPtr elem : traverseInheritance()) {
    ...
    activeVisibilities.insert(activeVisibilities.end(), visibilities.begin(),
                              visibilities.end());
  }
...
vector<VariantAssignPtr> MaterialAssign::getActiveVariantAssigns() const {
  vector<VariantAssignPtr> activeAssigns;
  for (ConstElementPtr elem : traverseInheritance()) {
    vector<VariantAssignPtr> assigns =
        elem->asA<MaterialAssign>()->getVariantAssigns();
    activeAssigns.insert(activeAssigns.end(), assigns.begin(), assigns.end());
  }
  return activeAssigns;
}
