...
template <> void stringToData(const string &str, string &data) { data = str; }

template <class T>
void stringToData(const string &str, enable_if_mx_vector_t<T> &data) {
...
template <class T>
void stringToData(const string &str, enable_if_mx_matrix_t<T> &data) {
  StringVec tokens = splitString(str, ARRAY_VALID_SEPARATORS);
...
template <class T>
void stringToData(const string &str, enable_if_std_vector_t<T> &data) {
  // This code path parses an array of arbitrary substrings, so we split the
  // string in a fashion that preserves substrings with internal spaces.
...
template <> void dataToString(const string &data, string &str) { str = data; }

template <class T>
void dataToString(const enable_if_mx_vector_t<T> &data, string &str) {
  for (size_t i = 0; i < data.numElements(); i++) {
    string token;
...
template <class T>
void dataToString(const enable_if_mx_matrix_t<T> &data, string &str) {
  for (size_t i = 0; i < data.numRows(); i++) {
    for (size_t j = 0; j < data.numColumns(); j++) {
      string token;
      dataToString(data[i][j], token);
      str += token;
      if (i + 1 < data.numRows() || j + 1 < data.numColumns()) {
        str += ARRAY_PREFERRED_SEPARATOR;
      }
    }
  }
...
template <class T>
void dataToString(const enable_if_std_vector_t<T> &data, string &str) {
...
template <class T> const string &getTypeString() { return TypedValue<T>::TYPE; }
...
template <class T>
ValuePtr TypedValue<T>::createFromString(const string &value) {
  try {
...
ValuePtr Value::createValueFromStrings(const string &value,
                                       const string &type) {
...
ScopedFloatFormatting::ScopedFloatFormatting(Value::FloatFormat format,
                                             int precision)
    : _format(Value::getFloatFormat()), _precision(Value::getFloatPrecision()) {
