...
bool Element::operator==(const Element &rhs) const {
  if (getCategory() != rhs.getCategory() || getName() != rhs.getName()) {
    return false;
  }
...
bool Element::operator!=(const Element &rhs) const { return !(*this == rhs); }
...
string Element::getNamePath(ConstElementPtr relativeTo) const {
  ...
  for (ConstElementPtr elem = getSelf(); elem; elem = elem->getParent()) {
    ...
    res = res.empty() ? elem->getName()
                      : elem->getName() + NAME_PATH_SEPARATOR + res;
  }
  return res;
}
...
void Element::unregisterChildElement(ElementPtr child) {
  ...
  _childOrder.erase(std::find(_childOrder.begin(), _childOrder.end(), child));
}

int Element::getChildIndex(const string &name) const {
  ElementPtr child = getChild(name);
  vector<ElementPtr>::const_iterator it =
...
ElementPtr Element::getRoot() {
  ...
  if (!root) {
    throw ExceptionOrphanedElement("Requested root of orphaned element: " +
                                   asString());
  }
  return root;
}
...
DocumentPtr Element::getDocument() { return getRoot()->asA<Document>(); }
...
Edge Element::getUpstreamEdge(size_t) const { return NULL_EDGE; }
...
void Element::validateRequire(bool expression, bool &res, string *message,
...
ValuePtr ValueElement::getDefaultValue() const {
  ...
  ConstInterfaceElementPtr interface =
      parent ? parent->asA<InterfaceElement>() : nullptr;
  if (interface) {
...
bool ValueElement::validate(string *message) const {
  ...
  if (hasInterfaceName()) {
    validateRequire(isA<Input>() || isA<Token>(), res, message,
                    "Only input and token elements support interface names");
    ConstNodeGraphPtr nodeGraph = getAncestorOfType<NodeGraph>();
    ConstInterfaceElementPtr decl =
        nodeGraph ? nodeGraph->getDeclaration() : nullptr;
    if (decl) {
      ValueElementPtr valueElem =
          decl->getActiveValueElement(getInterfaceName());
      validateRequire(valueElem != nullptr, res, message,
                      "Interface name not found in referenced declaration");
      if (valueElem) {
        ConstPortElementPtr portElem = asA<PortElement>();
        if (portElem && portElem->hasChannels()) {
          bool valid = portElem->validChannelsString(
              portElem->getChannels(), valueElem->getType(), getType());
          validateRequire(valid, res, message,
                          "Invalid channels string for interface name");
        } else {
          validateRequire(
              getType() == valueElem->getType(), res, message,
              "Interface name refers to value element of a different type");
        }
      }
    }
  }
  ...
  if (hasUnitType()) {
    ...
    if (!unittype.empty()) {
      ...
      validateRequire(unitTypeDef != nullptr, res, message,
                      "Unit type definition does not exist in document");
    }
  }
  if (hasUnit()) {
    ...
    validateRequire(foundUnit, res, message,
                    "Unit definition does not exist in document");
  }
...
string prettyPrint(ConstElementPtr elem) {
  string text;
  for (TreeIterator it = elem->traverseTree().begin();
       it != TreeIterator::end(); ++it) {
