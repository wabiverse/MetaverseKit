...
GLFramebufferPtr GLFramebuffer::create(unsigned int width, unsigned int height,
                                       unsigned channelCount,
                                       Image::BaseType baseType) {
  return GLFramebufferPtr(
      new GLFramebuffer(width, height, channelCount, baseType));
}
...
GLFramebuffer::GLFramebuffer(unsigned int width, unsigned int height,
    ...
    : _width(width), _height(height), _channelCount(channelCount),
      _baseType(baseType), _encodeSrgb(false), _framebuffer(0),
  ...
  GLTextureHandler::mapTextureFormatToGL(baseType, channelCount, true, glType,
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, glInternalFormat, _width, _height, 0, glFormat,
               glType, nullptr);
  ...
  if (status != GL_FRAMEBUFFER_COMPLETE) {
    ...
    throw ExceptionRenderError("Frame buffer object setup failed: " +
                               errorMessage);
  }
...
ImagePtr GLFramebuffer::getColorImage(ImagePtr image) {
  ...
  int glType, glFormat, glInternalFormat;
  GLTextureHandler::mapTextureFormatToGL(_baseType, _channelCount, false,
                                         glType, glFormat, glInternalFormat);

  bind();
  glPixelStorei(GL_PACK_ALIGNMENT, 1);
  glReadBuffer(GL_COLOR_ATTACHMENT0);
  glReadPixels(0, 0, image->getWidth(), image->getHeight(), glFormat, glType,
