...
void GlslMaterial::bindImages(ImageHandlerPtr imageHandler,
  ...
  for (const auto &uniform : publicUniforms->getVariableOrder()) {
    ...
    ImagePtr image =
        bindImage(filename, uniformVariable, imageHandler, samplingProperties);
    if (image) {
...
ImagePtr
GlslMaterial::bindImage(const FilePath &filePath,
  ...
  if (imageHandler->bindImage(image, samplingProperties)) {
    ...
    if (textureLocation >= 0) {
      _glProgram->bindUniform(uniformName, Value::createValue(textureLocation),
                              false);
      return image;
    }
  }
...
void GlslMaterial::bindLighting(LightHandlerPtr lightHandler,
  ...
  if (shadowState.shadowMap && _glProgram->hasUniform(HW::SHADOW_MAP)) {
    ...
    if (imageHandler->bindImage(shadowState.shadowMap, samplingProperties)) {
      ...
      int textureLocation = textureHandler->getBoundTextureLocation(
          shadowState.shadowMap->getResourceId());
      if (textureLocation >= 0) {
        _glProgram->bindUniform(HW::SHADOW_MAP,
                                Value::createValue(textureLocation));
      }
    }
  ...
  if (shadowState.ambientOcclusionMap &&
      _glProgram->hasUniform(HW::AMB_OCC_MAP)) {
    ImageSamplingProperties samplingProperties;
    samplingProperties.uaddressMode =
    ...
    if (imageHandler->bindImage(shadowState.ambientOcclusionMap,
                                samplingProperties)) {
      GLTextureHandlerPtr textureHandler =
          std::static_pointer_cast<GLTextureHandler>(imageHandler);
      int textureLocation = textureHandler->getBoundTextureLocation(
          shadowState.ambientOcclusionMap->getResourceId());
      if (textureLocation >= 0) {
        _glProgram->bindUniform(HW::AMB_OCC_MAP,
                                Value::createValue(textureLocation));
      }
    }
...
void GlslMaterial::drawPartition(MeshPartitionPtr part) const {
  ...
  glDrawElements(GL_TRIANGLES, (GLsizei)indexData.size(), GL_UNSIGNED_INT,
                 (void *)0);
...
void GlslMaterial::modifyUniform(const std::string &path, ConstValuePtr value,
