...
void GlslRenderer::initialize(RenderContextHandle renderContextHandle) {
  if (!_initialized) {
    ...
    _context =
        GLContext::create(_window, (HardwareContextHandle)renderContextHandle);
    if (!_context) {
      throw ExceptionRenderError(
          "Failed to create OpenGL context for renderer");
    }
  ...
  try {
    ...
    if (_program) {
      ...
      if (!_program->hasActiveAttributes()) {
        ...
        if (!_program->bind()) {
          throw ExceptionRenderError(
              "Cannot bind inputs without a valid program");
        }
        ...
        bool isTransparent =
            _program->getShader()->hasAttribute(HW::ATTR_TRANSPARENT);
        if (isTransparent) {
          glEnable(GL_BLEND);
        ...
        for (MeshPtr mesh : _geometryHandler->getMeshes()) {
          ...
          for (size_t i = 0; i < mesh->getPartitionCount(); i++) {
            ...
            if (isTransparent) {
              ...
              glCullFace(GL_FRONT);
              glDrawElements(GL_TRIANGLES, (GLsizei)indexData.size(),
...
void GlslRenderer::drawScreenSpaceQuad(const Vector2 &uvMin,
                                       const Vector2 &uvMax) {
  const float QUAD_VERTICES[] = {
      1.0f,     1.0f,     0.0f,  uvMax[0], uvMax[1], // position, texcoord
      1.0f,     -1.0f,    0.0f,  uvMax[0], uvMin[1], -1.0f,    -1.0f,   0.0f,
      uvMin[0], uvMin[1], -1.0f, 1.0f,     0.0f,     uvMin[0], uvMax[1]};
  const unsigned int QUAD_INDICES[] = {0, 1, 3, 1, 2, 3};
  ...
  for (const auto &pair : _program->getAttributesList()) {
    if (pair.first.find(HW::IN_POSITION) != std::string::npos) {
      ...
      glVertexAttribPointer(pair.second->location, 3, GL_FLOAT, GL_FALSE,
                            VERTEX_STRIDE * sizeof(float), (void *)0);
    }

    if (pair.first.find(HW::IN_TEXCOORD + "_") != std::string::npos) {
      glEnableVertexAttribArray(pair.second->location);
      glVertexAttribPointer(pair.second->location, 2, GL_FLOAT, GL_FALSE,
