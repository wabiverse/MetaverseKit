...
GLTextureHandler::GLTextureHandler(ImageLoaderPtr imageLoader)
  ...
  _boundTextureLocations.resize(maxTextureUnits,
                                GlslProgram::UNDEFINED_OPENGL_RESOURCE_ID);
}

bool GLTextureHandler::bindImage(
  ...
  if (textureUnit < 0) {
    std::cerr << "Exceeded maximum number of bound textures in "
                 "GLTextureHandler::bindImage"
              << std::endl;
    return false;
  }
  ...
  GLint minFilterType = mapFilterTypeToGL(samplingProperties.filterType,
                                          samplingProperties.enableMipmaps);
  GLint magFilterType =
      GL_LINEAR; // Magnification filters are more restrictive than minification
...
bool GLTextureHandler::createRenderResources(ImagePtr image,
  ...
  if (image->getResourceId() == GlslProgram::UNDEFINED_OPENGL_RESOURCE_ID) {
    ...
    if (resourceId == GlslProgram::UNDEFINED_OPENGL_RESOURCE_ID) {
      std::cerr << "Failed to generate render resource for texture"
                << std::endl;
      return false;
    }
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, glInternalFormat, image->getWidth(),
               image->getHeight(), 0, glFormat, glType,
...
int GLTextureHandler::mapAddressModeToGL(
    ImageSamplingProperties::AddressMode addressModeEnum) {
  const std::array<int, 4> ADDRESS_MODES{// Constant color. Use clamp to border
...
int GLTextureHandler::mapFilterTypeToGL(
...
void GLTextureHandler::mapTextureFormatToGL(Image::BaseType baseType,
                                            unsigned int channelCount,
                                            bool srgb, int &glType,
                                            int &glFormat,
