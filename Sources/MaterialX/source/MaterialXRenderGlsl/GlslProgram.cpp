...
GlslProgram::GlslProgram()
    : _programId(UNDEFINED_OPENGL_RESOURCE_ID), _shader(nullptr),
      _vertexArray(UNDEFINED_OPENGL_RESOURCE_ID) {}

GlslProgram::~GlslProgram() { clearBuiltData(); }
  ...
  if (!vertexShaderSource.empty()) {
    ...
    if (glStatus == GL_FALSE) {
      ...
      if (glInfoLogLength > 0) {
        ...
        glGetShaderInfoLog(vertexShaderId, glInfoLogLength, nullptr,
                           &vsErrorMessage[0]);
  ...
  if (!fragmentShaderSource.empty()) {
    ...
    if (glStatus == GL_FALSE) {
      ...
      if (glInfoLogLength > 0) {
        ...
        glGetShaderInfoLog(fragmentShaderId, glInfoLogLength, nullptr,
                           &fsErrorMessage[0]);
  ...
  if (stagesBuilt == desiredStages) {
    ...
    if (glStatus == GL_FALSE) {
      ...
      if (glInfoLogLength > 0) {
        ...
        glGetProgramInfoLog(_programId, glInfoLogLength, nullptr,
  ...
  for (const auto &input : inputs) {
    ...
    if (!stream) {
      throw ExceptionRenderError(
          "Geometry buffer could not be retrieved for binding: " + input.first +
...
void GlslProgram::bindPartition(MeshPartitionPtr part) {
  ...
  if (_indexBufferIds.find(part) != _indexBufferIds.end()) {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, indexBufferSize * sizeof(uint32_t),
...
ImagePtr
GlslProgram::bindTexture(unsigned int uniformType, int uniformLocation,
                         const FilePath &filePath, ImageHandlerPtr imageHandler,
                         const ImageSamplingProperties &samplingProperties) {
  if (uniformLocation >= 0 && uniformType >= GL_SAMPLER_1D &&
    ...
    if (imageHandler->bindImage(image, samplingProperties)) {
      GLTextureHandlerPtr textureHandler =
          std::static_pointer_cast<GLTextureHandler>(imageHandler);
      int textureLocation =
          textureHandler->getBoundTextureLocation(image->getResourceId());
...
MaterialX::ValuePtr
GlslProgram::findUniformValue(const string &uniformName,
                              const GlslProgram::InputMap &uniformList) {
  ...
  for (const auto &uniform : uniformList) {
    ...
    if (uniformLocation >= 0 && uniformType >= GL_SAMPLER_1D &&
      ...
      const string fileName(
          uniform.second->value ? uniform.second->value->getValueString() : "");
      ...
      if (fileName != HW::ENV_RADIANCE && fileName != HW::ENV_IRRADIANCE) {
        ImageSamplingProperties samplingProperties;
        samplingProperties.setProperties(uniform.first, publicUniforms);
        bindTexture(uniformType, uniformLocation, fileName, imageHandler,
                    samplingProperties);
      }
    }
  }
...
void GlslProgram::bindLighting(LightHandlerPtr lightHandler,
  ...
  ImageMap envImages = {
      {HW::ENV_RADIANCE, lightHandler->getIndirectLighting()
                             ? lightHandler->getEnvRadianceMap()
                             : imageHandler->getZeroImage()},
      {HW::ENV_IRRADIANCE, lightHandler->getIndirectLighting()
    ...
    if (image && hasUniform(env.first)) {
      ...
      if (imageHandler->bindImage(image, samplingProperties)) {
        ...
        if (uniform == HW::ENV_RADIANCE) {
          bindUniform(HW::ENV_RADIANCE_MIPS,
                      Value::createValue((int)image->getMaxMipCount()), false);
        }
      }
    }
  }
  bindUniform(HW::REFRACTION_TWO_SIDED,
              Value::createValue(lightHandler->getRefractionTwoSided()), false);
  ...
  if (albedoTable && hasUniform(HW::ALBEDO_TABLE)) {
    ...
    if (imageHandler->bindImage(albedoTable, samplingProperties)) {
      GLTextureHandlerPtr textureHandler =
          std::static_pointer_cast<GLTextureHandler>(imageHandler);
      int textureLocation =
          textureHandler->getBoundTextureLocation(albedoTable->getResourceId());
      if (textureLocation >= 0) {
...
void GlslProgram::bindUniform(const string &name, ConstValuePtr value,
                              bool errorIfMissing) {
  const GlslProgram::InputMap &uniformList = getUniformsList();
...
void GlslProgram::bindViewInformation(CameraPtr camera) {
  ...
  bindUniform(HW::VIEW_POSITION, Value::createValue(camera->getViewPosition()),
              false);
  bindUniform(HW::VIEW_DIRECTION,
              Value::createValue(camera->getViewDirection()), false);

  // World matrices
  Matrix44 worldInv = camera->getWorldMatrix().getInverse();
  bindUniform(HW::WORLD_MATRIX, Value::createValue(camera->getWorldMatrix()),
              false);
  bindUniform(HW::WORLD_TRANSPOSE_MATRIX,
              Value::createValue(camera->getWorldMatrix().getTranspose()),
              false);
  bindUniform(HW::WORLD_INVERSE_MATRIX, Value::createValue(worldInv), false);
  bindUniform(HW::WORLD_INVERSE_TRANSPOSE_MATRIX,
              Value::createValue(worldInv.getTranspose()), false);

  // View matrices
  Matrix44 viewInv = camera->getViewMatrix().getInverse();
  bindUniform(HW::VIEW_MATRIX, Value::createValue(camera->getViewMatrix()),
              false);
  bindUniform(HW::VIEW_TRANSPOSE_MATRIX,
              Value::createValue(camera->getViewMatrix().getTranspose()),
              false);
  bindUniform(HW::VIEW_INVERSE_MATRIX, Value::createValue(viewInv), false);
  bindUniform(HW::VIEW_INVERSE_TRANSPOSE_MATRIX,
              Value::createValue(viewInv.getTranspose()), false);

  // Projection matrices
  Matrix44 projInv = camera->getProjectionMatrix().getInverse();
  bindUniform(HW::PROJ_MATRIX,
              Value::createValue(camera->getProjectionMatrix()), false);
  bindUniform(HW::PROJ_TRANSPOSE_MATRIX,
              Value::createValue(camera->getProjectionMatrix().getTranspose()),
              false);
  bindUniform(HW::PROJ_INVERSE_MATRIX, Value::createValue(projInv), false);
  bindUniform(HW::PROJ_INVERSE_TRANSPOSE_MATRIX,
              Value::createValue(projInv.getTranspose()), false);
  ...
  bindUniform(HW::WORLD_VIEW_PROJECTION_MATRIX,
              Value::createValue(worldViewProj), false);
}

void GlslProgram::bindTimeAndFrame(float time, float frame) {
  if (_programId == UNDEFINED_OPENGL_RESOURCE_ID) {
    throw ExceptionRenderError(
        "Cannot bind time/frame without a valid program");
  }
...
void GlslProgram::unbind() const { glUseProgram(UNDEFINED_OPENGL_RESOURCE_ID); }
...
const GlslProgram::InputMap &GlslProgram::updateUniformsList() {
  ...
  if (_programId == UNDEFINED_OPENGL_RESOURCE_ID) {
    throw ExceptionRenderError(
        "Cannot parse for uniforms without a valid program");
  }
  ...
  for (int i = 0; i < uniformCount; i++) {
    glGetActiveUniform(_programId, GLuint(i), maxNameLength, nullptr,
                       &uniformSize, &uniformType, uniformName);
    GLint uniformLocation = glGetUniformLocation(_programId, uniformName);
    if (uniformLocation >= 0) {
      InputPtr inputPtr = std::make_shared<Input>(uniformLocation, uniformType,
                                                  uniformSize, EMPTY_STRING);
        ...
        if (inputIt != _uniformList.end()) {
          ...
          if (input->gltype == glType) {
            ...
            errors.push_back(
                ...
                "\". Colorspace: \"" +
                (!v->getColorSpace().empty() ? v->getColorSpace() : "<none>") +
                "\". GLType: " +
                std::to_string(mapTypeToOpenGLType(v->getType())));
...
const GlslProgram::InputMap &GlslProgram::updateAttributesList() {
  ...
  if (_programId == UNDEFINED_OPENGL_RESOURCE_ID) {
    throw ExceptionRenderError(
        "Cannot parse for attributes without a valid program");
  }
  ...
  for (int i = 0; i < numAttributes; i++) {
    ...
    glGetActiveAttrib(_programId, GLuint(i), maxNameLength, nullptr,
                      &attributeSize, &attributeType, attributeName);
    GLint attributeLocation = glGetAttribLocation(_programId, attributeName);
    if (attributeLocation >= 0) {
      InputPtr inputPtr = std::make_shared<Input>(
          attributeLocation, attributeType, attributeSize, EMPTY_STRING);
...
void GlslProgram::printUniforms(std::ostream &outputStream) {
  ...
  for (const auto &input : _uniformList) {
    ...
    outputStream << "Program Uniform: \"" << input.first
                 << "\". Location:" << location << ". GLtype: " << std::hex
                 << gltype << ". Size: " << std::dec << size;
    if (!type.empty())
