...
void GlslResourceBindingContext::emitDirectives(GenContext &context,
                                                ShaderStage &stage) {
  ...
  if (_separateBindingLocation) {
    ...
    if (!shaderStage.empty()) {
      generator.emitLine("#pragma shader_stage(" + shaderStage + ")", stage,
                         false);
    }
  }
...
void GlslResourceBindingContext::emitResourceBindings(
  ...
  if (hasValueUniforms) {
    ...
    for (auto uniform : uniforms.getVariableOrder()) {
      if (uniform->getType() != Type::FILENAME) {
        ...
        generator.emitVariableDeclaration(uniform, EMPTY_STRING, context, stage,
                                          false);
  ...
  // Second, emit all sampler uniforms as separate uniforms with separate layout
  // bindings
  for (auto uniform : uniforms.getVariableOrder()) {
    if (uniform->getType() == Type::FILENAME) {
      generator.emitString("layout (binding=" +
                               std::to_string(_separateBindingLocation
...
void GlslResourceBindingContext::emitStructuredResourceBindings(
    GenContext &context, const VariableBlock &uniforms, ShaderStage &stage,
    const std::string &structInstanceName, const std::string &arraySuffix) {
  ShaderGenerator &generator = context.getShaderGenerator();
  const Syntax &syntax = generator.getSyntax();

  // Glsl structures need to be aligned. We make a best effort to base align
  ...
  std::sort(
      memberOrder.begin(), memberOrder.end(),
  ...
  for (size_t i = 0; i < uniforms.size(); ++i) {
    ...
    generator.emitLineBegin(stage);
    generator.emitVariableDeclaration(uniforms[variableIndex], EMPTY_STRING,
                                      context, stage, false);
    generator.emitString(Syntax::SEMICOLON, stage);
    generator.emitLineEnd(stage, false);
  }
