...
void GeomPropValueNodeGlsl::createVariables(const ShaderNode &node,
                                            GenContext &,
                                            Shader &shader) const {
...
void GeomPropValueNodeGlsl::emitFunctionCall(const ShaderNode &node,
                                             GenContext &context,
...
void GeomPropValueNodeGlslAsUniform::createVariables(const ShaderNode &node,
                                                     GenContext &,
                                                     Shader &shader) const {
  const ShaderInput *geomPropInput = node.getInput(GEOMPROP);
  if (!geomPropInput || !geomPropInput->getValue()) {
...
void GeomPropValueNodeGlslAsUniform::emitFunctionCall(
    const ShaderNode &node, GenContext &context, ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
