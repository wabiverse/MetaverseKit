...
void SurfaceNodeGlsl::createVariables(const ShaderNode &, GenContext &context,
                                      Shader &shader) const {
  // TODO:
  ...
  // implementation can be set to depend on the PositionNodeGlsl, NormalNodeGlsl
  // ViewDirectionNodeGlsl and LightNodeGlsl nodes instead? This is where the
...
void SurfaceNodeGlsl::emitFunctionCall(const ShaderNode &node,
                                       GenContext &context,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::VERTEX) {
    ...
    if (!normal->isEmitted()) {
      ...
      shadergen.emitLine(prefix + normal->getVariable() + " = normalize((" +
                             HW::T_WORLD_INVERSE_TRANSPOSE_MATRIX + " * vec4(" +
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    shadergen.emitLine(
        "vec3 N = normalize(" + prefix + HW::T_NORMAL_WORLD + ")", stage);
    shadergen.emitLine("vec3 V = normalize(" + HW::T_VIEW_POSITION + " - " +
    ...
    if (bsdf) {
      ...
      if (context.getOptions().hwShadowMap) {
        shadergen.emitLine("vec3 shadowCoord = (" + HW::T_SHADOW_MATRIX +
                               " * vec4(" + prefix + HW::T_POSITION_WORLD +
                               ", 1.0)).xyz",
                           stage);
        shadergen.emitLine("shadowCoord = shadowCoord * 0.5 + 0.5", stage);
        shadergen.emitLine("vec2 shadowMoments = texture(" + HW::T_SHADOW_MAP +
                               ", shadowCoord.xy).xy",
                           stage);
        shadergen.emitLine(
      ...
      if (context.getOptions().hwAmbientOcclusion) {
        ShaderPort *texcoord = vertexData[HW::T_TEXCOORD + "_0"];
        shadergen.emitLine("vec2 ambOccUv = mx_transform_uv(" + prefix +
    ...
    if (bsdf) {
      ...
      shadergen.emitLine(outTransparency + " = mix(vec3(1.0), " +
...
void SurfaceNodeGlsl::emitLightLoop(const ShaderNode &node, GenContext &context,
                                    ShaderStage &stage,
