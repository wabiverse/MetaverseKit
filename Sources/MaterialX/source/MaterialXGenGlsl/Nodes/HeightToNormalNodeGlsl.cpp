...
void HeightToNormalNodeGlsl::createVariables(const ShaderNode &, GenContext &,
                                             Shader &) const {
  // Default filter kernels from ConvolutionNode are not used by this derived
  // class.
}

void HeightToNormalNodeGlsl::computeSampleOffsetStrings(
    const string &sampleSizeName, const string &offsetTypeString, unsigned int,
...
void HeightToNormalNodeGlsl::emitFunctionDefinition(const ShaderNode &,
                                                    GenContext &context,
...
void HeightToNormalNodeGlsl::emitFunctionCall(const ShaderNode &node,
                                              GenContext &context,
                                              ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    const ShaderGenerator &shadergen = context.getShaderGenerator();
    ...
    emitInputSamplesUV(node, sampleCount, filterWidth, filterSize, filterOffset,
                       sampleSizeFunctionUV, context, stage, sampleStrings);
    ...
    shadergen.emitLine(
        "float " + sampleName + "[" + std::to_string(sampleCount) + "]", stage);
