...
VkShaderGenerator::VkShaderGenerator() : GlslShaderGenerator() {
  ...
  _resourceBindingCtx =
      std::make_shared<MaterialX::VkResourceBindingContext>(0);
}
...
void VkShaderGenerator::emitInputs(GenContext &context,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::VERTEX) {
    ...
    if (!vertexInputs.empty()) {
      ...
      for (size_t i = 0; i < vertexInputs.size(); ++i) {
        ...
        emitVariableDeclaration(vertexInputs[i], _syntax->getInputQualifier(),
                                context, stage, false);
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    if (!vertexData.empty()) {
      ...
      for (size_t i = 0; i < vertexData.size(); ++i) {
        ...
        emitLineBegin(stage);
        emitString("layout (location = " + std::to_string(i) + ") ", stage);
        emitVariableDeclaration(vertexData[i], _syntax->getInputQualifier(),
                                context, stage, false);
        emitString(Syntax::SEMICOLON, stage);
        emitLineEnd(stage, false);
      }
...
void VkShaderGenerator::emitOutputs(GenContext &context,
                                    ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::VERTEX) {
    ...
    if (!vertexData.empty()) {
      for (size_t i = 0; i < vertexData.size(); ++i) {
        emitLineBegin(stage);
        emitString("layout (location = " + std::to_string(i) + ") ", stage);
        emitVariableDeclaration(vertexData[i], _syntax->getOutputQualifier(),
                                context, stage, false);
        emitString(Syntax::SEMICOLON, stage);
        emitLineEnd(stage, false);
      }
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    for (size_t i = 0; i < outputs.size(); ++i) {
      emitLineBegin(stage);
      emitString("layout (location = " + std::to_string(i) + ") ", stage);
      emitVariableDeclaration(outputs[i], _syntax->getOutputQualifier(),
                              context, stage, false);
      emitString(Syntax::SEMICOLON, stage);
      emitLineEnd(stage, false);
    }
...
VkShaderGenerator::getResourceBindingContext(GenContext & /*context*/) const {
