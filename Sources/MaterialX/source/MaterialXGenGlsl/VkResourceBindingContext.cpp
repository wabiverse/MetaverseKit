...
void VkResourceBindingContext::emitDirectives(GenContext &context,
                                              ShaderStage &stage) {
...
void VkResourceBindingContext::emitResourceBindings(
  ...
  for (auto uniform : uniforms.getVariableOrder()) {
    if (uniform->getType() == Type::FILENAME) {
      generator.emitString(
          "layout (binding=" + std::to_string(_hwUniformBindLocation++) + ") " +
...
void VkResourceBindingContext::emitStructuredResourceBindings(
    GenContext &context, const VariableBlock &uniforms, ShaderStage &stage,
