...
public:
  EditorColorPicker(ng::Widget *parent, const ng::Color &color)
    ...
    const std::array<std::string, 4> COLOR_LABELS = {"Red", "Green", "Blue",
                                                     "Alpha"};
    for (size_t i = 0; i < COLOR_LABELS.size(); i++) {
      ...
      _colorWidgets[i]->set_callback([this](float) {
        ng::Color value(_colorWidgets[0]->value(), _colorWidgets[1]->value(),
                        _colorWidgets[2]->value(), _colorWidgets[3]->value());
...
PropertyEditor::PropertyEditor()
    : _window(nullptr), _container(nullptr), _gridLayout2(nullptr),
      _gridLayout3(nullptr), _visible(false), _fileDialogsForImages(true) {}
...
void PropertyEditor::create(Viewer &parent) {
  ...
  _gridLayout2->set_col_alignment(
      {ng::Alignment::Minimum, ng::Alignment::Maximum});

  // 3 cell layout for label plus widget value pair.
  _gridLayout3 = new ng::GridLayout(ng::Orientation::Horizontal, 3,
                                    ng::Alignment::Minimum, 2, 2);
  _gridLayout3->set_col_alignment(
      {ng::Alignment::Minimum, ng::Alignment::Maximum, ng::Alignment::Maximum});
}

void PropertyEditor::addItemToForm(const mx::UIPropertyItem &item,
                                   const std::string &group,
                                   ng::Widget *container, Viewer *viewer,
                                   bool editable) {
  ...
  if (value->isA<int>()) {
    ...
    if (INVALID_INDEX != valueIndex) {
      ...
      comboBox->set_callback([path, viewer, enumeration,
                              enumValues](int index) {
        ...
        if (material) {
          if (index >= 0 && static_cast<size_t>(index) < enumValues.size()) {
            material->modifyUniform(path, enumValues[index]);
                     ...
                     static_cast<size_t>(index) < enumeration.size()) {
            material->modifyUniform(path, mx::Value::createValue(index),
                                    enumeration[index]);
          }
        }
      });
      ...
      intVar->set_callback([intVar, path, viewer](int /*unclamped*/) {
        ...
        if (material) {
          ...
          material->modifyUniform(path,
                                  mx::Value::createValue(intVar->value()));
        }
      });
      if (ui.uiMin) {
  ...
  // Float widget
    ...
    ng::FloatBox<float> *floatBox = createFloatWidget(
        threeColumns, label, value->asA<float>(), &ui,
        [viewer, path](float value) {
          mx::MaterialPtr material = viewer->getSelectedMaterial();
          if (material) {
  ...
  // String
    ...
    if (!v.empty()) {
      ...
      if (item.variable->getType() == mx::Type::FILENAME) {
        new ng::Label(twoColumns, label);
        ng::Button *buttonVar =
            new ng::Button(twoColumns, mx::FilePath(v).getBaseName());
        ...
        buttonVar->set_callback([buttonVar, path, viewer]() {
          ...
          mx::ShaderPort *uniform =
              material ? material->findUniform(path) : nullptr;
          if (uniform) {
            if (uniform->getType() == mx::Type::FILENAME) {
              ...
              if (handler) {
                ...
                if (!filename.empty()) {
                  uniform->setValue(
                      mx::Value::createValue<std::string>(filename));
...
void PropertyEditor::updateContents(Viewer *viewer) {
  ...
  if (publicUniforms) {
    ...
    mx::createUIPropertyGroups(elem->getDocument(), *publicUniforms, groups,
                               unnamedGroups, pathSeparator);
    ...
    for (auto it = groups.begin(); it != groups.end(); ++it) {
      ...
      // Verify that the uniform is editable, as some inputs may be optimized
      // out during compilation.
      if (material->findUniform(item.variable->getPath())) {
        addItemToForm(item,
                      (previousFolder == folder) ? mx::EMPTY_STRING : folder,
                      _container, viewer, true);
    ...
    for (auto it2 = unnamedGroups.begin(); it2 != unnamedGroups.end(); ++it2) {
      ...
      if (material->findUniform(item.variable->getPath())) {
        addItemToForm(item, addedLabel ? mx::EMPTY_STRING : "Shader Inputs",
                      _container, viewer, true);
        addedLabel = true;
        addedItems = true;
      }
...
ng::FloatBox<float> *createFloatWidget(ng::Widget *parent,
                                       const std::string &label, float value,
                                       const mx::UIProperties *ui,
...
ng::IntBox<int> *createIntWidget(ng::Widget *parent, const std::string &label,
                                 int value, const mx::UIProperties *ui,
  ...
  if (ui) {
    ...
    if (range.first != range.second) {
      std::pair<float, float> float_range((float)range.first,
                                          (float)range.second);
