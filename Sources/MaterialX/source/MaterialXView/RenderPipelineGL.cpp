...
std::shared_ptr<void>
GLRenderPipeline::createTextureBaker(unsigned int width, unsigned int height,
                                     mx::Image::BaseType baseType) {
  return std::static_pointer_cast<void>(
      mx::TextureBakerGlsl::create(width, height, baseType));
}
...
void GLRenderPipeline::updateAlbedoTable(int tableSize) {
  ...
  mx::GLFramebufferPtr framebuffer = mx::GLFramebuffer::create(
      tableSize, tableSize, 3, mx::Image::BaseType::FLOAT);
  framebuffer->bind();
  ...
  mx::ShaderPtr hwShader = mx::createAlbedoTableShader(
      genContext, stdLib, "__ALBEDO_TABLE_SHADER__");
  ...
  if (material->getProgram()->hasUniform(mx::HW::ALBEDO_TABLE_SIZE)) {
    std::static_pointer_cast<mx::GlslMaterial>(material)
        ->getProgram()
        ->bindUniform(mx::HW::ALBEDO_TABLE_SIZE,
...
mx::ImagePtr GLRenderPipeline::getShadowMap(int shadowMapSize) {
  ...
  if (!_viewer->_shadowMap) {
    ...
    if (_viewer->_shadowMaterial && _viewer->_shadowBlurMaterial) {
      // Create framebuffer.
      mx::GLFramebufferPtr framebuffer = mx::GLFramebuffer::create(
          shadowMapSize, shadowMapSize, 2, mx::Image::BaseType::FLOAT);
      framebuffer->bind();
      glClearColor(1.0f, 1.0f, 1.0f, 1.0f);
      ...
      for (unsigned int i = 0; i < _viewer->_shadowSoftness; i++) {
        ...
        if (imageHandler->bindImage(_viewer->_shadowMap,
                                    blurSamplingProperties)) {
          mx::GLTextureHandlerPtr textureHandler =
              std::static_pointer_cast<mx::GLTextureHandler>(imageHandler);
          int textureLocation = textureHandler->getBoundTextureLocation(
              _viewer->_shadowMap->getResourceId());
          if (textureLocation >= 0) {
            std::static_pointer_cast<mx::GlslMaterial>(
                _viewer->_shadowBlurMaterial)
                ->getProgram()
                ->bindUniform("image_file",
...
void GLRenderPipeline::renderFrame(void *, int shadowMapSize,
  ...
  lightHandler->setLightTransform(
      mx::Matrix44::createRotationY(lightRotation / 180.0f * PI));
  ...
  for (const auto &assignment : _viewer->_materialAssignments) {
    ...
    mx::GlslMaterialPtr material =
        std::dynamic_pointer_cast<mx::GlslMaterial>(assignment.second);
...
void GLRenderPipeline::bakeTextures() {
  ...
  glfwGetFramebufferSize(_viewer->m_glfw_window, &_viewer->m_fbsize[0],
                         &_viewer->m_fbsize[1]);
  glViewport(0, 0, _viewer->m_fbsize[0], _viewer->m_fbsize[1]);
  glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);
  glDrawBuffer(GL_BACK);
...
mx::ImagePtr GLRenderPipeline::getFrameImage() {
  ...
  mx::ImagePtr image =
      mx::Image::create((unsigned int)(size.x() * pixel_ratio),
  ...
  glReadPixels(0, 0, image->getWidth(), image->getHeight(), GL_RGB,
               GL_UNSIGNED_BYTE, image->getResourceBuffer());
