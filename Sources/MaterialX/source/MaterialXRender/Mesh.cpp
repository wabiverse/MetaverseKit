...
Mesh::Mesh(const string &name)
    : _name(name), _minimumBounds(MAX_FLOAT, MAX_FLOAT, MAX_FLOAT),
      _maximumBounds(-MAX_FLOAT, -MAX_FLOAT, -MAX_FLOAT),
      _sphereCenter(0.0f, 0.0f, 0.0f), _sphereRadius(0.0f), _vertexCount(0) {}

MeshStreamPtr Mesh::generateNormals(MeshStreamPtr positionStream) {
  // Create the normal stream.
  MeshStreamPtr normalStream = MeshStream::create(
      "i_" + MeshStream::NORMAL_ATTRIBUTE, MeshStream::NORMAL_ATTRIBUTE, 0);
...
MeshStreamPtr Mesh::generateTextureCoordinates(MeshStreamPtr positionStream) {
  ...
  std::fill(texcoordStream->getData().begin(), texcoordStream->getData().end(),
            0.0f);
...
MeshStreamPtr Mesh::generateTangents(MeshStreamPtr positionStream,
                                     ...
                                     MeshStreamPtr texcoordStream) {
  size_t vertexCount =
      positionStream->getData().size() / positionStream->getStride();
  size_t normalCount =
      normalStream->getData().size() / normalStream->getStride();
  size_t texcoordCount =
      texcoordStream->getData().size() / texcoordStream->getStride();
  if (vertexCount != normalCount || vertexCount != texcoordCount) {
    return nullptr;
  }

  // Create the tangent stream.
  MeshStreamPtr tangentStream = MeshStream::create(
      "i_" + MeshStream::TANGENT_ATTRIBUTE, MeshStream::TANGENT_ATTRIBUTE, 0);
  tangentStream->resize(positionStream->getSize());
  std::fill(tangentStream->getData().begin(), tangentStream->getData().end(),
            0.0f);
...
MeshStreamPtr Mesh::generateBitangents(MeshStreamPtr normalStream,
  ...
  MeshStreamPtr bitangentStream =
      MeshStream::create("i_" + MeshStream::BITANGENT_ATTRIBUTE,
...
void MeshStream::transform(const Matrix44 &matrix) {
  ...
  if (getType() == MeshStream::POSITION_ATTRIBUTE ||
    ...
    Matrix44 transformMatrix =
        isNormalStream ? matrix.getInverse().getTranspose() : matrix;
