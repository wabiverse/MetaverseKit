...
using GLTFMeshMatrixList =
    std::unordered_map<cgltf_mesh *, std::vector<Matrix44>>;

// Compute matrices for each mesh. Appends a transform for each transform
// instance
void computeMeshMatrices(GLTFMeshMatrixList &meshMatrices, cgltf_node *cnode) {
  ...
  if (cmesh) {
    ...
    Matrix44 positionMatrix =
        Matrix44((float)t[0], (float)t[1], (float)t[2], (float)t[3],
...
void computeMeshPaths(GLTFMeshPathList &meshPaths, cgltf_node *cnode,
                      FilePath path, size_t nodeCount, size_t meshCount) {
  string cnodeName = cnode->name
                         ? string(cnode->name)
                         : DEFAULT_NODE_PREFIX + std::to_string(nodeCount++);
  ...
  if (cmesh) {
    ...
    if (path.isEmpty()) {
      string meshName = cmesh->name
                            ? string(cmesh->name)
                            : DEFAULT_MESH_PREFIX + std::to_string(meshCount++);
...
void decodeVec4Tangents(MeshStreamPtr vec4TangentStream,
  ...
  tangentStream = MeshStream::create("i_" + MeshStream::TANGENT_ATTRIBUTE,
                                     MeshStream::TANGENT_ATTRIBUTE, 0);
  bitangentStream = MeshStream::create("i_" + MeshStream::BITANGENT_ATTRIBUTE,
                                       MeshStream::BITANGENT_ATTRIBUTE, 0);
...
bool CgltfLoader::load(const FilePath &filePath, MeshList &meshList,
  ...
  // Read file
  cgltf_result result =
      cgltf_parse_file(&options, input_filename.c_str(), &data);
  if (result != cgltf_result_success) {
    return false;
  }
  if (cgltf_load_buffers(&options, data, input_filename.c_str()) !=
  ...
  for (cgltf_size sceneIndex = 0; sceneIndex < data->scenes_count;
       ++sceneIndex) {
  ...
  for (size_t m = 0; m < data->meshes_count; m++) {
    ...
    for (size_t mtx = 0; mtx < positionMatrices.size(); mtx++) {
      ...
      for (cgltf_size primitiveIndex = 0;
           primitiveIndex < cmesh->primitives_count; ++primitiveIndex) {
        ...
        if (primitive->type != cgltf_primitive_type_triangles) {
          if (_debugLevel > 0) {
            std::cout << "Skip non-triangle indexed mesh: " << cmesh->name
                      << std::endl;
          }
          continue;
        }
        ...
        for (cgltf_size prim = 0; prim < primitive->attributes_count; prim++) {
          ...
          bool isPositionStream =
              (attribute->type == cgltf_attribute_type_position);
          bool isNormalStream =
              (attribute->type == cgltf_attribute_type_normal);
          bool isTangentStream =
              (attribute->type == cgltf_attribute_type_tangent);
          bool isColorStream = (attribute->type == cgltf_attribute_type_color);
          bool isTexCoordStream =
              (attribute->type == cgltf_attribute_type_texcoord);

          if (isPositionStream) {
            // Create position stream
            positionStream =
                MeshStream::create("i_" + MeshStream::POSITION_ATTRIBUTE,
                                   MeshStream::POSITION_ATTRIBUTE, streamIndex);
            mesh->addStream(positionStream);
            geomStream = positionStream;
          } else if (isNormalStream) {
            normalStream =
                MeshStream::create("i_" + MeshStream::NORMAL_ATTRIBUTE,
                                   MeshStream::NORMAL_ATTRIBUTE, streamIndex);
            mesh->addStream(normalStream);
            geomStream = normalStream;
          } else if (isTangentStream) {
            vec4TangentStream =
                MeshStream::create("i_" + MeshStream::TANGENT_ATTRIBUTE + "4",
                                   MeshStream::TANGENT_ATTRIBUTE, streamIndex);
            vec4TangentStream->setStride(
                MeshStream::STRIDE_4D); // glTF stores the bitangent sign in the
                                        // 4th component
            geomStream = vec4TangentStream;
            desiredVectorSize = 4;
          } else if (isColorStream) {
            colorStream =
                MeshStream::create("i_" + MeshStream::COLOR_ATTRIBUTE + "_" +
                                       std::to_string(colorAttrIndex),
                                   MeshStream::COLOR_ATTRIBUTE, streamIndex);
            ...
            texcoordStream =
                MeshStream::create("i_" + MeshStream::TEXCOORD_ATTRIBUTE + "_0",
                                   MeshStream::TEXCOORD_ATTRIBUTE, 0);
            ...
            if (_debugLevel > 0)
              std::cout << "Unknown stream type: "
                        << std::to_string(attribute->type) << std::endl;
          }
          ...
          if (geomStream) {
            ...
            if (_debugLevel > 0) {
              std::cout << "** Read stream: " << geomStream->getName()
                        << std::endl;
        ...
        if (_debugLevel > 0) {
          std::cout << "** Read indexing: Count = "
                    << std::to_string(indexCount) << std::endl;
        }
        if (indexAccessor) {
        ...
        mesh->setVertexCount(positionStream->getData().size() /
                             MeshStream::STRIDE_3D);
        ...
        // According to glTF spec. 3.7.2.1, tangents must be ignored when
        // normals are missing
        ...
        // Generate tangents, normals and texture coordinates if none are
        // provided
        if (!normalStream) {
        ...
        if (!vec4TangentStream) {
          MeshStreamPtr tangentStream = mesh->generateTangents(
