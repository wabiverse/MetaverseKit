...
bool StbImageLoader::saveImage(const FilePath &filePath, ConstImagePtr image,
                               bool verticalFlip) {
  ...
  if (!isFloat) {
    if (extension == PNG_EXTENSION) {
      returnValue = stbi_write_png(filePathName.c_str(), w, h, channels, data,
                                   w * image->getChannelCount());
      ...
      returnValue =
          stbi_write_jpg(filePathName.c_str(), w, h, channels, data, 100);
    }
  } else {
    if (extension == HDR_EXTENSION) {
      returnValue = stbi_write_hdr(filePathName.c_str(), w, h, channels,
...
ImagePtr StbImageLoader::loadImage(const FilePath &filePath) {
  ...
  string extension = filePath.getExtension();
  if (extension == HDR_EXTENSION) {
    buffer = stbi_loadf(filePath.asString().c_str(), &width, &height,
                        &channelCount, 0);
    baseType = Image::BaseType::FLOAT;
  } else {
    buffer = stbi_load(filePath.asString().c_str(), &width, &height,
                       &channelCount, 0);
