...
ImagePtr createUniformImage(unsigned int width, unsigned int height,
...
ImagePtr createImageStrip(const vector<ImagePtr> &imageVec) {
  ...
  ImagePtr imageStrip =
      Image::create(destWidth, destHeight, refImage->getChannelCount(),
                    refImage->getBaseType());
  ...
  for (unsigned int i = 0; i < imageVec.size(); i++) {
    ...
    if (!srcImage || srcImage->getWidth() != srcWidth ||
        srcImage->getHeight() != srcHeight ||
        srcImage->getChannelCount() != refImage->getChannelCount() ||
        srcImage->getBaseType() != refImage->getBaseType()) {
      throw Exception("Source images must have identical resolutions and "
                      "formats in createImageStrip");
    }
...
Image::Image(unsigned int width, unsigned int height, unsigned int channelCount,
             BaseType baseType)
    : _width(width), _height(height), _channelCount(channelCount),
      _baseType(baseType), _resourceBuffer(nullptr),
      _resourceBufferDeallocator(nullptr), _resourceId(0) {}

Image::~Image() { releaseResourceBuffer(); }

unsigned int Image::getBaseStride() const {
  if (_baseType == BaseType::FLOAT) {
    return 4;
  }
  if (_baseType == BaseType::HALF || _baseType == BaseType::UINT16) {
    return 2;
  }
...
void Image::setTexelColor(unsigned int x, unsigned int y, const Color4 &color) {
  ...
  if (_baseType == BaseType::FLOAT) {
    float *data = static_cast<float *>(_resourceBuffer) +
                  (y * _width + x) * _channelCount;
    for (unsigned int c = 0; c < writeChannels; c++) {
      data[c] = color[c];
    }
  } else if (_baseType == BaseType::HALF) {
    Half *data =
        static_cast<Half *>(_resourceBuffer) + (y * _width + x) * _channelCount;
    for (unsigned int c = 0; c < writeChannels; c++) {
      data[c] = (Half)color[c];
    }
  } else if (_baseType == BaseType::UINT16) {
    uint16_t *data = static_cast<uint16_t *>(_resourceBuffer) +
                     (y * _width + x) * _channelCount;
    for (unsigned int c = 0; c < writeChannels; c++) {
      data[c] = (uint16_t)std::round(
          color[c] * (float)std::numeric_limits<uint16_t>::max());
    }
  } else if (_baseType == BaseType::UINT8) {
    uint8_t *data = static_cast<uint8_t *>(_resourceBuffer) +
                    (y * _width + x) * _channelCount;
    for (unsigned int c = 0; c < writeChannels; c++) {
      data[c] = (uint8_t)std::round(color[c] *
...
Color4 Image::getTexelColor(unsigned int x, unsigned int y) const {
  ...
  if (_baseType == BaseType::FLOAT) {
    ...
    if (_channelCount == 4) {
      return Color4(data[0] / MAX_VALUE, data[1] / MAX_VALUE,
                    data[2] / MAX_VALUE, data[3] / MAX_VALUE);
    } else if (_channelCount == 3) {
      return Color4(data[0] / MAX_VALUE, data[1] / MAX_VALUE,
                    data[2] / MAX_VALUE, 1.0f);
    } else if (_channelCount == 2) {
...
ImagePtr Image::copy(unsigned int channelCount, BaseType baseType) const {
  ImagePtr newImage =
      Image::create(getWidth(), getHeight(), channelCount, baseType);
...
ImagePtr Image::applyBoxBlur() {
  ImagePtr blurImage =
      Image::create(getWidth(), getHeight(), getChannelCount(), getBaseType());
  ...
  for (int y = 0; y < (int)getHeight(); y++) {
    for (int x = 0; x < (int)getWidth(); x++) {
      ...
      for (int dx = -3; dx <= 3; dx++, weightIndex++) {
        int sx = std::min(std::max(x + dx, 0), (int)getWidth() - 1);
        blurColor +=
            blurImage1->getTexelColor(sx, y) * GAUSSIAN_KERNEL_7[weightIndex];
      }
...
ImagePair Image::splitByLuminance(float luminance) {
  ImagePtr underflowImage =
      Image::create(getWidth(), getHeight(), getChannelCount(), getBaseType());
  ImagePtr overflowImage =
      Image::create(getWidth(), getHeight(), getChannelCount(), getBaseType());
  ...
  for (unsigned int y = 0; y < getHeight(); y++) {
    for (unsigned int x = 0; x < getWidth(); x++) {
      ...
      Color4 underflowColor(std::min(envColor[0], luminance),
                            std::min(envColor[1], luminance),
                            std::min(envColor[2], luminance), 1.0f);
      Color4 overflowColor(std::max(envColor[0] - underflowColor[0], 0.0f),
                           std::max(envColor[1] - underflowColor[1], 0.0f),
                           std::max(envColor[2] - underflowColor[2], 0.0f),
                           1.0f);
