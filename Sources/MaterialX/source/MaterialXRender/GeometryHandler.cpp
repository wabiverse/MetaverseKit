...
MeshPtr GeometryHandler::createQuadMesh(const Vector2 &uvMin,
                                        const Vector2 &uvMax,
                                        bool flipTexCoordsHorizontally) {
  MeshStreamPtr quadPositions =
      MeshStream::create(HW::IN_POSITION, MeshStream::POSITION_ATTRIBUTE, 0);
  quadPositions->setStride(MeshStream::STRIDE_3D);
  quadPositions->getData().assign({1.0f, 1.0f, 0.0f, 1.0f, -1.0f, 0.0f, -1.0f,
                                   -1.0f, 0.0f, -1.0f, 1.0f, 0.0f});
  MeshStreamPtr quadTexCoords = MeshStream::create(
      HW::IN_TEXCOORD + "_0", MeshStream::TEXCOORD_ATTRIBUTE, 0);
  quadTexCoords->setStride(MeshStream::STRIDE_2D);
  if (!flipTexCoordsHorizontally) {
    quadTexCoords->getData().assign({uvMax[0], uvMax[1], uvMax[0], uvMin[1],
                                     uvMin[0], uvMin[1], uvMin[0], uvMax[1]});
  } else {
    quadTexCoords->getData().assign({uvMax[0], uvMin[1], uvMax[0], uvMax[1],
                                     uvMin[0], uvMax[1], uvMin[0], uvMin[1]});
  }
