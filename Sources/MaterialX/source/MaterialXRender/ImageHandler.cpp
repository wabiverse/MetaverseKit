...
ImagePtr ImageLoader::loadImage(const FilePath &) { return nullptr; }
...
ImageHandler::ImageHandler(ImageLoaderPtr imageLoader) {
  ...
  _zeroImage =
      createUniformImage(2, 2, 4, Image::BaseType::UINT8, Color4(0.0f));
}
...
bool ImageHandler::saveImage(const FilePath &filePath, ConstImagePtr image,
                             bool verticalFlip) {
  if (!image) {
    return false;
  }
...
ImagePtr ImageHandler::acquireImage(const FilePath &filePath,
                                    const Color4 &defaultColor) {
  // Resolve the input filepath.
  FilePath resolvedFilePath = filePath;
  if (_resolver) {
    resolvedFilePath =
        _resolver->resolve(resolvedFilePath, FILENAME_TYPE_STRING);
  }
  ...
  // No valid image was found, so generate a uniform texture with the given
  // default color.
  // TODO: This step assumes that the missing image and its default color are in
  // the same
  //       color space, which is not always the case.
  ImagePtr defaultImage =
      createUniformImage(1, 1, 4, Image::BaseType::UINT8, defaultColor);
...
bool ImageHandler::unbindImage(ImagePtr) { return false; }
...
bool ImageHandler::createRenderResources(ImagePtr, bool) { return false; }
...
ImagePtr ImageHandler::loadImage(const FilePath &filePath) {
  ...
  if (!filePath.isEmpty()) {
    if (!filePath.exists()) {
      std::cerr << string("Image file not found: ") + filePath.asString()
                << std::endl;
    } else if (!_imageLoaders.count(extension)) {
      std::cerr << string("Unsupported image extension: ") + filePath.asString()
                << std::endl;
    } else {
      std::cerr << string("Image loader failed to parse image: ") +
                       filePath.asString()
                << std::endl;
    }
  }
...
void ImageSamplingProperties::setProperties(const string &fileNameUniform,
  ...
  const int INVALID_MAPPED_INT_VALUE =
      -1; // Any value < 0 is not considered to be invalid
  ...
  uaddressMode = ImageSamplingProperties::AddressMode(
      intValue && intValue->isA<int>() ? intValue->asA<int>()
                                       : INVALID_MAPPED_INT_VALUE);

  port = uniformBlock.find(root + VADDRESS_MODE_SUFFIX);
  intValue = port ? port->getValue() : nullptr;
  vaddressMode = ImageSamplingProperties::AddressMode(
      intValue && intValue->isA<int>() ? intValue->asA<int>()
                                       : INVALID_MAPPED_INT_VALUE);

  port = uniformBlock.find(root + FILTER_TYPE_SUFFIX);
  intValue = port ? port->getValue() : nullptr;
  filterType = ImageSamplingProperties::FilterType(
      intValue && intValue->isA<int>() ? intValue->asA<int>()
                                       : INVALID_MAPPED_INT_VALUE);
...
bool ImageSamplingProperties::operator==(
    const ImageSamplingProperties &r) const {
  return (enableMipmaps == r.enableMipmaps && uaddressMode == r.uaddressMode &&
          vaddressMode == r.vaddressMode && filterType == r.filterType &&
