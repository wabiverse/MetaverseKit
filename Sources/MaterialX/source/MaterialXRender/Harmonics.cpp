...
double texelSolidAngle(unsigned int y, unsigned int width,
                       unsigned int height) {
...
Sh3ScalarCoeffs evalDirection(const Vector3d &dir) {
  ...
  return Sh3ScalarCoeffs(
      {BASIS_CONSTANT_0, BASIS_CONSTANT_1 * y, BASIS_CONSTANT_1 * z,
       BASIS_CONSTANT_1 * x, BASIS_CONSTANT_2 * x * y, BASIS_CONSTANT_2 * y * z,
       BASIS_CONSTANT_3 * (3.0 * z * z - 1.0), BASIS_CONSTANT_2 * x * z,
...
Sh3ColorCoeffs projectEnvironment(ConstImagePtr env, bool irradiance) {
  ...
  for (unsigned int y = 0; y < env->getHeight(); y++) {
    ...
    for (unsigned int x = 0; x < env->getWidth(); x++) {
      ...
      Color3d weightedColor(color[0] * texelWeight, color[1] * texelWeight,
                            color[2] * texelWeight);
...
ImagePtr normalizeEnvironment(ConstImagePtr env, float envRadiance,
  ...
  for (unsigned int y = 0; y < env->getHeight(); y++) {
    ...
    for (unsigned int x = 0; x < env->getWidth(); x++) {
      ...
      double texelRadiance =
          Color3d(color[0], color[1], color[2]).dot(LUMA_COEFFS_REC709);
  ...
  ImagePtr normEnv = Image::create(env->getWidth(), env->getHeight(),
                                   env->getChannelCount(), env->getBaseType());
  normEnv->createResourceBuffer();
  float envNormFactor =
      origEnvRadiance ? (float)(envRadiance / origEnvRadiance) : 1.0f;
  for (unsigned int y = 0; y < env->getHeight(); y++) {
    for (unsigned int x = 0; x < env->getWidth(); x++) {
      // Sample the color at these coordinates.
      Color4 color = env->getTexelColor(x, y);
...
void computeDominantLight(ConstImagePtr env, Vector3 &lightDir,
                          Color3 &lightColor) {
  // Reference:
  ...
  Color3d color =
      Color3d(std::max(vDir.dot(vEnvR), 0.0), std::max(vDir.dot(vEnvG), 0.0),
...
ImagePtr renderEnvironment(const Sh3ColorCoeffs &shEnv, unsigned int width,
                           unsigned int height) {
  ...
  for (unsigned int y = 0; y < env->getHeight(); y++) {
    ...
    for (unsigned int x = 0; x < env->getWidth(); x++) {
      ...
      Color4 outputColor((float)std::max(signalColor[0], 0.0),
                         (float)std::max(signalColor[1], 0.0),
                         (float)std::max(signalColor[2], 0.0), 1.0f);
...
ImagePtr renderReferenceIrradiance(ConstImagePtr env, unsigned int width,
                                   unsigned int height) {
  ...
  for (unsigned int outY = 0; outY < outImage->getHeight(); outY++) {
    std::cout << "Rendering irradiance map row " << outY << " of "
              << outImage->getHeight() << "..." << std::endl;
    ...
    for (unsigned int outX = 0; outX < outImage->getWidth(); outX++) {
      ...
      for (unsigned int inY = 0; inY < env->getHeight(); inY++) {
        ...
        double inTexelWeight =
            texelSolidAngle(inY, env->getWidth(), env->getHeight());
        for (unsigned int inX = 0; inX < env->getWidth(); inX++) {
          ...
          outColor += Color3d(envColor[0], envColor[1], envColor[2]) *
                      inTexelWeight * cosineWeight;
        }
      }

      // Normalize and store the output texel.
      outImage->setTexelColor(outX, outY,
                              Color4((float)(outColor[0] / PI),
                                     (float)(outColor[1] / PI),
                                     (float)(outColor[2] / PI), 1.0f));
    }
  }
