...
void NormalNodeMsl::createVariables(const ShaderNode &node, GenContext &,
                                    Shader &shader) const {
  ...
  if (space == WORLD_SPACE) {
    ...
    addStageConnector(HW::VERTEX_DATA, Type::VECTOR3, HW::T_NORMAL_OBJECT, vs,
                      ps);
  }
}

void NormalNodeMsl::emitFunctionCall(const ShaderNode &node,
                                     GenContext &context,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::VERTEX) {
    ...
    if (space == WORLD_SPACE) {
      ...
      if (!normal->isEmitted()) {
        normal->setEmitted();
        shadergen.emitLine(
            prefix + normal->getVariable() + " = " + HW::T_IN_NORMAL, stage);
      }
    }
  }
