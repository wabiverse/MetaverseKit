...
void LightCompoundNodeMsl::initialize(const InterfaceElement &element,
                                      GenContext &context) {
...
void LightCompoundNodeMsl::createVariables(const ShaderNode &,
                                           GenContext &context,
                                           Shader &shader) const {
...
void LightCompoundNodeMsl::emitFunctionDefinition(const ShaderNode &node,
                                                  GenContext &context,
...
void LightCompoundNodeMsl::emitFunctionDefinition(ClosureContext *cct,
                                                  GenContext &context,
                                                  ShaderStage &stage) const {
  const MslShaderGenerator &shadergen =
      static_cast<const MslShaderGenerator &>(context.getShaderGenerator());
  ...
  if (cct) {
    ...
    shadergen.emitLine(
        "void " + _functionName + cct->getSuffix(nodeType) +
...
void LightCompoundNodeMsl::emitFunctionCall(const ShaderNode &,
                                            GenContext &context,
                                            ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
