...
SurfaceNodeMsl::SurfaceNodeMsl()
  ...
  _callIndirect.setSuffix(Type::BSDF,
                          HwShaderGenerator::CLOSURE_CONTEXT_SUFFIX_INDIRECT);
  _callIndirect.addArgument(Type::BSDF,
                            ClosureContext::Argument(Type::VECTOR3, HW::DIR_V));
  // Emission context
  _callEmission.addArgument(Type::EDF,
...
void SurfaceNodeMsl::createVariables(const ShaderNode &, GenContext &context,
                                     Shader &shader) const {
  // TODO:
  // The surface shader needs position, normal, view position and light sources.
  // We should solve this by adding some dependency mechanism so this
  // implementation can be set to depend on the PositionNodeMsl, NormalNodeMsl
  // ViewDirectionNodeMsl and LightNodeMsl nodes instead? This is where the
  ...
  addStageUniform(HW::PRIVATE_UNIFORMS, Type::MATRIX44,
                  HW::T_WORLD_INVERSE_TRANSPOSE_MATRIX, vs);

  addStageConnector(HW::VERTEX_DATA, Type::VECTOR3, HW::T_POSITION_WORLD, vs,
                    ps);
  ...
  const MslShaderGenerator &shadergen =
      static_cast<const MslShaderGenerator &>(context.getShaderGenerator());
...
void SurfaceNodeMsl::emitFunctionCall(const ShaderNode &node,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    shadergen.emitLine("float3 V = normalize(" + HW::T_VIEW_POSITION + " - " +
                           prefix + HW::T_POSITION_WORLD + ")",
                       stage);
    ...
    if (bsdf) {
      ...
      if (context.getOptions().hwShadowMap) {
        shadergen.emitLine("float3 shadowCoord = (" + HW::T_SHADOW_MATRIX +
                               " * float4(" + prefix + HW::T_POSITION_WORLD +
                               ", 1.0)).xyz",
                           stage);
        shadergen.emitLine("shadowCoord.xy = shadowCoord.xy * 0.5 + 0.5",
                           stage);

        shadergen.emitLine("float2 shadowMoments = texture(" +
                               HW::T_SHADOW_MAP + ", shadowCoord.xy).xy",
                           stage);
        shadergen.emitLine(
            "float occlusion = mx_variance_shadow_occlusion(shadowMoments, "
            "shadowCoord.z)",
            stage);
      } else {
      ...
      if (context.getOptions().hwAmbientOcclusion) {
        ShaderPort *texcoord = vertexData[HW::T_TEXCOORD + "_0"];
        shadergen.emitLine("float2 ambOccUv = mx_transform_uv(" + prefix +
                               texcoord->getVariable() +
                               ", float2(1.0), float2(0.0))",
                           stage);
        shadergen.emitLine("occlusion = mix(1.0, texture(" + HW::T_AMB_OCC_MAP +
      ...
      shadergen.emitLineBreak(stage);
      shadergen.emitLine(outColor + " += occlusion * " +
    ...
    if (edf) {
      ...
      shadergen.emitLine(outColor + " += " + edf->getOutput()->getVariable(),
                         stage);
      shadergen.emitScopeEnd(stage);
      shadergen.emitLineBreak(stage);
    }
    ...
    if (bsdf) {
      ...
      if (context.getOptions().hwTransmissionRenderMethod ==
        ...
        shadergen.emitLine(outTransparency + " += " +
                               bsdf->getOutput()->getVariable() + ".response",
                           stage);
      }
      shadergen.emitScopeEnd(stage);
      ...
      shadergen.emitLine(outTransparency + " = mix(float3(1.0), " +
                             outTransparency + ", surfaceOpacity)",
                         stage);
      shadergen.emitScopeEnd(stage);
    }
...
void SurfaceNodeMsl::emitLightLoop(const ShaderNode &node, GenContext &context,
                                   ShaderStage &stage,
  ...
  if (context.getOptions().hwMaxActiveLightSources > 0) {
    ...
    shadergen.emitLine("for (int activeLightIndex = 0; activeLightIndex < "
                       "numLights; ++activeLightIndex)",
                       stage, false);

    shadergen.emitScopeBegin(stage);

    shadergen.emitLine("sampleLightSource(" + HW::T_LIGHT_DATA_INSTANCE +
                           "[activeLightIndex], " + prefix +
                           HW::T_POSITION_WORLD + ", lightShader)",
                       stage);
    shadergen.emitLine("float3 L = lightShader.direction", stage);
    shadergen.emitLineBreak(stage);

    shadergen.emitComment("Calculate the BSDF response for this light source",
                          stage);
    ...
    shadergen.emitLine(outColor + " += lightShader.intensity * " +
                           bsdf->getOutput()->getVariable() + ".response",
                       stage);
