...
LightNodeMsl::LightNodeMsl()
  ...
  _callEmission.addArgument(
      Type::EDF, ClosureContext::Argument(Type::VECTOR3, "light.direction"));
  _callEmission.addArgument(Type::EDF,
                            ClosureContext::Argument(Type::VECTOR3, "-L"));
}
...
void LightNodeMsl::createVariables(const ShaderNode &, GenContext &context,
                                   Shader &shader) const {
  ...
  lightUniforms.add(Type::VECTOR3, "direction",
...
void LightNodeMsl::emitFunctionCall(const ShaderNode &node, GenContext &context,
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    if (edf) {
      ...
      shadergen.emitComment("Apply quadratic falloff and adjust intensity",
                            stage);
      shadergen.emitLine(
