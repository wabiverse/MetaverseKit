...
void HeightToNormalNodeMsl::createVariables(const ShaderNode &, GenContext &,
                                            Shader &) const {
  // Default filter kernels from ConvolutionNode are not used by this derived
  // class.
}

void HeightToNormalNodeMsl::computeSampleOffsetStrings(
...
void HeightToNormalNodeMsl::emitFunctionDefinition(const ShaderNode &,
                                                   GenContext &context,
...
void HeightToNormalNodeMsl::emitFunctionCall(const ShaderNode &node,
                                             GenContext &context,
                                             ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    const ShaderGenerator &shadergen = context.getShaderGenerator();
