...
MslShaderGenerator::MslShaderGenerator()
  ...
  // <!-- <position> -->
  registerImplementation("IM_position_vector3_" + MslShaderGenerator::TARGET,
                         PositionNodeMsl::create);
  // <!-- <normal> -->
  registerImplementation("IM_normal_vector3_" + MslShaderGenerator::TARGET,
                         NormalNodeMsl::create);
  // <!-- <tangent> -->
  registerImplementation("IM_tangent_vector3_" + MslShaderGenerator::TARGET,
                         TangentNodeMsl::create);
  // <!-- <bitangent> -->
  registerImplementation("IM_bitangent_vector3_" + MslShaderGenerator::TARGET,
                         BitangentNodeMsl::create);
  // <!-- <texcoord> -->
  registerImplementation("IM_texcoord_vector2_" + MslShaderGenerator::TARGET,
                         HwTexCoordNode::create);
  registerImplementation("IM_texcoord_vector3_" + MslShaderGenerator::TARGET,
                         HwTexCoordNode::create);
  // <!-- <geomcolor> -->
  registerImplementation("IM_geomcolor_float_" + MslShaderGenerator::TARGET,
                         GeomColorNodeMsl::create);
  registerImplementation("IM_geomcolor_color3_" + MslShaderGenerator::TARGET,
                         GeomColorNodeMsl::create);
  registerImplementation("IM_geomcolor_color4_" + MslShaderGenerator::TARGET,
                         GeomColorNodeMsl::create);
  // <!-- <geompropvalue> -->
  elementNames = {
  ...
  registerImplementation("IM_geompropvalue_boolean_" +
                             MslShaderGenerator::TARGET,
                         GeomPropValueNodeMslAsUniform::create);
  registerImplementation("IM_geompropvalue_string_" +
                             MslShaderGenerator::TARGET,
                         GeomPropValueNodeMslAsUniform::create);

  // <!-- <frame> -->
  registerImplementation("IM_frame_float_" + MslShaderGenerator::TARGET,
                         FrameNodeMsl::create);
  // <!-- <time> -->
  registerImplementation("IM_time_float_" + MslShaderGenerator::TARGET,
                         TimeNodeMsl::create);

  // <!-- <surface> -->
  registerImplementation("IM_surface_" + MslShaderGenerator::TARGET,
                         SurfaceNodeMsl::create);
  registerImplementation("IM_surface_unlit_" + MslShaderGenerator::TARGET,
                         UnlitSurfaceNodeMsl::create);

  // <!-- <light> -->
  registerImplementation("IM_light_" + MslShaderGenerator::TARGET,
                         LightNodeMsl::create);

  // <!-- <point_light> -->
  registerImplementation("IM_point_light_" + MslShaderGenerator::TARGET,
                         LightShaderNodeMsl::create);
  // <!-- <directional_light> -->
  registerImplementation("IM_directional_light_" + MslShaderGenerator::TARGET,
                         LightShaderNodeMsl::create);
  // <!-- <spot_light> -->
  registerImplementation("IM_spot_light_" + MslShaderGenerator::TARGET,
                         LightShaderNodeMsl::create);

  // <!-- <heighttonormal> -->
  registerImplementation("IM_heighttonormal_vector3_" +
                             MslShaderGenerator::TARGET,
                         HeightToNormalNodeMsl::create);
  ...
  // <!-- <ND_transformpoint> ->
  registerImplementation("IM_transformpoint_vector3_" +
                             MslShaderGenerator::TARGET,
                         HwTransformPointNode::create);

  // <!-- <ND_transformvector> ->
  registerImplementation("IM_transformvector_vector3_" +
                             MslShaderGenerator::TARGET,
                         HwTransformVectorNode::create);

  // <!-- <ND_transformnormal> ->
  registerImplementation("IM_transformnormal_vector3_" +
                             MslShaderGenerator::TARGET,
  ...
  // <!-- <layer> -->
  registerImplementation("IM_layer_bsdf_" + MslShaderGenerator::TARGET,
                         ClosureLayerNode::create);
  registerImplementation("IM_layer_vdf_" + MslShaderGenerator::TARGET,
                         ClosureLayerNode::create);
  // <!-- <mix> -->
  registerImplementation("IM_mix_bsdf_" + MslShaderGenerator::TARGET,
                         ClosureMixNode::create);
  registerImplementation("IM_mix_edf_" + MslShaderGenerator::TARGET,
                         ClosureMixNode::create);
  // <!-- <add> -->
  registerImplementation("IM_add_bsdf_" + MslShaderGenerator::TARGET,
                         ClosureAddNode::create);
  registerImplementation("IM_add_edf_" + MslShaderGenerator::TARGET,
                         ClosureAddNode::create);
  // <!-- <multiply> -->
  elementNames = {
  ...
  // <!-- <thin_film> -->
  registerImplementation("IM_thin_film_bsdf_" + MslShaderGenerator::TARGET,
                         NopNode::create);

  // <!-- <surfacematerial> -->
  registerImplementation("IM_surfacematerial_" + MslShaderGenerator::TARGET,
                         MaterialNode::create);

  _lightSamplingNodes.push_back(ShaderNode::create(
      nullptr, "numActiveLightSources", NumLightsNodeMsl::create()));
  _lightSamplingNodes.push_back(ShaderNode::create(
      nullptr, "sampleLightSource", LightSamplerNodeMsl::create()));
}

ShaderPtr MslShaderGenerator::generate(const string &name, ElementPtr element,
  ...
  if (!resourceBindingCtx) {
    context.pushUserData(HW::USER_DATA_BINDING_CONTEXT,
                         MslResourceBindingContext::create());
    resourceBindingCtx = context.getUserData<HwResourceBindingContext>(
...
void MslShaderGenerator::MetalizeGeneratedShader(
    ShaderStage &shaderStage) const {
  std::string sourceCode = shaderStage.getSourceCode();

  // Used to convert shared code between GLSL pass by reference parameters to
  // MSL pass by reference. Converts "inout/out Type variableName" to "thread
  // Type& variableName"
  size_t pos = 0;
  {
    ...
    for (const auto &keyword : refKeywords) {
      ...
      while (pos != std::string::npos) {
        char preceeding = sourceCode[pos - 1],
             succeeding = sourceCode[pos + keyword.length()];
        bool isOutKeyword = (preceeding == '(' || preceeding == ',' ||
                             std::isspace(preceeding)) &&
                            std::isspace(succeeding) && succeeding != '\n';
        size_t beg = pos;
        pos += keyword.length();
        if (isOutKeyword) {
          ...
          std::string typeName =
              sourceCode.substr(typename_beg, typename_end - typename_beg);
          sourceCode.replace(beg, typename_end - beg,
                             "thread " + typeName + "&");
        }
  ...
  // Renames GLSL constructs that are used in shared code to MSL equivalent
  // constructs.
...
void MslShaderGenerator::emitGlobalVariables(GenContext &context,
  ...
  int buffer_slot =
      isVertexShader
          ? std::max(
                static_cast<int>(stage.getInputBlock(HW::VERTEX_INPUTS).size()),
                0)
          : 0;

  bool entryFunctionArgs =
      situation == EMIT_GLOBAL_SCOPE_CONTEXT_ENTRY_FUNCTION_RESOURCES;
  bool globalContextInit = situation == EMIT_GLOBAL_SCOPE_CONTEXT_MEMBER_INIT;
  ...
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
    ...
    if (globalContextConstructorInit) {
      ...
      emitLine(stage.getInputBlock(HW::VERTEX_DATA).getInstance() + ".pos)",
               stage, false);
      separator = ",";
    }
  }

  {
    ...
    if (!entryFunctionArgs) {
      if (isVertexShader) {
        for (const auto &it : vertex_inputs.getVariableOrder()) {
          ...
          if (globalContextInit) {
            emitString(vertex_inputs.getInstance() + "." + it->getName(),
                       stage);
          ...
          if (globalContextInit || globalContextConstructorParams ||
              globalContextConstructorInit)
            separator = ", ";
        ...
        if (globalContextInit) {
          ...
          emitLine(vertex_inputs.getName() + " " + vertex_inputs.getInstance(),
                   stage, !globalContextConstructorParams);
      ...
      emitString(vertex_inputs.getName() + " " + vertex_inputs.getInstance() +
                     " [[ stage_in ]]",
                 stage);
      separator = ", ";
    }
  ...
  for (const auto &it : stage.getUniformBlocks()) {
    ...
    if (isLightData) {
      emitString(separator, stage);
      if (entryFunctionArgs) {
        emitString(_syntax->getUniformQualifier() + " " + uniforms.getName() +
                       "_" + stage.getName() + "& " + uniforms.getInstance() +
                       "[[ buffer(" + std::to_string(buffer_slot++) + ") ]]",
                   stage);
      } else if (globalContextInit) {
        emitLine(uniforms.getInstance() + "." + uniforms.getInstance(), stage,
                 false);
      } else if (globalContextMembers || globalContextConstructorParams) {
        const string structArraySuffix =
            "[" + HW::LIGHT_DATA_MAX_LIGHT_SOURCES + "]";
        emitLine((globalContextConstructorParams
                      ? (_syntax->getUniformQualifier() + " ")
                      : std::string()) +
                     uniforms.getName() + " " + uniforms.getInstance() +
                     structArraySuffix,
                 stage, !globalContextConstructorParams);
      } else if (globalContextConstructorInit) {
        ...
        for (unsigned int l = 0; l < maxLights; ++l) {
          ...
          emitLine(uniforms.getInstance() + "[" + std::to_string(l) + "]",
                   stage, false);
        }
        emitScopeEnd(stage);
      }
        ...
        if (!uniforms.empty()) {
          for (size_t i = 0; i < uniforms.size(); ++i) {
            if (uniforms[i]->getType() != Type::FILENAME) {
              ...
              emitString(separator, stage);
              if (globalContextInit) {
                emitString(uniforms.getInstance() + "." +
                               uniforms[i]->getVariable(),
                           stage);
              } else if (globalContextMembers ||
                         globalContextConstructorParams) {
                emitLine(_syntax->getTypeName(uniforms[i]->getType()) + " " +
                             uniforms[i]->getVariable(),
                         stage, !globalContextConstructorParams);
              } else if (globalContextConstructorInit) {
                emitLine(uniforms[i]->getVariable() + "(" +
                             uniforms[i]->getVariable() + ")",
                         stage, false);
              }
              ...
              if (globalContextInit) {
                ...
                emitScopeEnd(stage);
                         ...
                         globalContextConstructorParams) {
                emitString(separator, stage);
                emitVariableDeclaration(uniforms[i], EMPTY_STRING, context,
                                        stage, false);
        ...
        if (!uniforms.empty()) {
          ...
          for (const ShaderPort *uniform : uniforms.getVariableOrder()) {
            if (uniform->getType() == Type::FILENAME) {
              emitString(separator, stage);
              emitString("texture2d<float> " +
                             TEXTURE_NAME(uniform->getVariable()),
                         stage);
              emitString(" [[texture(" + std::to_string(tex_slot) + ")]], ",
                         stage);
              emitString("sampler " + SAMPLER_NAME(uniform->getVariable()),
                         stage);
              emitString(" [[sampler(" + std::to_string(tex_slot++) + ")]]",
                         stage);
              emitLineEnd(stage, false);
            } else {
              hasUniforms = true;
    ...
    if (globalContextInit || entryFunctionArgs ||
        globalContextConstructorParams || globalContextConstructorInit)
      separator = ", ";
    else
      separator = "\n";
  }
  ...
  if (!isVertexShader) {
    ...
    for (auto &it : outputs.getVariableOrder()) {
      if (globalContextMembers) {
        emitLine(_syntax->getTypeName(it->getType()) + " " + it->getVariable(),
                 stage, true);
      }
    }
  }
...
void MslShaderGenerator::emitVertexStage(const ShaderGraph &graph,
                                         GenContext &context,
  ...
  emitScopeBegin(stage);
  {
    ...
    emitGlobalVariables(context, stage,
                        EMIT_GLOBAL_SCOPE_CONTEXT_CONSTRUCTOR_ARGS, true,
                        false);
    emitLine(") : ", stage, false);
    emitGlobalVariables(context, stage,
                        EMIT_GLOBAL_SCOPE_CONTEXT_CONSTRUCTOR_INIT, true,
                        false);
    emitLine("{}", stage, false);

    emitGlobalVariables(context, stage, EMIT_GLOBAL_SCOPE_CONTEXT_MEMBER_DECL,
                        true, false);
    ...
    emitScopeBegin(stage);
    {
      emitLine(vertexData.getName() + " " + vertexData.getInstance(), stage,
               true);
      emitLine("float4 hPositionWorld = " + HW::T_WORLD_MATRIX + " * float4(" +
                   HW::T_IN_POSITION + ", 1.0)",
               stage);
      emitLine(vertexData.getInstance() + ".pos" + " = " +
                   HW::T_VIEW_PROJECTION_MATRIX + " * hPositionWorld",
               stage);
  ...
  emitGlobalVariables(context, stage,
                      EMIT_GLOBAL_SCOPE_CONTEXT_ENTRY_FUNCTION_RESOURCES, true,
                      false);
  emitLine(")", stage, false);
  emitScopeBegin(stage);
  {
    emitString("\tGlobalContext ctx {", stage);
    emitGlobalVariables(context, stage, EMIT_GLOBAL_SCOPE_CONTEXT_MEMBER_INIT,
                        true, false);
    emitLine("}", stage, true);
...
void MslShaderGenerator::emitSpecularEnvironment(GenContext &context,
                                                 ShaderStage &stage) const {
  int specularMethod = context.getOptions().hwSpecularEnvironmentMethod;
  if (specularMethod == SPECULAR_ENVIRONMENT_FIS) {
    emitLibraryInclude("pbrlib/genglsl/lib/mx_environment_fis.glsl", context,
                       stage);
  } else if (specularMethod == SPECULAR_ENVIRONMENT_PREFILTER) {
    emitLibraryInclude("pbrlib/genglsl/lib/mx_environment_prefilter.glsl",
                       context, stage);
  } else if (specularMethod == SPECULAR_ENVIRONMENT_NONE) {
    emitLibraryInclude("pbrlib/genglsl/lib/mx_environment_none.glsl", context,
                       stage);
  } else {
...
void MslShaderGenerator::emitTransmissionRender(GenContext &context,
                                                ShaderStage &stage) const {
  int transmissionMethod = context.getOptions().hwTransmissionRenderMethod;
  if (transmissionMethod == TRANSMISSION_REFRACTION) {
    emitLibraryInclude("pbrlib/genglsl/lib/mx_transmission_refract.glsl",
                       context, stage);
  } else if (transmissionMethod == TRANSMISSION_OPACITY) {
    emitLibraryInclude("pbrlib/genglsl/lib/mx_transmission_opacity.glsl",
                       context, stage);
  } else {
    throw ExceptionShaderGenError("Invalid transmission render specified: '" +
                                  std::to_string(transmissionMethod) + "'");
  }
  emitLineBreak(stage);
}

void MslShaderGenerator::emitDirectives(GenContext &,
                                        ShaderStage &stage) const {
  // Add directives
...
void MslShaderGenerator::emitConstants(GenContext &context,
                                       ShaderStage &stage) const {
  const VariableBlock &constants = stage.getConstantBlock();
  if (!constants.empty()) {
    emitVariableDeclarations(constants, _syntax->getUniformQualifier(),
                             Syntax::SEMICOLON, context, stage);
    emitLineBreak(stage);
  }
}

void MslShaderGenerator::emitConstantBufferDeclarations(
    GenContext &context, HwResourceBindingContextPtr resourceBindingCtx,
    ShaderStage &stage) const {
  // Add all uniforms
  for (const auto &it : stage.getUniformBlocks()) {
    const VariableBlock &uniforms = *it.second;
    if (!uniforms.empty()) {
      ...
      if (resourceBindingCtx) {
        ...
        emitVariableDeclarations(uniforms, _syntax->getUniformQualifier(),
                                 Syntax::SEMICOLON, context, stage);
        emitLineBreak(stage);
      }
    }
...
void MslShaderGenerator::emitMetalTextureClass(GenContext &context,
                                               ShaderStage &stage) const {
  emitLibraryInclude("stdlib/genmsl/lib/mx_texture.metal", context, stage);
}

void MslShaderGenerator::emitLightData(GenContext &context,
                                       ShaderStage &stage) const {
  ...
  if (resourceBindingCtx) {
    ...
    emitScopeBegin(stage);
    emitVariableDeclarations(lightData, EMPTY_STRING, Syntax::SEMICOLON,
                             context, stage, false);
    emitScopeEnd(stage, true);
    emitLineBreak(stage);
    emitLine("uniform " + lightData.getName() + " " + structName +
                 structArraySuffix,
             stage);
  }
  emitLineBreak(stage);
...
void MslShaderGenerator::emitInputs(GenContext &context, ShaderStage &stage,
  ...
  for (size_t i = 0; i < inputs.size(); ++i) {
    string line = "";
    line += context.getShaderGenerator().getSyntax().getTypeName(
        inputs[i]->getType());
...
void MslShaderGenerator::emitInputs(GenContext &context,
                                    ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::VERTEX) {
...
void MslShaderGenerator::emitOutputs(GenContext &context,
                                     ShaderStage &stage) const {
  // Add vertex inputs
  auto emitOutputsOfShaderSource = [&](const VariableBlock &outputs) {
    if (!outputs.empty()) {
      ...
      emitVariableDeclarations(outputs, EMPTY_STRING, Syntax::SEMICOLON,
                               context, stage, false);
      emitScopeEnd(stage, true, false);
      emitLineBreak(stage);
      emitLineBreak(stage);
...
HwResourceBindingContextPtr
MslShaderGenerator::getResourceBindingContext(GenContext &context) const {
  return context.getUserData<HwResourceBindingContext>(
      HW::USER_DATA_BINDING_CONTEXT);
}

string
MslShaderGenerator::getVertexDataPrefix(const VariableBlock &vertexData) const {
...
void MslShaderGenerator::emitMathMatrixScalarMathOperators(
    GenContext &context, ShaderStage &stage) const {
  emitLibraryInclude("stdlib/genmsl/lib/mx_matscalaroperators.metal", context,
                     stage);
}

void MslShaderGenerator::emitPixelStage(const ShaderGraph &graph,
                                        GenContext &context,
                                        ShaderStage &stage) const {
  HwResourceBindingContextPtr resourceBindingCtx =
      getResourceBindingContext(context);
  ...
  if (lighting) {
    if (context.getOptions().hwMaxActiveLightSources > 0) {
      const unsigned int maxLights =
          std::max(1u, context.getOptions().hwMaxActiveLightSources);
      emitLine("#define " + HW::LIGHT_DATA_MAX_LIGHT_SOURCES + " " +
  ...
  bool needsLightBuffer =
      lighting && context.getOptions().hwMaxActiveLightSources > 0;
  ...
  emitScopeBegin(stage);
  {
    emitLine("GlobalContext(", stage, false);
    emitGlobalVariables(context, stage,
                        EMIT_GLOBAL_SCOPE_CONTEXT_CONSTRUCTOR_ARGS, false,
                        needsLightBuffer);
    emitLine(") : ", stage, false);
    emitGlobalVariables(context, stage,
                        EMIT_GLOBAL_SCOPE_CONTEXT_CONSTRUCTOR_INIT, false,
                        needsLightBuffer);
    emitLine("{}", stage, false);
    ...
    if (graph.hasClassification(ShaderNode::Classification::CLOSURE) &&
      ...
      emitLine(outputSocket->getVariable() + " = float4(0.0, 0.0, 0.0, 1.0)",
               stage);
      ...
      if (graph.hasClassification(ShaderNode::Classification::SHADER |
        ...
        // These will internally emit function calls for any dependent closure
        // nodes upstream.
      ...
      if (outputConnection) {
        ...
        if (!channels.empty()) {
          finalOutput = _syntax->getSwizzledVariable(
              finalOutput, outputConnection->getType(), channels,
        ...
        if (graph.hasClassification(ShaderNode::Classification::SURFACE)) {
          if (context.getOptions().hwTransparency) {
            emitLine("float outAlpha = clamp(1.0 - dot(" + finalOutput +
                     ...
                     stage);
            emitLine(outputSocket->getVariable() + " = float4(" + finalOutput +
                         ".color, outAlpha)",
                     stage);
            emitLine("if (outAlpha < " + HW::T_ALPHA_THRESHOLD + ")", stage,
                     false);
            emitScopeBegin(stage);
  ...
  // Add main function
  {
    ...
    emitGlobalVariables(context, stage,
                        EMIT_GLOBAL_SCOPE_CONTEXT_ENTRY_FUNCTION_RESOURCES,
                        false, needsLightBuffer);
    emitLine(")", stage, false);
    emitScopeBegin(stage);
    {
...
void MslShaderGenerator::emitLightFunctionDefinitions(
    const ShaderGraph &graph, GenContext &context, ShaderStage &stage) const {
  DEFINE_SHADER_STAGE(stage, Stage::PIXEL) {
...
void MslShaderGenerator::emitVariableDeclaration(const ShaderPort *variable,
  ...
  if (variable->getType() == Type::FILENAME) {
    ...
    if (variable->getType()->isArray() && variable->getValue()) {
      str += _syntax->getArrayVariableSuffix(variable->getType(),
                                             *variable->getValue());
    }
    ...
    // vertex stage and input to pixel stage. The only way to get these is with
    // geompropvalue_integer nodes.
    if (qualifier.empty() && variable->getType() == Type::INTEGER &&
        !assignValue && variable->getName().rfind(HW::T_IN_GEOMPROP, 0) == 0) {
...
ShaderNodeImplPtr
MslShaderGenerator::getImplementation(const NodeDef &nodedef,
                                      GenContext &context) const {
...
const std::set<string> IMMUTABLE_INPUTS = {
    // Geometric node inputs are immutable since a shader needs regeneration if
    // they change.
