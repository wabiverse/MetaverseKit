...
protected:
  void
  loadAdditionalLibraries(mx::DocumentPtr document,
  ...
  bool runRenderer(const std::string &shaderName, mx::TypedElementPtr element,
                   mx::GenContext &context, mx::DocumentPtr doc,
                   std::ostream &log,
  ...
  bool saveImage(const mx::FilePath &filePath, mx::ConstImagePtr image,
                 bool verticalFlip) const override;

  bool canBake() const override { return true; }

  void runBake(mx::DocumentPtr doc, const mx::FileSearchPath &imageSearchPath,
               const mx::FilePath &outputFilename,
               const GenShaderUtil::TestSuiteOptions::BakeSetting &bakeOptions,
...
// In addition to standard texture and shader definition libraries, additional
// lighting files are loaded in. If no files are specifed in the input options,
// a sample compound light type and a set of lights in a "light rig" are loaded
// in to a given document.
void MslShaderRenderTester::loadAdditionalLibraries(
    mx::DocumentPtr document, GenShaderUtil::TestSuiteOptions &options) {
...
// Create a light handler and populate it based on lights found in a given
// document
void MslShaderRenderTester::registerLights(
    mx::DocumentPtr document, const GenShaderUtil::TestSuiteOptions &options,
  ...
  mx::ImagePtr envRadiance =
      _renderer->getImageHandler()->acquireImage(options.radianceIBLPath);
  mx::ImagePtr envIrradiance =
      _renderer->getImageHandler()->acquireImage(options.irradianceIBLPath);
  REQUIRE(envRadiance);
  ...
  _lightHandler->setEnvSampleCount(options.enableReferenceQuality ? 4096
                                                                  : 1024);
}

//
// Create a renderer with the apporpraite image, geometry and light handlers.
// The light handler on the renderer is cleared on initialization to indicate no
// lighting is required. During code generation, if the element to validate
// requires lighting then the handler _lightHandler will be used.
//
...
bool MslShaderRenderTester::saveImage(const mx::FilePath &filePath,
                                      mx::ConstImagePtr image,
                                      bool verticalFlip) const {
  return _renderer->getImageHandler()->saveImage(filePath, image, verticalFlip);
}

bool MslShaderRenderTester::runRenderer(
    const std::string &shaderName, mx::TypedElementPtr element,
    mx::GenContext &context, mx::DocumentPtr doc, std::ostream &log,
    const GenShaderUtil::TestSuiteOptions &testOptions,
    RenderUtil::RenderProfileTimes &profileTimes,
    const mx::FileSearchPath &imageSearchPath, const std::string &outputPath,
    mx::ImageVec *imageVec) {
  std::cout << "Validating MSL rendering for: " << doc->getSourceUri()
            << std::endl;
  ...
  if (element && doc) {
    log << "------------ Run MSL validation with element: "
    ...
    for (auto options : optionsList) {
      ...
      std::string shaderPath =
          mx::FilePath(outputFilePath) / mx::FilePath(shaderName);
      mx::ShaderPtr shader;
      try {
        mx::ScopedTimer transpTimer(
            &profileTimes.languageTimes.transparencyTime);
        options.hwTransparency =
            mx::isTransparentSurface(element, shadergen.getTarget());
        transpTimer.endTimer();

        mx::ScopedTimer generationTimer(
            &profileTimes.languageTimes.generationTime);
      ...
      const std::string &pixelSourceCode =
      ...
      try {
        ...
        if (!testOptions.renderGeometry.isEmpty()) {
          if (!testOptions.renderGeometry.isAbsolute()) {
            geomPath = searchPath.find("resources/Geometry") /
                       testOptions.renderGeometry;
          } else {
        ...
        if (!geomHandler->hasGeometry(geomPath)) {
          // For test sphere and plane geometry perform a V-flip of texture
          // coordinates.
          const std::string baseName = geomPath.getBaseName();
          bool texcoordVerticalFlip =
              baseName == "sphere.obj" || baseName == "plane.obj";
        ...
        if (testOptions.dumpUniformsAndAttributes) {
          ...
          const MaterialX::MslProgram::InputMap &uniforms =
          ...
          for (const auto &uniform : uniforms) {
            ...
            mx::InputPtr input =
                pathElement ? pathElement->asA<mx::Input>() : nullptr;
            if (getUIProperties(input, target, uiProperties) > 0) {
              ...
              if (!uiProperties.enumerationValues.empty()) {
                log << ". Enum Values: {";
                for (size_t i = 0; i < uiProperties.enumerationValues.size();
                     i++)
              ...
              if (uiProperties.uiSoftMin)
                log << ". UI Soft Min: "
                    << uiProperties.uiSoftMin->getValueString();
              if (uiProperties.uiSoftMax)
                log << ". UI Soft Max: "
                    << uiProperties.uiSoftMax->getValueString();
        ...
        if (testOptions.renderImages) {
          {
            ...
            _renderer->getImageHandler()->setSearchPath(
                mx::getDefaultDataSearchPath());
            _renderer->setSize(
                static_cast<unsigned int>(testOptions.renderSize[0]),
                static_cast<unsigned int>(testOptions.renderSize[1]));
            _renderer->render();
          }
        ...
        log << ">> Refer to shader code in dump files: " << shaderPath
            << "_ps.metal and _vs.metal files" << std::endl;
...
void MslShaderRenderTester::runBake(
    mx::DocumentPtr doc, const mx::FileSearchPath &imageSearchPath,
    const mx::FilePath &outputFileName,
    const GenShaderUtil::TestSuiteOptions::BakeSetting &bakeOptions,
    std::ostream &log) {
  mx::ImageVec imageVec =
      _renderer->getImageHandler()->getReferencedImages(doc);
  auto maxImageSize = mx::getMaxDimensions(imageVec);
  const unsigned bakeWidth =
      std::max(bakeOptions.resolution, maxImageSize.first);
  const unsigned bakeHeight =
      std::max(bakeOptions.resolution, maxImageSize.second);

  mx::Image::BaseType baseType =
      bakeOptions.hdr ? mx::Image::BaseType::FLOAT : mx::Image::BaseType::UINT8;
  mx::TextureBakerPtr baker =
      mx::TextureBakerMsl::create(bakeWidth, bakeHeight, baseType);
  ...
  try {
    ...
    log << sourceUri.asString() + " failed baking process: " + e.what()
        << std::endl;
  }
}
