...
void elementToXml(ConstElementPtr elem, xml_node &xmlNode,
                  const XmlWriteOptions *writeOptions) {
  bool writeXIncludeEnable =
      writeOptions ? writeOptions->writeXIncludeEnable : true;
  ElementPredicate elementPredicate =
      writeOptions ? writeOptions->elementPredicate : nullptr;

  // Store attributes in XML.
  if (!elem->getName().empty()) {
    xmlNode.append_attribute(Element::NAME_ATTRIBUTE.c_str()) =
        elem->getName().c_str();
  }
      ...
      if (sourceUri != elem->getDocument()->getSourceUri()) {
        if (!writtenSourceFiles.count(sourceUri)) {
          if (!xmlNode.attribute(XINCLUDE_NAMESPACE.c_str())) {
            xmlNode.append_attribute(XINCLUDE_NAMESPACE.c_str()) =
                XINCLUDE_URL.c_str();
          }
          ...
          FilePath::Format includeFormat = includePath.isAbsolute()
                                               ? FilePath::FormatNative
                                               : FilePath::FormatPosix;
...
void processXIncludes(DocumentPtr doc, xml_node &xmlNode,
                      const FileSearchPath &searchPath,
                      const XmlReadOptions *readOptions) {
  // Search path for includes. Set empty and then evaluated once in the
  // iteration through xml includes.
  FileSearchPath includeSearchPath;

  XmlReadFunction readXIncludeFunction =
      readOptions ? readOptions->readXIncludeFunction : readFromXmlFile;
    ...
    if (xmlChild.name() == XINCLUDE_TAG) {
      ...
      if (readXIncludeFunction) {
        ...
        if (readOptions) {
          ...
          if (std::find(parents.begin(), parents.end(), filename) !=
              parents.end()) {
        ...
        XmlReadOptions xiReadOptions =
            readOptions ? *readOptions : XmlReadOptions();
        ...
        if (includeSearchPath.isEmpty()) {
          ...
          if (!parentUri.empty()) {
            ...
            if (!filePath.isEmpty()) {
              // Remove the file name from the path as we want the path to the
              // containing folder.
        ...
        readXIncludeFunction(library, filename, includeSearchPath,
                             &xiReadOptions);
...
void validateParseResult(const xml_parse_result &result,
  ...
  if (result.status == xml_parse_status::status_file_not_found ||
    ...
    throw ExceptionFileMissing("Failed to open file for reading: " +
                               filename.asString());
  }
...
XmlReadOptions::XmlReadOptions()
    : readComments(false), readNewlines(false), upgradeVersion(true),
...
XmlWriteOptions::XmlWriteOptions() : writeXIncludeEnable(true) {}
...
void readFromXmlBuffer(DocumentPtr doc, const char *buffer,
  ...
  xml_parse_result result =
      xmlDoc.load_string(buffer, getParseOptions(readOptions));
...
void readFromXmlFile(DocumentPtr doc, FilePath filename,
                     FileSearchPath searchPath,
                     const XmlReadOptions *readOptions) {
  searchPath.append(getEnvironmentPath());
...
void writeToXmlStream(DocumentPtr doc, std::ostream &stream,
                      const XmlWriteOptions *writeOptions) {
  xml_document xmlDoc;
...
void writeToXmlFile(DocumentPtr doc, const FilePath &filename,
                    const XmlWriteOptions *writeOptions) {
