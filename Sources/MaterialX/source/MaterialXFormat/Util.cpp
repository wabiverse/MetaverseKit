...
void getSubdirectories(const FilePathVec &rootDirectories,
  ...
  for (const FilePath &root : rootDirectories) {
    ...
    if (rootPath.exists()) {
      ...
      subDirectories.insert(std::end(subDirectories),
                            std::begin(childDirectories),
                            std::end(childDirectories));
    }
  }
}

void loadDocuments(const FilePath &rootPath, const FileSearchPath &searchPath,
    ...
    for (const FilePath &file : dir.getFilesInDirectory(MTLX_EXTENSION)) {
      if (!skipFiles.count(file) &&
        ...
        try {
          ...
          if (errors) {
            errors->push_back("Failed to load: " + filePath.asString() +
                              ". Error: " + e.what());
          }
        }
      }
...
StringSet loadLibraries(const FilePathVec &libraryFolders,
                        const FileSearchPath &searchPath, DocumentPtr doc,
  ...
  if (libraryFolders.empty()) {
    ...
    for (const FilePath &libraryPath : librarySearchPath) {
      for (const FilePath &path : libraryPath.getSubDirectories()) {
        for (const FilePath &filename :
             path.getFilesInDirectory(MTLX_EXTENSION)) {
...
void flattenFilenames(DocumentPtr doc, const FileSearchPath &searchPath,
                      StringResolverPtr customResolver) {
  for (ElementPtr elem : doc->traverseTree()) {
    ...
    if (customResolver &&
        customResolver->isResolvedType(FILENAME_TYPE_STRING)) {
      resolvedString =
          customResolver->resolve(resolvedString, FILENAME_TYPE_STRING);
    }
