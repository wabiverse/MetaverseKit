...
FilePath FilePath::getNormalized() const {
  ...
  for (const string &str : _vec) {
    ...
    if (str == PARENT_PATH_STRING && !res.isEmpty() &&
        res[res.size() - 1] != PARENT_PATH_STRING) {
      res._vec.pop_back();
      continue;
    }
  ...
  if (dir) {
    while (struct dirent *entry = readdir(dir)) {
      if (entry->d_type != DT_DIR &&
          FilePath(entry->d_name).getExtension() == extension) {
...
FilePath FilePath::getCurrentPath() {
  ...
  if (!GetCurrentDirectoryA(MAX_PATH, buf.data())) {
    throw Exception("Error in getCurrentPath: " +
...
FilePath FilePath::getModulePath() {
  ...
  while (true) {
    uint32_t reqSize =
        GetModuleFileNameA(NULL, buf.data(), (uint32_t)buf.size());
    if (!reqSize) {
      throw Exception("Error in getModulePath: " +
                      std::to_string(GetLastError()));
