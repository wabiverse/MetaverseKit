...
/// Returns the first renderable element from the given document. This element
/// can be used to generate a shader.
  ...
  for (mx::TypedElementPtr elem : elems) {
    ...
    if (node && node->getType() == mx::MATERIAL_TYPE_STRING) {
      std::vector<mx::NodePtr> shaderNodes =
          getShaderNodes(node, mx::SURFACE_SHADER_TYPE_STRING);
    ...
    mx::TypedElementPtr typedElem =
        renderableElement ? renderableElement->asA<mx::TypedElement>()
                          : nullptr;
    if (typedElem) {
...
EMSCRIPTEN_BINDINGS(Util) {
  BIND_FUNC("isTransparentSurface", mx::isTransparentSurface, 1, 2,
            mx::ElementPtr, const std::string &);
