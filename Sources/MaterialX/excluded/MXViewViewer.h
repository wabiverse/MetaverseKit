...
public:
  Viewer(const std::string &materialFilename, const std::string &meshFilename,
  ...
  void setTurntableEnabled(bool val) { _turntableEnabled = val; }

  // Set the total number of steps for one 360 degree rotation.
  void setTurntableSteps(int steps) { _turntableSteps = steps; }
  ...
  void setDrawEnvironment(bool enable) { _drawEnvironment = enable; }
  ...
  void setBakeWidth(unsigned int bakeWidth) { _bakeWidth = bakeWidth; }

  // Set the target height for texture baking.
  void setBakeHeight(unsigned int bakeHeight) { _bakeHeight = bakeHeight; }
  ...
  ng::Window *getWindow() const { return _window; }

  // Return the active image handler.
  mx::ImageHandlerPtr getImageHandler() const { return _imageHandler; }
  ...
  void bakeTextures() { _renderPipeline->bakeTextures(); }
...
private:
  ...
  bool mouse_motion_event(const ng::Vector2i &p, const ng::Vector2i &rel,
                          int button, int modifiers) override;
  bool mouse_button_event(const ng::Vector2i &p, int button, bool down,
                          int modifiers) override;
  ...
  // Return the ambient occlusion image, if any, associated with the given
  // material.
  ...
  void splitDirectLight(mx::ImagePtr envRadianceMap, mx::ImagePtr &indirectMap,
