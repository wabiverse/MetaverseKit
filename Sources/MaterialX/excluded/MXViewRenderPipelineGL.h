...
public:
  ...
  void initFramebuffer(int width, int height, void *color_texture) override;
  void resizeFramebuffer(int width, int height, void *color_texture) override;
  ...
  std::shared_ptr<void>
  createTextureBaker(unsigned int width, unsigned int height,
                     mx::Image::BaseType baseType) override;
  void renderFrame(void *color_texture, int shadowMapSize,
                   const char *dirLightNodeCat) override;
